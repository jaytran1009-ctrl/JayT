/**
 * JAYT EXPANDED BATCH CAPTURE RUNNER (138)
 * Directive: JAYT-138 — OFFICIAL LEAF-SOURCE ACQUISITION & SELF-RUNNING SUPPLY CYCLE
 * 
 * STRICT MANDATES:
 * 1. Captures >= 250 official leaf targets across 5 cohorts.
...
const outputDir = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_138', 'captures_138');
const seedsPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'seed_targets_138.json');
...
async function captureTarget(browser, target) {
  ...
  console.log(`[${captureStatus}] ${target.target_id} (${target.cohort}): ${target.name}`);
}

async function runBatch138() {
  console.log('========================================================================');
  console.log(`🚀 STARTING BATCH CAPTURE 138 (${targets.length} LEAF TARGETS)...`);
  console.log('========================================================================\n');
  ...
  for (let i = 0; i < targets.length; i += concurrency) {
    ...
    await Promise.all(batch.map(t => captureTarget(browser, t)));
  }

  await browser.close();

  console.log('\n========================================================================');
  console.log(`✅ BATCH CAPTURE 138 COMPLETED: ${targets.length} LEAF TARGETS CAPTURED.`);
  console.log(`📂 Output Directory: ${outputDir}`);
  console.log('========================================================================\n');
}

runBatch138().catch(err => {
  console.error('Fatal Error in runBatch138:', err);
  process.exit(1);
});
