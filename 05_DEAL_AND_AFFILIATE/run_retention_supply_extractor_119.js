/**
 * RETENTION SUPPLY EXTRACTOR & SSOT GENERATOR (JAYT-119)
 * 
 * Directives:
 * 1. Plain Life Language Classification:
 *    - LIMITED_TIME_DEAL ("ƯU ĐÃI CÓ HẠN")
 *    - PLANNING_MENU_PRICE ("GIÁ THAM KHẢO ĐỂ LẬP KÈO")
 *    - DAILY_UTILITY_SAVINGS ("TIỆN ÍCH TIẾT KIỆM THƯỜNG NHẬT")
 * 2. Zero technical jargon in display fields.
 * 3. 100% Physical file evidence on disk with SHA-256 verification.
 */

const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

const repoRoot = path.resolve(__dirname, '..');
const sotDir = path.join(repoRoot, '03_SOURCE_OF_TRUTH');

function getSha256(buf) {
  return crypto.createHash('sha256').update(buf).digest('hex');
}

function verifyDiskEvidence(relPath, claims) {
  const fullPath = path.join(repoRoot, relPath);
  if (!fs.existsSync(fullPath)) {
    throw new Error(`Physical file evidence missing: ${relPath}`);
  }
  const buf = fs.readFileSync(fullPath);
  const hash = getSha256(buf);
  const text = buf.toString('utf8');

  for (const claim of claims) {
    if (!text.includes(claim)) {
      throw new Error(`Claim mismatch in ${relPath}: Missing "${claim}"`);
    }
  }

  return { exists: true, sha256: hash, path: relPath };
}

// 1. LIMITED-TIME VERIFIED DEALS (4 DEALS)
const limitedTimeDeals = [
  {
    id: "DEAL_119_CGV_PAYDAY_30K",
    feed_category: "LIMITED_TIME_DEAL",
    category_badge: "🟢 ƯU ĐÃI CÓ HẠN",
    brand: "CGV Cinema Đà Nẵng",
    title: "Ting Ting Lương Về – Giảm 30K Khi Mua Từ 2 Vé",
    benefit: "GIẢM 30.000Đ",
    discount_numeric: 30000,
    promo_code: "PAYDAY",
    validity: "25/08 – 31/08/2026",
    expiry_date: "2026-08-31",
    scope: "Toàn bộ rạp CGV Đà Nẵng (Vincom & Vĩnh Trung)",
    terms: "Áp dụng mua trực tuyến trên app/web CGV từ 2 vé trở lên. Không cộng dồn ưu đãi khác.",
    official_url: "https://www.cgv.vn/default/news/cgv-payday",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_14_CGV_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "TING TING LƯƠNG VỀ – DEAL GIẢM NGAY 30K",
      "Từ 25/08 – 31/08/2026",
      "Giảm ngay 30.000Đ khi mua từ 02 vé trở lên",
      "PAYDAY",
      "Áp dụng tất cả các rạp, định dạng"
    ],
    slot: "SLOT_2000",
    sector: "CINEMA",
    persona: ["STUDENT", "OFFICE", "FAMILY", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "DEAL_119_CGV_MUA1TANG1",
    feed_category: "LIMITED_TIME_DEAL",
    category_badge: "🟢 ƯU ĐÃI CÓ HẠN",
    brand: "CGV Cinema Đà Nẵng",
    title: "Mua 1 Tặng 1 Vé Xem Phim CGV Với VNPAY",
    benefit: "MUA 1 TẶNG 1",
    discount_numeric: 80000,
    promo_code: "MUA1TANG1",
    validity: "Từ nay đến 30/09/2026",
    expiry_date: "2026-09-30",
    scope: "CGV Vincom Đà Nẵng & CGV Vĩnh Trung Plaza",
    terms: "Thanh toán qua cổng VNPAY-QR tại quầy hoặc app. Áp dụng vé 2D ghế Standard/VIP.",
    official_url: "https://www.cgv.vn/default/news/vnpay-cgv",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_14_CGV_LEAF_02/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "Hệ thống rạp CGV trên toàn quốc",
      "Từ nay - 30/09/2026",
      "Ưu đãi Mua 1 tặng 1 vé xem phim CGV",
      "MUA1TANG1",
      "VNPAY"
    ],
    slot: "SLOT_2000",
    sector: "CINEMA",
    persona: ["STUDENT", "OFFICE", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "DEAL_119_STARLIGHT_COMBO_10K",
    feed_category: "LIMITED_TIME_DEAL",
    category_badge: "🟢 ƯU ĐÃI CÓ HẠN",
    brand: "Starlight Cinema Đà Nẵng",
    title: "Hè Rộn Ràng – Deal Giảm 10K Combo Bắp Nước",
    benefit: "GIẢM 10.000Đ COMBO",
    discount_numeric: 10000,
    promo_code: "COMBOHE10K",
    validity: "16/06 đến 19/09/2026",
    expiry_date: "2026-09-19",
    scope: "Tầng 3-4 Tòa nhà Nguyễn Kim, 46 Điện Biên Phủ, Thanh Khê, Đà Nẵng",
    terms: "Áp dụng giảm trực tiếp khi mua combo bắp nước kèm vé xem phim tại quầy Starlight Đà Nẵng.",
    official_url: "https://starlight.vn/tin-tuc/he-ron-rang-deal-10k-san-sang.html",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_17_STARLIGHT_LEAF_03/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "HÈ RỘN RÀNG - DEAL 10K SẴN SÀNG",
      "COMBOHE10K",
      "GIẢM NGAY 10.000Đ trên tổng hóa đơn",
      "Starlight Đà Nẵng",
      "16/06 - 19/09/2026"
    ],
    slot: "SLOT_2000",
    sector: "CINEMA",
    persona: ["STUDENT", "FAMILY", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "DEAL_119_METIZ_U22_AND_SUPER_MONDAY",
    feed_category: "LIMITED_TIME_DEAL",
    category_badge: "🟢 ƯU ĐÃI CÓ HẠN",
    brand: "Metiz Cinema Đà Nẵng",
    title: "Khuyến Mãi Vé U22 & Super Monday (Thứ Hai Siêu Hạng)",
    benefit: "GIÁ VÉ ƯU ĐÃI ĐỊNH KỲ",
    discount_numeric: 30000,
    promo_code: "METIZ_U22",
    validity: "01/01/2026 – 31/12/2026",
    expiry_date: "2026-12-31",
    scope: "Tầng 1 Helio Center, Đường 2 Tháng 9, Hải Châu, Đà Nẵng",
    terms: "Áp dụng cho học sinh, sinh viên và thành viên Metiz Cinema Đà Nẵng vào Thứ Hai hàng tuần & các suất chiếu tiêu chuẩn.",
    official_url: "https://metiz.vn/tin-tuc-uu-dai/",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_15_METIZ_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "01/01/2026 - 31/12/2026",
      "SUPER MONDAY (THỨ HAI SIÊU HẠNG)",
      "KHUYẾN MÃI GIÁ VÉ U22",
      "Hotline: 0236 3630 689"
    ],
    slot: "SLOT_2000",
    sector: "CINEMA",
    persona: ["STUDENT", "ALL"],
    added_at: "2026-08-25T23:18:00+07:00"
  }
];

// 2. WATCHLIST DEALS (NEEDS RECHECK)
const watchlistDeals = [
  {
    id: "WATCHLIST_119_HIGHLANDS_JCB_30",
    feed_category: "WATCHLIST_RECHECK",
    category_badge: "⚠️ CẦN HỎI LẠI TẠI QUÁN",
    brand: "Highlands Coffee Đà Nẵng",
    title: "Ưu Đãi 30% Thanh Toán Thẻ JCB / Apple Pay",
    benefit: "GIẢM 30% (TỐI ĐA 30K)",
    discount_numeric: 20000,
    validity: "Đang kiểm tra hạn chót tại chi nhánh Đà Nẵng",
    expiry_date: null,
    scope: "Các chi nhánh Highlands Coffee Đà Nẵng",
    terms: "Áp dụng thanh toán chạm JCB trên Apple Pay. Cần hỏi nhân viên thu ngân trước khi gọi món.",
    official_url: "https://www.highlandscoffee.com.vn/vn/uu-dai-jcb-apple-pay.html",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_09_HIGHLANDS_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "ƯU ĐÃI 30% KHI THANH TOÁN QUA APPLE PAY BẰNG THẺ TÍN DỤNG VIETCOMBANK JCB",
      "13/08/2026"
    ],
    freshness: {
      max_recheck_days: 14,
      status: "NEEDS_STORE_INSPECTION"
    },
    slot: "SLOT_0730",
    sector: "COFFEE",
    persona: ["OFFICE", "STUDENT", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "WATCHLIST_119_WINMART_WINECO_20",
    feed_category: "WATCHLIST_RECHECK",
    category_badge: "⚠️ CẦN HỎI LẠI TẠI QUÁN",
    brand: "WinMart / WinMart+ Đà Nẵng",
    title: "Ưu Đãi Hội Viên WinLife -20% Rau Củ WinEco",
    benefit: "TIẾT KIỆM 20% CHO HỘI VIÊN",
    discount_numeric: 15000,
    validity: "Chương trình định kỳ cho hội viên",
    expiry_date: null,
    scope: "Hệ thống WinMart & WinMart+ toàn TP Đà Nẵng",
    terms: "Đọc số điện thoại đăng ký hội viên WinLife tại quầy thanh toán. Áp dụng các mặt hàng rau củ WinEco.",
    official_url: "https://winmart.vn/hoi-vien-winlife",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_30_WINMART_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "Ưu Đãi Hội Viên",
      "-20%",
      "Rau mầm cải ngọt WinEco",
      "14.800"
    ],
    freshness: {
      max_recheck_days: 14,
      status: "NEEDS_STORE_INSPECTION"
    },
    slot: "SLOT_1730",
    sector: "SHOPPING",
    persona: ["FAMILY", "OFFICE", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  }
];

// 3. PLANNING MENU PRICING & PUBLIC UTILITY SAVINGS
const planningMenuAndUtilities = [
  {
    id: "MENU_119_KFC_DZUT_DEAL_88K",
    feed_category: "PLANNING_MENU_PRICE",
    category_badge: "📋 GIÁ THAM KHẢO ĐỂ LẬP KÈO",
    brand: "KFC Đà Nẵng",
    item_name: "KFC Combo Dzựt Deal Hú Hồn 88K",
    listed_price_vnd: 88000,
    display_price_badge: "88.000₫ (Giá combo niêm yết)",
    description: "Gồm 2 Miếng Gà Giòn Cay/Truyền Thống + 1 Mì Ý Migaxuxi + 2 Ly Pepsi.",
    source_note: "Giá combo niêm yết công khai trên website KFC Việt Nam để bạn tham khảo chia tiền nhóm.",
    official_url: "https://kfcvietnam.com.vn/khuyen-mai/dzut-deal-hu-hon-88k",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_03_KFC_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "Dzựt Deal Hú Hồn 88K",
      "88.000₫",
      "138.000₫",
      "2 Miếng Gà + 1 Mì Ý Migaxuxi + 2 Ly"
    ],
    slot: "SLOT_1115",
    sector: "LUNCH",
    persona: ["STUDENT", "OFFICE", "FAMILY", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "MENU_119_JOLLIBEE_COMBO_73K",
    feed_category: "PLANNING_MENU_PRICE",
    category_badge: "📋 GIÁ THAM KHẢO ĐỂ LẬP KÈO",
    brand: "Jollibee Đà Nẵng",
    item_name: "Jollibee Combo Một Mình Ăn Ngon 73K",
    listed_price_vnd: 73000,
    display_price_badge: "73.000₫ (Giá combo niêm yết)",
    description: "Gồm 1 Miếng Gà Giòn Vui Vẻ + 1 Mì Ý Jolly + 1 Ly Nước Ngọt.",
    source_note: "Giá combo suất lẻ niêm yết tại jollibee.com.vn để bạn tham khảo bữa trưa.",
    official_url: "https://jollibee.com.vn/thuc-don/combo-1-nguoi",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_01_JOLLIBEE_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "MỘT MÌNH ĂN NGON",
      "1 Gà Giòn Vui Vẻ + 1 Mì Ý Jolly + 1",
      "73,000 ₫"
    ],
    slot: "SLOT_1115",
    sector: "LUNCH",
    persona: ["STUDENT", "OFFICE", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "MENU_119_PHELA_SPECIALTY",
    feed_category: "PLANNING_MENU_PRICE",
    category_badge: "📋 GIÁ THAM KHẢO ĐỂ LẬP KÈO",
    brand: "Phê La Đà Nẵng",
    item_name: "Phê La Ô Long Đặc Sản Việt Nam (55k - 65k)",
    listed_price_vnd: 55000,
    display_price_badge: "55.000₫ – 65.000₫ (Menu niêm yết)",
    description: "Trà Ô Long đặc sản, Phê Phin & các dòng đồ uống thủ công truyền thống.",
    source_note: "Giá menu công khai tại cửa hàng Nguyễn Văn Thoại & Bạch Đằng, Đà Nẵng.",
    official_url: "https://phela.vn",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_07_PHELA_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "SPECIALTY TEA & COFFEE",
      "Chuyện Phê Phin Đặc Sản – Cà Đặc Sả",
      "Ô Long Đặc Sản Việt Nam"
    ],
    slot: "SLOT_1415",
    sector: "COFFEE",
    persona: ["OFFICE", "STUDENT", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "MENU_119_GONGCHA_ALISAN",
    feed_category: "PLANNING_MENU_PRICE",
    category_badge: "📋 GIÁ THAM KHẢO ĐỂ LẬP KÈO",
    brand: "Gong Cha Đà Nẵng",
    item_name: "Gong Cha Trà Alisan Kem Sữa (Size M 53k / Size L 61k)",
    listed_price_vnd: 53000,
    display_price_badge: "53.000₫ – 61.000₫ (Menu niêm yết)",
    description: "Trà Alisan đặc biệt kết hợp lớp váng sữa Milk Foam béo ngậy đặc trưng.",
    source_note: "Giá menu niêm yết công khai trên website chính thức gongcha.com.vn.",
    official_url: "https://gongcha.com.vn/thuc-uong-dac-biet-gong-cha/",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_08_GONGCHA_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "THỨC UỐNG ĐẶC BIỆT GONG CHA",
      "Trà Alisan Kem Sữa",
      "Trà Oolong Kem Sữa"
    ],
    slot: "SLOT_1415",
    sector: "COFFEE",
    persona: ["STUDENT", "OFFICE", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "MENU_119_PHUCLONG_TEA_BAKERY",
    feed_category: "PLANNING_MENU_PRICE",
    category_badge: "📋 GIÁ THAM KHẢO ĐỂ LẬP KÈO",
    brand: "Phúc Long Đà Nẵng",
    item_name: "Phúc Long Trà Đào / Trà Vải / Trà Sữa (50k - 65k)",
    listed_price_vnd: 50000,
    display_price_badge: "50.000₫ – 65.000₫ (Menu niêm yết)",
    description: "Trà truyền thống & bánh ngọt tại các cửa hàng Phúc Long Đà Nẵng.",
    source_note: "Giá menu niêm yết công khai tại phuclong.com.vn.",
    official_url: "https://phuclong.com.vn",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_11_PHUCLONG_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "Chọn Phương Thức Nhận Hàng",
      "MENU",
      "Thức uống",
      "Trà"
    ],
    slot: "SLOT_1415",
    sector: "COFFEE",
    persona: ["OFFICE", "STUDENT", "ALL"],
    added_at: "2026-08-25T21:00:00+07:00"
  },
  {
    id: "MENU_119_GOGI_HOUSE_SIGNATURE",
    feed_category: "PLANNING_MENU_PRICE",
    category_badge: "📋 GIÁ THAM KHẢO ĐỂ LẬP KÈO",
    brand: "GoGi House Đà Nẵng",
    item_name: "GoGi House Combo Thịt Nướng & Buffet Xèo Xèo (529k - 629k)",
    listed_price_vnd: 529000,
    display_price_badge: "529.000₫ (Combo GoGi Signature)",
    description: "Combo Sườn LA & Dẻ sườn bò Mỹ chuẩn Hàn Quốc tại Nguyễn Tri Phương & Vincom Đà Nẵng.",
    source_note: "Giá menu niêm yết công khai tại gogi.com.vn để bạn tham khảo trước khi đặt bàn nhóm.",
    official_url: "https://gogi.com.vn/thuc-don",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_05_GOGI_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "Combo GoGi Signature",
      "529.000",
      "Combo Sườn LA và Dẻ sườn bò Mỹ",
      "629.000"
    ],
    slot: "SLOT_2000",
    sector: "LUNCH",
    persona: ["FAMILY", "OFFICE", "ALL"],
    added_at: "2026-08-25T23:18:00+07:00"
  },
  {
    id: "UTILITY_119_DANABUS_TRANSIT",
    feed_category: "DAILY_UTILITY_SAVINGS",
    category_badge: "🚌 TIỆN ÍCH TIẾT KIỆM THƯỜNG NHẬT",
    brand: "DanaBus Xe Buýt Đà Nẵng",
    item_name: "Xe Buýt Trợ Giá Đà Nẵng (6k - 8k / Vé Tháng SV 60k)",
    listed_price_vnd: 6000,
    display_price_badge: "6.000₫ – 8.000₫ (Vé lượt trợ giá)",
    description: "Mạng lưới xe buýt thông minh kết nối các tuyến nội thành Đà Nẵng, vé tháng học sinh sinh viên chỉ 60.000₫/tháng.",
    source_note: "Biểu giá trợ giá công cộng chính thức từ Sở GTVT TP Đà Nẵng & danabus.vn.",
    official_url: "https://danabus.vn",
    evidence: {
      physical_file: "05_DEAL_AND_AFFILIATE/batch_capture_109/captures_109/TARGET_108_22_DANABUS_LEAF_01/page.txt",
      file_sha256: "",
      last_verified_at: "2026-08-25T23:25:00+07:00"
    },
    claims_to_verify: [
      "Trung tâm điều hành giao thông thông minh Đà Nẵng",
      "LỘ TRÌNH TUYẾN",
      "ĐĂNG KÝ VÉ",
      "FOOD TOUR ĐÀ NẴNG BẰNG XE BUÝT"
    ],
    slot: "SLOT_0730",
    sector: "MOBILITY",
    persona: ["STUDENT", "OFFICE", "ALL"],
    added_at: "2026-08-25T23:18:00+07:00"
  }
];

function buildDailySupplyFeed119() {
  console.log('⚡ Đang đối soát chứng cứ đĩa cho Daily Supply Feed 119...');

  // Verify all evidence on disk & compute real SHA-256
  limitedTimeDeals.forEach(d => {
    const res = verifyDiskEvidence(d.evidence.physical_file, d.claims_to_verify);
    d.evidence.file_sha256 = res.sha256;
    // Map backwards compatibility fields for internal logic
    d.tier = "VERIFIED_DEAL";
    d.tier_label = "ƯU ĐÃI CÓ HẠN";
  });
  watchlistDeals.forEach(d => {
    const res = verifyDiskEvidence(d.evidence.physical_file, d.claims_to_verify);
    d.evidence.file_sha256 = res.sha256;
    d.tier = "NEEDS_RECHECK";
    d.tier_label = "CẦN HỎI LẠI TẠI QUÁN";
  });
  planningMenuAndUtilities.forEach(m => {
    const res = verifyDiskEvidence(m.evidence.physical_file, m.claims_to_verify);
    m.evidence.file_sha256 = res.sha256;
    m.tier = "PUBLIC_MENU_PRICING";
    m.tier_label = m.feed_category === 'DAILY_UTILITY_SAVINGS' ? "TIỆN ÍCH TIẾT KIỆM THƯỜNG NHẬT" : "GIÁ THAM KHẢO ĐỂ LẬP KÈO";
  });

  const feed119 = {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    dataset_version: "119.0.0",
    generated_at: "2026-08-25T23:25:00+07:00",
    release_directive: "JAYT-119-PREMIUM-CLARITY-AND-SUPPLY-FOCUS",
    classification_policy: "PLAIN_LANGUAGE_3_GROUPS_WITH_5_ATTRIBUTE_VALIDATION",
    metrics_summary: {
      limited_time_deals_count: limitedTimeDeals.length,
      watchlist_deals_count: watchlistDeals.length,
      planning_menu_and_utilities_count: planningMenuAndUtilities.length,
      watchlist_locations_count: 26,
      local_device_notes_count: 5
    },
    // Modern Plain Language Fields
    limited_time_deals: limitedTimeDeals,
    watchlist_deals: watchlistDeals,
    planning_menu_and_utilities: planningMenuAndUtilities,
    // Internal Bridge Fields
    verified_savings: limitedTimeDeals,
    needs_recheck_deals: watchlistDeals,
    public_menu_combos: planningMenuAndUtilities
  };

  const outputPath = path.join(sotDir, 'daily_supply_feed_119.json');
  fs.writeFileSync(outputPath, JSON.stringify(feed119, null, 2), 'utf8');
  console.log(`✅ Đã tạo thành công: ${outputPath}`);
  return feed119;
}

buildDailySupplyFeed119();
