...
console.log('📊 [EVIDENCE-BUNDLE-BUILDER-088B] Xây dựng Evidence Bundles chuẩn Semantics & Relational Locality 088B...');

const bundles = [];

// 1. Metiz Cinema - Giá Ưu Đãi U22 55.000đ (Thứ Ba đến Thứ Năm)
...
if (metizPolicy && metizBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_METIZ_U22_RECURRING',
    brand: 'Metiz Cinema',
    sector: 'CINEMA',
...
if (cgvPaydayPolicy && cgvVincomBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_CGV_PAYDAY_30K_TIME_BOUNDED',
    brand: 'CGV Cinemas',
    sector: 'CINEMA',
...
if (cgvMemberPolicy && cgvVinhTrungBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_CGV_MEMBERSHIP_LOYALTY',
    brand: 'CGV Cinemas',
    sector: 'CINEMA',
...
if (metizMemberPolicy && metizBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_METIZ_MEMBER_2026_LOYALTY',
    brand: 'Metiz Cinema',
    sector: 'CINEMA',
...
if (gongChaPolicy && gongChaBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_GONGCHA_MEMBERSHIP_LOYALTY',
...
if (pheLaBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_PHELA_STORE_NETWORK',
    brand: 'Phê La',
    sector: 'COFFEE_TEA',
...
if (jollibeeBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088B_JOLLIBEE_STORE_NETWORK',
...
console.log(`\n🎉 Đã xây dựng thành công ${bundles.length} Evidence Bundles 088B!`);
const tierSummary = {};
bundles.forEach(b => tierSummary[b.tier] = (tierSummary[b.tier] || 0) + 1);
console.log('Phân bố theo Tier Chuẩn 088B:');
console.log(tierSummary);

const manifest = {
  manifest_id: 'EVIDENCE_BUNDLES_MANIFEST_088B',
...
const outPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_088b', 'evidence_bundles_088b.json');
fs.writeFileSync(outPath, JSON.stringify(manifest, null, 2), 'utf8');
console.log(`💾 Đã lưu Evidence Bundles 088B tại: ${outPath}`);
