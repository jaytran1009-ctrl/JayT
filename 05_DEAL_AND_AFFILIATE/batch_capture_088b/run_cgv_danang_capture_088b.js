...
const targets = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'selected_cgv_danang_theaters_088b.json'), 'utf8')
);

const outBaseDir = path.join(__dirname, 'captures_088b');
...
async function runCgvDaNangCapture() {
  console.log(`🚀 [CGV-DANANG-CDP-088B] Quét Chrome CDP thật 3 rạp CGV Đà Nẵng...`);
  ...
  const manifest = {
    batch_id: 'BATCH_088B_CGV_DANANG_THEATERS',
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    console.log(`\n[${i + 1}/${targets.length}] [${t.brand}] ${t.theater_name} -> ${t.resolved_url}`);
  ...
  const manifestPath = path.join(outBaseDir, 'batch_manifest_088b.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
  console.log(`\n🎉 [CGV-DANANG-COMPLETE] Đã hoàn tất quét 3 rạp CGV Đà Nẵng. Manifest lưu tại: ${manifestPath}`);
}

runCgvDaNangCapture().catch(err => {
  console.error('Fatal CGV Da Nang capture error:', err);
  process.exit(1);
});
