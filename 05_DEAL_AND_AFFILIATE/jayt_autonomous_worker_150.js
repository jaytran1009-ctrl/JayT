/**
 * JAYT REAL-SUPPLY WORKER ENGINE (150)
...
const lockFilePath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'worker_150.lock');
  ...
  if (fs.existsSync(lockFilePath)) {
    try {
      ...
      if (isProcessAlive && lockAge < LOCK_TTL_MS) {
        console.error(`⚠️ [SUPPLY-LOCK-150] Active worker already running (PID: ${lockData.pid}, Age: ${Math.round(lockAge / 1000)}s). Aborting concurrent run.`);
      ...
      console.log(`ℹ️ [SUPPLY-LOCK-150] Stale lock recovered (Reason: ${staleReceipt.stale_reason}).`);
    } catch (_) {}
  }

  const newLock = {
    worker_identifier: 'JAYT_REAL_SUPPLY_WORKER_150',
    pid: process.pid,
...
/**
 * Main Real-Supply Engine Runner 150
 */
async function runSupplyEngine150(options = {}) {
  const isSmoke = Boolean(options.isSmoke);
  const batchCap = options.limit || (isSmoke ? 2 : DEFAULT_BATCH_CAP);
  const executionOrigin = 'MANUAL_TRIGGERED'; // Honest execution origin (Workstream 1 & 4)

  console.log('========================================================================');
  console.log(`🚀 JAYT-150: RUNNING REAL-SUPPLY ENGINE (${isSmoke ? 'SMOKE_RUN' : 'MANUAL_BATCH'})`);
  console.log(`- Transparent Execution Origin: ${executionOrigin}`);
  console.log(`- Host Scheduler Status: SCHEDULER_BLOCKED_ON_THIS_HOST (No fake OS claims)`);
  ...
  try {
    ...
    // Priority Cohort Sorting (Directive JAYT-150 Workstream 4):
    ...
    const runManifest = {
      ...
      worker_identifier: 'JAYT_REAL_SUPPLY_WORKER_150',
      directive: 'JAYT-150: EXECUTION CREDIBILITY RESET & REAL-SUPPLY CONTINUITY',
      execution_mode: isSmoke ? 'SMOKE_RUN' : 'MANUAL_BATCH',
      execution_origin: executionOrigin,
      host_scheduler_status: 'SCHEDULER_BLOCKED_ON_THIS_HOST',
      executed_at: new Date().toISOString(),
      run_directory: runOutputDir,
    ...
    console.log('\n========================================================================');
    console.log(`✅ SUPPLY ENGINE 150 BATCH COMPLETED: ${executionResults.length} processed.`);
...
if (require.main === module) {
  ...
  let limit = DEFAULT_BATCH_CAP; // Default 20
  let isSmoke = false;
  ...
  runSupplyEngine150({ limit, isSmoke }).then(res => {
