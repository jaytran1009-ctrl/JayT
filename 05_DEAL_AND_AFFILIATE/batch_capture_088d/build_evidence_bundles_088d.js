...
const dir088d = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_088d', 'captures_088d');
...
console.log('📊 [EVIDENCE-BUNDLE-BUILDER-088D] Xây dựng Evidence Bundles mở rộng 088D...');
...
if (metizPolicy && metizBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_METIZ_U22_RECURRING',
    brand: 'Metiz Cinema',
    sector: 'CINEMA',
...
if (cgvPaydayPolicy && cgvVincomBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_CGV_PAYDAY_30K_TIME_BOUNDED',
    brand: 'CGV Cinemas',
    sector: 'CINEMA',
...
// 3. LOYALTY_POLICY_REFERENCE: Galaxy Cinema Membership 2026
const galaxyMemberPolicy = getArtifact(dir088c, 'TARGET_088C_589');
const galaxyDaNangBranch = getArtifact(dir088d, 'TARGET_088D_062');

if (galaxyMemberPolicy && galaxyDaNangBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_GALAXY_MEMBERSHIP_2026_LOYALTY',
    brand: 'Galaxy Cinema',
    sector: 'CINEMA',
    ...
    pieces: {
      ...
      danang_branch: {
        quote: 'Địa chỉ: Tầng 3, TTTM Co.opmart Đà Nẵng - 478 Điện Biên Phủ, Phường Thanh Khê, TP. Đà Nẵng',
        artifact_path: galaxyDaNangBranch.path,
        artifact_sha256: galaxyDaNangBranch.hash,
        branch_name: 'Galaxy Cinema Co.opmart Đà Nẵng (478 Điện Biên Phủ, Thanh Khê, Đà Nẵng)'
      }
    }
  });
}
...
if (cgvMemberPolicy && cgvVinhTrungBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_CGV_MEMBERSHIP_LOYALTY',
    brand: 'CGV Cinemas',
    sector: 'CINEMA',
...
if (metizMemberPolicy && metizBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_METIZ_MEMBER_2026_LOYALTY',
    brand: 'Metiz Cinema',
    sector: 'CINEMA',
...
if (gongChaPolicy && gongChaBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_GONGCHA_MEMBERSHIP_LOYALTY',
...
if (dominosPolicy) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_DOMINOS_PIZZA_PROMOTIONS',
...
const pheLaBranchDirect = getArtifact(dir088d, 'TARGET_088D_177');
if (pheLaBranchDirect) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_PHELA_STORE_NETWORK',
    brand: 'Phê La',
    sector: 'COFFEE_TEA',
    ...
    freshness: {
      checked_at: '2026-08-25T13:20:00+07:00',
      recheck_due_at: '2026-09-25T13:20:00+07:00'
    },
    pieces: {
      danang_branch_bach_dang: {
        quote: 'Số 36 - 38 đường Bạch Đằng, Phường Hải Châu, TP Đà Nẵng',
        artifact_path: pheLaBranchDirect.path,
        artifact_sha256: pheLaBranchDirect.hash
      }
    }
  });
}
...
const jollibeeStores = getArtifact(dir088d, 'TARGET_088D_093');
if (jollibeeStores) {
  bundles.push({
    bundle_id: 'BUNDLE_088D_JOLLIBEE_STORE_NETWORK',
    brand: 'Jollibee Vietnam',
    sector: 'FNB_FASTFOOD',
    title: 'Hệ thống cửa hàng chính thức Jollibee tại Đà Nẵng (Vincom, Tiểu La, Lý Thái Tổ, Đống Đa, MM Mega Market)',
    tier: 'SIGNAL_ONLY',
    validity_type: 'STORE_NETWORK_EVIDENCE',
    freshness: {
      checked_at: '2026-08-25T13:20:00+07:00',
    ...
    pieces: {
      danang_branch_vincom: {
        ...
        artifact_path: jollibeeStores.path,
        artifact_sha256: jollibeeStores.hash
      },
      ...
      danang_branch_ly_thai_to: {
        quote: '99 Lý Thái Tổ, Phường Thanh Khê, Thành phố Đà Nẵng',
        artifact_path: jollibeeStores.path,
        artifact_sha256: jollibeeStores.hash
      }
    }
...
console.log(`\n🎉 Đã xây dựng thành công ${bundles.length} Evidence Bundles 088D!`);
const tierSummary = {};
bundles.forEach(b => tierSummary[b.tier] = (tierSummary[b.tier] || 0) + 1);
console.log('Phân bố theo Tier Chuẩn 088D:');
console.log(tierSummary);

const manifest = {
  manifest_id: 'EVIDENCE_BUNDLES_MANIFEST_088D',
...
const outPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_088d', 'evidence_bundles_088d.json');
fs.writeFileSync(outPath, JSON.stringify(manifest, null, 2), 'utf8');
console.log(`💾 Đã lưu Evidence Bundles 088D tại: ${outPath}`);
