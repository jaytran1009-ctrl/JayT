...
const targets = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'selected_paired_targets_088d.json'), 'utf8')
);

const outBaseDir = path.join(__dirname, 'captures_088d');
...
async function runPairedCapture() {
  console.log(`🚀 [PAIRED-CDP-088D] Bắt đầu quét Chrome CDP thật ${targets.length} paired targets...`);
  ...
  const manifest = {
    batch_id: 'BATCH_088D_PAIRED_SUPPLY_EXPANSION',
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    console.log(`\n[${i + 1}/${targets.length}] [${t.target_type}] [${t.sector}] ${t.brand} -> ${t.resolved_url}`);
  ...
  const manifestPath = path.join(outBaseDir, 'batch_manifest_088d.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
  console.log(`\n🎉 [PAIRED-COMPLETE] Đã hoàn tất quét 32 paired targets. Manifest lưu tại: ${manifestPath}`);
}

runPairedCapture().catch(err => {
  console.error('Fatal paired capture error:', err);
  process.exit(1);
});
