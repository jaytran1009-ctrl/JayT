...
const targets = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'selected_batch_targets_087a.json'), 'utf8')
);

const outBaseDir = path.join(__dirname, 'captures_087a');
...
async function runReplayBatch() {
  console.log(`🚀 [REPLAY-CDP-087A] Bắt đầu quét lại CDP thật ${targets.length} targets thuần túy từ DOM lineage...`);
  ...
  const manifestPath = path.join(outBaseDir, 'batch_manifest_087a.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
  console.log(`\n🎉 [REPLAY-COMPLETE] Đã hoàn tất quét 40 targets DOM-lineage. Manifest lưu tại: ${manifestPath}`);
}

runReplayBatch().catch(err => {
  console.error('Fatal replay batch error:', err);
  process.exit(1);
});
