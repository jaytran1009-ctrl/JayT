/**
 * JAYT COMPLETE AUTONOMOUS SUPPLY ENGINE (146)
 * Directive: JAYT-146: CHUYỂN SCHEDULER TỪ SMOKE MODE SANG SUPPLY ENGINE THẬT
 * 
 * CORE PIPELINE INVARIANTS:
 * 1. Scheduled Production-Safe Cycle: Batch cap up to 20 due items, prioritized (Leaf -> Index -> Locator).
 * 2. Process Lock with TTL & Stale Recovery Receipt.
 * 3. Strict Memory Reader with ZERO string fallback.
 * 4. Immutable Unique Run Directory: runs/RUN_YYYYMMDD_HHMMSS_<nonce>/
 * 5. End-to-End Per-Run Pipeline:
 *    Capture Native -> Verify Receipt & Disk Hashes -> Parse Semantic Root -> Verify Address Locality -> Classify 5-step -> Dynamic Discovery -> Update Registry -> Emit Run Manifest.
...
const lockFilePath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'worker_146.lock');
  ...
  if (fs.existsSync(lockFilePath)) {
    try {
      ...
      if (isProcessAlive && lockAge < LOCK_TTL_MS) {
        console.error(`⚠️ [SUPPLY-LOCK] Active worker already running (PID: ${lockData.pid}, Age: ${Math.round(lockAge / 1000)}s). Aborting concurrent run.`);
      ...
      console.log(`ℹ️ [SUPPLY-LOCK] Stale lock recovered (Reason: ${staleReceipt.stale_reason}).`);
    } catch (_) {}
  }
...
/**
 * Dynamic Leaf Discovery from Source DOM
 */
function extractDiscoveredLeafUrls(htmlContent, baseUrl, brandId, brandName) {
  ...
  while ((match = hrefRegex.exec(htmlContent)) !== null) {
    ...
    try {
      ...
      const isSameHost = resolved.hostname === new URL(baseUrl).hostname;
      const pathLower = resolved.pathname.toLowerCase();

      const hasKeyword = promoKeywords.some(kw => pathLower.includes(kw));
      if (isSameHost && hasKeyword && pathLower.length > 5) {
        const cleanUrl = resolved.origin + resolved.pathname;
        const linkHash = crypto.createHash('sha256').update(cleanUrl).digest('hex').substring(0, 12);
        discovered.push({
          url: cleanUrl,
          parent_source_url: baseUrl,
          brand_id: brandId,
  ...
  return discovered.filter(d => {
    if (seen.has(d.url) || d.url === baseUrl) return false;
    seen.add(d.url);
    return true;
  });
}
...
/**
 * Main Autonomous Supply Engine Runner
 */
async function runSupplyEngine146(options = {}) {
  ...
  console.log('========================================================================');
  console.log(`🚀 JAYT-146: RUNNING AUTONOMOUS SUPPLY ENGINE (${isScheduledCycle ? 'SCHEDULED_CYCLE' : (isSmoke ? 'SMOKE_RUN' : 'BATCH_RUN')})`);
  ...
  try {
    ...
    // 2. Load Registry & Select Due Items with Prioritization
    if (!fs.existsSync(registryPath)) {
      // Copy from 145 registry if not yet exists
      const oldRegPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'autonomous_schedule_registry_145.json');
      if (fs.existsSync(oldRegPath)) {
        fs.copyFileSync(oldRegPath, registryPath);
      } else {
    ...
    console.log(`📋 Total targets registered: ${registry.items.length} | Due items: ${dueItems.length}`);
    ...
    if (dueItems.length === 0) {
      ...
      const idleReceipt = {
        run_id: runId,
        worker_identifier: 'JAYT_AUTONOMOUS_SUPPLY_WORKER_145',
        mode: isScheduledCycle ? 'SCHEDULED_CYCLE' : 'IDLE_CYCLE',
    ...
    // Prioritization: 1. Leaves -> 2. Utilities/Index -> 3. Locators
    const priorityWeight = item => {
      if (item.cohort === 'COHORT_B_OFFERS' || item.target_type?.includes('LEAF')) return 1;
      if (item.cohort === 'COHORT_C_UTILITY' || item.target_type?.includes('INDEX')) return 2;
      return 3;
    };

    dueItems.sort((a, b) => priorityWeight(a) - priorityWeight(b));
    const selectedItems = dueItems.slice(0, batchCap);
    console.log(`🎯 Prioritized & Selected ${selectedItems.length} items for this cycle.`);
    ...
    for (let i = 0; i < selectedItems.length; i++) {
      ...
      // Step E: 5-Step Classification
      let classification = 'UNKNOWN';
      if (!verifiedReceipt.is_receipt_trusted) {
        classification = 'CAPTURE_RECEIPT_INVALID';
      ...
      // Step F: Dynamic Leaf Discovery (from index or locator pages)
      if (verifiedReceipt.is_receipt_trusted && rawHtml) {
        const discovered = extractDiscoveredLeafUrls(rawHtml, verifiedReceipt.final_url || item.url, item.brand_id, item.brand_name);
        for (const disc of discovered) {
          const exists = registry.items.some(r => r.url === disc.url);
          if (!exists) {
            ...
            registry.items.push({
              ...
              last_status: 'DISCOVERED_PENDING_CAPTURE',
      ...
      // Step G: Update item in registry
      ...
      if (itemInReg) {
        ...
        if (isHttpError || isNetworkFail) {
          ...
          itemInReg.last_status = classification;
    ...
    registry.total_items = registry.items.length;
    ...
    const runManifest = {
      ...
      summary: {
        ...
        new_leaf_urls_discovered: newDiscoveriesCount,
        total_registered_targets_now: registry.items.length,
        classification_breakdown: {
          ...
          CAPTURE_RECEIPT_INVALID: executionResults.filter(r => r.classification === 'CAPTURE_RECEIPT_INVALID').length
        }
      },
    ...
    console.log('\n========================================================================');
    console.log(`✅ SUPPLY ENGINE CYCLE COMPLETED: ${executionResults.length} processed, ${newDiscoveriesCount} newly discovered.`);
    console.log(`- Run ID: ${runId}`);
    console.log(`- Run Output: ${runOutputDir}`);
    console.log(`- Staging Gate: ${stagingDecision} (${progressMilestone})`);
    console.log(`- Receipt: ${workerReceiptPath}`);
    console.log('========================================================================\n');
...
if (require.main === module) {
  ...
  runSupplyEngine146({ limit, isSmoke, isScheduledCycle }).then(res => {
