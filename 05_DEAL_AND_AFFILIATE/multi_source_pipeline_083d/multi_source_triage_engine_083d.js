/**
 * JAYT STRICT ACTIVE VALIDITY & RECEIPT SCHEMA ENGINE (083D)
 * Directive: JAYT-083D-ACTIVE-VALIDITY-AND-RECEIPT-SCHEMA
 * 
 * Fail-Closed Completeness Rules:
 * 1. MANDATORY RECEIPT SCHEMA:
 *    - `receipt_id`
 *    - `target_url` (or `source_url`)
 *    - `captured_at` (or `checked_at`)
 *    - `artifact_path`
 *    - `artifact_sha256`
 *    - `artifact_type` (strictly allowlisted)
 * 2. ALLOWLISTED ARTIFACT TYPES:
 *    `['html', 'text', 'pdf', 'dom_html', 'raw_html', 'extracted_text']`
 * 3. NO ARBITRARY SELF-DECLARED VALIDITY:
 *    Validity rule must be grounded in physical receipt metadata and verified `validity_snippet`.
 * 4. FRESHNESS TTL ENFORCEMENT:
 *    - Daily / Flash deal: capture age <= 24 hours (86,400,000 ms).
 *    - Weekly / Recurring deal: capture age <= 7 days (604,800,000 ms).
 *    - Specific valid_to deal: capture age <= 30 days (2,592,000,000 ms) AND valid_to >= refDate.
 *    - Capture exceeding TTL -> `LINEAGE_VALID_HISTORICAL_ONLY`.
 * 5. STRICT SNIPPET DISJOINTNESS:
 *    Rejects duplicates, substring overlaps, and overlapping offset ranges.
 */
...
const FRESHNESS_TTL_MS = {
  DAILY_DEAL: 24 * 60 * 60 * 1000,       // 24 hours
  WEEKLY_RECURRING: 7 * 24 * 60 * 60 * 1000, // 7 days
  SPECIFIC_VALID_TO: 30 * 24 * 60 * 60 * 1000 // 30 days
};
...
/**
 * Validates physical receipt completeness, allowlisted artifact type, and cryptographic integrity.
 */
function verifyReceiptCompletenessStrict083d(evidenceBundle) {
  ...
  // 9. Extract artifact references and artifact_type
  ...
  if (!artifactPath) {
    artifactPath = receiptData.artifact_path || receiptData.snapshot_path;
    artifactSha = receiptData.artifact_sha256 || receiptData.snapshot_sha256;
    artifactType = receiptData.artifact_type || (artifactPath ? path.extname(artifactPath).replace('.', '').toLowerCase() : null);
  }

  if (receiptData.artifact_type) {
    artifactType = receiptData.artifact_type.toLowerCase();
  }

  // 10. Mandatory allowlisted artifact_type check
  if (!artifactType || !ALLOWED_ARTIFACT_TYPES.has(artifactType)) {
    return { valid: false, failure_reason: `INVALID_OR_MISSING_ARTIFACT_TYPE: '${artifactType}'. Allowed: [${Array.from(ALLOWED_ARTIFACT_TYPES).join(', ')}]` };
  }
...
/**
 * Enforces Freshness TTL and Active Validity Rules.
 */
function checkTemporalActiveValidity083d(validitySpec, capturedAt, refDate = new Date()) {
  if (!capturedAt || isNaN(Date.parse(capturedAt))) {
  ...
  if (!validitySpec || typeof validitySpec !== 'object') {
    return { is_active: false, reason: 'NO_VALIDITY_SPEC_BOUND_TO_EVIDENCE' };
  }
  ...
  return { is_active: false, reason: 'NO_VALID_ACTIVE_SCHEDULE_OR_RECURRING_PROOF' };
}

/**
 * Strict evaluation of a single signal in 083D.
 */
function evaluateSignalStrict083d(signal, options = {}) {
  ...
  const binding = verifyReceiptCompletenessStrict083d({
    ...(evidence_bundle || {}),
  ...
  // Extract validity spec from evidence bundle (bound to verified receipt)
  const validitySpec = evidence_bundle?.validity_spec || binding.receipt_data?.validity_spec;
  const temporalCheck = checkTemporalActiveValidity083d(
    validitySpec,
    binding.captured_at,
...
module.exports = {
  ...
  FRESHNESS_TTL_MS,
  normalizeUrlStrict,
  verifyReceiptCompletenessStrict083d,
  validateDisjointSnippets,
  checkTemporalActiveValidity083d,
  evaluateSignalStrict083d
};
