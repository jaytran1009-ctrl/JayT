/**
 * JAYT MULTI-TIER DATA PRESERVATION & REAL-VALUE SUPPLY ENGINE (155)
 ...
 * 3. Workstream A Containment: 154 quarantined, zero post-run mutation on sealed artifacts.
 * 4. Workstream B Multi-Tier Data Preservation:
 *    - Equation 1: raw_anchors_observed = policy_rejected + canonical_candidates_before_dedupe
 *    - Equation 2: canonical_candidates_before_dedupe = canonical_duplicates + unique_eligible_leaves
 *    - Zero manual tinkering. If mismatch, exit RECONCILIATION_FAILED.
 * 5. Workstream C Value-Based Selection:
 *    - Strictly filter candidates having explicit offer signals (price/discount, terms, validity, scope).
 *    - Strictly exclude menus, homepages, trailers, PR, generic news, language switches, navigation, expired, traffic news without subsidies.
 * 6. Workstream D Real-Value Stratified Allocation (Max 10/cohort, Max 3/brand):
 *    - Honest shortfall reporting; zero backfilling with junk.
 * 7. Workstream E 5-State Resolution Taxonomy:
...
const lockFilePath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'worker_155.lock');
  ...
  if (fs.existsSync(lockFilePath)) {
    try {
      ...
      if (isProcessAlive && lockAge < LOCK_TTL_MS) {
        console.error(`⚠️ [SUPPLY-LOCK-155] Active worker already running (PID: ${lockData.pid}, Age: ${Math.round(lockAge / 1000)}s). Aborting concurrent run.`);
      ...
      console.log(`ℹ️ [SUPPLY-LOCK-155] Stale lock recovered (Reason: ${staleReceipt.stale_reason}).`);
    } catch (_) {}
  }

  const newLock = {
    worker_identifier: 'JAYT_VALUE_SUPPLY_WORKER_155',
    pid: process.pid,
...
/**
 * Strict Policy Negative Filter for URL Link Discovery
 */
function isRejectedByNegativePolicy(rawUrl, parentUrl, anchorText, surroundingText) {
  ...
  try {
    ...
    // 2. System, Auth, Cart, Navigation paths
    const forbiddenPaths = [
      ...
      '/menu', '/menu/', '/thuc-don', '/thuc-don/', '/product', '/product/',
      '/help', '/help/', '/faq', '/faq/'
    ];
    ...
    // 4. Strict Content Filters: Movies, Trailers, PR, Language Switch, Empty Labels
    const combinedContent = `${anchorText} ${surroundingText} ${rawUrl}`.toLowerCase();
    if (/(?:trailer|review|bom tấn|phim hay|đạo diễn|diễn viên|chiếu sớm|lịch chiếu|nhện nhọ|moana|minions|hulk|zombie|quang tuấn|lầu chú hỏa)/i.test(combinedContent)) {
      return { rejected: true, reason: 'NOISE_FILTER_MOVIE_TRAILER_OR_SYNOPSIS' };
    }
    if (/(?:khai trương|flagship|hà nội|hồ chí minh|sài gòn|câu chuyện|thương hiệu|chúc mừng|kỷ niệm|trao tặng)/i.test(combinedContent)) {
      return { rejected: true, reason: 'NOISE_FILTER_BRAND_PR_OR_STORE_OPENING' };
    }
    if (/^(?:tin tức|ưu đãi|xem thêm|chi tiết|read more|learn more|get template|here|students|teachers|chuyển sang tiếng việt)$/i.test(anchorText.trim())) {
      return { rejected: true, reason: 'NOISE_FILTER_GENERIC_LABEL_OR_NAV_BUTTON' };
    }
    if (/(?:lộ trình nâng tầm giao thông|bài toán dịch chuyển|số hóa và đồng bộ)/i.test(combinedContent)) {
      return { rejected: true, reason: 'NOISE_FILTER_TRAFFIC_NEWS_WITHOUT_SUBSIDY' };
    }
...
/**
 * Strict Offer Value Signal Evaluator (Workstream C)
 */
function evaluateOfferValueSignals(anchorText, surroundingDomText, canonicalUrl) {
  const text = `${anchorText} ${surroundingDomText} ${canonicalUrl}`.toLowerCase();
  
  // Signal 1: Price / Discount / Gift
  const hasPriceOrDiscount = /(?:\d{1,3}(?:\.\d{3})+|\d+k|\d+đ|vnd|đồng|giá chỉ|chỉ từ|\d+%\s*giảm|giảm\s*\d+%|mua\s*1\s*tặng\s*1|free|miễn\s*phí|tặng\s*kèm|voucher|coupon|cashback|combo)/i.test(text);
  
  // Signal 2: Terms / Membership / Student
  const hasTermsOrAudience = /(?:sinh viên|học sinh|u22|thành viên|member|student|hssv|education|starter-pack|áp dụng cho)/i.test(text);

  // Signal 3: Time / Date / Validity
  const hasValidityDate = /(?:áp dụng từ|đến ngày|hạn sử dụng|từ ngày|áp dụng đến|\d{1,2}\/\d{1,2}|\d{1,2}\.\d{1,2}|hàng tuần|thứ 2|thứ 3|thứ 4|thứ 5|thứ 6|thứ 7|chủ nhật|tháng|2\/9)/i.test(text);

  // Signal 4: Scope / Brand
  const hasScope = /(?:đà nẵng|toàn quốc|hệ thống|chi nhánh|cơ sở|áp dụng tại)/i.test(text);

  let score = 0;
  if (hasPriceOrDiscount) score += 40;
  if (hasTermsOrAudience) score += 30;
  if (hasValidityDate) score += 20;
  if (hasScope) score += 10;

  const isHighQualityOfferCandidate = hasPriceOrDiscount || (hasTermsOrAudience && (hasValidityDate || hasScope));

  return {
    is_offer_candidate: isHighQualityOfferCandidate,
    score,
    signals: {
      has_price_discount: hasPriceOrDiscount,
      has_terms_audience: hasTermsOrAudience,
      has_validity_date: hasValidityDate,
      has_scope: hasScope
    }
  };
}
...
/**
 * Strict Multi-Tier Discovery Scanner
 */
async function scanMultiTierLineageLinks(browser, htmlContent, pageUrl, parentReceiptPath, brandId, brandName, cohort151) {
  ...
  try {
    ...
    for (const item of rawAnchors) {
      ...
      const policyRes = isRejectedByNegativePolicy(item.raw_href, pageUrl, item.anchor_text, item.surrounding_dom_text);
      if (policyRes.rejected) {
        policyRejected.push({ raw_href: item.raw_href, reason: policyRes.reason });
        continue;
      }

      const valueSignals = evaluateOfferValueSignals(item.anchor_text, item.surrounding_dom_text, policyRes.canonicalUrl);
      if (!valueSignals.is_offer_candidate) {
        policyRejected.push({ raw_href: item.raw_href, reason: 'NO_OFFER_VALUE_SIGNALS_IN_DOM_CONTEXT' });
        continue;
      }
      ...
      canonicalCandidates.push({
        ...
        offer_score: valueSignals.score,
        signals: valueSignals.signals,
...
/**
 * Main Value Supply Engine Runner 155
 */
async function runSupplyEngine155() {
  const executionOrigin = 'MANUAL_TRIGGERED';

  console.log('========================================================================');
  console.log('🚀 JAYT-155: MULTI-TIER DATA PRESERVATION & REAL-VALUE SUPPLY ENGINE');
  console.log(`- Transparent Execution Origin: ${executionOrigin}`);
  console.log(`- Host Scheduler Status: SCHEDULER_BLOCKED_ON_THIS_HOST`);
  console.log(`- Strict Multi-Tier Invariance: raw_anchors = rejected + candidates_before_dedupe`);
  console.log('========================================================================\n');
  ...
  try {
    ...
    console.log('\n--- WORKSTREAM B: MULTI-TIER DISCOVERY ACROSS 32 ROOTS + PROMO SOURCES ---');
    ...
    for (let i = 0; i < uniqueDiscoverySources.length; i++) {
      ...
      if (!isError && rawHtml) {
        ...
        const scanRes = await scanMultiTierLineageLinks(browser, rawHtml, verifiedReceipt.final_url || src.root_url, receiptJsonPath, src.brand_id, src.brand_name, src.cohort_151);
    ...
    // --- WORKSTREAM B: MATHEMATICAL DEDUPLICATION & MULTI-TIER RECONCILIATION ---
    ...
    console.log('\n--- WORKSTREAM B: MULTI-TIER DATA PRESERVATION AUDIT ---');
    console.log(`- Tier 1: Raw Anchors Observed: ${totalRawAnchorsObserved}`);
    console.log(`- Tier 2: Policy Rejected: ${totalPolicyRejected}`);
    ...
    console.log(`- Tier 5: Unique Eligible Offer Leaves: ${uniqueEligibleLeaves.length}`);
    ...
    // Persist Discovery Lineage Ledger 155
    const ledgerData = {
      ledger_id: 'DISCOVERY_LINEAGE_LEDGER_155',
      directive: 'JAYT-155: KHÔI PHỤC TÍNH TOÀN VẸN & BATCH NGUỒN CUNG GIÁ TRỊ THẬT',
    ...
    console.log(`💾 Persisted sealed Discovery Lineage Ledger 155 to: ${ledgerPath}`);

    // --- WORKSTREAM D: REAL VALUE STRATIFIED SELECTION (UP TO 30 LEAVES, MAX 3/BRAND) ---
    console.log('\n--- WORKSTREAM D: REAL VALUE STRATIFIED SELECTION (MAX 10/COHORT, MAX 3/BRAND) ---');
    
    function selectRealValueLeaves(candidates, cohortName, maxCohort, maxBrand) {
      const cohortCandidates = candidates
        .filter(c => c.cohort_151 === cohortName)
        .sort((a, b) => b.offer_score - a.offer_score);
    ...
    const stratA = selectRealValueLeaves(uniqueEligibleLeaves, 'COHORT_A_CINEMA_ENTERTAINMENT', COHORT_CAP, BRAND_CAP);
    const stratB = selectRealValueLeaves(uniqueEligibleLeaves, 'COHORT_B_FNB_COFFEE', COHORT_CAP, BRAND_CAP);
    const stratC = selectRealValueLeaves(uniqueEligibleLeaves, 'COHORT_C_TRANSIT_STUDENT', COHORT_CAP, BRAND_CAP);

    const stratifiedBatch = [...stratA.selected, ...stratB.selected, ...stratC.selected];
    console.log(`📊 Real Value Stratified Allocation Plan:`);
    ...
    console.log(`- Total Real-Value Batch Size: ${stratifiedBatch.length} leaves (Zero backfill with junk)`);

    // --- WORKSTREAM E: LEAF RE-CAPTURE & 5-STATE EVIDENCE RESOLUTION ---
    console.log(`\n--- WORKSTREAM E: LEAF RE-CAPTURE & 5-STATE EVIDENCE RESOLUTION (${stratifiedBatch.length} LEAVES) ---`);
    const leafResults = [];
    ...
    for (let j = 0; j < stratifiedBatch.length; j++) {
      ...
      console.log(`[${j + 1}/${stratifiedBatch.length}] [${leaf.cohort_151}] (Score: ${leaf.offer_score}) Re-capturing ${leaf.brand_name} -> ${leaf.canonical_url} ("${leaf.anchor_text.substring(0, 40)}")`);
      ...
      const targetDef = {
        ...
        target_type: 'REAL_VALUE_OFFER_LEAF',
    ...
    const registry155Data = {
      registry_id: 'AUTONOMOUS_SCHEDULE_REGISTRY_155',
      directive: 'JAYT-155: KHÔI PHỤC TÍNH TOÀN VẸN & BATCH NGUỒN CUNG GIÁ TRỊ THẬT',
      created_at: new Date().toISOString(),
    ...
    fs.writeFileSync(registryPath, JSON.stringify(registry155Data, null, 2), 'utf8');
    ...
    const runManifest = {
      ...
      worker_identifier: 'JAYT_VALUE_SUPPLY_WORKER_155',
      directive: 'JAYT-155: KHÔI PHỤC TÍNH TOÀN VẸN & BATCH NGUỒN CUNG GIÁ TRỊ THẬT',
      execution_mode: 'REAL_VALUE_SUPPLY_CAMPAIGN',
      execution_origin: executionOrigin,
      host_scheduler_status: 'SCHEDULER_BLOCKED_ON_THIS_HOST',
      executed_at: new Date().toISOString(),
      run_directory: runOutputDir,
      project_memory_verified: memory,
      workstream_a_containment_status: '154_MUTATIONS_QUARANTINED_AND_DISCLOSED',
      workstream_b_multi_tier_preservation: ledgerData.reconciliation,
    ...
    console.log('\n========================================================================');
    console.log(`✅ REAL-VALUE SUPPLY CAMPAIGN 155 COMPLETED:`);
    ...
    console.log(`- Unique Eligible Leaves in Ledger: ${uniqueEligibleLeaves.length}`);
    console.log(`- Real-Value Leaves Re-captured: ${leafResults.length} across 3 cohorts (Max ${BRAND_CAP}/brand)`);
...
module.exports = {
  ...
  releaseLock,
  readMemoryStrict,
  isRejectedByNegativePolicy,
  evaluateOfferValueSignals,
  scanMultiTierLineageLinks,
  runSupplyEngine155
};
