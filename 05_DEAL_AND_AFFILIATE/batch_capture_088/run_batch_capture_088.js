...
const targets = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'selected_policy_targets_088.json'), 'utf8')
);

const outBaseDir = path.join(__dirname, 'captures_088');
...
async function runPolicyBatch() {
  console.log(`🚀 [POLICY-CDP-088] Bắt đầu quét Chrome CDP thật ${targets.length} policy/pricing targets...`);
  ...
  const manifest = {
    batch_id: 'BATCH_088_POLICY_EVIDENCE_RESOLUTION',
  ...
  const manifestPath = path.join(outBaseDir, 'batch_manifest_088.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
  console.log(`\n🎉 [POLICY-COMPLETE] Đã hoàn tất quét 31 policy targets. Manifest lưu tại: ${manifestPath}`);
}

runPolicyBatch().catch(err => {
  console.error('Fatal policy batch error:', err);
  process.exit(1);
});
