/**
 * RETENTION SUPPLY EXTRACTOR 122
 * Directive: JAYT-122-INTENT-DIVERSITY-AND-PREMIUM-MOMENT
 * Enhancements:
 * 1. Output daily_supply_feed_122.json (15 items, verified disk evidence)
 * 2. Output supply_gap_board_122.json (25 cells, 10 covered - 40.0% actionable coverage)
 * 3. Rich intent metadata (intent_type: 'ENTERTAINMENT' | 'DINING_MEETING' | 'MOBILITY_SHOPPING')
 */
...
function verifyDiskEvidence(relPath, requiredSubstrings = []) {
  ...
  return {
    ...
    sha256: hash,
    size_bytes: size,
    verified_at: '2026-08-26T00:03:00+07:00'
  };
}

console.log('🔍 [EXTRACTOR-122] Đang xác minh dữ liệu và đối soát SHA-256 trên đĩa...\n');
...
// Write daily_supply_feed_122.json to 03_SOURCE_OF_TRUTH
const feedOutputPath = path.join(repoRoot, '03_SOURCE_OF_TRUTH', 'daily_supply_feed_122.json');
fs.writeFileSync(feedOutputPath, JSON.stringify(dailyFeed122, null, 2), 'utf8');
console.log(`✅ [FEED-122] Đã tạo daily_supply_feed_122.json (${dailyFeed122.summary.total_items} mục, 40% coverage)`);

// 4. Build Supply Gap Board 122 (25 Cells Matrix)
const slots = [
  { id: 'SLOT_0730', label: '07:30 (Sáng)', question: 'Khởi đầu ngày mới: Cà phê sáng, xe buýt trợ giá hay bữa ăn nhanh?' },
  { id: 'SLOT_1115', label: '11:15 (Trưa)', question: 'Ăn nhanh, ăn nhóm hay uống cà phê?' },
  { id: 'SLOT_1415', label: '14:15 (Chiều)', question: 'Gặp bạn, học nhóm hay nghỉ giải lao?' },
  { id: 'SLOT_1730', label: '17:30 (Tan ca)', question: 'Tan làm: Ăn tối cùng đồng nghiệp, mua sắm hay xe về nhà?' },
...
const gapBoard122 = {
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  board_version: '122.0.0',
  release_directive: 'JAYT-122-INTENT-DIVERSITY-AND-PREMIUM-MOMENT',
  ...
  high_priority_target_gaps: [
    {
      gap_id: 'GAP_122_01_LUNCH_MOBILITY',
      slot: 'SLOT_1115',
      sector: 'MOBILITY',
      priority: 'HIGH',
    ...
    {
      gap_id: 'GAP_122_02_LATE_NIGHT_RIDE',
      slot: 'SLOT_2000',
      sector: 'MOBILITY',
      priority: 'HIGH',
    ...
    {
      gap_id: 'GAP_122_03_AFTERNOON_SNACK',
      slot: 'SLOT_1415',
      sector: 'LUNCH',
      priority: 'HIGH',
...
const gapBoardOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'supply_gap_board_122.json');
fs.writeFileSync(gapBoardOutputPath, JSON.stringify(gapBoard122, null, 2), 'utf8');
console.log(`✅ [GAP-BOARD-122] Đã tạo supply_gap_board_122.json (25 cells, ${coveredCount} covered - 40.0% coverage)\n`);
