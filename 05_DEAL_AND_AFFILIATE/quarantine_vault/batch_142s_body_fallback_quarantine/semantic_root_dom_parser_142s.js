/**
 * JAYT SEMANTIC-ROOT LEAF DOM PARSER (142S)
 * Directive: JAYT-142S: SEMANTIC-ROOT RECOVERY & RECEIPT-TRUST REBUILD
 * 
 * STRICT ARCHITECTURAL CONSTRAINTS:
 * 1. Strictly identifies an isolated `content_root` container (main, article, promo container).
 * 2. Mandatorily excludes: header, nav, footer, sidebar, modal, booking widget (Mua vé nhanh, Quick buy), mega-menu, hidden elements.
 * 3. Title, price, discount, schedule, validity, and scope MUST ALL BE DESCENDANTS of the same `content_root`.
 * 4. If no valid `content_root` exists -> returns NON_OFFER_PAGE_OR_SHELL (0 extracted fields).
 * 5. Rejects extracting global price nodes (e.g. "Miễn phí" in quick-buy booking bar).
 */
...
/**
 * Browser evaluation function executed inside Puppeteer
 */
function browserParseSemanticRootLeaf() {
  ...
  // 1. Remove all forbidden global UI widgets directly before content_root identification
  ...
  // Clone document to avoid modifying original if needed, but in evaluate we work directly on DOM
  for (const sel of forbiddenSelectors) {
    document.querySelectorAll(sel).forEach(el => el.remove());
  }

  // 2. Identify candidate content_root
  const rootCandidates = Array.from(document.querySelectorAll('main, article, [class*="article"], [class*="content"], [class*="detail"], [class*="promo"], [class*="post"]'));
  let contentRoot = null;

  for (const el of rootCandidates) {
    const text = normalize(el.innerText);
    // Must contain substantial content (at least 80 chars) and contain a heading
    if (text.length >= 80 && el.querySelector('h1, h2, h3')) {
      contentRoot = el;
      break;
    }
  }

  // Fallback: If no dedicated container, check if body has substantial content with heading
  if (!contentRoot) {
    const bodyText = normalize(document.body.innerText);
    if (bodyText.length >= 100 && document.querySelector('h1, h2')) {
  ...
  for (const el of titleCandidates) {
    const text = normalize(el.innerText);
    if (text.length >= 5 && text.length <= 150 && !/^(mua vé nhanh|đăng nhập|giỏ hàng|error)/i.test(text)) {
      titleNode = {
 ...
 */
async function parseLeafSemanticRootNative(html, leafUrl, receiptTruth, browser) {
  const page = await browser.newPage();
  await page.setRequestInterception(true);
  page.on('request', req => req.abort());
  await page.setContent(html, { waitUntil: 'domcontentloaded', timeout: 5000 });

  const rawDom = await page.evaluate(browserParseSemanticRootLeaf);
  await page.close();
...
module.exports = {
  computeSha256,
  browserParseSemanticRootLeaf,
  parseLeafSemanticRootNative
};
