/**
 * JAYT STRICT SEMANTIC-ROOT LEAF DOM PARSER (143)
 ...
 * 2. Excludes header, nav, footer, aside, booking widgets, menus, popups, modals.
 * 3. Requires authentic article/main container with verified headline matching page scope.
 * 4. Yields NON_OFFER_PAGE_OR_SHELL when no valid isolated content_root exists.
 */
 ...
 */
function browserParseStrictSemanticRoot143() {
  ...
  // 1. Purge global widgets, navigation, footers, popups, and booking bars
  ...
  for (const el of titleCandidates) {
    ...
    if (text.length >= 5 && text.length <= 150 && !/^(tin liên quan|mua vé nhanh|đăng nhập|giỏ hàng|error)/i.test(text)) {
      titleNode = { tag: el.tagName.toLowerCase(), selector: getDomSelector(el), outer_html: el.outerHTML, text: text };
      break;
    }
  }
 ...
 */
async function parseStrictSemanticRootLeaf143(html, leafUrl, receiptTruth, browser) {
  const page = await browser.newPage();
  await page.setRequestInterception(true);
  page.on('request', req => req.abort());
  await page.setContent(html, { waitUntil: 'domcontentloaded', timeout: 5000 });

  const rawDom = await page.evaluate(browserParseStrictSemanticRoot143);
  await page.close();
...
module.exports = {
  computeSha256,
  browserParseStrictSemanticRoot143,
  parseStrictSemanticRootLeaf143
};
