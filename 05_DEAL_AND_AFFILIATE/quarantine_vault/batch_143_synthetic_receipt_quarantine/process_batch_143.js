/**
 * JAYT BATCH 143 PROCESSING RUNNER (NATIVE RECEIPT TRUTH & STRICT 5-STEP ORDER)
...
const { verifyStrictRawCaptureReceipt143 } = require('./strict_receipt_truth_verifier_143');
const { verifyNormalizedAddressUnits143 } = require('./address_unit_locality_verifier_143');
const { parseStrictSemanticRootLeaf143 } = require('./strict_semantic_root_dom_parser_143');
...
const brandRegistryOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'brand_locality_registry_143.json');
const batchTableOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_143_table.json');
const registry143Path = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'fresh_source_registry_143.json');

async function processBatch143() {
  console.log('========================================================================');
  console.log('🔬 JAYT-143: EVALUATING FRESH CAPTURES WITH NATIVE RECEIPT TRUTH');
  console.log('========================================================================\n');
  ...
  for (const item of localityItems) {
    ...
    const receiptTruth = verifyStrictRawCaptureReceipt143(folder);
    const html = fs.existsSync(path.join(folder, 'page.html')) ? fs.readFileSync(path.join(folder, 'page.html'), 'utf8') : '';

    const localityEval = await verifyNormalizedAddressUnits143(html, item.url, receiptTruth, browser);
  ...
  const brandRegistry = {
    registry_id: 'BRAND_LOCALITY_REGISTRY_143',
    directive: 'JAYT-143: FRESH-CAPTURE RESET & AUTONOMOUS VERIFIED-SUPPLY LOOP',
    ...
    locality_distribution: {
      ...
      LOCALITY_PENDING_RECEIPT_VALIDATION: brandLocalityResults.filter(b => b.locality_status === 'LOCALITY_PENDING_RECEIPT_VALIDATION').length
    },
    receipt_trust_audit: {
  ...
  for (const item of nonLocatorItems) {
    ...
    const parsed = await parseStrictSemanticRootLeaf143(html, item.url, receiptTruth, browser);
    ...
    if (collidingCaptureIds.has(item.capture_id)) {
      ...
      classificationReason = 'Raw capture receipt is missing mandatory network/provenance fields.';
    } else if (receiptTruth.http_status >= 400) {
      // HTTP Error / Blocked
  ...
  const batchTable143 = {
    batch_id: 'BATCH_CAPTURE_143_TABLE',
    directive: 'JAYT-143: FRESH-CAPTURE RESET & AUTONOMOUS VERIFIED-SUPPLY LOOP',
    generated_at: new Date().toISOString(),
    ...
    governance_statement: 'Capture mới hoàn toàn 143 với receipt ghi trực tiếp từ browser network events, chuẩn hóa address units và áp dụng đúng 5 bước phân loại. Zero candidate staging hay live deploy.',
    items: evaluatedLeaves
  };

  fs.writeFileSync(batchTableOutputPath, JSON.stringify(batchTable143, null, 2), 'utf8');

  // Build Fresh Source Registry 143 (21 Brand Sources Monitored)
  const sources143 = brandLocalityResults.map((b, idx) => ({
    source_id: `SRC_143_${String(idx + 1).padStart(2, '0')}`,
    brand_id: b.brand_id,
    brand_name: b.brand_name,
    canonical_url: b.locator_url,
    state: b.raw_receipt_truth.http_status >= 400 ? 'HTTP_ERROR_BACKOFF' : 'OFFICIAL_SOURCE_MONITORED',
    http_status: b.raw_receipt_truth.http_status,
    backoff_policy: b.raw_receipt_truth.http_status >= 400 ? '7_DAYS_BACKOFF' : 'NONE',
    next_check_due: '2026-09-03T02:00:00.000Z'
  }));

  const registry143 = {
    registry_id: 'FRESH_SOURCE_REGISTRY_143',
    directive: 'JAYT-143: FRESH-CAPTURE RESET & AUTONOMOUS VERIFIED-SUPPLY LOOP',
    generated_at: new Date().toISOString(),
    governance_statement: `21 official brand sources monitored in Batch 143 across cinema, dining, tea/coffee, entertainment, and public utilities.`,
    sources: sources143
  };

  fs.writeFileSync(registry143Path, JSON.stringify(registry143, null, 2), 'utf8');

  console.log('\n========================================================================');
  console.log(`✅ 143 PROCESSING COMPLETE: ${evaluatedLeaves.length} EVALUATED.`);
  ...
  console.log(`- Metric Conservation Check: ${batchTable143.conservation_check} == ${evaluatedLeaves.length}`);
  console.log(`📂 Output Brand Registry: ${brandRegistryOutputPath}`);
  console.log(`📂 Output Batch Table: ${batchTableOutputPath}`);
  console.log(`📂 Output Fresh Source Registry: ${registry143Path}`);
  console.log('========================================================================\n');
}
