...
(function() {
  ...
  // --- PRESET SCENARIOS & CONTEXT INTELLIGENCE ---
  const CALC_PRESETS = [
    { name: '🎬 Galaxy Happy Day (T3)', item: 50000, voucher: 0, ship: 0, surcharge: 0, desc: 'Xem Phim Thứ Ba (Galaxy Cinema)' },
    { name: '☕ Cà phê Gặp gỡ Đà Nẵng', item: 45000, voucher: 10000, ship: 0, surcharge: 0, desc: 'Kèo Cà phê Bạn bè Đà Nẵng' },
    { name: '🍗 Ăn trưa Đồng nghiệp (F&B)', item: 80000, voucher: 20000, ship: 15000, surcharge: 0, desc: 'Ăn Trưa Combo F&B' },
    { name: '🛍️ Đặt món Giao tận nơi (App)', item: 120000, voucher: 30000, ship: 20000, surcharge: 0, desc: 'Đặt Món Giao Hàng App' }
  ];

  const DISTRICT_TIPS = {
    'Hải Châu': 'Khu vực trung tâm, tập trung rạp chiếu phim, chuỗi ẩm thực & cà phê ven sông Hàn.',
    'Thanh Khê': 'Gần trục Điện Biên Phủ, thuận tiện xem phim & ăn uống cùng bạn bè.',
    'Sơn Trà': 'Khu vực bán đảo & ven biển Mỹ Khê, nhộn nhịp ẩm thực đêm & cà phê view đẹp.',
    'Ngũ Hành Sơn': 'Gần Làng Đại Học & bãi biển Non Nước, nhiều hàng quán ưu đãi sinh viên.',
    'Liên Chiểu': 'Khu vực Bách Khoa, Sư Phạm & KCN Hòa Khánh, mức giá tiết kiệm cho sinh viên.',
    'Cẩm Lệ': 'Khu dân cư nhộn nhịp gần cầu Đỏ, nhiều quán ăn gia đình & cà phê sân vườn.'
  };

  const RHYTHM_TIPS = {
    'RHYTHM_MORNING': 'Khởi đầu ngày mới với cà phê & điểm tâm Đà Nẵng.',
    'RHYTHM_LUNCH': 'Nghỉ trưa & ăn trưa đồng nghiệp với các combo tiết kiệm.',
    'RHYTHM_AFTERNOON': 'Cà phê trà chiều & nạp năng lượng sau giờ làm/học.',
    'RHYTHM_AFTER_WORK': 'Tan ca rủ bạn xem phim, dạo phố & ăn tối thư giãn.'
  };
  ...
  function renderHero() {
    ...
    const currentDistrictTip = DISTRICT_TIPS[state.selectedDistrict] || 'Đà Nẵng';
    const currentRhythmTip = RHYTHM_TIPS[state.selectedRhythm] || 'Lịch ưu đãi định kỳ';
    ...
    return `
      <section class="apex-hero-card">
        ...
        <p class="apex-hero-desc">
          Theo dõi lịch ưu đãi định kỳ từ nguồn công khai cho ${state.persona === 'student' ? 'sinh viên' : state.persona === 'office' ? 'người đi làm' : 'mọi người'} Đà Nẵng. Không sử dụng dữ liệu suy đoán.<br>
          <span style="display:inline-block;margin-top:4px;color:var(--gold-champagne-light);font-size:11.5px;">📍 ${currentDistrictTip} · ${currentRhythmTip}</span>
        </p>
  ...
  function renderHonestEmptyState() {
    return `
      <div class="apex-empty-state-card">
        ...
        <div class="apex-empty-title">JAYT PUBLIC BETA — MINH BẠCH & BẢO VỆ NGƯỜI DÙNG</div>
    ...
    return `
      <section class="apex-calc-card">
        ...
        <div style="margin-bottom:14px;">
          <label class="apex-input-label">⚡ Chọn nhanh kịch bản chi tiêu:</label>
          <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px;">
            ${CALC_PRESETS.map((p, idx) => `
              <button class="apex-calc-preset-btn" data-preset-idx="${idx}" type="button">
                ${p.name}
              </button>
            `).join('')}
          </div>
...
👉 Xem đối soát minh bạch tại JayT Đà Nẵng!`;
    ...
    return `
      <section class="apex-group-plan-box">
        <div class="apex-section-header">
          <div class="apex-section-title">✨ Kế Hoạch Nhóm & Rủ Bạn (Group Plan Builder)</div>
          <div class="apex-section-sub">Tùy biến nội dung kèo đi chơi và chia sẻ tin nhắn chuẩn đẹp</div>
        </div>

        <div class="apex-group-plan-grid">
          <div class="apex-plan-edit-pane">
            <div style="font-size:11.5px;font-weight:800;color:var(--pine-forest-main);margin-bottom:10px;text-transform:uppercase;">
              ✏️ TÙY BIẾN CHI TIẾT KÈO ĐI CHƠI
            </div>
            <div class="apex-input-item">
              <label class="apex-input-label">Tên kèo / Nội dung</label>
              <input type="text" class="apex-text-field" id="in-gp-title" value="${esc(gp.title)}">
            </div>
            <div class="apex-input-item">
              <label class="apex-input-label">Thời gian</label>
              <input type="text" class="apex-text-field" id="in-gp-time" value="${esc(gp.timeSlot)}">
            </div>
            <div class="apex-input-item">
              <label class="apex-input-label">Địa điểm chi nhánh</label>
              <input type="text" class="apex-text-field" id="in-gp-location" value="${esc(gp.location)}">
            </div>
            <div class="apex-input-item">
              <label class="apex-input-label">Hoạt động cụ thể</label>
              <input type="text" class="apex-text-field" id="in-gp-activity" value="${esc(gp.activity)}">
            </div>
            <div class="apex-input-item">
              <label class="apex-input-label">Điều kiện cần chuẩn bị trước</label>
              <input type="text" class="apex-text-field" id="in-gp-conditions" value="${esc(gp.conditions)}">
            </div>
          </div>

          <div class="apex-plan-preview-pane">
            <div>
              <div style="font-size:11.5px;font-weight:800;color:var(--pine-forest-main);margin-bottom:10px;text-transform:uppercase;">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;width:100%;margin-top:10px;">
              ...
              <button class="apex-btn-outline" id="btn-copy-plan-url">🔗 Sao Chép Liên Kết Kế Hoạch (URL Hash)</button>
  ...
  // --- MAIN MOUNT ---
  function mount() {
    // Sync tab or plan payload from URL hash if present
    try {
      const hash = window.location.hash.replace('#', '');
      if (hash.startsWith('plan=')) {
        const planJson = decodeURIComponent(hash.substring(5));
        const parsedPlan = JSON.parse(planJson);
        if (parsedPlan && typeof parsedPlan === 'object') {
          if (parsedPlan.title) state.groupPlan.title = parsedPlan.title;
          if (parsedPlan.timeSlot) state.groupPlan.timeSlot = parsedPlan.timeSlot;
          if (parsedPlan.location) state.groupPlan.location = parsedPlan.location;
          if (parsedPlan.activity) state.groupPlan.activity = parsedPlan.activity;
          if (parsedPlan.conditions) state.groupPlan.conditions = parsedPlan.conditions;
          state.activeNav = 'group_plan';
        }
      } else if (['dashboard', 'schedule_7d', 'radar', 'calculator', 'group_plan', 'staging_review'].includes(hash)) {
  ...
  function bindEvents() {
    ...
    // Calculator Preset clicks
    document.querySelectorAll('.apex-calc-preset-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const idx = Number(btn.getAttribute('data-preset-idx'));
        const preset = CALC_PRESETS[idx];
        if (preset) {
          state.calculator.scenario_name = preset.desc || preset.name;
          state.calculator.item_price = preset.item;
          state.calculator.voucher_discount = preset.voucher;
          state.calculator.shipping_fee = preset.ship;
          state.calculator.surcharge = preset.surcharge;
          mount();
          showToast(`⚡ Đã chọn: ${preset.name}`);
        }
      });
    });
    ...
    // Group Plan input live sync
    const inGpTitle = document.getElementById('in-gp-title');
    const inGpTime = document.getElementById('in-gp-time');
    const inGpLocation = document.getElementById('in-gp-location');
    const inGpActivity = document.getElementById('in-gp-activity');
    const inGpConditions = document.getElementById('in-gp-conditions');

    if (inGpTitle) {
      const updateGroupPlanPreview = () => {
        state.groupPlan.title = inGpTitle.value;
        state.groupPlan.timeSlot = inGpTime ? inGpTime.value : '';
        state.groupPlan.location = inGpLocation ? inGpLocation.value : '';
        state.groupPlan.activity = inGpActivity ? inGpActivity.value : '';
        state.groupPlan.conditions = inGpConditions ? inGpConditions.value : '';

        const totalCalc = Math.max(0, state.calculator.item_price - state.calculator.voucher_discount + state.calculator.shipping_fee + state.calculator.surcharge);
        const shareText = `[JAYT ĐÀ NẴNG 43 - KẾ HOẠCH ĐI CHƠI TIẾT KIỆM]
📍 Kèo: ${state.groupPlan.title}
⏰ Thời gian: ${state.groupPlan.timeSlot}
🏢 Địa điểm: ${state.groupPlan.location}
🍿 Hoạt động: ${state.groupPlan.activity}
⚠️ Điều kiện: ${state.groupPlan.conditions}
💰 Dự tính thực trả: ${money(totalCalc)} / người
👉 Xem đối soát minh bạch tại JayT Đà Nẵng!`;
        const previewEl = document.getElementById('share-card-content');
        if (previewEl) previewEl.textContent = shareText;
      };

      [inGpTitle, inGpTime, inGpLocation, inGpActivity, inGpConditions].forEach(el => {
        if (el) el.addEventListener('input', updateGroupPlanPreview);
      });
    }
    ...
    // Copy plan URL hash
    const btnCopyUrl = document.getElementById('btn-copy-plan-url');
    if (btnCopyUrl) {
      btnCopyUrl.addEventListener('click', () => {
        const payload = encodeURIComponent(JSON.stringify(state.groupPlan));
        const fullUrl = `${window.location.origin}${window.location.pathname}#plan=${payload}`;
        navigator.clipboard.writeText(fullUrl).then(() => {
          showToast('🔗 Đã sao chép liên kết kế hoạch!');
        }).catch(() => {
