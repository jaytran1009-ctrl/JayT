...
console.log('========================================================================');
console.log('🔍 JAYT-138: SELF-RUNNING SUPPLY CYCLE & RED-TEAM AUDIT (260 TARGETS)');
console.log('========================================================================\n');

const repoRoot = path.resolve(__dirname, '..');
const compilerPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'generic_compiler_138.js');
...
test('Sum of all categories equals exact total leaf targets evaluated (260 == 260)', () => {
  ...
  assert.strictEqual(m.total_targets_evaluated, 260, 'Expected 260 total targets');
  assert.strictEqual(sum, 260, 'Metric conservation broken');
  assert.strictEqual(m.metric_conservation_check, 260, 'Manifest internal check failed');
});

console.log('\n--- GATE 2: QUANTIFIED BENEFIT OFFER ENFORCEMENT ---');
test('Candidate bundle has explicit quantified numeric benefit clause ("Giảm ngay 30.000Đ...")', () => {
  for (const cand of manifest.evidence_bundle_candidates) {
    const offerQuote = cand.evidence_bundle.offer.quote;
    const hasQuantifiedBenefit = /(?:\d+[\.,]\d+|\d+ ?%|mua \d+ tặng \d+|giảm ngay \d+|giá vé chỉ \d+|combo hè \d+|deal \d+)/i.test(offerQuote);
    assert(hasQuantifiedBenefit, `Violation: Unquantified offer passed into candidates: "${offerQuote}"`);
  }
});

console.log('\n--- GATE 3: ANTI-FOOTER & ANTI-COPYRIGHT GATE ---');
...
console.log('\n--- GATE 5: DUAL RELATIONAL LINEAGE CANDIDATE INTEGRITY ---');
...
test('Manifest 138 contains ZERO "VERIFIED" statuses or CTA buy/book buttons', () => {
...
test('generic_compiler_138.js contains ZERO hardcoded brands, venues, malls, landmarks, or voucher codes', () => {
...
console.log('\n--- GATE 8: PRODUCTION LOCK & ZERO LIVE DEPLOYMENT IN 138 ---');
...
if (failCount > 0) {
  ...
  console.log('✨ ALL 8 JAYT-138 SELF-RUNNING SUPPLY CYCLE GATES PASSED 100% CLEAN!');
  process.exit(0);
}
