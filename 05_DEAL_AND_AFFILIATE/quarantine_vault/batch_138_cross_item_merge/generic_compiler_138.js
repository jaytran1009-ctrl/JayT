/**
 * JAYT SELF-RUNNING SUPPLY CYCLE COMPILER (138)
 ...
 *    Generic headings / PR wrappers demoted to INCOMPLETE_OFFER_BENEFIT_UNPROVEN.
 * 2. Gate 2: Strict Locality Gate: Anti-footer, anti-copyright, anti-year-as-house-number. Word boundary enforced on P./TP.
 ...
 * 5. Day-of-week usability coverage analysis across 7 days.
 * 6. Automated Staging Gate Evaluation (Threshold: >= 10 candidates, >= 3 cohorts, >= 5 days/week).
 * 7. Metric conservation invariance (exact 260 == 260 sum).
 * 8. ZERO live deployment / catalog production locked.
 */
...
const rawCapturesDir = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_138', 'captures_138');
const manifestPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_138_manifest.json');
...
function compileSupplyCycleBatch138() {
  console.log('🚀 [SUPPLY-CYCLE-COMPILER-138] Khởi chạy biên dịch Self-Running Supply Cycle trên 260 Leaf Targets...');

  const targets = fs.readdirSync(rawCapturesDir).filter(d => d.startsWith('TARGET_138_'));
  console.log(`📂 Phát hiện ${targets.length} mục tiêu trong Raw Corpus 138.\n`);
  ...
  // Automated Staging Gate Evaluation
  ...
  const manifest138 = {
    manifest_id: 'BATCH_CAPTURE_138_MANIFEST',
    directive: 'JAYT-138 — OFFICIAL LEAF-SOURCE ACQUISITION & SELF-RUNNING SUPPLY CYCLE',
    generated_at: new Date().toISOString(),
    governance_statement: 'Toàn bộ 260 mục tiêu leaf được biên dịch qua Self-Running Supply Cycle Compiler. 0% generic headings, 0% footer false-positives. Zero deal gán VERIFIED trong 138.',
    summary_metrics: {
  ...
  fs.writeFileSync(manifestPath, JSON.stringify(manifest138, null, 2), 'utf8');

  console.log('========================================================================');
  console.log('📊 KẾT QUẢ BIÊN DỊCH SELF-RUNNING SUPPLY CYCLE 138 (260 TARGETS):');
  console.log(`- Tổng số mục tiêu đánh giá: ${targets.length} / 260`);
  ...
  console.log(`- Metric Conservation Check: ${manifest138.summary_metrics.metric_conservation_check} == ${targets.length}`);
  console.log(`- Weekly Days Covered: ${daysOfWeekCount} days (${Array.from(weeklyCoverageAccumulator).join(', ')})`);
  console.log(`- Cohorts Covered: ${distinctCohortsWithCandidates}`);
  console.log(`- Automated Staging Decision: 🎯 [${automatedDecision}]`);
  console.log(`- Manifest 138 Path: ${manifestPath}`);
  console.log('========================================================================\n');
}

compileSupplyCycleBatch138();
