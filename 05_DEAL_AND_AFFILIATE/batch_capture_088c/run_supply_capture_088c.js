...
const targets = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'selected_supply_targets_088c.json'), 'utf8')
);

const outBaseDir = path.join(__dirname, 'captures_088c');
...
async function runSupplyCapture() {
  console.log(`🚀 [SUPPLY-CDP-088C] Bắt đầu quét Chrome CDP thật ${targets.length} targets mới...`);
  ...
  const manifest = {
    batch_id: 'BATCH_088C_SUPPLY_EXPANSION',
  ...
  const manifestPath = path.join(outBaseDir, 'batch_manifest_088c.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
  console.log(`\n🎉 [SUPPLY-COMPLETE] Đã hoàn tất quét 22 supply targets. Manifest lưu tại: ${manifestPath}`);
}

runSupplyCapture().catch(err => {
  console.error('Fatal supply capture error:', err);
  process.exit(1);
});
