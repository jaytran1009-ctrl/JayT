...
console.log('📊 [RELATIONAL-BUNDLE-BUILDER-088A] Xây dựng Evidence Bundles 2 mảnh Relational Locality...');

const bundles = [];

// 1. Metiz Cinema - Khuyến Mãi Giá Vé U22 Định Kỳ (55.000đ)
...
if (metizPolicy && metizBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_METIZ_U22_RECURRING',
    brand: 'Metiz Cinema',
    sector: 'CINEMA',
    title: 'Giá vé 2D 55.000đ dành cho thành viên U22 (Thứ Ba đến Thứ Năm)',
    tier: 'RECURRING_POLICY_CANDIDATE',
    computable_saving_vnd: 'Giá vé 55.000đ (tiết kiệm so với giá cơ sở)',
    ...
    pieces: {
      ...
      danang_branch: {
        ...
        branch_name: 'Metiz Cinema Đà Nẵng (Helio Center)'
      }
    }
  });
}

// 2. CGV Cinemas - Deal Giảm 30K Lương Về (25/08 - 31/08/2026)
const cgvPaydayPolicy = getArtifact(dir088, 'TARGET_088_175');
const cgvBranch = getArtifact(dir088a, 'TARGET_088A_BR_107');

if (cgvPaydayPolicy && cgvBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_CGV_PAYDAY_30K_TIME_BOUNDED',
    brand: 'CGV Cinemas',
    sector: 'CINEMA',
    title: 'Deal Ting Ting Lương Về: Giảm ngay 30.000đ khi mua từ 2 vé (Mã PAYDAY)',
    tier: 'TIME_BOUNDED_CANDIDATE',
    computable_saving_vnd: 'Giảm 30.000đ khi mua từ 2 vé',
    ...
    pieces: {
      ...
      danang_branch: {
        quote: 'Đà Nẵng',
        artifact_path: cgvBranch.path,
        artifact_sha256: cgvBranch.hash,
        branch_name: 'Hệ thống rạp CGV tại Đà Nẵng (CGV Vĩnh Trung Plaza & CGV Vincom Đà Nẵng)'
      }
    }
  });
}

// 3. CGV Cinemas - Chính Sách Hội Viên CGV Membership
const cgvMemberPolicy = getArtifact(dir088, 'TARGET_088_104');
if (cgvMemberPolicy && cgvBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_CGV_MEMBERSHIP_LOYALTY',
    brand: 'CGV Cinemas',
    sector: 'CINEMA',
    ...
    pieces: {
      ...
      danang_branch: {
        ...
        artifact_path: cgvBranch.path,
        artifact_sha256: cgvBranch.hash,
        branch_name: 'CGV Đà Nẵng'
      }
    }
  });
}

// 4. Metiz Cinema - Chính Sách Ưu Đãi Thành Viên Metiz 2026
const metizMemberPolicy = getArtifact(dir088, 'TARGET_088_150');
if (metizMemberPolicy && metizBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_METIZ_MEMBER_2026_LOYALTY',
    brand: 'Metiz Cinema',
    sector: 'CINEMA',
...
// 5. Gong Cha Vietnam - Chính Sách Tích Điểm Thành Viên
...
if (gongChaPolicy && gongChaBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_GONGCHA_MEMBERSHIP_LOYALTY',
...
// 6. Phê La - Mạng Lưới Cửa Hàng Trực Tiếp Đà Nẵng
const pheLaBranch = getArtifact(dir088a, 'TARGET_088A_BR_137');
if (pheLaBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_PHELA_STORE_NETWORK',
    brand: 'Phê La',
    sector: 'COFFEE_TEA',
...
// 7. Jollibee Vietnam - Mạng Lưới Cửa Hàng Trực Tiếp Đà Nẵng
const jollibeeBranch = getArtifact(dir088a, 'TARGET_088A_BR_128');
if (jollibeeBranch) {
  bundles.push({
    bundle_id: 'BUNDLE_088A_JOLLIBEE_STORE_NETWORK',
...
console.log(`\n🎉 Đã xây dựng thành công ${bundles.length} Relational Evidence Bundles!`);
const tierSummary = {};
bundles.forEach(b => tierSummary[b.tier] = (tierSummary[b.tier] || 0) + 1);
console.log('Phân bố theo Tier Chuẩn:');
console.log(tierSummary);

const manifest = {
  manifest_id: 'RELATIONAL_EVIDENCE_BUNDLES_MANIFEST_088A',
...
const outPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_088a', 'evidence_bundles_088a.json');
fs.writeFileSync(outPath, JSON.stringify(manifest, null, 2), 'utf8');
console.log(`💾 Đã lưu Relational Evidence Bundles 088A tại: ${outPath}`);
