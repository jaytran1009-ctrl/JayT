...
const targets = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'selected_branch_targets_088a.json'), 'utf8')
);

const outBaseDir = path.join(__dirname, 'captures_088a');
...
async function runBranchBatch() {
  console.log(`🚀 [BRANCH-CDP-088A] Bắt đầu quét Chrome CDP thật ${targets.length} branch locator targets...`);
  ...
  const manifest = {
    batch_id: 'BATCH_088A_BRANCH_RESOLUTION',
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    console.log(`\n[${i + 1}/${targets.length}] [${t.brand}] -> ${t.resolved_url}`);
  ...
  const manifestPath = path.join(outBaseDir, 'batch_manifest_088a.json');
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
  console.log(`\n🎉 [BRANCH-COMPLETE] Đã hoàn tất quét 24 branch targets. Manifest lưu tại: ${manifestPath}`);
}

runBranchBatch().catch(err => {
  console.error('Fatal branch batch error:', err);
  process.exit(1);
});
