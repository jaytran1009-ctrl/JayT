/**
 * JAYT STRICT RECEIPT-ONLY VALIDITY & SCHEMA ENGINE (083E)
 * Directive: JAYT-083E-RECEIPT-ONLY-VALIDITY
 * 
 * Strict Zero-Trust Rules:
 * 1. RECEIPT-ONLY VALIDITY:
 *    - `validity_spec` MUST be read exclusively from `receipt_data.validity_spec` in the hashed receipt file.
 *    - Any attempt by signal or evidence_bundle to supply, inject, or override `validity_spec` -> Rejected with `VALIDITY_SPEC_NOT_RECEIPT_BOUND`.
 * 2. EXPLICIT LITERAL ARTIFACT TYPE:
 *    - Receipt MUST contain an explicit string field `artifact_type`.
 *    - Zero inference from field names (like dom_html) or file extensions.
 *    - Allowed types: `['html', 'text', 'pdf', 'dom_html', 'raw_html', 'extracted_text']`.
 * 3. SNIPPET-TO-RULE GROUNDING:
 *    - The `recurring_rule` or `valid_to` in receipt's validity_spec must be grounded in and substantiated by the verified `validity_snippet`.
 * 4. FRESHNESS TTL LIMITS:
 *    - Daily: <= 24h (86,400s)
 *    - Recurring: <= 7d (604,800s)
 *    - Specific valid_to: <= 30d (2,592,000s) AND valid_to >= refDate.
 * 5. SEPARATION OF HISTORICAL VS ACTIVE:
 *    - Unproven / expired / legacy receipts fail closed to `LINEAGE_VALID_HISTORICAL_ONLY`.
 */
...
/**
 * Validates physical receipt completeness with strict explicit artifact_type.
 */
function verifyReceiptCompletenessStrict083e(evidenceBundle) {
  ...
  // 7. Mandatory URL normalization comparison
  ...
  // 9. Mandatory EXPLICIT literal artifact_type (Zero Inference)
  ...
  } else if (receiptData.artifacts?.primary?.artifact_type) {
    artifactType = receiptData.artifacts.primary.artifact_type.trim().toLowerCase();
  }

  if (!artifactType) {
    return { valid: false, failure_reason: 'RECEIPT_MISSING_EXPLICIT_ARTIFACT_TYPE: Receipt must explicitly declare "artifact_type" string field.' };
  }
 ...
 * Also verifies semantic grounding between receipt's validity_spec and verified validity_snippet.
 */
function checkTemporalActiveValidity083e(receiptValiditySpec, capturedAt, validitySnippet, refDate = new Date()) {
  if (!capturedAt || isNaN(Date.parse(capturedAt))) {
  ...
  if (is_recurring && recurring_rule) {
    // Grounding check: verified snippet must substantiate recurring rule
...
/**
 * Strict evaluation of a single signal in 083E.
 */
function evaluateSignalStrict083e(signal, options = {}) {
  ...
  // 1. Zero-Trust Check: Reject if caller supplied self-declared validity_spec in signal or evidence_bundle
  ...
  const binding = verifyReceiptCompletenessStrict083e({
    ...(evidence_bundle || {}),
  ...
  // Exclusively read validity_spec from receipt JSON on disk
  const receiptValiditySpec = binding.receipt_data?.validity_spec || null;
  const temporalCheck = checkTemporalActiveValidity083e(
    receiptValiditySpec,
    binding.captured_at,
...
module.exports = {
  ...
  FRESHNESS_TTL_MS,
  normalizeUrlStrict,
  verifyReceiptCompletenessStrict083e,
  validateDisjointSnippets,
  checkTemporalActiveValidity083e,
  evaluateSignalStrict083e
};
