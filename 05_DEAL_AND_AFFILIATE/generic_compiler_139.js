/**
 * JAYT ATOMIC PROMOTION UNIT COMPILER (139)
 * Directive: JAYT-139 — ATOMIC PROMOTION UNIT EXTRACTION & CANONICAL LEAF VERIFICATION
 * 
 * STRICT ARCHITECTURAL CONSTRAINTS:
 * 1. Tier 1 (Discovery-Only): RSS, hubs, category roots, listings classified as DISCOVERY_ONLY_LISTING. Zero candidate generation.
 * 2. Tier 2 (Canonical Leaf): Each leaf has a unique promotion_unit_id.
 * 3. Atomicity Gate: Offer, terms, validity MUST all share the same promotion_unit_id within the single canonical leaf.
 *    Cross-item multi-promo merges are blocked into CROSS_ITEM_MERGE_BLOCKED.
 * 4. Gate 1: Offer must contain an explicit quantified benefit (price, discount amount, %, structured BOGO, code with quantified reward).
 * 5. Gate 2: Strict Locality Gate: Anti-footer, anti-copyright, anti-year-as-house-number. Word boundary enforced on P./TP.
 * 6. Dual relational lineage yields EVIDENCE_BUNDLE_CANDIDATE (NOT VERIFIED).
 * 7. Automated Staging Gate Evaluation (Threshold: >= 10 candidates, >= 3 cohorts, >= 5 days/week).
 * 8. Metric conservation invariance (exact 360 == 360 sum).
...
function compileAtomicBatch139() {
  console.log('🚀 [ATOMIC-UNIT-COMPILER-139] Khởi chạy biên dịch Atomic Promotion Units trên 360 Leaf Targets...');

  const targets = fs.readdirSync(rawCapturesDir).filter(d => d.startsWith('TARGET_139_'));
  console.log(`📂 Phát hiện ${targets.length} mục tiêu trong Raw Corpus 139.\n`);
  ...
  for (const tId of targets) {
    ...
    if (offerFrag && termsFrag && validityFrag && scopeFrag) {
      // Check maximum semantic offset distance within single leaf article (reject cross-promo stitching > 4000 chars distance)
  ...
  const manifest139 = {
    manifest_id: 'BATCH_CAPTURE_139_MANIFEST',
    directive: 'JAYT-139 — ATOMIC PROMOTION UNIT EXTRACTION & CANONICAL LEAF VERIFICATION',
    generated_at: new Date().toISOString(),
    governance_statement: 'Toàn bộ 360 mục tiêu leaf được biên dịch qua Atomic Promotion Unit Compiler. 0% cross-item merges, 0% listing candidates. Zero deal gán VERIFIED trong 139.',
    summary_metrics: {
  ...
  fs.writeFileSync(manifestPath, JSON.stringify(manifest139, null, 2), 'utf8');

  console.log('========================================================================');
  console.log('📊 KẾT QUẢ BIÊN DỊCH ATOMIC PROMOTION UNITS 139 (360 TARGETS):');
  ...
  console.log(`- DISCOVERY_ONLY_LISTING (Listing/RSS không tạo candidate): ${discoveryOnlyListing.length}`);
  console.log(`- CROSS_ITEM_MERGE_BLOCKED (Chặn ghép mảnh cách xa/khác bài): ${crossItemMergeBlocked.length}`);
  ...
  console.log(`- Metric Conservation Check: ${manifest139.summary_metrics.metric_conservation_check} == ${targets.length}`);
  console.log(`- Weekly Days Covered: ${daysOfWeekCount} days`);
  console.log(`- Automated Staging Decision: 🎯 [${automatedDecision}]`);
  console.log(`- Manifest 139 Path: ${manifestPath}`);
  console.log('========================================================================\n');
}

compileAtomicBatch139();
