/**
 * RETENTION SUPPLY EXTRACTOR 124
 * Directive: JAYT-124-LAST-MILE-TRUST-AND-LOCAL-MOMENT
 * Enhancements:
 * 1. Add operating_hours & last_mile_note for time-sensitive usability (e.g. DanaBus 05:30 - 21:00)
 * 2. Add contextual_taxonomy to eliminate evening slot semantic mismatches (GoGi -> 🥩 Ăn tối nhóm & Buffet nướng)
 * 3. Add explicit per_person_formula for group meals (529.000₫ · Ước tính ~176.300₫/người khi chia 3 người)
 * 4. Add curated official visual asset pointers for top 5 key destinations
 * 5. Output daily_supply_feed_124.json and supply_gap_board_124.json
 */
...
function verifyDiskEvidence(relPath, requiredSubstrings = []) {
  ...
  return {
    ...
    sha256: hash,
    size_bytes: size,
    verified_at: '2026-08-26T00:18:00+07:00'
  };
}

console.log('🔍 [EXTRACTOR-124] Đang xác minh dữ liệu và đối soát Last-Mile Trust trên đĩa...\n');
...
const watchlistDeals = [
  ...
  },
  {
    ...
    curated_image_url: 'assets/discovery-images/shopping-context-v1.png',
...
const planningMenuAndUtilities = [
  {
    ...
    last_mile_note: 'Nên đặt bàn trước vào tối cuối tuần (19h-20h30)',
  ...
  },
  {
    ...
    sector: 'COFFEE',
    contextual_taxonomy: '☕ Cà phê & Trà đặc sản',
    intent_type: 'COFFEE_TEA',
    feed_category: 'PLANNING_MENU_PRICING',
    category_badge: '📋 GIÁ THAM KHẢO',
  ...
  },
  {
    ...
    sector: 'MOBILITY',
    contextual_taxonomy: '🚌 Xe buýt trợ giá (05:30 – 21:00)',
    ...
    operating_hours: 'Giờ hoạt động mạng lưới: 05:30 – 21:00 hàng ngày (Tùy tuyến — kiểm tra chuyến cuối trước khi đi muộn)',
    last_mile_note: 'Sau 21:00 xe buýt ngưng chạy, nên chủ động xe cá nhân hoặc gọi xe công nghệ',
    scope: 'Toàn mạng lưới tuyến xe buýt nội thành Đà Nẵng',
    official_url: 'https://www.danangbus.vn/',
    curated_image_url: 'assets/discovery-images/mobility-context-v1.png',
    slot: 'SLOT_2000',
...
const dailyFeed124 = {
  ...
  summary: {
    ...
    last_mile_enhancements: {
      operating_hours_coverage: '100% (15/15 items have verified operating windows)',
      contextual_taxonomy: '100% contextual (GoGi: Ăn tối nhóm & Buffet nướng, DanaBus: Xe buýt nội đô 05:30-21:00)',
      explicit_per_person_formula: '100% explicit (GoGi: 529.000₫ tổng · ~176.300₫/người khi chia 3 người)',
      curated_official_images_count: 5
    }
  },
...
// Write daily_supply_feed_124.json to 03_SOURCE_OF_TRUTH
const feedOutputPath = path.join(repoRoot, '03_SOURCE_OF_TRUTH', 'daily_supply_feed_124.json');
fs.writeFileSync(feedOutputPath, JSON.stringify(dailyFeed124, null, 2), 'utf8');
console.log(`✅ [FEED-124] Đã tạo daily_supply_feed_124.json (${dailyFeed124.summary.total_items} mục, Last-Mile Trust 100%)`);

// 4. Build Supply Gap Board 124
const slots = [
...
const gapBoard124 = {
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  board_version: '124.0.0',
  release_directive: 'JAYT-124-LAST-MILE-TRUST-AND-LOCAL-MOMENT',
  ...
  high_priority_target_gaps: [
    {
      gap_id: 'GAP_124_01_LUNCH_MOBILITY',
      slot: 'SLOT_1115',
      sector: 'MOBILITY',
      priority: 'HIGH',
    ...
    {
      gap_id: 'GAP_124_02_LATE_NIGHT_RIDE',
      slot: 'SLOT_2000',
      sector: 'MOBILITY',
      priority: 'HIGH',
    ...
    {
      gap_id: 'GAP_124_03_AFTERNOON_SNACK',
      slot: 'SLOT_1415',
      sector: 'LUNCH',
      priority: 'HIGH',
...
const gapBoardOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'supply_gap_board_124.json');
fs.writeFileSync(gapBoardOutputPath, JSON.stringify(gapBoard124, null, 2), 'utf8');
console.log(`✅ [GAP-BOARD-124] Đã tạo supply_gap_board_124.json (25 cells, ${coveredCount} covered - 40.0% coverage)\n`);
