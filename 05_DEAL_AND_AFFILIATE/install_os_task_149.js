/**
 * JAYT OS TASK INSTALLER & LAUNCHER (149)
...
const launcherBatPath = path.join(userHome, 'run_jayt_worker_149.bat');
const taskName = 'JAYT_AUTONOMOUS_SUPPLY_WORKER_149';
const scriptPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'jayt_autonomous_worker_149.js');
const nodeExePath = process.execPath;

// 1. Create robust Launcher Batch with UTF-8 support
const batContent = `@echo off
chcp 65001 > nul
"${nodeExePath}" "${scriptPath}" --scheduled-cycle
`;
...
function installAndVerifyOsTask149() {
  console.log('========================================================================');
...
$argList = "\`"$scriptPath\`" --scheduled-cycle"
...
`;
    const psScriptPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'register_task_149.ps1');
    const BOM = '\uFEFF';
