...
const lockFilePath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'worker_148.lock');
  ...
  if (fs.existsSync(lockFilePath)) {
    try {
      ...
      if (isProcessAlive && lockAge < LOCK_TTL_MS) {
        console.error(`⚠️ [SUPPLY-LOCK-148] Active worker already running (PID: ${lockData.pid}, Age: ${Math.round(lockAge / 1000)}s). Aborting concurrent run.`);
      ...
      console.log(`ℹ️ [SUPPLY-LOCK-148] Stale lock recovered (Reason: ${staleReceipt.stale_reason}).`);
    } catch (_) {}
  }

  const newLock = {
    worker_identifier: 'JAYT_AUTONOMOUS_SUPPLY_WORKER_148',
    pid: process.pid,
...
/**
 * DOM-Lineage Dynamic Discovery (Workstream 2)
...
async function extractContentRootLineageLinks(browser, htmlContent, pageUrl, parentReceiptPath, brandId, brandName) {
  ...
  try {
    ...
    const rawLinkData = await page.evaluate(() => {
      // Find candidate semantic content root (reject pure generic 'div' or 'body' without meaningful class/id)
      ...
      // Check if root is overly generic
      ...
      for (let i = 0; i < anchorNodes.length; i++) {
        ...
        // Exclude if anchor is inside navigation/header/footer/sidebar
    ...
    for (const rawItem of (rawLinkData.links || [])) {
      // Lineage Check 1: Non-empty anchor text (length >= 3)
      ...
      // Lineage Check 2: Content root selector must not be generic
      ...
      // Full outer HTML hash (no truncation)
...
/**
 * Main Autonomous Supply Engine Runner 148
 */
async function runSupplyEngine148(options = {}) {
  const isScheduledCycle = Boolean(options.isScheduledCycle);
  const isSmoke = Boolean(options.isSmoke);
  const executionOrigin = options.origin || (isScheduledCycle ? 'OS_TRIGGERED' : 'MANUAL_TRIGGERED');
  const batchCap = options.limit || (isSmoke ? 2 : DEFAULT_BATCH_CAP);

  console.log('========================================================================');
  console.log(`🚀 JAYT-148: RUNNING AUTONOMOUS SUPPLY ENGINE (${isScheduledCycle ? 'SCHEDULED_CYCLE' : (isSmoke ? 'SMOKE_RUN' : 'MANUAL_BATCH')})`);
  console.log(`- Execution Origin: ${executionOrigin}`);
  ...
  try {
    ...
    // Priority Cohort Sorting (Directive JAYT-148 Section 4):
    ...
    for (let i = 0; i < selectedItems.length; i++) {
      ...
      // Step E: 6-Step Taxonomy Classification (Workstream 4: Explicit ERROR_OR_BLOCKED_SOURCE)
      ...
      // Step F: Precise DOM-Lineage Discovery (Workstream 2)
    ...
    // 4. Strict Reconciliation Invariance (Workstream 5)
    ...
    console.log('\n========================================================================');
    console.log(`✅ SUPPLY ENGINE 148 CYCLE COMPLETED: ${executionResults.length} processed.`);
    console.log(`- Run ID: ${runId}`);
    console.log(`- Origin: ${executionOrigin}`);
...
if (require.main === module) {
  ...
  if (originIdx !== -1 && args[originIdx + 1]) {
    origin = args[originIdx + 1];
  } else if (isScheduledCycle && process.env.OS_TRIGGERED === '1') {
    origin = 'OS_TRIGGERED';
  }
  ...
  runSupplyEngine148({ limit, isSmoke, isScheduledCycle, origin }).then(res => {
