/**
 * JAYT STRICT SEMANTIC-ROOT LEAF DOM PARSER (143R)
 ...
 */
function browserParseStrictSemanticRoot143R() {
 ...
 */
async function parseStrictSemanticRootLeaf143R(html, leafUrl, receiptTruth, browser) {
  const page = await browser.newPage();
  await page.setRequestInterception(true);
  page.on('request', req => req.abort());
  await page.setContent(html, { waitUntil: 'domcontentloaded', timeout: 5000 });

  const rawDom = await page.evaluate(browserParseStrictSemanticRoot143R);
  await page.close();
  ...
  function buildProvenance(node) {
    ...
    return {
      ...
      raw_html_sha256: receiptTruth.fresh_hashes ? receiptTruth.fresh_hashes.html_sha256 : null,
      screenshot_sha256: receiptTruth.fresh_hashes ? receiptTruth.fresh_hashes.screenshot_sha256 : null,
...
module.exports = {
  computeSha256,
  browserParseStrictSemanticRoot143R,
  parseStrictSemanticRootLeaf143R
};
