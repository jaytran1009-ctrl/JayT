/**
 * JAYT BATCH 144 PROCESSING RUNNER (32 BRANDS, 101 URLS, 5-STEP ORDER)
...
const brandRegistryOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'brand_locality_registry_144.json');
const batchTableOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_144_table.json');
const registry144Path = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'fresh_source_registry_144.json');

async function processBatch144() {
  console.log('========================================================================');
  console.log('🔬 JAYT-144: EVALUATING MULTI-SOURCE FRESH CAPTURES (5-STEP ORDER)');
  console.log('========================================================================\n');
  ...
  // Phase 1: Process Cohort A (Locality Store Locators - 32 Brands)
  console.log('--- PHASE 1: EVALUATING COHORT A STORE LOCATORS (32 BRANDS) ---');
  ...
  const brandRegistry = {
    registry_id: 'BRAND_LOCALITY_REGISTRY_144',
    directive: 'JAYT-144: SCHEDULER THẬT, QUÉT ĐA NGUỒN VÀ VÒNG LẶP CUNG ỨNG TỰ VẬN HÀNH',
  ...
  // Phase 2: Collision Detection across all captures
  ...
  console.log('\n--- PHASE 3: 5-STEP EVALUATION ACROSS COHORTS B & C ---');
  ...
  const batchTable144 = {
    batch_id: 'BATCH_CAPTURE_144_TABLE',
    directive: 'JAYT-144: SCHEDULER THẬT, QUÉT ĐA NGUỒN VÀ VÒNG LẶP CUNG ỨNG TỰ VẬN HÀNH',
    generated_at: new Date().toISOString(),
    ...
    governance_statement: 'Tái xử lý 144 trên 32 thương hiệu và 101 URLs với Certified Native Harness. Zero synthetic defaults. Production feed locked.',
    items: evaluatedLeaves
  };

  fs.writeFileSync(batchTableOutputPath, JSON.stringify(batchTable144, null, 2), 'utf8');

  // Build Fresh Source Registry 144 (32 Brands)
  const sources144 = brandLocalityResults.map((b, idx) => ({
    source_id: `SRC_144_${String(idx + 1).padStart(2, '0')}`,
    brand_id: b.brand_id,
  ...
  const registry144 = {
    registry_id: 'FRESH_SOURCE_REGISTRY_144',
    directive: 'JAYT-144: SCHEDULER THẬT, QUÉT ĐA NGUỒN VÀ VÒNG LẶP CUNG ỨNG TỰ VẬN HÀNH',
    generated_at: new Date().toISOString(),
    governance_statement: `32 official brand sources monitored in Batch 144 under certified native event capture harness.`,
    sources: sources144
  };

  fs.writeFileSync(registry144Path, JSON.stringify(registry144, null, 2), 'utf8');

  console.log('\n========================================================================');
  console.log(`✅ 144 PROCESSING COMPLETE: ${evaluatedLeaves.length} EVALUATED.`);
  ...
  console.log(`- Metric Conservation Check: ${batchTable144.conservation_check} == ${evaluatedLeaves.length}`);
  console.log(`📂 Output Brand Registry: ${brandRegistryOutputPath}`);
  console.log(`📂 Output Batch Table: ${batchTableOutputPath}`);
  console.log(`📂 Output Fresh Source Registry: ${registry144Path}`);
  console.log('========================================================================\n');
}
