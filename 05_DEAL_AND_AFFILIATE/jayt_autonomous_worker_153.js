/**
 * JAYT STRATIFIED LEAF RESOLUTION & THREE-COHORT CAMPAIGN ENGINE (153)
 ...
 * 3. Discovery Lineage Ledger: Persists 100% of discovered leaf links with complete DOM lineage.
 * 4. Stratified Leaf Allocation (10/10/10):
 *    - Cohort A (Cinema/Entertainment): Max 10 leaves, Max 3 per brand.
 *    - Cohort B (F&B/Coffee): Max 10 leaves, Max 3 per brand.
 *    - Cohort C (Transit/Student): Max 10 leaves, Max 3 per brand.
 *    - ZERO brand monopolization (Galaxy Cinema capped at <= 3).
...
const lockFilePath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'worker_153.lock');
  ...
  if (fs.existsSync(lockFilePath)) {
    try {
      ...
      if (isProcessAlive && lockAge < LOCK_TTL_MS) {
        console.error(`⚠️ [SUPPLY-LOCK-153] Active worker already running (PID: ${lockData.pid}, Age: ${Math.round(lockAge / 1000)}s). Aborting concurrent run.`);
      ...
      console.log(`ℹ️ [SUPPLY-LOCK-153] Stale lock recovered (Reason: ${staleReceipt.stale_reason}).`);
    } catch (_) {}
  }

  const newLock = {
    worker_identifier: 'JAYT_STRATIFIED_SUPPLY_WORKER_153',
    pid: process.pid,
...
async function extractContentRootLineageLinks(browser, htmlContent, pageUrl, parentReceiptPath, brandId, brandName, cohort151) {
  ...
  try {
    ...
    const rawLinkData = await page.evaluate(() => {
      const candidateRoots = Array.from(document.querySelectorAll('article, [class*="post"], [class*="article"], [class*="promo"], [class*="news"], [class*="event"], [class*="content"], main, [class*="menu"], [class*="banner"], [class*="slider"], [class*="grid"], [class*="list"], [class*="deals"]'));
      
      const promoKeywords = ['khuyen-mai', 'uu-dai', 'tin-tuc', 'promotions', 'offers', 'deal', 'events', 'combo', 'student', 've-thang', 'tuyen-xe', 've-tau', 'u22', 'giam-gia', 'gia-ve', 'membership', 'gia-tot', 'coupon', 'voucher', 'bang-gia'];
      const results = [];
...
/**
 * Main Stratified Supply Campaign Runner 153
 */
async function runStratifiedSupplyEngine153() {
  const executionOrigin = 'MANUAL_TRIGGERED';

  console.log('========================================================================');
  console.log('🚀 JAYT-153: STRATIFIED LEAF RESOLUTION & SUPPLY DIVERSITY CAMPAIGN');
  console.log(`- Transparent Execution Origin: ${executionOrigin}`);
  console.log(`- Host Scheduler Status: SCHEDULER_BLOCKED_ON_THIS_HOST`);
  console.log(`- Stratification: Cohort A (max ${COHORT_CAP}), Cohort B (max ${COHORT_CAP}), Cohort C (max ${COHORT_CAP})`);
  console.log(`- Brand Cap: Max ${BRAND_CAP} leaves per brand`);
  console.log('========================================================================\n');
  ...
  try {
    ...
    console.log('\n--- WORKSTREAM A: DISCOVER & PERSIST FULL LINEAGE LEDGER (32 ROOTS) ---');
    const rootResults = [];
    ...
    for (let i = 0; i < OFFICIAL_32_ROOTS.length; i++) {
      ...
      console.log(`[${i + 1}/${OFFICIAL_32_ROOTS.length}] [${rootDef.cohort_151}] Scanning ${rootDef.brand_name} -> ${rootDef.root_url}`);
    ...
    console.log(`\n✨ Workstream A Discovery Finished: 32 roots scanned -> ${discoveredCandidates.length} valid leaf links discovered across 3 cohorts (${totalRejectedCandidates} rejected by negative/lineage filter).`);

    // Persist full discovery ledger
    const ledgerData = {
      ledger_id: 'DISCOVERY_LINEAGE_LEDGER_153',
      directive: 'JAYT-153: STRATIFIED LEAF RESOLUTION & SUPPLY DIVERSITY',
    ...
    console.log(`💾 Persisted 100% of discovered lineage records (${discoveredCandidates.length}) to: ${ledgerPath}`);

    // --- WORKSTREAM B: STRATIFIED BATCH SELECTION (10/10/10, Max 3/brand) ---
    console.log('\n--- WORKSTREAM B: STRATIFIED BATCH SELECTION (10/10/10, MAX 3/BRAND) ---');
    
    function selectStratifiedLeaves(candidates, cohortName, maxCohort, maxBrand) {
    ...
    const stratA = selectStratifiedLeaves(discoveredCandidates, 'COHORT_A_CINEMA_ENTERTAINMENT', COHORT_CAP, BRAND_CAP);
    const stratB = selectStratifiedLeaves(discoveredCandidates, 'COHORT_B_FNB_COFFEE', COHORT_CAP, BRAND_CAP);
    const stratC = selectStratifiedLeaves(discoveredCandidates, 'COHORT_C_TRANSIT_STUDENT', COHORT_CAP, BRAND_CAP);

    const stratifiedBatch = [...stratA.selected, ...stratB.selected, ...stratC.selected];
    console.log(`📊 Stratified Allocation Plan:`);
    ...
    console.log(`- Total Stratified Batch Size: ${stratifiedBatch.length} leaves (Zero brand monopolization)`);

    // --- WORKSTREAM C & D: LEAF RE-CAPTURE & EVIDENCE RESOLUTION ---
    console.log(`\n--- WORKSTREAM C & D: LEAF RE-CAPTURE & EVIDENCE RESOLUTION (${stratifiedBatch.length} LEAVES) ---`);
    const leafResults = [];
    ...
    for (let j = 0; j < stratifiedBatch.length; j++) {
      ...
      console.log(`[${j + 1}/${stratifiedBatch.length}] [${leaf.cohort_151}] Re-capturing ${leaf.brand_name} -> ${leaf.canonical_url} ("${leaf.anchor_text}")`);
    ...
    const registry153Data = {
      registry_id: 'AUTONOMOUS_SCHEDULE_REGISTRY_153',
      directive: 'JAYT-153: STRATIFIED LEAF RESOLUTION & SUPPLY DIVERSITY',
      created_at: new Date().toISOString(),
    ...
    fs.writeFileSync(registryPath, JSON.stringify(registry153Data, null, 2), 'utf8');
    ...
    const runManifest = {
      ...
      worker_identifier: 'JAYT_STRATIFIED_SUPPLY_WORKER_153',
      directive: 'JAYT-153: STRATIFIED LEAF RESOLUTION & SUPPLY DIVERSITY',
      execution_mode: 'STRATIFIED_THREE_COHORT_CAMPAIGN',
      execution_origin: executionOrigin,
      host_scheduler_status: 'SCHEDULER_BLOCKED_ON_THIS_HOST',
      executed_at: new Date().toISOString(),
      run_directory: runOutputDir,
      project_memory_verified: memory,
      workstream_a_discovery_ledger: {
        total_discovered_lineage_links: discoveredCandidates.length,
        ledger_path: ledgerPath,
        cohort_breakdown: ledgerData.cohort_breakdown
      },
      workstream_b_stratified_allocation: {
      ...
      workstream_c_d_evidence_resolution: {
    ...
    console.log('\n========================================================================');
    console.log(`✅ STRATIFIED THREE-COHORT CAMPAIGN 153 COMPLETED:`);
    console.log(`- Run ID: ${runId}`);
    console.log(`- Discovery Ledger: ${discoveredCandidates.length} lineage records persisted`);
    console.log(`- Stratified Leaves Re-captured: ${leafResults.length} across 3 cohorts (Max ${BRAND_CAP}/brand)`);
...
if (require.main === module) {
  runStratifiedSupplyEngine153().then(res => {
