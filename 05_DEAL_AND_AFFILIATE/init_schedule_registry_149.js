/**
 * JAYT SCHEDULE REGISTRY INITIALIZER (149)
...
const registryOutputPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'autonomous_schedule_registry_149.json');
...
function initScheduleRegistry149() {
  ...
  const registryData = {
    registry_id: 'AUTONOMOUS_SCHEDULE_REGISTRY_149',
    directive: 'JAYT-149: END SCHEDULER THEATRE, PROVEN AUTONOMY & REAL-SUPPLY RECOVERY',
  ...
  console.log(`✅ [REGISTRY-149] Initialized ${registryItems.length} schedulable targets (${registryData.stale_sources_count} marked SOURCE_PATH_STALE) at: ${registryOutputPath}`);
  return registryData;
}

if (require.main === module) {
  initScheduleRegistry149();
}
