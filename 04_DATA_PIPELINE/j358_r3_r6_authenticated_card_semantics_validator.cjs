/**
 * J358-R3-R6: Authenticated Card Semantics Validator
 * 
 * Mandate:
 ...
 */
function authenticateCardIdentifier(idValue, meta, candidate) {
  ...
  if (!CARD_SCOPED_ID_REGEX.test(trimmed)) {
    return {
      ...
      raw_id: trimmed,
      namespace: null,
      reason: 'INVALID_SCHEMA_PATTERN: Does not match card-scoped schema'
    };
  }
  ...
  // Authenticity Proof (Req 16): ID must be present in sealed capture metadata/manifest mapping
  let isAuthenticated = false;
  let mappingKey = null;

  if (meta && typeof meta === 'object') {
    if (Array.isArray(meta.authenticated_offer_ids) && meta.authenticated_offer_ids.includes(trimmed)) {
      isAuthenticated = true;
      mappingKey = `meta.authenticated_offer_ids[${meta.authenticated_offer_ids.indexOf(trimmed)}]`;
    } else if (meta.offer_card_manifest && typeof meta.offer_card_manifest === 'object' && meta.offer_card_manifest[trimmed]) {
      isAuthenticated = true;
      mappingKey = `meta.offer_card_manifest["${trimmed}"]`;
    } else if (meta.sealed_offer_id_mapping && typeof meta.sealed_offer_id_mapping === 'object' && meta.sealed_offer_id_mapping[trimmed]) {
      isAuthenticated = true;
      mappingKey = `meta.sealed_offer_id_mapping["${trimmed}"]`;
    }
  }

  if (candidate && typeof candidate === 'object') {
    if (candidate.authenticated_offer_id && candidate.authenticated_offer_id === trimmed) {
      isAuthenticated = true;
      mappingKey = 'candidate.authenticated_offer_id';
    }
  }
  ...
  if (!isAuthenticated) {
    return {
      ...
      raw_id: trimmed,
      namespace,
      reason: 'HELD__UNAUTHENTICATED_CARD_IDENTIFIER: Offer ID is not present in sealed capture metadata/manifest mapping'
    };
  }
  ...
  return {
    ...
    reason: 'AUTHENTICATED_BY_SEALED_METADATA_MAPPING'
  };
}
...
function evaluateCandidate(arg1, arg2, arg3, arg4) {
  ...
  if (uniqueCards.length > 1) {
    const hasDistinguishingId = uniqueCards.some(c => {
      ...
      return authenticateCardIdentifier(oid, meta, candidate).isValid;
    });
  ...
  // Step 6: Authenticate Card Identifier (Req 16)
  const rawOfferId = chosenCard.attrs['data-offer-id'] || chosenCard.attrs.id || null;
  const idAuth = authenticateCardIdentifier(rawOfferId, meta, candidate);
