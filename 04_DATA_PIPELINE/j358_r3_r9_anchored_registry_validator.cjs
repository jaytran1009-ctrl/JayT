/**
 * J358-R3-R9: Anchored Registry Context Validator
 * 
 * Mandate & Invariants (Work Order J358-R3-R9-REMOVE-CUSTOM-REGISTRY-SUBSTITUTION):
 * 1. Remove customRegistryPath and every caller-selected registry source from the public evaluator
 *    and exported API. Public evaluation must only load the canonical anchored registry (Req 15).
 * 2. Enforce the registry anchor independently: exact canonical path plus hard-coded digest or
 *    public-key signature verification. Sidecar alone is not an authenticity anchor (Req 16).
 * 3. Keep any alternate registry support internal to test-only code, unexported and structurally
 *    incapable of returning VERIFIED through the public evaluator (Req 17).
 * 4. Add attacks using a schema-valid forged registry whose leaf, raw SHA, metadata SHA, selector,
 *    range and card ID all match a forged fixture; caller path override, path alias and symlink/junction
 *    variants must all fail closed at public API (Req 18).
 * 5. Distinct fail-closed reason codes: HELD__CALLER_REGISTRY_OVERRIDE_FORBIDDEN,
 *    HELD__REGISTRY_ANCHOR_VERIFICATION_FAILED, HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED,
 *    HELD__LEAF_NOT_IN_SEALED_REGISTRY, HELD__METADATA_HASH_NOT_IN_SEALED_REGISTRY,
 *    HELD__REGISTRY_SOURCE_SHA_MISMATCH (Req 15, 16).
 * 6. Preserved Protections: Unconditional broad shell rejection, card ID authenticity, directed locality,
 *    validity semantics, zero real promotions (0 VERIFIED / 10 HELD), staging invariance (Req 20).
 */
...
const CANONICAL_REGISTRY_PATH = path.resolve(__dirname, '..', '06_TRUST_AND_EVIDENCE', 'batch_19_r3_r8_replay', 'SEALED_METADATA_REGISTRY.json');
const EXPECTED_REGISTRY_SHA = 'd592d72fff5d923d93c939acc863bcdc4829bcd68e8cc90b3b61274066f0b1e4';

const CANONICAL_R4_EXTENSION_PATH = path.resolve(__dirname, '..', '06_TRUST_AND_EVIDENCE', 'batch_19_r4_targeted_offer_card_vault', 'SEALED_METADATA_REGISTRY_R4_EXTENSION.json');
const EXPECTED_R4_EXTENSION_SHA = '97e8111405979b34dd514497c6b136b45ae586ba52ff181a3218612bd0ae2af5';

/**
 * Load and independently verify the canonical anchored registry.
 * Strictly accepts NO arguments; cannot be pointed to any caller-supplied path.
 * Enforces canonical path resolution (preventing aliases, relative paths, symlinks/junctions).
 * Enforces exact hard-coded digest verification against EXPECTED_REGISTRY_SHA.
 * Supports anchored R4 extension if present, verifying exact path realpath and hard-coded digest.
 */
function loadCanonicalAnchoredRegistry() {
  const normalizedTarget = path.normalize(CANONICAL_REGISTRY_PATH);
  if (!fs.existsSync(normalizedTarget)) {
    return {
      ok: false,
      code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED',
      error: 'Canonical anchored metadata registry not found: ' + normalizedTarget
    };
  }

  // Prevent path alias, symlink, or junction substitution by verifying realpath
  let realPath;
  try {
    ...
    return {
      ok: false,
      code: 'HELD__REGISTRY_ANCHOR_VERIFICATION_FAILED',
      error: 'Failed to resolve canonical realpath: ' + err.message
    };
  }

  if (path.resolve(realPath).toLowerCase() !== path.resolve(normalizedTarget).toLowerCase()) {
    return {
      ok: false,
      code: 'HELD__REGISTRY_ANCHOR_VERIFICATION_FAILED',
      error: 'Registry path is an alias, symlink, or junction'
    };
  }

  try {
    const raw = fs.readFileSync(normalizedTarget);
    const fileSha = sha256(raw);
    if (fileSha !== EXPECTED_REGISTRY_SHA) {
      return {
        ok: false,
        code: 'HELD__REGISTRY_ANCHOR_VERIFICATION_FAILED',
        error: 'Canonical metadata registry digest mismatch: ' + fileSha + ' !== ' + EXPECTED_REGISTRY_SHA
      };
    }
    const parsed = JSON.parse(raw.toString('utf8'));
    if (!parsed || parsed.registry_version !== 'J358-R3-R8-TRUST-ROOT-V1' || !parsed.leaves) {
      return {
        ok: false,
        code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED',
        error: 'Invalid canonical registry schema'
      };
    }

    // Load and verify anchored R4 extension if present
    let extensionInfo = null;
    if (fs.existsSync(CANONICAL_R4_EXTENSION_PATH)) {
      const extNormalized = path.normalize(CANONICAL_R4_EXTENSION_PATH);
      const extRealPath = fs.realpathSync(extNormalized);
      if (path.resolve(extRealPath).toLowerCase() !== path.resolve(extNormalized).toLowerCase()) {
        return {
          ok: false,
          code: 'HELD__REGISTRY_ANCHOR_VERIFICATION_FAILED',
          error: 'Extension registry path is an alias, symlink, or junction'
        };
      }
      const extRaw = fs.readFileSync(extNormalized);
      const extSha = sha256(extRaw);
      if (extSha !== EXPECTED_R4_EXTENSION_SHA) {
        return {
          ok: false,
          code: 'HELD__REGISTRY_ANCHOR_VERIFICATION_FAILED',
          error: 'Anchored extension registry digest mismatch: ' + extSha + ' !== ' + EXPECTED_R4_EXTENSION_SHA
        };
      }
      const extParsed = JSON.parse(extRaw.toString('utf8'));
      if (!extParsed || extParsed.registry_version !== 'J358-R4-TRUST-EXTENSION-V1' || !extParsed.leaves) {
        return {
          ok: false,
          code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED',
          error: 'Invalid extension registry schema'
        };
      }
      Object.assign(parsed.leaves, extParsed.leaves);
      extensionInfo = { path: extNormalized, sha256: extSha, realPath: extRealPath, leaves_count: Object.keys(extParsed.leaves).length };
    }

    return { ok: true, registry: parsed, sha256: fileSha, realPath, extension: extensionInfo };
  } catch (e) {
    return {
      ok: false,
      code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED',
      error: 'Canonical metadata registry read error: ' + e.message
    };
  }
}
 ...
 */
function authenticateCardIdentifier(rawOfferId, chosenCard, rawBuffer, meta, leafEntry, options = {}) {
  // 1. Check offer ID presence
  ...
  // 3. Sealed Registry Leaf Entry Check (Req 16, 17)
  if (!leafEntry || typeof leafEntry !== 'object') {
    return {
      isValid: true,
      isAuthenticated: false,
      raw_id: trimmed,
      namespace,
      reason: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED: No registered leaf entry in sealed trust root'
    };
  }

  // 4. Look up permitted card in sealed registry leaf entry
  const allowedCards = leafEntry.allowed_cards || {};
  const boundCard = allowedCards[trimmed];
  if (!boundCard || typeof boundCard !== 'object') {
    return {
      isValid: true,
      isAuthenticated: false,
      raw_id: trimmed,
      namespace,
      reason: 'HELD__UNAUTHENTICATED_CARD_IDENTIFIER: Offer ID "' + trimmed + '" is not registered in sealed trust root for leaf "' + leafEntry.leaf_id + '"'
    };
  }

  // 5. Multi-Attribute Binding Verification against sealed registry (Req 16, 17)
  // (a) DOM Selector binding check
  if (chosenCard && boundCard.dom_selector) {
    const sel = boundCard.dom_selector.toLowerCase();
    ...
    if (!selectorMatches) {
      return {
        ...
        raw_id: trimmed,
        namespace,
        reason: 'HELD__MANIFEST_CARD_SELECTOR_MISMATCH: Bound DOM selector "' + boundCard.dom_selector + '" does not match chosen card <' + chosenCard.tagName + ' class="' + (chosenCard.attrs.class || '') + '">'
      };
    }
  }

  // (b) Card Byte Range binding check
  if (chosenCard && boundCard.card_byte_range) {
    let boundStart, boundEnd;
    if (Array.isArray(boundCard.card_byte_range)) {
      boundStart = boundCard.card_byte_range[0];
      boundEnd = boundCard.card_byte_range[1];
    } else if (typeof boundCard.card_byte_range === 'object') {
      boundStart = boundCard.card_byte_range.start;
      boundEnd = boundCard.card_byte_range.end;
    }
  ...
  const mappingKey = 'sealed_registry.leaves["' + leafEntry.leaf_id + '"].allowed_cards["' + trimmed + '"]';
  const manifestDigest = leafEntry.manifest_digest || sha256(Buffer.from(JSON.stringify(boundCard), 'utf8'));
  ...
  return {
    ...
    binding_proof: {
      leaf_id: leafEntry.leaf_id,
      source_sha256: leafEntry.raw_sha256,
      dom_selector: boundCard.dom_selector,
      card_byte_range: boundCard.card_byte_range,
      manifest_digest: manifestDigest
    },
    digest: sha256(trimmed),
    reason: 'AUTHENTICATED_BY_IMMUTABLE_SEALED_TRUST_ROOT'
  };
}
...
function evaluateCandidate(arg1, arg2, arg3, arg4) {
  ...
  // REQ 19: Check for path traversal in leaf ID
  if (!leafId || typeof leafId !== 'string') {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__LEAF_ID_MISSING: No leaf identifier provided in candidate or metadata',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['leaf_identity']
    };
  }
  if (leafId.includes('/') || leafId.includes('\\') || leafId.includes('..')) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__LEAF_PATH_TRAVERSAL_REJECTED: Leaf identifier contains path traversal characters',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['leaf_identity']
  ...
  // REQ 15, 16: Check for forbidden caller override attempts
  ...
  for (const forbiddenKey of FORBIDDEN_CALLER_KEYS) {
    if (options && options[forbiddenKey] !== undefined) {
      return {
        b19_id: b19Id,
        brand,
        status: 'HELD',
        held_reason: 'HELD__CALLER_REGISTRY_OVERRIDE_FORBIDDEN: Caller-supplied registry or allowlist is strictly forbidden; must use canonical anchored registry',
        preconditions_passed: false,
        offer_card: null,
        dimensions: {},
        missing_dimensions: ['sealed_trust_root']
      };
    }
  }

  // REQ 15, 16, 17: Unconditionally load canonical anchored registry
  const registryResult = loadCanonicalAnchoredRegistry();
  if (!registryResult.ok) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: registryResult.code + ': ' + registryResult.error,
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['sealed_trust_root']
  ...
  const sealedRegistry = registryResult.registry;
  const leafEntry = sealedRegistry.leaves[leafId];
  if (!leafEntry) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__LEAF_NOT_IN_SEALED_REGISTRY: Leaf identifier "' + leafId + '" is not registered in sealed trust root',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['sealed_trust_root']
  ...
  // REQ 17: Authenticate metadata bytes and raw source bytes against sealed registry
  if (!rawBuffer || !Buffer.isBuffer(rawBuffer)) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__RAW_BUFFER_MISSING_OR_INVALID: Raw HTML buffer is required',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['raw_source_bytes']
    };
  }
  const rawSha = sha256(rawBuffer);
  if (rawSha !== leafEntry.raw_sha256) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__REGISTRY_SOURCE_SHA_MISMATCH: Raw buffer SHA (' + rawSha + ') does not match registered leaf source SHA (' + leafEntry.raw_sha256 + ')',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
  ...
  if (!meta || typeof meta !== 'object') {
    return {
      ...
      brand,
      status: 'HELD',
      held_reason: 'HELD__METADATA_OBJECT_MISSING: Metadata object is null or missing',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['metadata_integrity']
    };
  }
  const metaBytes = options.metadataBytes || (meta && meta._rawMetadataBytes ? meta._rawMetadataBytes : Buffer.from(JSON.stringify(meta), 'utf8'));
  const metaSha = sha256(metaBytes);
  if (metaSha !== leafEntry.meta_sha256) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__METADATA_HASH_NOT_IN_SEALED_REGISTRY: Metadata hash (' + metaSha + ') does not match registered leaf metadata hash (' + leafEntry.meta_sha256 + ')',
      preconditions_passed: false,
      offer_card: null,
      dimensions: {},
      missing_dimensions: ['metadata_integrity']
  ...
  if (uniqueCards.length > 1) {
    const hasDistinguishingId = uniqueCards.some(c => {
      ...
      return authenticateCardIdentifier(oid, c, rawBuffer, meta, leafEntry, { leaf_id: leafId }).isValid;
    });
  ...
  // Step 6: Authenticate Card Identifier strictly via sealed registry (Req 15, 16, 17)
...
module.exports = {
  ...
  // Low-level structural helpers exported for testing only (cannot grant VERIFIED)
  isExplicitOfferCard,
  parseHTMLToDOM,
  findAllRawByteSpans
};
