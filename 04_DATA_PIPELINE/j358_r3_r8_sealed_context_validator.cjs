/**
 * J358-R3-R8: Sealed Context Validator
 * 
 * Mandate & Invariants (Work Order J358-R3-R8-REQUIRE-SEALED-METADATA-CONTEXT):
 * 1. Public promotion evaluator must require sealed metadata context; absence of a registered context fails closed (Req 15).
 * 2. Load metadata hash allowlist and leaf bindings from an immutable sealed registry owned by verification path,
 *    never from candidate, metadata object, evaluator options or caller-provided allowlist (Req 16).
 * 3. Authenticate metadata bytes before parsing manifest; bind registry entry to leaf ID, metadata file digest,
 *    raw source digest and permitted manifest digest (Req 17).
 * 4. Do not export any raw/pure function capable of returning VERIFIED without registry check (Req 18).
 * 5. Distinct fail-closed reason codes: HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED, HELD__CALLER_ALLOWLIST_OVERRIDE_FORBIDDEN,
 *    HELD__LEAF_NOT_IN_SEALED_REGISTRY, HELD__METADATA_HASH_NOT_IN_SEALED_REGISTRY, HELD__REGISTRY_SOURCE_SHA_MISMATCH (Req 19).
 * 
 * Mandate:
...
const SEALED_REGISTRY_PATH = path.resolve(__dirname, '..', '06_TRUST_AND_EVIDENCE', 'batch_19_r3_r8_replay', 'SEALED_METADATA_REGISTRY.json');
const EXPECTED_REGISTRY_SHA = 'd592d72fff5d923d93c939acc863bcdc4829bcd68e8cc90b3b61274066f0b1e4';

function loadSealedRegistry(customPath = null) {
  const regPath = customPath || SEALED_REGISTRY_PATH;
  if (!fs.existsSync(regPath)) {
    return { ok: false, code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED', error: 'Sealed metadata registry not found: ' + regPath };
  }
  try {
    const raw = fs.readFileSync(regPath);
    const fileSha = sha256(raw);
    if (fileSha !== EXPECTED_REGISTRY_SHA && !customPath) {
      return { ok: false, code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED', error: 'Sealed metadata registry digest mismatch: ' + fileSha };
    }
    const parsed = JSON.parse(raw.toString('utf8'));
    if (!parsed || parsed.registry_version !== 'J358-R3-R8-TRUST-ROOT-V1' || !parsed.leaves) {
      return { ok: false, code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED', error: 'Invalid registry schema' };
    }
    return { ok: true, registry: parsed, sha256: fileSha };
  } catch (e) {
    return { ok: false, code: 'HELD__SEALED_CONTEXT_MISSING_OR_UNREGISTERED', error: 'Sealed metadata registry read error: ' + e.message };
  }
}
...
function evaluateCandidate(arg1, arg2, arg3, arg4) {
  ...
  // REQ 16: Reject caller-supplied allowlists or attempts to override sealed trust root
  if (options && (options.validMetadataHashes || options.customAllowlist || options.allowlist)) {
    return {
      b19_id: b19Id,
      brand,
      status: 'HELD',
      held_reason: 'HELD__CALLER_ALLOWLIST_OVERRIDE_FORBIDDEN: Caller-supplied metadata allowlist is strictly forbidden; must use sealed registry',
  ...
  // REQ 15, 16, 17: Mandatory Sealed Metadata Context Check from Registry
  const registryResult = loadSealedRegistry(options.customRegistryPath || null);
