...
const JAYT_STOREFRONT_VERSION = 'v3.424.3-staging.bp';
...
function renderSpotlightLargeCard(item) {
  ...
  return `
    <div class="hero-sidecar-daily-pick-bp" data-item-id="${itemId}">
      <div class="sidecar-badge-row">
        <span class="sidecar-badge-verified">✓ ĐÃ ĐỐI SOÁT</span>
        <span class="sidecar-badge-time">T6, T7, CN</span>
      </div>
      <h3 class="sidecar-title">${item.title}</h3>
      <p class="sidecar-fact">Áp dụng tại các cụm rạp CGV TP. Đà Nẵng khi thanh toán qua VNPAY-QR.</p>
      <div class="sidecar-action-row">
        <button class="btn-sidecar-main" data-card-detail="${itemId}">
          Xem lịch chiếu & điều kiện &rarr;
        </button>
        <a href="${item.official_source_url}" target="_blank" rel="noopener noreferrer" class="sidecar-source-link">
          Nguồn: cgv.vn &nearr;
        </a>
      </div>
    </div>
...
function renderDailyGuideHome() {
  ...
  return `
    <div class="home-guide-layout-bp">
      <!-- 1. THE CITY IS THE INTERFACE HERO (DRAGON BRIDGE REAL PHOTOGRAPH IS THE MAIN SUBJECT) -->
      <section class="hero-city-interface-bp" aria-label="Hero Cẩm Nang Thành Phố Đà Nẵng">
        <!-- Real Photograph Backdrop with Soft Bottom Scrim -->
        <div class="hero-photo-stage">
          <img 
            ...
            class="hero-dragon-bridge-img-bp" 
            id="hero-main-photo"
            loading="eager"
          />
          <div class="hero-soft-scrim"></div>
          <div class="hero-photo-credit-tag">
            📍 Cầu Rồng & Sông Hàn &bull; 📷 Tác giả Bùi Thụy Đào Nguyên (CC BY-SA 3.0)
          </div>
        </div>

        <div class="hero-interface-grid-bp">
          <!-- Left Quiet Space: Editorial Headline & Minimal Discovery Controls (Max 30% Area) -->
          <div class="hero-quiet-space">
            <div class="hero-kicker-pill-bp">
              <span class="pulse-dot"></span> 📍 ĐÀ NẴNG &bull; CITY GUIDE
            </div>
            <h1 class="hero-headline-bp">
              Đà Nẵng, Chọn Điều Hay Mỗi Ngày
            </h1>
            <p class="hero-subcopy-bp">
              Cẩm nang tuyển chọn độc lập cho người trẻ và cư dân Đà Nẵng.
            </p>

            <!-- 3 Minimal Text Discovery Chips + 1 Subtle Icon Filter Button -->
            <div class="hero-minimal-discovery-bar">
              <button class="minimal-chip active" data-quick-slot="EVENING">🌙 Tối nay</button>
              <button class="minimal-chip" data-quick-loc="HAI_CHAU">📍 Gần tôi</button>
              <button class="minimal-chip" data-quick-budget="UNDER_100K">💰 Dưới 100k</button>
              <button class="btn-minimal-filter-icon" id="btn-trigger-filter-drawer" aria-label="Mở bộ lọc chi tiết" title="Mở bộ lọc chi tiết">
                <span>⚡</span>
              </button>
            </div>
          </div>

          <!-- Right Floating Sidecar: Daily Pick (Max 380px, Frosted Glass, Not Billboard) -->
          <div class="hero-sidecar-slot">
