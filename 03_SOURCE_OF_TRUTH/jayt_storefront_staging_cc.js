/**
 * JAYT COMMUNITY OS — LOCAL EDITORIAL COMMERCE (SECTION CC)
 * Release Candidate: v3.426.7-staging.cc
 * Governing Directive: JAYT-245 Section CC (Lines 1940-1971)
 * Key Architecture:
 *  - Living Magazine Cover First Fold (Cầu Rồng Sông Hàn)
 *  - Đà Nẵng Living Palette (Warm Sand, River Navy, Sunset Amber, Ocean Cyan)
 *  - 3 Core Visual Modules: Nhịp Hôm Nay, Góc Địa Phương, Hành Trình Quyết Định
 *  - Generous Breathing Room & Editorial Storytelling
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.426.7-staging.cc',
    environment: 'STAGING_REVIEW_CC',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.426.7-staging.cc';
...
function getCurrentTimeSlotInfo() {
  ...
  if (hour >= 5 && hour < 11) {
    return { slot: 'SLOT_MORNING', label: 'Sáng nay', greeting: 'Chào buổi sáng Đà Nẵng', momentTag: '🌅 BUỔI SÁNG: CÀ PHÊ & ĐIỂM TÂM' };
  } else if (hour >= 11 && hour < 14) {
    return { slot: 'SLOT_LUNCH', label: 'Trưa nay', greeting: 'Bữa trưa năng lượng tại Đà Nẵng', momentTag: '🍜 BUỔI TRƯA: ĂN TRƯA TIẾP SỨC' };
  } else if (hour >= 14 && hour < 18) {
    return { slot: 'SLOT_AFTERNOON', label: 'Chiều nay', greeting: 'Chiều mát Đà Nẵng', momentTag: '☕ BUỔI CHIỀU: TỰ HỌC & LÀM VIỆC' };
  } else {
    return { slot: 'SLOT_EVENING', label: 'Tối nay', greeting: 'Đêm Đà Nẵng rực rỡ', momentTag: '✨ BUỔI TỐI: DẠO SÔNG HÀN & RẠP PHIM' };
  }
}

// RESOLVE DISTINCT CTA ACTION CONTRACT (CC MANDATE #6)
...
function renderHeroCivicCityNote() {
  return `
    <div class="hero-civic-note-ca" role="region" aria-label="Điểm Nhấn Đô Thị Hôm Nay">
      <div class="civic-note-kicker-ca">📍 ĐIỂM HẸN VĂN HÓA ĐÔ THỊ</div>
      <h3 class="civic-note-title-ca">Cầu Rồng Sông Hàn</h3>
      <p class="civic-note-desc-ca">
        Điểm hẹn ngắm cảnh ven sông Hàn. Phun lửa & nước vào 21:00 Thứ 7 & Chủ Nhật hàng tuần.
      </p>
      <div class="civic-note-link-row">
        <a href="https://danang.gov.vn" target="_blank" rel="noopener noreferrer" class="civic-source-link">
          Cổng TP Đà Nẵng (danang.gov.vn) &rarr;
        </a>
      </div>
    </div>
  `;
...
// Render Editorial Story Feature Card ("Góc Địa Phương" — CC Mandate #4)
...
// 1. HOME VIEW (LOCAL EDITORIAL COMMERCE — CC MANDATE #3 & #4)
function renderDailyGuideHome() {
  ...
  return `
    <div class="home-editorial-layout-cc">
      <!-- 1. LIVING MAGAZINE COVER FIRST FOLD (CC MANDATE #3) -->
      <section class="magazine-cover-hero-cc" aria-label="Bìa Cẩm Nang Sống Đà Nẵng">
        <div class="magazine-cover-stage">
          <img 
            ...
            loading="eager"
          />
          <div class="magazine-cover-scrim"></div>
          
          <div class="magazine-credit-tag" tabindex="0" role="note" aria-label="Thông tin bản quyền ảnh Cầu Rồng">
            📍 Cầu Rồng Sông Hàn &bull; 📷 Bùi Thụy Đào Nguyên (CC BY-SA 3.0)
          </div>
        </div>

        <div class="magazine-cover-overlay">
          <div class="magazine-header-group">
            <span class="magazine-issue-tag">ĐÀ NẴNG &bull; LOCAL EDITORIAL &bull; ${timeInfo.momentTag}</span>
            <h1 class="magazine-main-title">
              Đà Nẵng, Nhịp Sống Hôm Nay
            </h1>
            <p class="magazine-subhead">
              Cẩm nang phong cách sống đô thị, ẩm thực bản địa, di chuyển công cộng và tiện ích học đường.
            </p>

            <!-- 3 FIRST-FOLD LIVING ACTIONS -->
            <div class="magazine-three-actions">
              <button class="btn-living-action btn-action-food" data-scroll-to="journey-food">
                🍜 Ăn gì gần đây?
              </button>
              <button class="btn-living-action btn-action-leisure" data-scroll-to="journey-leisure">
                ✨ Tối nay đi đâu?
              </button>
              <button class="btn-living-action btn-action-wallet" data-nav="WALLET">
                🎟️ Có quyền lợi gì?
              </button>
            </div>
          </div>
          ...
          <!-- COMPACT CIVIC NOTE -->
          <div class="magazine-civic-note-wrapper">
            ${renderHeroCivicCityNote()}
          </div>
        </div>
      </section>

      <!-- 2. MODULE 1: NHỊP HÔM NAY (DAILY RHYTHM CHIPS — CC MANDATE #4) -->
      ...
      <!-- 3. MODULE 2: GÓC ĐỊA PHƯƠNG (EDITORIAL STORY FEATURE — CC MANDATE #4) -->
      <section class="section-local-editorial-feature" aria-label="Góc Địa Phương">
        ${renderLocalEditorialFeatureCard(huynhThucKhang)}
      </section>

      <!-- 4. MODULE 3: HÀNH TRÌNH QUYẾT ĐỊNH (5 DECISION RAILS — CC MANDATE #4) -->
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Local Editorial &bull; v3.426.7-staging.cc</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.426.7-staging.cc &bull; Staging Review CC</span>
          </div>
        </div>
      </footer>
