/**
 * JAYT COMMUNITY OS — DEAL & VOUCHER DISCOVERY STOREFRONT (SECTION CV)
 * Release Candidate: v3.429.0-staging.cv
 * Governing Directive: JAYT-245 Section CV (Lines 2366-2383) - Deal, Voucher & Smart Shopping Discovery
 * Architecture Upgrades:
 *  - First-Fold Transformation: 3 Core Customer Shopping Actions (Deals, Vouchers, Price Check)
 *  - 4-Tier Transparent Directory: 8 Verified Deals, 14 Official Programmes, 18 Utilities, 10 Radars (Total 50 Items)
 *  - 3-Lane Verified Voucher Wallet: Active Codes, Official Portals, Radar Channels
 *  - 100% Real Photography with Transparent Derivative Labels (HD 1280 Web Optimized)
 *  - Strict Zero Fake Social Proof (No 5-star claims, No fake quotes)
 *  - Compact 52px Header & Strict 390x844 Mobile First-Fold Contract
 */
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.429.0-staging.cv',
    environment: 'STAGING_REVIEW_CV',
    theme: 'DEAL_VOUCHER_DISCOVERY',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.429.0-staging.cv';
...
// 1. PRODUCT-FIRST STOREFRONT HOME VIEW (SECTION CV)
function renderDailyGuideHome() {
  const timeInfo = getCurrentTimeSlotInfo();
  const topDeals = JAYT_DISCOVERY_ITEMS.filter(i => i.tier_level === 'TIER_1_DEAL').slice(0, 4);

  return `
    <div class="cr-experience-container">
      <!-- 1. LANDMARK-LED SHOPPING HERO (3 ACTION ENTRY POINTS) -->
      <section class="hero-landmark-cr vivid-dragon-hero-cf" aria-label="Bìa Khám Phá Deal & Tiện Ích Đà Nẵng">
        <img 
        ...
        <div class="hero-landmark-content">
          ...
          <h1 class="hero-headline-cr vivid-main-title">
            Khám Phá Deal, Voucher & Nhịp Sống Đà Nẵng
          </h1>
          <p class="hero-subhead-cr vivid-subhead">
            Cổng thông tin đối soát ưu đãi thật, quyền lợi học đường 3 làn và tiện ích đô thị chính thống tại Đà Nẵng.
          </p>

          <!-- 3 CORE ACTION ENTRY POINTS IN FIRST FOLD -->
      ...
      <!-- 3. FEATURED VERIFIED DEALS SECTION (TIER 1 DEALS) -->
      <section id="featured-deals-section" aria-label="Ưu Đãi Đã Đối Soát Hôm Nay">
        <div class="section-flow-header">
          <div>
            <h2 class="section-flow-title">🔥 Ưu Đãi Đã Đối Soát Hôm Nay</h2>
            <p class="section-flow-subtitle">Giá niêm yết thực tế, điều kiện áp dụng rõ ràng và nguồn xác thực từ thương hiệu.</p>
          </div>
          <button class="section-flow-viewall" data-nav="EXPLORE" data-tier-filter="TIER_1_DEAL">
            Xem tất cả deal &rarr;
          </button>
        </div>

        <div class="cards-layout-grid">
          ${topDeals.map(item => renderRailCard(item)).join('')}
        </div>
      </section>

      <!-- 4. REAL PHOTO SPOTLIGHT: CULINARY HERITAGE (MÌ QUẢNG HUỲNH THÚC KHÁNG) -->
      ...
      <!-- 5. REAL PHOTO SPOTLIGHT: GREEN TRANSIT (CẦU SÔNG HÀN & DANABUS) -->
      ...
      <!-- 6. REAL PHOTO SPOTLIGHT: KNOWLEDGE & PROMENADE (BẠCH ĐẰNG) -->
      ...
      <!-- 7. REAL PHOTO SPOTLIGHT: HERITAGE & EVENING LEISURE (BẢO TÀNG CHĂM) -->
  ...
  root.innerHTML = `
    <div class="jayt-app-shell theme-${currentTheme}">
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            <svg width="28" height="28" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path d="M19 6 V19 C19 24.5 15.5 28 10 28 C6.5 28 4.2 26 3.5 24 C5.2 24 7 22.8 7.5 20.8 C8 18.5 6.5 16.5 4.5 16.5 C3.8 16.5 3 16.8 2.5 17.2 C3.5 10.5 11 6 19 6 Z" fill="url(#jflow-logo-grad-cv)"/>
              <path d="M19 14 C23.5 14 27.5 17 29.5 21 C31 24 32.5 28 33.5 31 C29.5 29 25 27.5 19 27.5 V21.5 C22 21.5 24.5 23 26 24.5 C24.5 20 20.5 17.5 16 17 L19 14 Z" fill="url(#jflow-logo-grad-cv)" opacity="0.9"/>
              <circle cx="27" cy="8" r="3.5" fill="#f43f5e"/>
              <defs>
                <linearGradient id="jflow-logo-grad-cv" x1="4" y1="4" x2="34" y2="34" gradientUnits="userSpaceOnUse">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Deal, Voucher & Tiện Ích &bull; v3.429.0-staging.cv</span>
            </div>
          </div>
      ...
      <footer style="text-align: center; padding: 32px 16px; font-size: 0.8rem; color: var(--text-muted); border-top: 1px solid var(--border-color);" role="contentinfo">
        <div>JayT Đà Nẵng &bull; Chuẩn Deal & Voucher Discovery &bull; v3.429.0-staging.cv</div>
      </footer>
