/**
 * JAYT COMMUNITY OS — 7-DEPARTMENT CONSOLIDATED STOREFRONT (SECTION CY)
 * Release Candidate: v3.432.0-staging.cy
 * Governing Directive: JAYT-245 Section CY (Lines 2423-2440) - Unified Brief Pipeline
 * Pipeline Architecture:
 *   data/tier evidence -> customer flow -> component system -> photo/asset gate -> responsive QA -> Council -> CEO Review
 * Core Customer Promise:
 *   "Đà Nẵng Để Sống Hay Hơn Hôm Nay — Khám Phá Deal, Voucher Thật & Ra Quyết Định Mua Sắm Thông Minh"
 */
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.432.0-staging.cy',
    environment: 'STAGING_REVIEW_CY',
    theme: 'CONSOLIDATED_UNIFIED_BRIEF',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.432.0-staging.cy';
...
// 1. PRODUCT-FIRST STOREFRONT HOME VIEW (SECTION CY)
function renderDailyGuideHome() {
  ...
  return `
    <div class="cr-experience-container">
      <!-- 1. PRODUCT-FIRST STAGE HERO (CY 2-SECOND PROMISE & 3 INTENT TARGETS) -->
      <section class="hero-landmark-cr vivid-dragon-hero-cf" aria-label="Bìa Đặc Quyền Đà Nẵng">
        ...
        <div class="hero-landmark-content">
          ...
          <!-- 3 CORE 44px TOUCH TARGETS (CY 5-SECOND INTENT SELECTION) -->
  ...
  root.innerHTML = `
    <div class="jayt-app-shell theme-${currentTheme}">
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            <svg width="28" height="28" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path d="M19 6 V19 C19 24.5 15.5 28 10 28 C6.5 28 4.2 26 3.5 24 C5.2 24 7 22.8 7.5 20.8 C8 18.5 6.5 16.5 4.5 16.5 C3.8 16.5 3 16.8 2.5 17.2 C3.5 10.5 11 6 19 6 Z" fill="url(#jflow-logo-grad-cy)"/>
              <path d="M19 14 C23.5 14 27.5 17 29.5 21 C31 24 32.5 28 33.5 31 C29.5 29 25 27.5 19 27.5 V21.5 C22 21.5 24.5 23 26 24.5 C24.5 20 20.5 17.5 16 17 L19 14 Z" fill="url(#jflow-logo-grad-cy)" opacity="0.9"/>
              <circle cx="27" cy="8" r="3.5" fill="#f43f5e"/>
              <defs>
                <linearGradient id="jflow-logo-grad-cy" x1="4" y1="4" x2="34" y2="34" gradientUnits="userSpaceOnUse">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Sống Hay Hơn Mỗi Ngày &bull; v3.432.0-staging.cy</span>
            </div>
          </div>
      ...
      <footer style="text-align: center; padding: 32px 16px; font-size: 0.8rem; color: var(--text-muted); border-top: 1px solid var(--border-color);" role="contentinfo">
        <div>JayT Đà Nẵng &bull; Chuẩn Brief Hội Đồng CY &bull; v3.432.0-staging.cy</div>
      </footer>
