/**
 * JAYT COMMUNITY OS — THREE-LANE WALLET & OFFICIAL VALUE LAYER (SECTION BY)
 * Release Candidate: v3.426.4-staging.by
 * Governing Directive: JAYT-245 Section BY (Lines 1830-1855)
 * Key Architecture:
 *  - Three-Lane Information Architecture (Dùng ngay / Cổng chính thức / Theo dõi)
 *  - Official Value Layer (3 Questions: Là gì? Ai kiểm tra? Mở ở đâu?)
 *  - Strict 32x32px Brand Mark Contract
 *  - Single City Note in Hero
 *  - Zero Residual Modal Blur
 */

// 0. SILENT ZERO-PII OBSERVABILITY (STRICT PUBLIC DOM ISOLATION — BY MANDATE #1)
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.426.4-staging.by',
    environment: 'STAGING_REVIEW_BY',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.426.4-staging.by';
...
const JAYT_DISCOVERY_ITEMS = [
  {
    ...
    "badge_label": "KÊNH LIÊN KẾT THANH TOÁN",
    ...
    "summary_text": "Thông tin lịch chiếu và chính sách cổng thanh toán VNPAY-QR tại các cụm rạp CGV Đà Nẵng.",
  ...
  },
  {
    ...
    "evidence_status": "Bằng chứng thông tin sự kiện niêm yết • Đối soát dominos.vn",
...
// 2. THREE-LANE WALLET ENTRIES (13 STRUCTURED ENTRIES)
const JAYT_WALLET_ENTRIES = [
  {
    ...
    "evidence_status": "Bằng chứng văn bản chính thức • Đối soát education.github.com"
  },
  {
    "lane": "LANE_DUNG_NGAY",
    "lane_label": "DÙNG NGAY (ĐÃ XÁC THỰC)",
  ...
  },
  {
    ...
    "merchant_name": "CGV Cinemas x VNPAY-QR",
    "title": "CGV Cinemas: Cổng Thông Tin Cụm Rạp Đà Nẵng",
    "service_description": "Lịch chiếu phim, suất chiếu tiêu chuẩn và cổng thanh toán VNPAY-QR tại các rạp CGV Đà Nẵng.",
    "target_audience": "Khán giả điện ảnh có sử dụng ứng dụng ngân hàng liên kết VNPAY-QR.",
    "official_portal_guide": "Tra cứu lịch chiếu và thể lệ tại website cgv.vn hoặc tại quầy vé rạp CGV.",
    "official_source_url": "https://www.cgv.vn",
    "public_action_type": "VIEW_CONDITIONS",
    "action_button_label": "Mở cổng thông tin CGV →",
    "evidence_status": "Bằng chứng niêm yết chính thức • Đối soát cgv.vn"
  },
  {
    "lane": "LANE_CONG_CHINH_THUC",
    "lane_label": "CỔNG CHÍNH THỨC (ĐANG KIỂM TRA ĐIỀU KIỆN)",
...
function resolvePublicActionContract(item) {
  ...
  return {
    action_type: 'VIEW_OFFICIAL_SOURCE',
    label: item.action_label || 'Xem nguồn chính thức &rarr;',
    url: item.official_source_url
  };
}
...
// Render single City Note in Hero (BY Mandate #1 & #2)
function renderHeroCityNote(item) {
  if (!item) return '';
  const action = resolvePublicActionContract(item);

  return `
    <div class="city-note-card-bq" role="region" aria-label="Điểm Nhấn Đô Thị — ${item.title}">
      <div class="city-note-header">
        <span class="city-note-kicker">${item.badge_label || 'TIỆN ÍCH TIÊU BIỂU'}</span>
      ...
      <div class="card-body-content">
        ...
        <div class="card-field-specs">
          ...
          ${item.conditions_limit ? `
            <div class="field-spec-row">
              <span class="spec-label">ℹ️ Thông tin:</span>
...
// 1. HOME VIEW (RESTORED HIERARCHY — BY MANDATE #1)
function renderDailyGuideHome() {
  ...
  const primaryDailyPick = JAYT_DISCOVERY_ITEMS[0] || null;
    ...
    <div class="home-guide-layout-bq">
      ...
      <section class="hero-cinematic-arrival-bq" aria-label="Hero Cẩm Nang Thành Phố Đà Nẵng">
        ...
        <div class="hero-cinematic-overlay-content">
          ...
          <!-- EXACTLY ONE CITY NOTE (BY MANDATE #1) -->
      ...
      <section class="section-city-routes-bn" aria-label="3 Lộ Trình Khám Phá">
        <div class="section-header-editorial">
          <span class="section-kicker kicker-fac">${timeInfo.kicker}</span>
        ...
        <div class="city-routes-grid-bn">
          <div class="city-route-card-bn route-food" role="button" tabindex="0" data-gateway-target="AN_GI">
            ...
            <div class="route-info-box">
              ...
              <p class="route-subcopy">Ẩm thực đường phố Huỳnh Thúc Kháng, Chợ Cồn, điểm tâm và cà phê Đà Nẵng.</p>
          ...
          <div class="city-route-card-bn route-places" role="button" tabindex="0" data-gateway-target="DI_DAU">
            ...
            <div class="route-info-box">
              ...
              <h3 class="route-headline">Đi chơi & Dịch vụ công</h3>
              <p class="route-subcopy">Suất chiếu rạp phim, dạo Cầu Rồng, bờ Sông Hàn và mạng lưới xe buýt DanaBus.</p>
          ...
          <div class="city-route-card-bn route-tools" role="button" tabindex="0" data-gateway-target="MUA_SAM">
            ...
            <div class="route-info-box">
              ...
              <h3 class="route-headline">Đặc quyền học đường</h3>
              <p class="route-subcopy">Gói hỗ trợ sinh viên từ GitHub, Notion, Canva và chính sách xe buýt thành phố.</p>
      ...
      <!-- 4. HAI CỔNG DESTINATIONS (BUY DECISION & WALLET) -->
...
// 3. THREE-LANE WALLET VIEW WITH OFFICIAL VALUE LAYER (BY MANDATE #2 & #3)
function renderOfficialValueCard(entry) {
  ...
  return `
    <article class="value-layer-card" data-lane="${entry.lane}" data-category="${entry.category}">
      ...
      <!-- OFFICIAL VALUE LAYER: 3 STRUCTURED QUESTIONS (BY MANDATE #2) -->
...
function renderThreeLaneWalletView() {
  ...
  return `
    <div class="three-lane-wallet-page">
      ...
      <!-- THREE-LANE FILTER BAR (BY MANDATE #3) -->
      ...
      <!-- THREE-LANE CARDS GRID (BY MANDATE #2 & #4) -->
...
function renderAppShell() {
  ...
  root.innerHTML = `
    <div class="jayt-app-shell theme-${currentTheme}">
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Community OS &bull; v3.426.4-staging.by</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.426.4-staging.by &bull; Staging Review BY</span>
          </div>
        </div>
      </footer>
      ...
      <!-- GLOBAL TOAST & MODAL CONTAINER (STRICT ZERO-RESIDUAL BLUR) -->
...
// MODAL LIFECYCLE CONTROLLER (ZERO RESIDUAL BLUR)
