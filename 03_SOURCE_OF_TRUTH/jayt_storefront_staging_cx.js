/**
 * JAYT COMMUNITY OS — UNIFIED PRODUCT SYSTEM & 5-SEC TASK GATE (SECTION CX)
 * Release Candidate: v3.431.0-staging.cx
 * Governing Directive: JAYT-245 Section CX (Lines 2404-2420) - Customer Experience Gate
 * Architecture Upgrades:
 *  - 2-Second Customer Promise: "Đặc quyền thật cho hôm nay ở Đà Nẵng"
 *  - 5-Second Intent Selector: 3 Touch Targets (44px min-height) -> "Deal dùng hôm nay", "Voucher chính thức", "Kiểm tra trước khi mua"
 *  - First-Card Tier Transparency: 4 Clear Tier Badges with pricing, verified terms, and proof
 *  - Action Transparency: Honest destinations, zero fake codes, zero affiliate redirects
 *  - Dynamic Visual Rhythm: Hero -> Deal Radar (6 cards) -> 3-Lane Voucher Wallet -> Moment Collections -> Buy Decision Hub
 *  - Mobile Comfort: Strict 390x844 single-viewport contract without clipping
 */
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.431.0-staging.cx',
    environment: 'STAGING_REVIEW_CX',
    theme: 'PRODUCT_SYSTEM_CX_GATE',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.431.0-staging.cx';
...
// 1. PRODUCT-FIRST STOREFRONT HOME VIEW (SECTION CX)
function renderDailyGuideHome() {
  ...
  return `
    <div class="cr-experience-container">
      <!-- 1. PRODUCT-FIRST STAGE HERO (CX 2-SECOND PROMISE & 3 INTENT TARGETS) -->
      <section class="hero-landmark-cr vivid-dragon-hero-cf" aria-label="Bìa Đặc Quyền Đà Nẵng">
        ...
        <div class="hero-landmark-content">
          ...
          <!-- 3 CORE 44px TOUCH TARGETS (CX 5-SECOND INTENT SELECTION) -->
  ...
  root.innerHTML = `
    <div class="jayt-app-shell theme-${currentTheme}">
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            <svg width="28" height="28" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path d="M19 6 V19 C19 24.5 15.5 28 10 28 C6.5 28 4.2 26 3.5 24 C5.2 24 7 22.8 7.5 20.8 C8 18.5 6.5 16.5 4.5 16.5 C3.8 16.5 3 16.8 2.5 17.2 C3.5 10.5 11 6 19 6 Z" fill="url(#jflow-logo-grad-cx)"/>
              <path d="M19 14 C23.5 14 27.5 17 29.5 21 C31 24 32.5 28 33.5 31 C29.5 29 25 27.5 19 27.5 V21.5 C22 21.5 24.5 23 26 24.5 C24.5 20 20.5 17.5 16 17 L19 14 Z" fill="url(#jflow-logo-grad-cx)" opacity="0.9"/>
              <circle cx="27" cy="8" r="3.5" fill="#f43f5e"/>
              <defs>
                <linearGradient id="jflow-logo-grad-cx" x1="4" y1="4" x2="34" y2="34" gradientUnits="userSpaceOnUse">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Sống Hay Hơn Mỗi Ngày &bull; v3.431.0-staging.cx</span>
            </div>
          </div>
      ...
      <footer style="text-align: center; padding: 32px 16px; font-size: 0.8rem; color: var(--text-muted); border-top: 1px solid var(--border-color);" role="contentinfo">
        <div>JayT Đà Nẵng &bull; Chuẩn CX Gate Storefront &bull; v3.431.0-staging.cx</div>
      </footer>
