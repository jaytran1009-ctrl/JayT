/**
 * JAYT COMMUNITY OS — LOCAL EDITORIAL COMMERCE STOREFRONT (SECTION CO)
 * Release Candidate: v3.427.8-staging.co
 * Governing Directive: JAYT-245 Section CO (Lines 2209-2231) - Asset-First Intervention
 * Recomposed Architecture:
 *  - Board B "Đà Nẵng Mở Cửa": Warm Sand, Ocean Blue, Coral Action Highlights
 *  - Visual Slate Admitted: Full-bleed rich culinary, transit, library, and evening scenes
 *  - Broken Template Removed: Asymmetric Magazine Mosaic (Large Story + Horizontal Rail + Panoramic Transit)
 *  - Zero Empty Media Frames / Zero Yellow Placeholder Boxes
 *  - Zero Internal Toolbars / Zero Experiment Selectors
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.427.8-staging.co',
    environment: 'STAGING_REVIEW_CO',
    theme: 'DA_NANG_MO_CUA_ASSET_FIRST',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.427.8-staging.co';
...
const JAYT_DISCOVERY_ITEMS = [
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_transit_danabus_photo.svg",
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_study_library_photo.svg",
    "visual_attribution": "📚 Thư viện KHTH Đà Nẵng (46 Bạch Đằng &bull; danang.gov.vn)"
  },
  {
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_food_mi_quang_photo.svg",
  ...
  },
  {
    ...
    "visual_archetype": "VERIFIED_PLACE",
    "specific_action_label": "📍 Chỉ đường Google Maps",
    "visual_asset_url": "assets/images/danang_afterhours_river_photo.svg",
    "visual_attribution": "✨ Dạo Sông Hàn & Cầu Rồng &bull; TP. Đà Nẵng"
  },
  {
...
// Render Full-Bleed Editorial Feature Showcase (Zero Empty Boxes)
function renderFullBleedSpotlight(item, kickerText) {
  const isSaved = savedItemIds.has(item.item_id);
  const action = resolveActionContract(item);

  return `
    <article class="primary-archetype-card archetype-${item.visual_archetype.toLowerCase()} magazine-spotlight-cn" data-item-id="${item.item_id}">
      <!-- FULL-BLEED VISUAL STAGE -->
      ${item.visual_asset_url ? `
        <div class="spotlight-visual-stage-cn">
          <img 
            src="${item.visual_asset_url}" 
            alt="${item.title}" 
            class="spotlight-visual-img-cn" loading="eager"
          />
  ...
  return `
    <article class="rail-card-cn" data-item-id="${item.item_id}">
      <div class="rail-header-row">
        <span class="rail-tier-tag">${item.badge_label}</span>
...
// 1. RECOMPOSED HOME VIEW (SECTION CO: BOARD B "ĐÀ NẴNG MỞ CỬA")
function renderDailyGuideHome() {
  ...
  const banhTrang = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('BANH_TRANG')) || JAYT_DISCOVERY_ITEMS[3];

  const danaBus = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('DANABUS')) || JAYT_DISCOVERY_ITEMS[4];
  const tngoBike = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('TNGO')) || JAYT_DISCOVERY_ITEMS[5];
  const riversideWalk = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('SONG_HAN')) || JAYT_DISCOVERY_ITEMS[6];

  const library = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('LIBRARY') || i.item_id.includes('THU_VIEN')) || JAYT_DISCOVERY_ITEMS[7];
  const githubEdu = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('GITHUB')) || JAYT_DISCOVERY_ITEMS[8];
  const notionEdu = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('NOTION')) || JAYT_DISCOVERY_ITEMS[9];

  const chamMuseum = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('CHAM') || i.item_id.includes('BAO_TANG')) || JAYT_DISCOVERY_ITEMS[10];
  const cgvCinema = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('CGV')) || JAYT_DISCOVERY_ITEMS[11];
  const metizCinema = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('METIZ')) || JAYT_DISCOVERY_ITEMS[12];

  return `
    <div class="home-editorial-layout-cn">
      <!-- 1. ARRIVAL: VIVID DRAGON BRIDGE & COASTAL HERO -->
      <section class="vivid-dragon-hero-cf hero-coastal-cn" aria-label="Bìa Cẩm Nang Sống Đà Nẵng">
        ...
        <div class="vivid-hero-overlay">
          <div class="vivid-header-group">
            ...
            <div class="vivid-actions-bar">
              <button class="btn-primary-action-cf btn-coral-action-cn" data-nav="FOOD_JOURNEY">
                🍜 Ăn gì gần đây?
              </button>
      ...
      <!-- 2. NARRATIVE SECTION 1: ẨM THỰC XỨ QUẢNG (FULL-BLEED FEATURE + ACTION RAIL) -->
      <section class="narrative-journey-module" id="journey-food" aria-label="Hành Trình Ẩm Thực">
        ...
        <div class="editorial-mosaic-layout">
          <!-- Large Left Feature -->
          <div class="mosaic-feature-col">
            ${renderFullBleedSpotlight(huynhThucKhang, '📍 GỢI Ý ĐIỂM TÂM TRUNG TÂM')}
          </div>

          <!-- Right Horizontal/Vertical Action Rail -->
          <div class="mosaic-rail-col">
            <h3 class="rail-section-heading">Lựa chọn điểm tâm & giải khát khác</h3>
            <div class="editorial-rail-scroll">
              ${renderRailCard(choCon)}
              ${renderRailCard(bachDangCafe)}
              ${renderRailCard(banhTrang)}
            </div>
          </div>
        </div>
      </section>

      <!-- 3. NARRATIVE SECTION 2: DI CHUYỂN & GIAO THÔNG XANH (FULL-BLEED PANORAMIC) -->
      <section class="narrative-journey-module" id="journey-transport" aria-label="Hành Trình Di Chuyển">
        ...
        <div class="editorial-mosaic-layout">
          <div class="mosaic-feature-col">
            ${renderFullBleedSpotlight(danaBus, '🚌 TIỆN ÍCH GIAO THÔNG CÔNG CỘNG')}
          </div>
          <div class="mosaic-rail-col">
            <h3 class="rail-section-heading">Phương tiện kết nối linh hoạt</h3>
            <div class="editorial-rail-scroll">
              ${renderRailCard(tngoBike)}
              ${renderRailCard(riversideWalk)}
            </div>
          </div>
        </div>
      </section>

      <!-- 4. NARRATIVE SECTION 3: HỌC TẬP & TRI THỨC (LIBRARY SHOWCASE) -->
      <section class="narrative-journey-module" id="journey-study" aria-label="Hành Trình Học Tập">
        ...
        <div class="editorial-mosaic-layout">
          <div class="mosaic-feature-col">
            ${renderFullBleedSpotlight(library, '📚 KHÔNG GIAN TRI THỨC CÔNG CỘNG')}
          </div>
          <div class="mosaic-rail-col">
            <h3 class="rail-section-heading">Tiện ích học đường chính thống</h3>
            <div class="editorial-rail-scroll">
              ${renderRailCard(githubEdu)}
              ${renderRailCard(notionEdu)}
            </div>
          </div>
        </div>
      </section>

      <!-- 5. NARRATIVE SECTION 4: VĂN HÓA & GIẢI TRÍ TỐI NAY (AFTER-HOURS MOMENT) -->
      <section class="narrative-journey-module" id="journey-leisure" aria-label="Hành Trình Thư Giãn">
        ...
        <div class="editorial-mosaic-layout">
          <div class="mosaic-feature-col">
            ${renderFullBleedSpotlight(chamMuseum, '🏛️ ĐIỂM ĐẾN VĂN HÓA DI SẢN')}
          </div>
          <div class="mosaic-rail-col">
            <h3 class="rail-section-heading">Suất chiếu rạp & thư giãn tối nay</h3>
            <div class="editorial-rail-scroll">
              ${renderRailCard(cgvCinema)}
              ${renderRailCard(metizCinema)}
            </div>
          </div>
        </div>
      </section>
  ...
  return `
    <div class="leisure-journey-page" id="leisure-journey-root">
      ...
      <div class="journey-results-section">
        ${filtered.length > 0 ? `
          <div class="journey-spotlight-wrapper">
            ${renderFullBleedSpotlight(spotlight, '📍 LỰA CHỌN NỔI BẬT BUỔI TỐI')}
          </div>
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Đà Nẵng Mở Cửa &bull; v3.427.8-staging.co</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.427.8-staging.co &bull; Section CO Asset-First Release</span>
          </div>
        </div>
      </footer>
