/**
 * JAYT COMMUNITY OS — DESIGN ESCALATION (SECTION CK)
 * Release Candidate: v3.427.4-staging.ck
 * Governing Directive: JAYT-245 Section CK (Lines 2130-2151)
 * Studio Implementation: 3 Distinct Art-Direction Boards with Live Switcher
 *  - Board A: Sông Hàn Afterglow (Cinematic Night & Riverfront, Deep Blue/Amber)
 *  - Board B: Đà Nẵng Mở Cửa (Airy Coastal & Market Editorial, Warm Sand/Coral/Ocean)
 *  - Board C: Thành Phố Trong Một Ngày (Bold Urban Route & Time-Stream)
 */
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.427.4-staging.ck',
    environment: 'STAGING_REVIEW_CK',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.427.4-staging.ck';
...
const JAYT_DISCOVERY_ITEMS = [
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_danabus_art_002.svg",
    "visual_attribution": "🚌 DanaBus Đà Nẵng (danangbus.vn &bull; Open Civic Data)"
  },
  {
  ...
  },
  {
    ...
    "visual_attribution": "📚 Thư viện KHTH Đà Nẵng (46 Bạch Đằng)"
  },
  {
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_cham_museum_art_003.svg",
    "visual_attribution": "🏛️ Bảo tàng Điêu khắc Chăm (chammuseum.vn)"
  },
  {
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_mi_quang_art_001.svg",
    "visual_attribution": "🍜 Phố Huỳnh Thúc Kháng &bull; JayT Editorial Art"
  },
  {
...
// 3. SPA & STUDIO STATE
let activeBoard = 'BOARD_A'; // 'BOARD_A' | 'BOARD_B' | 'BOARD_C'
...
try {
  if (typeof localStorage !== 'undefined') {
    ...
    const board = localStorage.getItem('jayt_active_board');
...
function getCurrentTimeSlotInfo() {
  ...
  if (hour >= 5 && hour < 11) {
    return { slot: 'SLOT_MORNING', label: 'Sáng nay', momentTag: '🌅 BUỔI SÁNG &bull; CÀ PHÊ & ĐIỂM TÂM ĐÀ THÀNH' };
  } else if (hour >= 11 && hour < 14) {
    return { slot: 'SLOT_LUNCH', label: 'Trưa nay', momentTag: '🍜 BUỔI TRƯA &bull; ẨM THỰC TIẾP SỨC ĐÀ NẴNG' };
  } else if (hour >= 14 && hour < 18) {
    return { slot: 'SLOT_AFTERNOON', label: 'Chiều nay', momentTag: '☕ BUỔI CHIỀU &bull; TỰ HỌC & LÀM VIỆC VEN SÔNG' };
  } else {
    return { slot: 'SLOT_EVENING', label: 'Tối nay', momentTag: '✨ BUỔI TỐI &bull; DẠO SÔNG HÀN & RẠP PHIM ĐÀ THÀNH' };
  }
}
...
function resolveActionContract(item) {
  ...
  if (item.visual_archetype === 'OFFICIAL_PROGRAMME') {
    btnClass = 'btn-action-civic';
  } else if (item.visual_archetype === 'RADAR') {
...
// Render Compact Civic City Note
function renderHeroCivicCityNote() {
  return `
    <div class="hero-civic-note-cf" role="region" aria-label="Điểm Nhấn Đô Thị Hôm Nay">
      <div class="civic-note-inner-cf">
        <span class="civic-note-pin">📍</span>
        <div class="civic-note-text">
          <strong class="civic-note-title">Cầu Rồng Sông Hàn:</strong>
          <span class="civic-note-detail">Phun lửa & nước vào 21:00 Thứ Bảy & Chủ Nhật.</span>
        </div>
        <a href="https://danang.gov.vn" target="_blank" rel="noopener noreferrer" class="civic-note-link" aria-label="Mở cổng TP Đà Nẵng">
          danang.gov.vn &rarr;
        </a>
      </div>
    </div>
...
// Render Magazine-Style Primary Showcase Card
function renderPrimarySpotlightCard(item, kickerText) {
  const isSaved = savedItemIds.has(item.item_id);
  const action = resolveActionContract(item);

  return `
    <article class="primary-archetype-card archetype-${item.visual_archetype.toLowerCase()} magazine-spotlight-cj" data-item-id="${item.item_id}">
      <!-- VERIFIED EDITORIAL ARTWORK CROP -->
      ${item.visual_asset_url ? `
        <div class="spotlight-visual-stage-cj">
          <img 
            src="${item.visual_asset_url}" 
            alt="${item.title}" 
            class="spotlight-visual-img-cj" 
            loading="lazy"
          />
          <div class="spotlight-visual-attribution-tag">
            ${item.visual_attribution}
          </div>
        </div>
      ` : ''}

      <div class="card-kicker-strip">
        <span class="archetype-badge">${kickerText || '📍 GỢI Ý NỔI BẬT THEO KHU VỰC'}</span>
      ...
      <div class="card-main-content">
        <span class="card-brand-label">${item.brand}</span>
        <h3 class="card-headline">${item.title}</h3>
        
        ${item.curation_story ? `
          <p class="card-story-narrative">📖 ${item.curation_story}</p>
        ` : `
          <p class="card-editorial-narrative">${item.editorial_lead || item.summary_text}</p>
        `}

        <!-- HONEST RANKING RATIONALE BOX -->
        ${item.ranking_rationale ? `
          <div class="ranking-rationale-box">
            <span class="rationale-label">🔍 Tiêu chí xếp hạng:</span>
            <span class="rationale-text">${item.ranking_rationale}</span>
          </div>
        ` : ''}
        
        <div class="card-meta-details-row">
          <div class="meta-pill">📍 ${item.scope_text}</div>
          <div class="meta-pill">⏳ ${item.timing_window}</div>
          ${item.transit_hint ? `<div class="meta-pill transit-pill">${item.transit_hint}</div>` : ''}
        </div>
      </div>

      <div class="card-actions-strip">
        <a href="${action.url}" target="_blank" rel="noopener noreferrer" class="btn-primary-outcome ${action.btnClass}">
          ${action.label}
        </a>
        ${item.map_query_url ? `
          <a href="${item.map_query_url}" target="_blank" rel="noopener noreferrer" class="btn-map-shortcut" title="Chỉ đường trên Google Maps">
            📍 Chỉ đường Maps
          </a>
        ` : ''}
      </div>
      ...
      <div class="card-provenance-footer">
        <span>ℹ️ ${item.evidence_status}</span>
      </div>
    </article>
  `;
}

// Render Compact Secondary Item Card
function renderCompactSecondaryCard(item) {
  const isSaved = savedItemIds.has(item.item_id);
  const action = resolveActionContract(item);

  return `
    <article class="compact-editorial-card archetype-${item.visual_archetype.toLowerCase()}" data-item-id="${item.item_id}">
      <div class="compact-header-row">
        <span class="compact-tier-tag">${item.badge_label}</span>
      ...
      <div class="compact-body">
        ...
        <p class="compact-desc">${item.curation_story || item.summary_text}</p>
        <div class="compact-meta">📍 ${item.scope_text}</div>
        ${item.transit_hint ? `<div class="compact-transit">${item.transit_hint}</div>` : ''}
      </div>

      <div class="compact-footer-row">
        <a href="${action.url}" target="_blank" rel="noopener noreferrer" class="btn-compact-outcome ${action.btnClass}">
          ${action.label}
        </a>
        ${item.map_query_url ? `
          <a href="${item.map_query_url}" target="_blank" rel="noopener noreferrer" class="btn-map-icon" title="Xem vị trí">
            📍 Maps
          </a>
        ` : ''}
      </div>
...
// STUDIO ART-DIRECTION BOARD SWITCHER BAR
function renderBoardStudioSwitcher() {
  return `
    <section class="board-studio-bar-ck" role="region" aria-label="Studio Chọn Art Direction">
      <div class="studio-bar-inner">
        <div class="studio-label-group">
          <span class="studio-icon">🎨</span>
          <div>
            <strong class="studio-title">CEO Design Escalation &bull; 3 Hướng Art Direction:</strong>
            <span class="studio-desc">Chọn phong cách thị giác để trải nghiệm và nghiệm thu thẩm mỹ trực tiếp:</span>
          </div>
        </div>

        <div class="studio-buttons-group">
          <button class="btn-studio-board ${activeBoard === 'BOARD_A' ? 'active' : ''}" data-set-board="BOARD_A">
            <span class="b-tag">Board A</span>
            <span class="b-name">🌃 Sông Hàn Afterglow</span>
          </button>
          <button class="btn-studio-board ${activeBoard === 'BOARD_B' ? 'active' : ''}" data-set-board="BOARD_B">
            <span class="b-tag">Board B</span>
            <span class="b-name">🏖️ Đà Nẵng Mở Cửa</span>
          </button>
          <button class="btn-studio-board ${activeBoard === 'BOARD_C' ? 'active' : ''}" data-set-board="BOARD_C">
            <span class="b-tag">Board C</span>
            <span class="b-name">🧭 Thành Phố Trong Ngày</span>
          </button>
        </div>
      </div>
    </section>
...
// 1. HOME VIEW (DISPATCHED BY ACTIVE BOARD)
function renderDailyGuideHome() {
  const timeInfo = getCurrentTimeSlotInfo();

  const huynhThucKhang = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('HUYNH_THUC_KHANG')) || JAYT_DISCOVERY_ITEMS[0];
  const choCon = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('CHO_CON')) || JAYT_DISCOVERY_ITEMS[1];
  const bachDangCafe = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('BACH_DANG')) || JAYT_DISCOVERY_ITEMS[2];

  const danaBus = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('DANABUS')) || JAYT_DISCOVERY_ITEMS[3];
  const tngoBike = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('TNGO')) || JAYT_DISCOVERY_ITEMS[4];
  const riversideWalk = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('SONG_HAN')) || JAYT_DISCOVERY_ITEMS[5];

  const library = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('LIBRARY') || i.item_id.includes('THU_VIEN')) || JAYT_DISCOVERY_ITEMS[6];
  const githubEdu = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('GITHUB')) || JAYT_DISCOVERY_ITEMS[7];
  const notionEdu = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('NOTION')) || JAYT_DISCOVERY_ITEMS[8];

  const chamMuseum = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('CHAM') || i.item_id.includes('BAO_TANG')) || JAYT_DISCOVERY_ITEMS[9];
  const cgvCinema = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('CGV')) || JAYT_DISCOVERY_ITEMS[10];
  const metizCinema = JAYT_DISCOVERY_ITEMS.find(i => i.item_id.includes('METIZ')) || JAYT_DISCOVERY_ITEMS[11];

  let boardHeroSrc = 'assets/images/dragon_bridge_hero_001.jpg';
  let boardHeadline = 'Khám Phá Nhịp Sống & Tiện Ích Đà Nẵng';
  let boardSubhead = 'Cẩm nang kết nối ẩm thực bản địa, di chuyển công cộng và tiện ích học đường chính thống.';
  let boardClass = 'board-mode-a';

  if (activeBoard === 'BOARD_A') {
    boardHeroSrc = 'assets/images/board_a_afterglow_hero.svg';
    boardHeadline = 'Sông Hàn Afterglow & Nhịp Sống Đêm Đà Thành';
    boardSubhead = 'Khám phá ẩm thực đêm, dạo bộ ven sông Hàn, ngắm Cầu Rồng và trải nghiệm văn hóa sau giờ học.';
    boardClass = 'board-mode-a';
  } else if (activeBoard === 'BOARD_B') {
    boardHeroSrc = 'assets/images/board_b_coastal_hero.svg';
    boardHeadline = 'Đà Nẵng Mở Cửa & Sắc Màu Thành Phố Biển';
    boardSubhead = 'Không gian khám phá chợ truyền thống, ẩm thực xứ Quảng và lộ trình du lịch cộng đồng thoáng đãng.';
    boardClass = 'board-mode-b';
  } else if (activeBoard === 'BOARD_C') {
    boardHeroSrc = 'assets/images/board_c_urban_hero.svg';
    boardHeadline = 'Đà Nẵng 24 Giờ & Hành Trình Đô Thị Thông Minh';
    boardSubhead = 'Dòng chảy trải nghiệm xuyên suốt: Sáng điểm tâm, trưa làm việc, chiều buýt xanh, tối ngắm Cầu Rồng.';
    boardClass = 'board-mode-c';
  }

  return `
    <div class="home-editorial-layout-ck ${boardClass}">
      <!-- STUDIO SWITCHER -->
      ${renderBoardStudioSwitcher()}

      <!-- 1. ARRIVAL HERO -->
      <section class="vivid-dragon-hero-cf hero-board-adaptive" aria-label="Bìa Cẩm Nang Sống Đà Nẵng">
        <div class="vivid-hero-stage">
          <img 
            src="${boardHeroSrc}" 
            alt="${boardHeadline}" 
            class="vivid-hero-image" 
            id="hero-main-photo"
            loading="eager"
          />
          <div class="vivid-hero-scrim-light"></div>
          
          <div class="vivid-credit-tag" tabindex="0" role="note" aria-label="Thông tin bản quyền ảnh">
            📍 Đà Nẵng &bull; 📷 JayT Visual Studio (CC BY-SA 3.0/4.0)
          </div>
        </div>

        <div class="vivid-hero-overlay">
          <div class="vivid-header-group">
            <span class="vivid-issue-tag">${timeInfo.momentTag}</span>
            <h1 class="vivid-main-title">${boardHeadline}</h1>
            <p class="vivid-subhead">${boardSubhead}</p>

            <!-- 3 CONCISE ACTION BUTTONS -->
            <div class="vivid-actions-bar">
              <button class="btn-primary-action-cf" data-nav="FOOD_JOURNEY">
                🍜 Ăn gì gần đây?
              </button>
              <div class="chips-horizontal-group">
                <button class="chip-secondary-cf" data-nav="LEISURE_JOURNEY">
                  ✨ Đi đâu tối nay?
                </button>
                <button class="chip-secondary-cf" data-nav="WALLET">
                  🎟️ Ví quyền lợi
                </button>
              </div>
            </div>
          ...
          <!-- COMPACT LIGHTWEIGHT CIVIC NOTE -->
          <div class="vivid-civic-wrapper">
            ${renderHeroCivicCityNote()}
          </div>
        </div>
      </section>

      <!-- 2. TODAY'S MOMENT HIGHLIGHTS -->
      <section class="section-todays-edit-cj" aria-label="Nhịp Sống Hôm Nay">
        <div class="edit-section-header">
          <div>
            <span class="edit-kicker">⏳ NHỊP SỐNG THEO THỜI ĐIỂM &bull; ${activeBoard}</span>
            <h2 class="edit-title">Khám Phá Đúng Điểm &bull; Đúng Lúc</h2>
          </div>
        </div>

        <div class="todays-edit-tiles-grid">
          <div class="edit-tile tile-morning" data-nav="FOOD_JOURNEY" role="button" tabindex="0">
            <span class="tile-tag">🌅 SÁNG SỚM</span>
            <h3 class="tile-heading">Cà Phê Ven Sông Hàn & Điểm Tâm</h3>
            <p class="tile-desc">Phố Huỳnh Thúc Kháng & Cà phê đường Bạch Đằng đón nắng mai.</p>
            <span class="tile-action">Khám phá ăn sáng &rarr;</span>
          </div>

          <div class="edit-tile tile-lunch" data-nav="FOOD_JOURNEY" role="button" tabindex="0">
            <span class="tile-tag">🍜 BỮA TRƯA</span>
            <h3 class="tile-heading">Mì Quảng & Ẩm Thực Tiếp Sức</h3>
            <p class="tile-desc">Gợi ý quán ăn chuẩn vị, giá niêm yết rõ ràng tại các quận nội thành.</p>
            <span class="tile-action">Xem điểm ăn trưa &rarr;</span>
          </div>

          <div class="edit-tile tile-transit" data-explore-gw="DI_DAU" role="button" tabindex="0">
            <span class="tile-tag">🚌 DI CHUYỂN</span>
            <h3 class="tile-heading">Buýt Xanh DanaBus & Xe Đạp TNGO</h3>
            <p class="tile-desc">Tuyến R16 Sân Bay - Bến Xe và các trạm xe đạp công cộng ven sông.</p>
            <span class="tile-action">Tra cứu lộ trình &rarr;</span>
          </div>

          <div class="edit-tile tile-night" data-nav="LEISURE_JOURNEY" role="button" tabindex="0">
            <span class="tile-tag">✨ BUỔI TỐI</span>
            <h3 class="tile-heading">Cầu Rồng, Bảo Tàng & Rạp Chiếu Phim</h3>
            <p class="tile-desc">Lịch phun lửa 21:00 cuối tuần và suất chiếu rạp CGV/Metiz tối nay.</p>
            <span class="tile-action">Lên lịch tối nay &rarr;</span>
          </div>
        </div>
      </section>

      <!-- 3. NARRATIVE COLLECTION 1: ẨM THỰC BẢN ĐỊA ĐÀ THÀNH -->
      <section class="narrative-journey-module" id="journey-food" aria-label="Hành Trình Ẩm Thực">
        <div class="narrative-module-header">
          <div>
            <span class="journey-kicker-tag">🍜 HÀNH TRÌNH 1 &bull; ẨM THỰC BẢN ĐỊA</span>
            <h2 class="journey-title-ca">Hương Vị Đà Thành & Điểm Hẹn Ăn Uống</h2>
          </div>
          <button class="btn-journey-see-all" data-nav="FOOD_JOURNEY">Mở lộ trình ăn uống &rarr;</button>
        </div>

        <div class="narrative-grid-layout">
          <div class="narrative-spotlight-col">
            ${renderPrimarySpotlightCard(huynhThucKhang, '📍 GỢI Ý ĐIỂM TÂM TRUNG TÂM')}
          </div>
          <div class="narrative-secondary-col">
            ${renderCompactSecondaryCard(choCon)}
            ${renderCompactSecondaryCard(bachDangCafe)}
          </div>
        </div>
      </section>

      <!-- 4. NARRATIVE COLLECTION 2: DI CHUYỂN & GIAO THÔNG XANH -->
      <section class="narrative-journey-module" id="journey-transport" aria-label="Hành Trình Di Chuyển">
        <div class="narrative-module-header">
          <div>
            <span class="journey-kicker-tag">🚌 HÀNH TRÌNH 2 &bull; DI CHUYỂN THÔNG MINH</span>
            <h2 class="journey-title-ca">Xe Buýt Công Cộng & Xe Đạp Đô Thị</h2>
          </div>
          <button class="btn-journey-see-all" data-explore-gw="DI_DAU">Xem các tuyến buýt & trạm xe &rarr;</button>
        </div>

        <div class="narrative-grid-layout">
          <div class="narrative-spotlight-col">
            ${renderPrimarySpotlightCard(danaBus, '🚌 TIỆN ÍCH GIAO THÔNG CÔNG CỘNG')}
          </div>
          <div class="narrative-secondary-col">
            ${renderCompactSecondaryCard(tngoBike)}
            ${renderCompactSecondaryCard(riversideWalk)}
          </div>
        </div>
      </section>

      <!-- 5. NARRATIVE COLLECTION 3: HỌC TẬP & LÀM VIỆC -->
      <section class="narrative-journey-module" id="journey-study" aria-label="Hành Trình Học Tập">
        <div class="narrative-module-header">
          <div>
            <span class="journey-kicker-tag">🎓 HÀNH TRÌNH 3 &bull; HỌC TẬP & LÀM VIỆC</span>
            <h2 class="journey-title-ca">Không Gian Tự Học & Công Cụ Sinh Viên</h2>
          </div>
          <button class="btn-journey-see-all" data-explore-gw="MUA_SAM">Xem không gian học tập &rarr;</button>
        </div>

        <div class="narrative-grid-layout">
          <div class="narrative-spotlight-col">
            ${renderPrimarySpotlightCard(library, '📚 KHÔNG GIAN TRI THỨC CÔNG CỘNG')}
          </div>
          <div class="narrative-secondary-col">
            ${renderCompactSecondaryCard(githubEdu)}
            ${renderCompactSecondaryCard(notionEdu)}
          </div>
        </div>
      </section>

      <!-- 6. NARRATIVE COLLECTION 4: VĂN HÓA, DI SẢN & GIẢI TRÍ TỐI NAY -->
      <section class="narrative-journey-module" id="journey-leisure" aria-label="Hành Trình Thư Giãn">
        <div class="narrative-module-header">
          <div>
            <span class="journey-kicker-tag">✨ HÀNH TRÌNH 4 &bull; GIẢI TRÍ & VĂN HÓA TỐI NAY</span>
            <h2 class="journey-title-ca">Văn Hóa, Di Sản & Lịch Rạp Phim</h2>
          </div>
          <button class="btn-journey-see-all" data-nav="LEISURE_JOURNEY">Mở lịch đi chơi tối &rarr;</button>
        </div>

        <div class="narrative-grid-layout">
          <div class="narrative-spotlight-col">
            ${renderPrimarySpotlightCard(chamMuseum, '🏛️ ĐIỂM ĐẾN VĂN HÓA DI SẢN')}
          </div>
          <div class="narrative-secondary-col">
            ${renderCompactSecondaryCard(cgvCinema)}
            ${renderCompactSecondaryCard(metizCinema)}
          </div>
        </div>
      </section>

      <!-- 7. DESTINATIONS DUAL HUB -->
      <section class="narrative-journey-module" id="journey-shopping" aria-label="Hành Trình Tiện Ích">
        <div class="narrative-module-header">
          <div>
            <span class="journey-kicker-tag">🛒 HÀNH TRÌNH 5 &bull; TIỆN ÍCH TIÊU DÙNG & ĐẶC QUYỀN</span>
            <h2 class="journey-title-ca">Cổng Tư Vấn Giá Thực & Ví Quyền Lợi</h2>
          </div>
        </div>

        <div class="destinations-dual-grid-ca">
          <div class="dest-card-ca dest-card-buy" data-nav="BUY_DECISION" role="button" tabindex="0">
            <span class="dest-badge-ca">🛡️ TƯ VẤN TIÊU DÙNG</span>
            <h3 class="dest-title-ca">Mua món này có hời không?</h3>
            <p class="dest-desc-ca">Tra cứu giá thực, điều kiện ẩn và chính sách đổi trả trước khi thanh toán tại Đà Nẵng.</p>
            <span class="dest-action-link-ca">Kiểm tra thông tin mua sắm &rarr;</span>
          </div>

          <div class="dest-card-ca dest-card-wallet" data-nav="WALLET" role="button" tabindex="0">
            <span class="dest-badge-ca">🎟️ VÍ QUYỀN LỢI ĐÃ PHÂN CẤP</span>
            <h3 class="dest-title-ca">Ví Thông Tin & Đặc Quyền Học Đường</h3>
            <p class="dest-desc-ca">Tra cứu 3 làn: Dùng ngay, Cổng chính thức và Kênh theo dõi. Nguồn gốc rõ ràng.</p>
            <span class="dest-action-link-ca">Mở ví thông tin & đặc quyền &rarr;</span>
          </div>
        </div>
      </section>
    </div>
...
function renderFoodJourneyRoute() {
  ...
  const spotlight = filtered[0] || foodItems[0];
  const secondary = filtered.slice(1);

  return `
    <div class="food-journey-page" id="food-journey-root">
      <div class="journey-editorial-hero">
        <button class="btn-back-home" data-nav="HOME">&larr; Quay lại Trang Chủ</button>
        <span class="section-kicker kicker-fac">🍜 LỘ TRÌNH 1 &bull; ẨM THỰC BẢN ĐỊA ĐÀ NẴNG</span>
        <h1 class="journey-main-title">Ăn Gì Gần Bạn & Hương Vị Đà Thành</h1>
        <p class="journey-main-subcopy">
          Chọn bữa ăn và quận bạn đang ở để nhận đề xuất địa điểm chính xác kèm hướng dẫn di chuyển công cộng.
        </p>
      </div>

      <!-- FILTER CONTROLS BAR -->
      <div class="journey-filters-card">
        <div class="filter-row">
          <label class="filter-title">Bữa ăn:</label>
          <div class="filter-pills-row">
            <button class="food-filter-btn ${activeMealMoment === 'ALL' ? 'active' : ''}" data-meal-moment="ALL">Tất cả</button>
            <button class="food-filter-btn ${activeMealMoment === 'SANG' ? 'active' : ''}" data-meal-moment="SANG">🌅 Điểm tâm sáng</button>
            <button class="food-filter-btn ${activeMealMoment === 'TRUA' ? 'active' : ''}" data-meal-moment="TRUA">🍜 Bữa trưa</button>
            <button class="food-filter-btn ${activeMealMoment === 'CHIEU' ? 'active' : ''}" data-meal-moment="CHIEU">☕ Cà phê / Trà</button>
            <button class="food-filter-btn ${activeMealMoment === 'TOI' ? 'active' : ''}" data-meal-moment="TOI">🍲 Bữa tối / Ăn đêm</button>
          </div>
        </div>

        <div class="filter-row">
          <label class="filter-title">Quận:</label>
          <div class="filter-pills-row">
            <button class="food-loc-btn ${activeLocality === 'ALL' ? 'active' : ''}" data-food-loc="ALL">Toàn thành phố</button>
            <button class="food-loc-btn ${activeLocality === 'HAI_CHAU' ? 'active' : ''}" data-food-loc="HAI_CHAU">Hải Châu</button>
            <button class="food-loc-btn ${activeLocality === 'SON_TRA' ? 'active' : ''}" data-food-loc="SON_TRA">Sơn Trà</button>
            <button class="food-loc-btn ${activeLocality === 'THANH_KHE' ? 'active' : ''}" data-food-loc="THANH_KHE">Thanh Khê</button>
            <button class="food-loc-btn ${activeLocality === 'NGU_HANH_SON' ? 'active' : ''}" data-food-loc="NGU_HANH_SON">Ngũ Hành Sơn</button>
            <button class="food-loc-btn ${activeLocality === 'LIEN_CHIEU' ? 'active' : ''}" data-food-loc="LIEN_CHIEU">Liên Chiểu</button>
          </div>
        </div>
      </div>

      <!-- RESULTS SECTION -->
      <div class="journey-results-section">
        ${filtered.length > 0 ? `
          <div class="journey-spotlight-wrapper">
            ${renderPrimarySpotlightCard(spotlight, '📍 GỢI Ý PHÙ HỢP BỘ LỌC HIỆN TẠI')}
          </div>

          ${secondary.length > 0 ? `
            <h3 class="secondary-section-title">Các điểm ẩm thực khác trong khu vực (${secondary.length})</h3>
            <div class="cards-layout-grid">
              ${secondary.map(item => renderCompactSecondaryCard(item)).join('')}
            </div>
          ` : ''}
        ` : `
          <div class="empty-state-box">
            <h3>Chưa tìm thấy quán ăn phù hợp bộ lọc này</h3>
            <p>Vui lòng chọn "Toàn thành phố" hoặc bữa ăn khác để xem danh sách gợi ý.</p>
            <button class="btn-modal-primary" id="btn-reset-food-filter">Đặt lại bộ lọc ẩm thực</button>
          </div>
        `}
      </div>
    </div>
...
function renderLeisureJourneyRoute() {
  ...
  const spotlight = filtered[0] || leisureItems[0];
  const secondary = filtered.slice(1);

  return `
    <div class="leisure-journey-page" id="leisure-journey-root">
      <div class="journey-editorial-hero">
        <button class="btn-back-home" data-nav="HOME">&larr; Quay lại Trang Chủ</button>
        <span class="section-kicker kicker-fac">✨ LỘ TRÌNH 2 &bull; GIẢI TRÍ & VĂN HÓA TỐI NAY</span>
        <h1 class="journey-main-title">Đi Đâu Tối Nay Tại Đà Nẵng?</h1>
        <p class="journey-main-subcopy">
      ...
      <div class="journey-filters-card">
        <div class="filter-row">
          <label class="filter-title">Nhu cầu giải trí:</label>
          <div class="filter-pills-row">
            <button class="leisure-filter-btn ${activeLeisureCategory === 'ALL' ? 'active' : ''}" data-leisure-cat="ALL">Tất cả</button>
            <button class="leisure-filter-btn ${activeLeisureCategory === 'CINEMA' ? 'active' : ''}" data-leisure-cat="CINEMA">🎬 Rạp chiếu phim</button>
            <button class="leisure-filter-btn ${activeLeisureCategory === 'HERITAGE' ? 'active' : ''}" data-leisure-cat="HERITAGE">🏛️ Văn hóa & Di sản</button>
            <button class="leisure-filter-btn ${activeLeisureCategory === 'RIVERSIDE' ? 'active' : ''}" data-leisure-cat="RIVERSIDE">🌉 Dạo Sông Hàn & Cầu Rồng</button>
          </div>
        </div>
      </div>
      ...
      <div class="journey-results-section">
        ${filtered.length > 0 ? `
          <div class="journey-spotlight-wrapper">
            ${renderPrimarySpotlightCard(spotlight, '📍 LỰA CHỌN NỔI BẬT BUỔI TỐI')}
          </div>

          ${secondary.length > 0 ? `
            <h3 class="secondary-section-title">Các điểm tham quan & giải trí khác (${secondary.length})</h3>
            <div class="cards-layout-grid">
              ${secondary.map(item => renderCompactSecondaryCard(item)).join('')}
            </div>
          ` : ''}
        ` : `
          <div class="empty-state-box">
            <h3>Không có điểm giải trí phù hợp</h3>
            <button class="btn-modal-primary" id="btn-reset-leisure-filter">Xem tất cả địa điểm</button>
          </div>
        `}
      </div>
    </div>
    ...
    <div class="three-lane-wallet-page">
      <section class="wallet-hero-section">
        <div class="wallet-header-editorial">
          ...
          <p class="wallet-subcopy">Phân chia 3 làn thông tin rõ ràng: Dùng ngay, Cổng chính thức và Kênh theo dõi. Nguồn gốc rõ ràng, cập nhật định kỳ.</p>
        </div>
      </section>
...
// 5. EXPLORE DIRECTORY VIEW (50 ITEMS)
  ...
  return `
    <div class="explore-directory-page">
      ...
      <div class="cards-layout-grid directory-grid">
        ${filtered.length > 0 
          ? filtered.map(item => renderCompactSecondaryCard(item)).join('')
          : `
            <div class="empty-state-box">
  ...
  return `
    <div class="buy-decision-hub-page">
      ...
      <div class="decision-search-box">
        <input type="text" id="input-check-deal" placeholder="Nhập tên món hàng, đường link hoặc cửa hàng (vd: Tai nghe Sony, Pizza Domino's)..." />
        <button id="btn-submit-check" class="btn-modal-primary">Kiểm Tra Ngay</button>
      </div>

      <div id="decision-result-container" class="decision-result-box" style="display: none;"></div>
      ...
      <div class="decision-truth-card">
        ...
        <div class="truth-grid">
          <div class="truth-point">1. Bằng chứng niêm yết chính hãng</div>
          <div class="truth-point">2. Không phát sinh phụ phí ẩn</div>
          <div class="truth-point">3. Đúng phạm vi phục vụ tại Đà Nẵng</div>
          <div class="truth-point">4. Chính sách đổi trả & bảo hành rõ ràng</div>
        </div>
      </div>
    </div>
  `;
  ...
  return `
    <div class="saved-collection-page">
      ...
      ${savedItems.length > 0 
        ? `
          <div class="cards-layout-grid">
            ${savedItems.map(item => renderCompactSecondaryCard(item)).join('')}
          </div>
        `
        : `
          <div class="empty-state-box">
...
function renderAppShell() {
  ...
  root.innerHTML = `
    <div class="jayt-app-shell theme-${currentTheme} board-${activeBoard.toLowerCase()}">
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Design Escalation &bull; v3.427.4-staging.ck</span>
            </div>
          </div>
          ...
          <nav class="nav-links-desktop" role="navigation" aria-label="Điều hướng chính">
            ...
            <button class="nav-btn ${activeView === 'FOOD_JOURNEY' ? 'active' : ''}" data-nav="FOOD_JOURNEY">Ăn gì gần đây</button>
            <button class="nav-btn ${activeView === 'LEISURE_JOURNEY' ? 'active' : ''}" data-nav="LEISURE_JOURNEY">Tối nay đi đâu</button>
            <button class="nav-btn ${activeView === 'WALLET' ? 'active' : ''}" data-nav="WALLET">Ví Quyền Lợi (3 Làn)</button>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            <span class="footer-title">JayT Đà Nẵng &bull; Local Editorial Commerce</span>
            <p class="footer-mission">
              Nền tảng cẩm nang cộng đồng Đà Nẵng. Studio đang chạy 3 Art Direction Boards (Board A, Board B, Board C) theo chỉ thị CK.
            </p>
            <span class="footer-version">Phiên bản: v3.427.4-staging.ck &bull; Design Escalation CK</span>
          </div>
        </div>
      </footer>
      ...
      <nav class="jayt-mobile-bottom-nav" role="navigation" aria-label="Điều hướng di động">
        ...
        <button class="mobile-nav-btn ${activeView === 'FOOD_JOURNEY' ? 'active' : ''}" data-nav="FOOD_JOURNEY">
          <span class="nav-icon">🍜</span>
          <span class="nav-text">Ăn gì</span>
        </button>
        <button class="mobile-nav-btn ${activeView === 'LEISURE_JOURNEY' ? 'active' : ''}" data-nav="LEISURE_JOURNEY">
          <span class="nav-icon">✨</span>
          <span class="nav-text">Đi đâu</span>
        </button>
...
function attachStudioBoardEvents() {
  document.querySelectorAll('[data-set-board]').forEach(btn => {
    btn.addEventListener('click', () => {
      activeBoard = btn.dataset.setBoard;
      try {
        if (typeof localStorage !== 'undefined') localStorage.setItem('jayt_active_board', activeBoard);
      } catch (e) {}
      renderAppShell();
      attachGlobalEvents();
      renderCurrentView();
      showToast(`Đã chuyển sang ${btn.innerText.trim()}`);
    });
  });
}
        ...
        <div class="jayt-modal-box" role="dialog" aria-modal="true" aria-labelledby="report-modal-title">
          ...
          <div class="modal-body">
            <p style="font-size: 0.9rem; color: var(--text-secondary); margin-bottom: 8px;">
              Chia sẻ thông tin tiện ích công hoặc chính sách tại Đà Nẵng để cộng đồng tham khảo.
            </p>
            ...
            <input 
              ...
              placeholder="Tên đơn vị hoặc đường link chính thức (vd: cgv.vn, danangbus.vn)..." 
              maxlength="250"
...
function attachHomeEvents() {
  ...
  document.querySelectorAll('[data-explore-gw]').forEach(btn => {
    btn.addEventListener('click', () => {
      activeView = 'EXPLORE';
      activeGateway = btn.dataset.exploreGw;
...
function attachFoodJourneyEvents() {
  ...
  const resetBtn = document.getElementById('btn-reset-food-filter');
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      activeMealMoment = 'ALL';
      activeLocality = 'ALL';
      renderCurrentView();
    });
  }
...
function attachLeisureJourneyEvents() {
  ...
  const resetBtn = document.getElementById('btn-reset-leisure-filter');
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      activeLeisureCategory = 'ALL';
      activeLocality = 'ALL';
      renderCurrentView();
    });
  }
      ...
      resultBox.innerHTML = `
        <div class="result-card-inner">
          <div class="result-header">
            <span class="result-badge-failclosed">ℹ️ KẾT QUẢ ĐỐI SOÁT MINH BẠCH</span>
            <h4>${val}</h4>
          </div>
          <div class="result-checklist">
            <div class="check-item-row pass">✓ Nguồn thông tin: Niêm yết tại cổng chính hãng</div>
            <div class="check-item-row pass">✓ Chi phí phát sinh: Đang kiểm tra phụ phí theo khu vực</div>
            <div class="check-item-row pass">✓ Phục vụ tại Đà Nẵng: Sẵn sàng tại các chi nhánh nội thành</div>
            <div class="check-item-row pass">✓ Chính sách đổi trả: Áp dụng theo quy định của nhà cung cấp</div>
          </div>
          <div class="result-recommendation">
            💡 <strong>Lời khuyên tiêu dùng:</strong> Kiểm tra trực tiếp hóa đơn và các điều khoản phụ phí tại quầy trước khi thanh toán.
          </div>
        </div>
      `;
