/**
 * JAYT COMMUNITY OS — CURATED LOCAL DISCOVERY EXPERIENCE (SECTION DN)
 * Release Candidate: v3.442.0-staging.dn
 * Governing Directive: JAYT-245 Section DN (Lines 2803-2833)
 */
...
(function() {
  ...
  if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
    window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
      version: 'v3.442.0-staging.dn',
      environment: 'STAGING_REVIEW_DN',
      theme: 'CURATED_LOCAL_DISCOVERY',
      dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
    });
  }

  if (typeof window !== 'undefined') {
    window.JAYT_STOREFRONT_VERSION = 'v3.442.0-staging.dn';
  }
  ...
  function getCurrentTimeSlotInfo() {
    ...
    if (hour >= 5 && hour < 11) {
      return { slot: 'SLOT_MORNING', label: 'Sáng nay', momentTag: '🌅 BUỔI SÁNG &bull; ĐIỂM TÂM & CÀ PHÊ VEN SÔNG', momentDesc: 'Khung giờ điểm tâm mì Quảng, cà phê sáng Hải Châu và buýt DanaBus kết nối liên quận.' };
  ...
  // 1. CURATED LOCAL DISCOVERY HOME VIEW (SECTION DN)
  function renderDailyGuideHome() {
    const timeInfo = getCurrentTimeSlotInfo();
    const featuredCards = JAYT_DISCOVERY_ITEMS.slice(0, 6);

    // Data-driven dynamic counts from runtime ledger (Section DN Invariant)
    const anGiCount = JAYT_DISCOVERY_ITEMS.filter(i => i.gateway_group === 'AN_GI').length;
    const diDauCount = JAYT_DISCOVERY_ITEMS.filter(i => i.gateway_group === 'DI_DAU').length;
    const tienIchCount = JAYT_DISCOVERY_ITEMS.filter(i => i.tier_level === 'TIER_3_UTILITY').length;
    const hocTapCount = JAYT_DISCOVERY_ITEMS.filter(i => i.gateway_group === 'MUA_SAM').length;
    ...
    return `
      <div class="cr-experience-container">
        ...
        <div class="bento-hero-grid" aria-label="Bảng Điều Khiển Khám Phá Hôm Nay">
          ...
          <section class="bento-tile-main-stage hero-landmark-cr vivid-dragon-hero-cf" aria-label="Bìa Khám Phá Đà Nẵng">
            ...
            <div class="bento-stage-content hero-landmark-content">
              ...
              <!-- HERO DECISION HIERARCHY (SECTION DN: STATE-AWARE PRIMARY CTA) -->
        ...
        <!-- 2. QUICK DISCOVERY CATEGORIES STRIP (SECTION DN: DATA-DRIVEN RUNTIME COUNTERS) -->
        <div class="journey-category-strip" role="region" aria-label="Lối Tắt Khám Phá Theo Nhu Cầu">
          <a href="#featured-deals-section" class="journey-cat-card" data-nav="FOOD_JOURNEY" aria-label="Khám phá Ẩm thực bản địa">
            ...
            <div class="journey-cat-count">${anGiCount} Địa điểm dẫn nguồn chính thức</div>
          </a>
          <a href="#featured-deals-section" class="journey-cat-card" data-nav="LEISURE_JOURNEY" aria-label="Khám phá Đi chơi và Văn hóa">
            ...
            <div class="journey-cat-count">${diDauCount} Điểm tham quan & rạp chiếu phim</div>
          </a>
          <a href="#featured-deals-section" class="journey-cat-card" data-nav="EXPLORE" data-tier-filter="TIER_3_UTILITY" aria-label="Khám phá Tiện ích công cộng">
            ...
            <div class="journey-cat-count">${tienIchCount} Tiện ích công cộng TP. Đà Nẵng</div>
          </a>
          <a href="#featured-deals-section" class="journey-cat-card" data-nav="EXPLORE" data-tier-filter="TIER_2_PROGRAMME" aria-label="Khám phá Học tập và Chương trình chính thức">
            ...
            <div class="journey-cat-count">${hocTapCount} Cổng thông tin học tập</div>
          </a>
        </div>

        <!-- 3. COMPACT CIVIC TICKER (SECTION DN: QUARANTINED SCHEDULE CLAIM, HONEST NEUTRAL FALLBACK) -->
        ...
        <!-- 4. RAIL 1: ĐANG ĐÁNG CHÚ Ý HÔM NAY (CURATED TOP ITEMS) -->
        <section id="featured-deals-section" aria-label="Đang Đáng Chú Ý Hôm Nay">
          <div class="section-flow-header">
            <div>
              <span class="discovery-section-kicker">TIÊU ĐIỂM HÔM NAY</span>
              <h2 class="section-flow-title">🔥 Đang Đáng Chú Ý Hôm Nay Tại Đà Nẵng</h2>
              <p class="section-flow-subtitle">Các chương trình chính thức và tiện ích đô thị nổi bật. Mở thẻ để xem căn cứ đối soát và điều kiện áp dụng.</p>
            </div>
        ...
        <!-- 5. RAIL 2: CỔNG THÔNG TIN & CHƯƠNG TRÌNH HỌC ĐƯỜNG (SECTION DN: HONEST ATTESTATION) -->
        ...
        <!-- 6. RAIL 3: MOMENT-CURATED REAL PHOTO SPOTLIGHTS -->
        <section aria-label="Điểm Hẹn & Tiện Ích Theo Khoảnh Khắc">
          <div class="section-flow-header">
            <div>
              <span class="discovery-section-kicker">KHÔNG GIAN ĐÔ THỊ</span>
              <h2 class="section-flow-title">📍 Không Gian Tiêu Biểu Theo Khoảnh Khắc</h2>
  ...
  // 4. OFFICIAL PROGRAMMES & PORTALS VIEW (SECTION DN)
  ...
  // 8. PROGRESSIVE DISCLOSURE DETAIL DRAWER WITH FULL TAB-CYCLE FOCUS TRAP (SECTION DN)
    ...
    root.innerHTML = `
      <div class="jayt-app-shell theme-${currentTheme}">
          ...
          <div class="header-inner">
            <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT Đà Nẵng">
              <svg width="28" height="28" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path d="M19 6 V19 C19 24.5 15.5 28 10 28 C6.5 28 4.2 26 3.5 24 C5.2 24 7 22.8 7.5 20.8 C8 18.5 6.5 16.5 4.5 16.5 C3.8 16.5 3 16.8 2.5 17.2 C3.5 10.5 11 6 19 6 Z" fill="url(#jflow-logo-grad-dn)"/>
                <path d="M19 14 C23.5 14 27.5 17 29.5 21 C31 24 32.5 28 33.5 31 C29.5 29 25 27.5 19 27.5 V21.5 C22 21.5 24.5 23 26 24.5 C24.5 20 20.5 17.5 16 17 L19 14 Z" fill="url(#jflow-logo-grad-dn)" opacity="0.9"/>
                <circle cx="27" cy="8" r="3.5" fill="#f43f5e"/>
                <defs>
                  <linearGradient id="jflow-logo-grad-dn" x1="4" y1="4" x2="34" y2="34" gradientUnits="userSpaceOnUse">
              ...
              <div class="brand-text-group">
                ...
                <span class="brand-tagline">Khám Phá Mỗi Ngày &bull; v3.442.0-staging.dn</span>
              </div>
            </div>
        ...
        <footer style="text-align: center; padding: 32px 16px; font-size: 0.8rem; color: var(--text-muted); border-top: 1px solid var(--border-color);" role="contentinfo">
          <div>JayT Đà Nẵng &bull; Chuẩn Khám Phá Bản Địa DN &bull; v3.442.0-staging.dn</div>
        </footer>
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
      JAYT_STOREFRONT_VERSION: 'v3.442.0-staging.dn',
