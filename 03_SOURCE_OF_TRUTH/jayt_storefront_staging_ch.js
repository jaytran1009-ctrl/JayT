/**
 * JAYT COMMUNITY OS — LOCAL EDITORIAL COMMERCE WITH VISUAL SUPPLY (SECTION CH)
 * Release Candidate: v3.427.2-staging.ch
 * Governing Directive: JAYT-245 Section CH (Lines 2073-2089)
 * Key Architecture:
 *  - Rights-Cleared Local Visual Supply Integration (0 AI fake imagery)
 *  - Visual Provenance & Safe-Crop Attributions
 *  - Honest Audited Ranking Rationale (No Fake Personalization/AI Claims)
 *  - Rebalanced Narrative Modules: Visual Hero + Reason-to-Go + Differentiated Action
 *  - Vivid Panoramic Hero (Cầu Rồng Center Stage, Light Non-Intrusive Scrim)
 *  - 4 Visual Archetypes: Verified Place, Official Programme, Radar, Voucher Ready
 *  - Zero Commercial Assertions & Zero Absolute Claims
 */
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.427.2-staging.ch',
    environment: 'STAGING_REVIEW_CH',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.427.2-staging.ch';

// 1. DATA LEDGER (50 SANITIZED TIERED ITEMS WITH VISUAL ASSETS)
const JAYT_DISCOVERY_ITEMS = [
  ...
  },
  {
    ...
    "visual_attribution": "🚌 Lộ trình xe buýt DanaBus (danangbus.vn)"
  },
  {
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_library_study_006.svg",
    "visual_attribution": "📚 Thư viện Khoa học Tổng hợp Đà Nẵng"
  },
  {
  ...
  },
  {
    ...
    "visual_asset_url": "assets/images/danang_mi_quang_food_001.svg",
    "visual_attribution": "🍜 Phố Huỳnh Thúc Kháng &bull; JayT Editorial"
  },
  {
...
function getCurrentTimeSlotInfo() {
  ...
  if (hour >= 5 && hour < 11) {
    return { slot: 'SLOT_MORNING', label: 'Sáng nay', greeting: 'Chào buổi sáng Đà Nẵng', momentTag: '🌅 BUỔI SÁNG &bull; CÀ PHÊ & ĐIỂM TÂM' };
  } else if (hour >= 11 && hour < 14) {
    return { slot: 'SLOT_LUNCH', label: 'Trưa nay', greeting: 'Bữa trưa năng lượng tại Đà Nẵng', momentTag: '🍜 BUỔI TRƯA &bull; ĂN TRƯA TIẾP SỨC' };
  } else if (hour >= 14 && hour < 18) {
    return { slot: 'SLOT_AFTERNOON', label: 'Chiều nay', greeting: 'Chiều mát Đà Nẵng', momentTag: '☕ BUỔI CHIỀU &bull; TỰ HỌC & LÀM VIỆC' };
  } else {
    return { slot: 'SLOT_EVENING', label: 'Tối nay', greeting: 'Đêm Đà Nẵng rực rỡ', momentTag: '✨ BUỔI TỐI &bull; DẠO SÔNG HÀN & RẠP PHIM' };
  }
}

// RESOLVE OUTCOME-DRIVEN ACTION CONTRACT
    ...
    <div class="hero-civic-note-cf" role="region" aria-label="Điểm Nhấn Đô Thị Hôm Nay">
      <div class="civic-note-inner-cf">
        ...
        <div class="civic-note-text">
          ...
          <span class="civic-note-detail">Phun lửa & nước vào 21:00 cuối tuần.</span>
        </div>
...
// Render Archetype-Driven Primary Spotlight Card with Rights-Cleared Visual Asset (CH Mandates #2, #3, #4)
function renderPrimarySpotlightCard(item, kickerText) {
  ...
  return `
    <article class="primary-archetype-card archetype-${item.visual_archetype.toLowerCase()} has-visual-ch" data-item-id="${item.item_id}">
      <!-- AUTHENTIC LOCAL VISUAL CROP (CH MANDATE #2, #3) -->
      ${item.visual_asset_url ? `
        <div class="spotlight-visual-stage-ch">
          <img 
            src="${item.visual_asset_url}" 
            alt="${item.title}" 
            class="spotlight-visual-img-ch" 
            loading="lazy"
          />
...
// Render Compact Secondary Item Card with Visual Cue & Reason to Choose
...
// 1. HOME VIEW (VIVID HERO + TRUE NARRATIVE MODULES WITH VISUAL SUPPLY)
function renderDailyGuideHome() {
  ...
  return `
    <div class="home-editorial-layout-cf">
      <!-- 1. VIVID DRAGON BRIDGE HERO FIRST FOLD -->
      ...
      <!-- 2. MODULE 1: NHỊP HÔM NAY (DAILY RHYTHM CHIPS) -->
      <section class="section-daily-rhythm-cc" aria-label="Nhịp Sống Hôm Nay">
        <div class="rhythm-header">
          <span class="rhythm-kicker">⏳ NHỊP SỐNG THEO THỜI ĐIỂM</span>
          <h2 class="rhythm-title">Chọn Trải Nghiệm Phù Hợp Với Bạn Lúc Này</h2>
        </div>
        <div class="rhythm-chips-bar">
          <button class="rhythm-chip active" data-nav="FOOD_JOURNEY">🌅 Sáng: Cà phê & Điểm tâm</button>
          <button class="rhythm-chip" data-nav="FOOD_JOURNEY">🍜 Trưa: Ẩm thực tiếp sức</button>
          <button class="rhythm-chip" data-explore-gw="MUA_SAM">☕ Chiều: Tự học & Làm việc</button>
          <button class="rhythm-chip" data-nav="LEISURE_JOURNEY">✨ Tối: Dạo Sông Hàn & Rạp phim</button>
        </div>
      </section>

      <!-- 3. TRUE NARRATIVE MODULE 1: ẨM THỰC ĐÀ THÀNH (WITH VISUAL CROP) -->
      ...
      <!-- 4. TRUE NARRATIVE MODULE 2: DI CHUYỂN & GIAO THÔNG XANH (WITH VISUAL CROP) -->
      ...
      <!-- 5. TRUE NARRATIVE MODULE 3: HỌC TẬP & LÀM VIỆC (WITH VISUAL CROP) -->
      ...
      <!-- 6. TRUE NARRATIVE MODULE 4: ĐI CHƠI & THƯ GIÃN TỐI NAY (WITH VISUAL CROP) -->
      ...
      <!-- 7. TRUE NARRATIVE MODULE 5: MUA SẮM TIẾT KIỆM & THIẾT YẾU -->
      <section class="narrative-journey-module" id="journey-shopping" aria-label="Hành Trình Mua Sắm">
        <div class="narrative-module-header">
          <div>
            <span class="journey-kicker-tag">🛒 HÀNH TRÌNH 5 &bull; MUA SẮM TIẾT KIỆM & THIẾT YẾU</span>
            <h2 class="journey-title-ca">Siêu Thị Tiêu Dùng & Cổng Tra Cứu Thông Minh</h2>
          </div>
        </div>
...
// 4. THREE-LANE WALLET VIEW WITH OFFICIAL VALUE LAYER
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Local Editorial &bull; v3.427.2-staging.ch</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.427.2-staging.ch &bull; Staging Review CH</span>
          </div>
        </div>
      </footer>
