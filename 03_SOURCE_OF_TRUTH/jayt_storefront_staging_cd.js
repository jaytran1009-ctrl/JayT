/**
 * JAYT COMMUNITY OS — MOBILE FIRST-FOLD COMPOSITION & LOCAL EDITORIAL (SECTION CD)
 * Release Candidate: v3.426.8-staging.cd
 * Governing Directive: JAYT-245 Section CD (Lines 1974-1993)
 * Key Architecture:
 *  - Rigorous 390x844 Mobile First-Fold Contract (Zero Truncation, Zero Clipping)
 *  - 1 Primary CTA + 2 Horizontal Compact Chips (No Vertical Block Stacking)
 *  - Lightweight Non-Intrusive Civic City Note (<25% of Hero)
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.426.8-staging.cd',
    environment: 'STAGING_REVIEW_CD',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.426.8-staging.cd';
...
// Render Compact Civic City Note (CD Mandate #4: Lightweight, Non-Intrusive, <25% of Hero)
...
// Render Editorial Story Feature Card ("Góc Địa Phương")
function renderLocalEditorialFeatureCard(item) {
  const isSaved = savedItemIds.has(item.item_id);
  const action = resolveActionContract(item);

  return `
    <article class="local-editorial-feature-card" data-item-id="${item.item_id}">
      <div class="feature-story-badge-row">
        <span class="feature-story-kicker">📖 GÓC ĐỊA PHƯƠNG &bull; CÂU CHUYỆN ĐÀ THÀNH</span>
        <button class="btn-card-save ${isSaved ? 'saved' : ''}" data-save-id="${item.item_id}" aria-label="${isSaved ? 'Đã lưu' : 'Lưu lại'}">
          ${isSaved ? '❤️' : '🤍'}
        </button>
      </div>

      <div class="feature-story-content">
        <h3 class="feature-story-title">${item.title}</h3>
        <p class="feature-story-lead">${item.editorial_lead || item.summary_text}</p>
        
        <div class="feature-story-meta-grid">
          <div class="meta-item">📍 <strong>Địa điểm:</strong> ${item.scope_text}</div>
          <div class="meta-item">⏳ <strong>Thời gian:</strong> ${item.timing_window}</div>
          ${item.transit_hint ? `<div class="meta-item transit-item">${item.transit_hint}</div>` : ''}
        </div>
      </div>

      <div class="feature-story-action-row">
        <a href="${action.url}" target="_blank" rel="noopener noreferrer" class="btn-spotlight-action ${action.btnClass}">
          ${action.icon} ${action.label}
        </a>
        <span class="feature-story-source">ℹ️ ${item.evidence_status}</span>
      </div>
    </article>
  `;
}
...
// 1. HOME VIEW (CD MANDATE: ROBUST MOBILE FIRST-FOLD & LOCAL EDITORIAL)
function renderDailyGuideHome() {
  ...
  return `
    <div class="home-editorial-layout-cd">
      <!-- 1. LIVING MAGAZINE COVER FIRST FOLD (CD MANDATE #2, #3, #4) -->
      <section class="magazine-cover-hero-cd" aria-label="Bìa Cẩm Nang Sống Đà Nẵng">
        ...
        <div class="magazine-cover-overlay-cd">
          <div class="magazine-header-group-cd">
            ...
            <!-- 1 PRIMARY CTA + 2 COMPACT HORIZONTAL CHIPS (CD MANDATE #3) -->
            <div class="magazine-compact-actions-cd">
              <button class="btn-primary-action-cd" data-scroll-to="journey-food">
                🍜 Ăn gì gần đây?
              </button>
              <div class="chips-horizontal-row-cd">
                <button class="chip-secondary-cd" data-scroll-to="journey-leisure">
                  ✨ Đi đâu?
                </button>
          ...
          <!-- COMPACT LIGHTWEIGHT CIVIC NOTE (CD MANDATE #4) -->
      ...
      <section class="section-daily-rhythm-cc" aria-label="Nhịp Sống Hôm Nay">
        ...
        <div class="rhythm-chips-bar">
          <button class="rhythm-chip active" data-scroll-to="journey-food">🌅 Sáng: Cà phê & Điểm tâm</button>
          <button class="rhythm-chip" data-scroll-to="journey-food">🍜 Trưa: Ẩm thực tiếp sức</button>
          <button class="rhythm-chip" data-scroll-to="journey-study">☕ Chiều: Tự học & Làm việc</button>
          <button class="rhythm-chip" data-scroll-to="journey-leisure">✨ Tối: Dạo Sông Hàn & Rạp phim</button>
        </div>
      </section>

      <!-- 3. MODULE 2: GÓC ĐỊA PHƯƠNG (EDITORIAL STORY FEATURE) -->
      <section class="section-local-editorial-feature" aria-label="Góc Địa Phương">
        ${renderLocalEditorialFeatureCard(huynhThucKhang)}
      </section>
      ...
      <section class="section-living-journey-ca" id="journey-food" aria-label="Hành Trình Ẩm Thực">
        <div class="journey-header-ca">
          ...
          <button class="btn-journey-see-all" data-explore-gw="AN_GI">Xem tất cả quán ăn &rarr;</button>
        </div>
      ...
      <section class="section-living-journey-ca" id="journey-leisure" aria-label="Hành Trình Thư Giãn">
        <div class="journey-header-ca">
          ...
          <button class="btn-journey-see-all" data-explore-gw="DI_DAU">Xem các điểm tham quan &rarr;</button>
        </div>
...
// 4. BUY DECISION HUB VIEW (INTERACTIVE FAIL-CLOSED ENGINE)
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Local Editorial &bull; v3.426.8-staging.cd</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.426.8-staging.cd &bull; Staging Review CD</span>
          </div>
        </div>
      </footer>
