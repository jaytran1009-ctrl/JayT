/**
 * JAYT COMMUNITY OS — THREE CLOSED-LOOP DECISION JOURNEYS (SECTION CE)
 * Release Candidate: v3.426.9-staging.ce
 * Governing Directive: JAYT-245 Section CE (Lines 1996-2016)
 * Key Architecture:
 *  - 3 Closed-Loop Action Journeys: Ăn gì gần đây, Tối nay đi đâu, Ví quyền lợi
 *  - Deterministic District & Meal Context Filtering with Maps Navigation
 *  - Mobile First-Fold Contract Preservation (390x844 Zero Clipping)
 *  - Living Magazine Cover First Fold (Cầu Rồng Sông Hàn)
 *  - Đà Nẵng Living Palette (Warm Sand, River Navy, Sunset Amber, Ocean Cyan)
 *  - Three-Lane Wallet UX & Interactive Buy Decision Hub
...
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.426.9-staging.ce',
    environment: 'STAGING_REVIEW_CE',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.426.9-staging.ce';
...
let activeTier = 'ALL';
let activeMealMoment = 'ALL'; // 'ALL' | 'SANG' | 'TRUA' | 'CHIEU' | 'TOI'
let activeLeisureCategory = 'ALL'; // 'ALL' | 'CINEMA' | 'HERITAGE' | 'RIVERSIDE'
...
// RESOLVE DISTINCT CTA ACTION CONTRACT
function resolveActionContract(item) {
  let btnClass = 'btn-cta-official';
  let icon = '🌐';

  if (item.action_label && item.action_label.includes('lịch')) {
    btnClass = 'btn-cta-cinema';
    icon = '📅';
  } else if (item.action_label && (item.action_label.includes('vé') || item.action_label.includes('xe'))) {
    btnClass = 'btn-cta-transport';
    icon = '🚌';
  } else if (item.action_label && item.action_label.includes('giờ mở cửa')) {
    btnClass = 'btn-cta-civic';
    icon = '🏛️';
  } else if (item.tier === 'RADAR_SOURCE') {
    btnClass = 'btn-cta-radar';
    icon = '🔔';
  }
  ...
  return {
    btnClass,
    icon,
    label: (item.action_label || 'Xem cổng thông tin chính thức →'),
...
function renderHeroCivicCityNote() {
  return `
    <div class="hero-civic-note-cd" role="region" aria-label="Điểm Nhấn Đô Thị Hôm Nay">
      <div class="civic-note-inner-cd">
...
// Render Contextual Recommendation Card ("Đề Xuất Cho Lúc Này")
function renderContextualHighlightCard(item, kickerText) {
  const isSaved = savedItemIds.has(item.item_id);
  const action = resolveActionContract(item);

  return `
    <article class="contextual-spotlight-card" data-item-id="${item.item_id}">
      <div class="spotlight-kicker-bar">
        <span class="spotlight-badge">${kickerText || '⭐ ĐỀ XUẤT CHO LÚC NÀY'}</span>
      ...
      <div class="spotlight-body-content">
        <span class="spotlight-brand-tag">${item.brand}</span>
        <h3 class="spotlight-headline">${item.title}</h3>
        <p class="spotlight-summary-text">${item.summary_text}</p>
        
        <div class="spotlight-info-grid">
          <div class="info-row">📍 <strong>Địa điểm:</strong> ${item.scope_text}</div>
          <div class="info-row">⏳ <strong>Thời gian:</strong> ${item.timing_window}</div>
          ${item.transit_hint ? `<div class="info-row transit-row">${item.transit_hint}</div>` : ''}
        </div>
      </div>

      <div class="spotlight-action-footer">
        <a href="${action.url}" target="_blank" rel="noopener noreferrer" class="btn-spotlight-action ${action.btnClass}">
          ${action.icon} ${action.label}
        </a>
        ${item.map_query_url ? `
          <a href="${item.map_query_url}" target="_blank" rel="noopener noreferrer" class="btn-map-action" title="Xem trên Google Maps">
            📍 Chỉ đường
          </a>
        ` : ''}
      </div>

      <div class="spotlight-provenance-bar">
        <span class="provenance-text">ℹ️ ${item.evidence_status}</span>
      </div>
    </article>
  `;
}
...
function renderCompactSecondaryCard(item) {
  ...
  return `
    <article class="compact-secondary-card" data-item-id="${item.item_id}">
      <div class="secondary-header">
        <span class="secondary-tier-badge">${item.badge_label}</span>
        <button class="btn-card-save ${isSaved ? 'saved' : ''}" data-save-id="${item.item_id}" aria-label="${isSaved ? 'Đã lưu' : 'Lưu lại'}" title="${isSaved ? 'Đã lưu' : 'Lưu lại'}">
          ${isSaved ? '❤️' : '🤍'}
        </button>
      </div>
      ...
      <div class="secondary-body">
        <span class="secondary-brand">${item.brand}</span>
        <h4 class="secondary-title">${item.title}</h4>
        <p class="secondary-summary">${item.summary_text}</p>
        <div class="secondary-meta">📍 ${item.scope_text}</div>
        ${item.transit_hint ? `<div class="secondary-transit">${item.transit_hint}</div>` : ''}
      </div>

      <div class="secondary-footer">
        <a href="${action.url}" target="_blank" rel="noopener noreferrer" class="btn-secondary-action ${action.btnClass}">
          ${action.icon} ${action.label}
        </a>
        ${item.map_query_url ? `
          <a href="${item.map_query_url}" target="_blank" rel="noopener noreferrer" class="btn-map-mini" title="Xem vị trí">
            📍 Vị trí
          </a>
        ` : ''}
      </div>

      <div class="secondary-provenance">
...
// 1. HOME VIEW (LOCAL EDITORIAL WITH CLOSED-LOOP ROUTING)
function renderDailyGuideHome() {
  ...
  return `
    <div class="home-editorial-layout-cd">
      <!-- 1. LIVING MAGAZINE COVER FIRST FOLD -->
      <section class="magazine-cover-hero-cd" aria-label="Bìa Cẩm Nang Sống Đà Nẵng">
        <div class="magazine-cover-stage-cd">
          <img 
            src="assets/images/dragon_bridge_hero_001.jpg" 
            alt="Toàn cảnh Cầu Rồng bắc qua Sông Hàn rực rỡ tại trung tâm TP. Đà Nẵng" 
            class="magazine-cover-image-cd" 
            id="hero-main-photo"
            loading="eager"
          />
          <div class="magazine-cover-scrim-cd"></div>
          
          <div class="magazine-credit-tag-cd" tabindex="0" role="note" aria-label="Thông tin bản quyền ảnh Cầu Rồng">
            📍 Cầu Rồng &bull; 📷 Bùi Thụy Đào Nguyên (CC BY-SA 3.0)
          </div>
        </div>

        <div class="magazine-cover-overlay-cd">
          <div class="magazine-header-group-cd">
            <span class="magazine-issue-tag-cd">${timeInfo.momentTag}</span>
            <h1 class="magazine-main-title-cd">
              Khám Phá Nhịp Sống & Tiện Ích Đà Nẵng
            </h1>
            <p class="magazine-subhead-cd">
              Cẩm nang kết nối ẩm thực bản địa, di chuyển công cộng và tiện ích học đường chính thống.
            </p>

            <!-- 3 CLOSED-LOOP ACTION CTA BUTTONS (CE MANDATE #2) -->
            <div class="magazine-compact-actions-cd">
              <button class="btn-primary-action-cd" data-nav="FOOD_JOURNEY">
                🍜 Ăn gì gần đây?
              </button>
              <div class="chips-horizontal-row-cd">
                <button class="chip-secondary-cd" data-nav="LEISURE_JOURNEY">
                  ✨ Đi đâu tối nay?
                </button>
                <button class="chip-secondary-cd" data-nav="WALLET">
                  🎟️ Ví quyền lợi
                </button>
              </div>
            </div>
          ...
          <div class="magazine-civic-note-wrapper-cd">
      ...
      <!-- 3. MODULE 2: GÓC ĐỊA PHƯƠNG (EDITORIAL STORY FEATURE) -->
      <section class="section-local-editorial-feature" aria-label="Góc Địa Phương">
        ${renderContextualHighlightCard(huynhThucKhang, '📖 GÓC ĐỊA PHƯƠNG &bull; PHỐ ẨM THỰC')}
      </section>

      <!-- 4. MODULE 3: HÀNH TRÌNH QUYẾT ĐỊNH (5 DECISION RAILS) -->
      
      <!-- RAIL 1: ẨM THỰC -->
      <section class="section-living-journey-ca" id="journey-food" aria-label="Hành Trình Ẩm Thực">
        <div class="journey-header-ca">
          <div>
            <span class="journey-kicker-tag">🍜 HÀNH TRÌNH 1 &bull; ẨM THỰC ĐÀ THÀNH</span>
            <h2 class="journey-title-ca">Ăn Gì Gần Bạn & Hương Vị Bản Địa</h2>
          </div>
          <button class="btn-journey-see-all" data-nav="FOOD_JOURNEY">Mở lộ trình ăn uống &rarr;</button>
        </div>

        <div class="journey-grid-hierarchical">
          <div class="journey-spotlight-area">
            ${renderContextualHighlightCard(huynhThucKhang, '⭐ ĐIỂM ĐẾN NỔI BẬT')}
          </div>
          <div class="journey-secondary-area">
      ...
      <!-- RAIL 2: DI CHUYỂN -->
      <section class="section-living-journey-ca" id="journey-transport" aria-label="Hành Trình Di Chuyển">
        <div class="journey-header-ca">
          <div>
            <span class="journey-kicker-tag">🚌 HÀNH TRÌNH 2 &bull; DI CHUYỂN & ĐI LẠI</span>
        ...
        <div class="journey-grid-hierarchical">
          <div class="journey-spotlight-area">
            ${renderContextualHighlightCard(danaBus, '🚌 TIỆN ÍCH CÔNG CỘNG')}
          </div>
          <div class="journey-secondary-area">
      ...
      <!-- RAIL 3: HỌC TẬP -->
      <section class="section-living-journey-ca" id="journey-study" aria-label="Hành Trình Học Tập">
        <div class="journey-header-ca">
          <div>
        ...
        <div class="journey-grid-hierarchical">
          <div class="journey-spotlight-area">
            ${renderContextualHighlightCard(library, '📚 KHÔNG GIAN TRI THỨC')}
          </div>
          <div class="journey-secondary-area">
      ...
      <!-- RAIL 4: THƯ GIÃN -->
      <section class="section-living-journey-ca" id="journey-leisure" aria-label="Hành Trình Thư Giãn">
        <div class="journey-header-ca">
          <div>
            <span class="journey-kicker-tag">✨ HÀNH TRÌNH 4 &bull; ĐI CHƠI & THƯ GIÃN TỐI NAY</span>
        ...
        <div class="journey-grid-hierarchical">
          <div class="journey-spotlight-area">
            ${renderContextualHighlightCard(chamMuseum, '🏛️ DI SẢN VĂN HÓA')}
          </div>
          <div class="journey-secondary-area">
      ...
      <!-- RAIL 5: MUA SẮM -->
      <section class="section-living-journey-ca" id="journey-shopping" aria-label="Hành Trình Mua Sắm">
        <div class="journey-header-ca">
          <div>
...
// 2. CLOSED-LOOP JOURNEY 1: ĂN GÌ GẦN ĐÂY (CE MANDATE #2)
      ...
      <div class="journey-results-section">
        ${filtered.length > 0 ? `
          <div class="journey-spotlight-wrapper">
            ${renderContextualHighlightCard(spotlight, '⭐ ĐỀ XUẤT HÀNG ĐẦU CHO BẠN')}
          </div>

          ${secondary.length > 0 ? `
            <h3 class="secondary-section-title">Các điểm ẩm thực khác gần khu vực (${secondary.length})</h3>
...
// 3. CLOSED-LOOP JOURNEY 2: ĐI ĐÂU TỐI NAY (CE MANDATE #2)
  ...
  return `
    <div class="leisure-journey-page" id="leisure-journey-root">
      ...
      <div class="journey-results-section">
        ${filtered.length > 0 ? `
          <div class="journey-spotlight-wrapper">
            ${renderContextualHighlightCard(spotlight, '⭐ LỰA CHỌN NỔI BẬT TỐI NAY')}
          </div>
        ...
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Local Editorial &bull; v3.426.9-staging.ce</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.426.9-staging.ce &bull; Staging Review CE</span>
          </div>
        </div>
      </footer>
