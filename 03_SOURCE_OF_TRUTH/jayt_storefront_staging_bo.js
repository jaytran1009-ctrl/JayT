...
const JAYT_STOREFRONT_VERSION = 'v3.424.2-staging.bo';
...
function renderSpotlightLargeCard(item) {
  ...
  return `
    <div class="hero-daily-pick-card-bn" data-item-id="${itemId}">
      <div class="daily-pick-badge-row">
        <span class="dp-badge-verified">✓ ƯU ĐÃI ĐÃ ĐỐI SOÁT</span>
        <span class="dp-badge-time">📅 Thứ 6, Thứ 7, Chủ Nhật</span>
      </div>
      <h3 class="daily-pick-title">${item.title}</h3>
      <p class="daily-pick-meta">Áp dụng tại các cụm rạp CGV TP. Đà Nẵng khi thanh toán qua VNPAY-QR.</p>
      <div class="daily-pick-action-row">
        <button class="btn-daily-pick-main" data-card-detail="${itemId}">
          Xem lịch chiếu & điều kiện &rarr;
        </button>
        <a href="${item.official_source_url}" target="_blank" rel="noopener noreferrer" class="btn-daily-pick-link">
          Nguồn cgv.vn &nearr;
        </a>
      </div>
    </div>
...
function renderDailyGuideHome() {
  ...
  return `
    <div class="home-guide-layout-bo">
      <!-- 1. REAL DA NANG HERO ARRIVAL (DRAGON_BRIDGE_HERO_001 PHOTOGRAPH) -->
      <section class="hero-danang-arrival-bo" aria-label="Hero Cẩm Nang Đà Nẵng">
        <!-- Real Photograph Backdrop with Credit & Scrim -->
        <div class="hero-image-backdrop-wrap">
          <img 
            ...
            class="hero-dragon-bridge-img" 
            id="hero-main-photo"
            loading="eager"
          />
          <div class="hero-image-overlay-scrim"></div>
          <div class="hero-photo-credit-badge">
            📍 Cầu Rồng & Sông Hàn &bull; 📷 Tác giả Bùi Thụy Đào Nguyên (CC BY-SA 3.0)
          </div>
        </div>

        <div class="hero-arrival-inner-bo">
          <!-- Top Kicker & Editorial Title (Tối đa 8 từ) -->
          <div class="hero-kicker-pill">
            <span class="pulse-dot"></span> 📍 ĐÀ NẴNG &bull; CITY GUIDE
          </div>
          <h1 class="hero-arrival-headline">
            Đà Nẵng, Chọn Điều Hay Mỗi Ngày
          </h1>
          <p class="hero-arrival-subcopy">
            Cẩm nang tuyển chọn độc lập cho người trẻ và cư dân Đà Nẵng.
          </p>

          <!-- 3 Quick-Choice Discovery Chips + 1 Filter Button (Không Form To) -->
          <div class="hero-quick-discovery-bar">
            <button class="quick-chip active" data-quick-slot="EVENING">🌙 Tối nay</button>
            <button class="quick-chip" data-quick-loc="HAI_CHAU">📍 Gần tôi (Hải Châu)</button>
            <button class="quick-chip" data-quick-budget="UNDER_100K">💰 Dưới 100k</button>
            <button class="btn-open-filter-drawer" id="btn-trigger-filter-drawer" aria-label="Mở bộ lọc chi tiết">
              <span>⚡ Bộ lọc</span>
            </button>
          </div>

          <!-- Hero Dominant Daily Pick Object -->
          <div class="hero-daily-pick-wrap">
