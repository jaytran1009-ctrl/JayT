/**
 * JAYT COMMUNITY OS — STRICT VISUAL HIERARCHY & CLAIM ISOLATION (SECTION BV)
 * Release Candidate: v3.426.1-staging.bv
 * Governing Directive: JAYT-245 Section BV (Lines 1768-1785)
 * Architecture:
 *  - Silent Observability (Zero public DOM injection)
 *  - Strict Logo Size Contract (32x32px brand mark)
 *  - Single City Note in Hero (No stacked deals)
 *  - Public Claim Kill Switch (Enforces evidence binding per item)
 *  - Dedicated Voucher Wallet Route
 *  - 50 Tiered Items in Explore Directory
 */

// 0. SILENT ZERO-PII OBSERVABILITY (STRICT PUBLIC DOM ISOLATION — BV MANDATE #2)
if (typeof window !== 'undefined' && window.JAYT_OBSERVABILITY) {
  window.JAYT_OBSERVABILITY.recordEvent('STOREFRONT_INITIALIZED', {
    version: 'v3.426.1-staging.bv',
    environment: 'STAGING_REVIEW_BV',
    dom_isolation: 'STRICT_ZERO_DOM_INJECTION'
  });
}

const JAYT_STOREFRONT_VERSION = 'v3.426.1-staging.bv';

// PUBLIC CLAIM KILL SWITCH (BV MANDATE #4)
// When true: all commercial claims/vouchers fall back to official source links or monitoring.
const PUBLIC_CLAIM_KILL_SWITCH = false;

// 1. DATA LEDGER (50 TIERED ITEMS)
...
// 2. VOUCHER LEDGER (12 VOUCHERS ACROSS 4 ACTIONS)
const JAYT_VOUCHERS = [
  {
...
// 3. VISUAL ASSETS
const JAYT_VISUAL_ASSETS = {
  ASSET_HERO_DRAGON_BRIDGE: {
    title: 'Cầu Rồng bắc qua Sông Hàn',
    source_type: 'DOCUMENTARY_PHOTO',
    licence: 'CC BY-SA 3.0',
    attribution: 'Bùi Thụy Đào Nguyên (Wikimedia Commons)',
    canonical_path: 'assets/images/dragon_bridge_hero_001.jpg'
  }
};
...
let activeLocality = 'ALL'; // 'ALL' | 'HAI_CHAU' | 'SON_TRA' | 'THANH_KHE' | 'NGU_HANH_SON' | 'LIEN_CHIEU' | 'CAM_LE'
let activeTier = 'ALL';
...
// TIME-AWARE CONTEXT HELPER (WO-BU-C2-004)
...
// PUBLIC CLAIM RESOLVER (BV MANDATE #4)
function resolvePublicActionContract(item) {
  if (PUBLIC_CLAIM_KILL_SWITCH || !item.allow_field_claims || item.tier === 'RADAR_SOURCE') {
    return {
      action_type: 'VIEW_OFFICIAL_SOURCE',
      label: 'Xem nguồn chính thức &rarr;',
      url: item.official_source_url,
      isKillSwitched: PUBLIC_CLAIM_KILL_SWITCH
    };
  }
  return {
    action_type: item.action_type || 'VIEW_OFFICIAL_SOURCE',
    label: item.action_label || 'Xem chi tiết &rarr;',
    code_text: item.code_text || null,
    url: item.official_source_url,
...
// Helper: Render exactly ONE compact City Note in Hero (BV Mandate #3)
function renderHeroCityNote(item) {
  if (!item) return '';
  const action = resolvePublicActionContract(item);
  const isCopy = action.action_type === 'COPY_CODE' && action.code_text;

  return `
    <div class="city-note-card-bq" role="region" aria-label="City Note — ${item.title}">
      <div class="city-note-header">
        <span class="city-note-kicker">${item.badge_label || 'ĐIỂM NHẤN HÔM NAY'}</span>
        <span class="city-note-timing">⏳ ${item.timing_window || 'Hôm nay'}</span>
      </div>
      <h3 class="city-note-title">${item.title}</h3>
      <p class="city-note-summary">${item.summary_text}</p>
      
      <div class="city-note-meta-row">
        <span class="city-note-location">📍 ${item.scope_text}</span>
      </div>

      <div class="city-note-actions">
        ${isCopy ? `
          <button class="btn-city-note-action btn-copy-code" data-copy-code="${action.code_text}" data-voucher-id="${item.item_id}" aria-label="Sao chép mã ${action.code_text}">
            📋 Sao chép: <strong>${action.code_text}</strong>
          </button>
        ` : `
          <a href="${action.url}" target="_blank" rel="noopener noreferrer" class="btn-city-note-action">
            ${action.label}
          </a>
        `}
      </div>
      
      <div class="city-note-trust-footer">
    ...
    <article class="editorial-card" data-item-id="${item.item_id}" data-gateway="${item.gateway_group}" data-tier="${item.tier}" data-locality="${item.locality_tag}">
      ...
      <div class="card-body-content">
        ...
        <div class="card-field-specs">
          ...
          ${item.conditions_limit ? `
            <div class="field-spec-row">
              <span class="spec-label">⚠️ Điều kiện:</span>
              <span class="spec-val">${item.conditions_limit}</span>
            </div>
          ` : ''}
        </div>
      </div>
      ...
      <div class="card-footer-actions">
        ${isCopy ? `
          <button class="btn-card-primary btn-copy-code" data-copy-code="${action.code_text}" data-voucher-id="${item.item_id}">
            📋 ${action.label}
          </button>
        ` : `
        ...
        `}
        <a href="${item.official_source_url}" target="_blank" rel="noopener noreferrer" class="btn-card-detail" title="Xem nguồn kiểm chứng">
          Nguồn gốc &nearr;
        </a>
      </div>
...
// 1. HOME VIEW (CINEMATIC ARRIVAL BQ — RESTORED HIERARCHY BV)
function renderDailyGuideHome() {
  ...
  // BV MANDATE #3: Hero contains MAXIMUM ONE City Note!
  const primaryDailyPick = verifiedDeals[0] || null;
    ...
    <div class="home-guide-layout-bq">
      ...
      <section class="hero-cinematic-arrival-bq" aria-label="Hero Cẩm Nang Thành Phố Đà Nẵng">
        ...
        <div class="hero-cinematic-overlay-content">
          ...
          <!-- EXACTLY ONE CITY NOTE (BV MANDATE #3) -->
          <div class="hero-city-note-slot">
            ${primaryDailyPick ? renderHeroCityNote(primaryDailyPick) : ''}
          </div>
        </div>
      </section>
      ...
      <section class="section-city-routes-bn" aria-label="3 Lộ Trình Khám Phá">
        <div class="section-header-editorial">
          ...
          <h2 class="section-title-editorial">Khám Phá Theo Nhu Của Bạn</h2>
        </div>
      ...
      <!-- 3. TIỆN ÍCH ĐÔ THỊ ĐÀ NẴNG (4 ITEMS) -->
      ...
      <!-- 4. HAI CỔNG DESTINATIONS (BUY DECISION & VOUCHERS — BV MANDATE #6) -->
      <section class="feature-destinations-section">
        ...
        <div class="destination-card dest-voucher" data-nav="VOUCHERS" role="button" tabindex="0">
          <span class="dest-badge">VÍ ƯU ĐÃI & VOUCHER (12 MỤC)</span>
          <h3 class="dest-title">Ví Ưu Đãi & Voucher Đã Đối Soát</h3>
          <p class="dest-sub">Tổng hợp mã khuyến mãi, vé xem phim U22, đặc quyền học sinh sinh viên với bằng chứng văn bản chính thống.</p>
          <span class="dest-cta">Mở ví ưu đãi cộng đồng &rarr;</span>
        </div>
      </section>

      <!-- 5. CHƯƠNG TRÌNH CHÍNH THỨC (3 ITEMS) -->
      <section class="section-editorial-rail" aria-label="Chương Trình Chính Thức">
        <div class="section-header-editorial">
          <span class="section-kicker kicker-prog">🏛️ CHƯƠNG TRÌNH & ĐẶC QUYỀN HỌC ĐƯỜNG</span>
          <h2 class="section-title-editorial">Chính Sách & Hỗ Trợ Công Cộng</h2>
        </div>
        <div class="cards-layout-grid">
          ${officialItems.slice(0, 3).map(item => renderEditorialCardWithPhoto(item)).join('')}
        </div>
      </section>

      <!-- 6. RADAR THEO DÕI NGUỒN CUNG (PILLS) -->
      <section class="section-radar-stream" aria-label="Radar Nguồn Đang Giám Sát">
        <div class="section-header-editorial">
          <span class="section-kicker kicker-radar">📡 RADAR NGUỒN CUNG ĐANG GIÁM SÁT</span>
          <h2 class="section-title-editorial">Thương Hiệu & Địa Điểm Trong Tầm Ngắm</h2>
        </div>
        <div class="radar-pills-wrap">
          ${radarItems.map(item => `
            <div class="radar-brand-pill" data-item-id="${item.item_id}">
              <span class="radar-dot"></span>
              <span class="radar-name">${item.brand}</span>
              <span class="radar-status-text">Đang theo dõi</span>
            </div>
          `).join('')}
        </div>
      </section>
...
// 2. EXPLORE DIRECTORY VIEW (50 ITEMS WITH FILTERS)
  ...
  return `
    <div class="explore-directory-page">
      <div class="directory-header-editorial">
        <span class="section-kicker kicker-deal">📚 THƯ MỤC NGUỒN CUNG TP. ĐÀ NẴNG</span>
...
// 3. VOUCHER WALLET VIEW (BS DEDICATED ROUTE — BV MANDATE #6)
function renderVoucherTicket(v) {
  const isCopy = v.public_action === 'COPY_CODE' && v.code_text;
    ...
    <div class="voucher-wallet-page">
      ...
      <section class="wallet-controls-section">
        <div class="wallet-filter-tabs" role="tablist">
          ...
          <button class="wallet-tab-btn" data-wallet-filter="FOOD_BEVERAGE">🍔 Ăn uống</button>
          <button class="wallet-tab-btn" data-wallet-filter="ENTERTAINMENT">🎬 Giải trí</button>
          <button class="wallet-tab-btn" data-wallet-filter="STUDY_TOOLS">🎓 Học tập</button>
          <button class="wallet-tab-btn" data-wallet-filter="TRANSPORT">🚌 Đi lại</button>
          <button class="wallet-tab-btn" data-wallet-filter="SHOPPING">🛍️ Mua sắm</button>
        </div>
      </section>

      <section class="wallet-tickets-grid" id="wallet-tickets-container">
        ${otherVouchers.map(v => renderVoucherTicket(v)).join('')}
      </section>
    </div>
  `;
}
  ...
  return `
    <div class="buy-decision-hub-page">
      <div class="decision-header-editorial">
        <span class="section-kicker kicker-deal">🛡️ CỔNG TƯ VẤN MUA SẮM MINH BẠCH</span>
      ...
      <div class="decision-search-box">
        <input type="text" id="input-check-deal" placeholder="Nhập tên món hàng, đường link khuyến mãi hoặc cửa hàng..." />
      ...
      <div class="decision-truth-card">
        ...
        <p>Hệ thống chỉ xác nhận "Hời thật" khi có đủ 4 yếu tố: Bằng chứng niêm yết chính hãng, Không phát sinh phí ẩn, Đúng giá trị thực tế tại Đà Nẵng, và Chính sách đổi trả rõ ràng.</p>
      </div>
    </div>
  `;
}
  ...
  return `
    <div class="saved-collection-page">
      <div class="saved-header-editorial">
        <span class="section-kicker kicker-deal">❤️ VÍ TIỆN ÍCH ĐÃ LƯU</span>
        <h1 class="saved-headline">Danh Sách Địa Điểm Của Bạn (${savedItems.length})</h1>
        <p class="saved-subcopy">Các địa điểm, ưu đãi và quyền lợi bạn đã đánh dấu để sử dụng khi cần.</p>
      </div>
      ...
      ${savedItems.length > 0 
        ...
        : `
          <div class="empty-state-box">
            ...
            <h3>Bạn chưa lưu địa điểm hoặc ưu đãi nào</h3>
...
// 6. APP SHELL & NAVIGATION (STRICT LOGO SIZE CONTRACT — BV MANDATE #1)
function renderAppShell() {
  ...
  root.innerHTML = `
    <div class="jayt-app-shell theme-${currentTheme}">
      <header class="jayt-header-sticky" role="banner">
        <div class="header-inner">
          <div class="brand-lockup" role="button" tabindex="0" data-nav="HOME" aria-label="Về trang chủ JayT">
            <!-- STRICT 32x32px LOGO CONTRACT (BV MANDATE #1) -->
            ...
            <div class="brand-text-group">
              ...
              <span class="brand-tagline">Community OS &bull; v3.426.1-staging.bv</span>
            </div>
          </div>
      ...
      <footer class="jayt-footer-unified" role="contentinfo">
        <div class="footer-inner">
          <div class="footer-brand-col">
            ...
            <span class="footer-version">Phiên bản: v3.426.1-staging.bv &bull; Staging Review BV</span>
          </div>
        </div>
      </footer>
      ...
      <div id="jayt-modal-root" class="jayt-modal-backdrop" hidden></div>
    </div>
  `;
}
...
function attachGlobalEvents() {
  ...
  // Report source modal with Zero-PII & Input Sanitization
...
function attachVoucherWalletEvents() {
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(codeToCopy).then(() => {
          showToast(`✓ Đã sao chép mã ưu đãi: ${codeToCopy}`);
        }).catch(() => {
  ...
  document.querySelectorAll('.btn-save-wallet').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      showToast('✓ Đã lưu voucher vào ví cá nhân');
    });
  });

  document.querySelectorAll('.wallet-tab-btn').forEach(tab => {
    tab.addEventListener('click', () => {
      document.querySelectorAll('.wallet-tab-btn').forEach(t => t.classList.remove('active'));
...
function attachBuyDecisionEvents() {
  ...
  if (submitBtn && input) {
    submitBtn.addEventListener('click', () => {
      ...
      showToast('Đang đối soát giá thực và chính sách bảo hành...');
    });
  }
}
  ...
  document.querySelectorAll('[data-save-id]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (savedItemIds.has(itemId)) {
        ...
        showToast('✓ Đã lưu vào ví cá nhân');
      }
      try {
        if (typeof localStorage !== 'undefined') {
