/**
 * JAYT COMPLETE AUTONOMOUS SUPPLY ENGINE RED-TEAM TEST SUITE (149)
...
const { readMemoryStrict, isRejectedByNegativeFilter, verifyOsExecutionTrace } = require('../05_DEAL_AND_AFFILIATE/jayt_autonomous_worker_149');

console.log('========================================================================');
console.log('🧪 JAYT-149: PROVEN AUTONOMOUS SUPPLY ENGINE & SCHEDULER REALITY AUDIT');
console.log('========================================================================\n');
...
async function runSupplyEngine149Audit() {
  ...
  test('Active pipeline contains zero static dictionary or string fallback patterns', () => {
    ...
    for (const dir of targetDirs) {
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.js') && !f.includes('quarantine') && !f.includes('test_supply_engine_149.js'));
  ...
  test('Windows Task Scheduler physically registers JAYT_AUTONOMOUS_SUPPLY_WORKER_149 with Last Result 0', () => {
    const taskName = 'JAYT_AUTONOMOUS_SUPPLY_WORKER_149';
  ...
  console.log('\n--- GATE 3: SCHEDULED TASK ACTION & WORKING DIRECTORY VERIFICATION ---');
  test('Task action invokes node.exe --scheduled-cycle with working directory set to project root', () => {
    const taskName = 'JAYT_AUTONOMOUS_SUPPLY_WORKER_149';
    const queryCmd = `schtasks /query /tn "${taskName}" /fo LIST /v`;
    const queryOut = execSync(queryCmd, { encoding: 'utf8' });

    assert(queryOut.includes('--scheduled-cycle'), 'Task To Run must include --scheduled-cycle');
    assert(queryOut.includes('jayt_autonomous_worker_149.js'), 'Task To Run must point to jayt_autonomous_worker_149.js');
    assert(queryOut.includes('Start In:') && queryOut.includes('JayT-D'), 'Start In must be set to project directory');
    console.log('     Verified task action: executes node.exe --scheduled-cycle with exact working directory.');
  });

  console.log('\n--- GATE 4: RUNTIME OS ORIGIN VERIFICATION (OS_SCHEDULED_TRIGGERED_VERIFIED) ---');
  test('OS-triggered scheduled cycle recorded OS_SCHEDULED_TRIGGERED_VERIFIED via runtime OS verification', () => {
    ...
    assert.strictEqual(manifest.execution_origin, 'OS_SCHEDULED_TRIGGERED_VERIFIED');
    assert.strictEqual(manifest.summary.items_processed, 20);
    assert(manifest.os_execution_evidence, 'Manifest must contain physical OS query evidence');
    console.log(`     Verified OS_SCHEDULED_TRIGGERED_VERIFIED batch: processed ${manifest.summary.items_processed} items into ${latestDirName}.`);
  });

  console.log('\n--- GATE 5: 6-STEP TAXONOMY & SOURCE REPAIR ERROR CLASSIFICATION ---');
  test('Error pages are classified as ERROR_OR_BLOCKED_SOURCE and marked SOURCE_PATH_STALE', () => {
    ...
    assert.strictEqual(b.ERROR_OR_BLOCKED_SOURCE, 5, 'Expected 5 blocked/error sources in this batch');
    assert.strictEqual(b.INCOMPLETE_OFFER_EVIDENCE, 2, 'Expected 2 incomplete offer pages in this batch');
  ...
  if (failCount > 0) {
    ...
    console.log('✨ ALL 9 JAYT-149 AUTONOMOUS SUPPLY ENGINE RED-TEAM TESTS PASSED 100% CLEAN!');
    process.exit(0);
  }
}

if (require.main === module) {
  runSupplyEngine149Audit();
}
