...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_208_certification');
...
async function runCertifyPromotedSources208() {
  console.log('========================================================================');
  console.log('🌐 JAYT-208: 3-GATE PRODUCTION DEPLOYMENT & PROMOTED SOURCES CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.348.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.348.0&t=${ts}`);
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3348 = bodyText.includes('OS 3.348') || bodyText.includes('Real Savings OS 3.348');
  ...
  console.log('   - OS 3.348 Rendered:                ' + (domReport.hasOs3348 ? '🟢 PASS' : '❌ FAIL'));

  const gate2Pass = (domReport.blueCount === 12 && domReport.purpleCount === 17 && domReport.totalDailyCards === 29 && domReport.hasHeadline && domReport.hasOs3348);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Desktop Light
  const desktopLightPath = path.join(artifactsDir, 'screenshot_208_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View: ' + desktopLightPath);

  // 2. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_208_mobile_light.png');
  ...
  // 3. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_208_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_208_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_208_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_208_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_208_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_208_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-208 PROMOTED SAVINGS DIRECTORY.');
  }
}

if (require.main === module) {
  runCertifyPromotedSources208().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyPromotedSources208 };
