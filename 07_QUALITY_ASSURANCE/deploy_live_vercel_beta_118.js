/**
 * LIVE VERCEL PRODUCTION DEPLOYMENT & PARITY AUDIT (118)
...
const receiptPath = path.join(repoRoot, '08_RELEASE_VAULT', 'DEPLOYMENT_RECEIPT_118.json');
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'live_beta_118');
...
async function deployAndAudit118() {
  console.log('🚀 [DEPLOY-118] Bắt đầu quy trình Deploy Live Vercel Beta & Audit 118...\n');
  ...
  const viewports = [
    { name: 'live_118_mobile_390px', width: 390, height: 844 },
    { name: 'live_118_desktop_1440px', width: 1440, height: 900 }
  ];
  ...
  for (const vp of viewports) {
    ...
    // Assert Tier 1 (4 Verified deals)
    ...
    // Assert Tier 3 (7 Menu combos & utilities)
    const menuCards = await page.$$eval('.state-public-menu .apex-deal-4q-card', els => els.length);
    console.log(`   [${vp.name}] Đã render ${menuCards} thẻ Tier 3 menu combo niêm yết & tiện ích.`);
  ...
  const receipt = {
    release_id: 'JAYT_118_REAL_SUPPLY_AND_HONEST_VALIDATION',
    version: 'v3.235.0',
    directive: 'JAYT-118-REAL-SUPPLY-AND-HONEST-VALIDATION',
    ...
    metrics_summary: {
      ...
      tier_1_verified_savings_count: 4,
      tier_2_needs_recheck_deals_count: 2,
      tier_3_public_menu_combos_count: 7,
    ...
    ethics_and_validation_summary: {
      report_117_status: 'SIMULATED_USABILITY_SCENARIOS — NOT REAL USER RESEARCH',
      disclaimer_active: true,
      anonymized_entities: ['P01', 'P02', 'P03', 'P04', 'P05', 'P06', 'P07', 'P08'],
      canonical_protocol_file: '08_RELEASE_VAULT/CANONICAL_USABILITY_TESTING_PROTOCOL_118.md',
  ...
  console.log(`\n✅ [DEPLOY-118] Đã phát hành DEPLOYMENT_RECEIPT_118.json tại:\n   ${receiptPath}\n`);
}

deployAndAudit118().catch(err => {
