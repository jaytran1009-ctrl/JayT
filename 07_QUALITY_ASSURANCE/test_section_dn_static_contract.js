/**
 * SECTION DN — STATIC CONTRACT & TICKER CLAIM ISOLATION VALIDATOR
 * Governing: JAYT-245 Section DN (Lines 2803-2833)
 */
const fs = require('fs');
const path = require('path');

const PROJECT_ROOT = 'd:/Công Việc MMO/OPC JayT/JayT-Dự Án Giá Trị Cộng Đồng';
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_dn.js');
const indexPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/index.html');

console.log('========================================================================');
console.log('🛡️  SECTION DN — STATIC CONTRACT & TICKER ISOLATION VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const dnModule = require(storefrontPath);
const items = dnModule.JAYT_DISCOVERY_ITEMS;
const walletEntries = dnModule.JAYT_WALLET_ENTRIES;
...
// Test 15: HTML only references jayt_storefront_staging_dn.js once
const htmlContent = fs.readFileSync(indexPath, 'utf8');
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_dn\.js"><\/script>/g) || [];
...
assertTest('Version string is v3.442.0-staging.dn', htmlContent.includes('v3.442.0-staging.dn'));
...
// Test 20: Ticker Quarantine (Section DN Directive) - Unverified schedule claim removed
...
// Test 21: Ticker Neutral Fallback (Section DN Directive)
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_dn_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION DN STATIC CONTRACT INTEGRITY PASS (21/21)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION DN STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
