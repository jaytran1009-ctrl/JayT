/**
 * JAYT-242: MASTER WORK ORDER & COMMUNITY OS GOVERNANCE QA GATE
 * 1. Exact Version Parity Gate (v3.398.0 strictly synced across all SOT files).
 * 2. 100% 43 Supply Cards resolve to PHYSICAL EVIDENCE BUNDLES on disk.
 * 3. Supply Truth Matrix Verification (6 Deals, 10 Programs, 16 Venues, 11 Radars).
 * 4. P0 Pure Radar Remediation: ZERO "NÊN MUA", ZERO "NÊN CHỜ", ZERO prices, ZERO commercial affiliate links in Tier 4.
...
function runGate() {
  console.log('========================================================================');
  console.log('🏛️ JAYT-242: MASTER WORK ORDER & COMMUNITY OS GOVERNANCE QA GATE');
  console.log('========================================================================\n');

  let violations = [];

  // 1. Exact Version Parity (v3.398.0)
  const EXPECTED_VERSION = 'v3.398.0';
  ...
  if (!index.includes(EXPECTED_VERSION) && !index.includes('3.398.0')) violations.push(`index.html does not contain ${EXPECTED_VERSION}`);
  ...
  // 4. P0 Pure Radar Remediation Gate (Zero Unproven Claims)
  const tier4Cards = supplyCards.filter(c => c.tier === 'TIER_4_RADAR');
  tier4Cards.forEach(card => {
    if (card.net_paid_price) violations.push(`Tier 4 Card ${card.card_id} must NOT have net_paid_price`);
    if (card.current_price) violations.push(`Tier 4 Card ${card.card_id} must NOT have current_price`);
    if (card.price_history_summary) violations.push(`Tier 4 Card ${card.card_id} must NOT have price_history_summary`);
    if (card.advisor_verdict) violations.push(`Tier 4 Card ${card.card_id} must NOT have advisor_verdict`);
    if (card.tracking_url && card.tracking_url.includes('go.isclix.com')) {
      violations.push(`Tier 4 Card ${card.card_id} must NOT have commercial affiliate tracking link`);
    }
  });

  // Check DOM for unproven claims
  if (apex.includes('NÊN MUA NGAY') || apex.includes('NÊN CHỜ SALE')) {
    violations.push('Found unproven "NÊN MUA / NÊN CHỜ" claim in apex interface (must be pure Radar)');
  }
  if (apex.includes('go.isclix.com')) {
    violations.push('Found commercial affiliate tracking URL in apex interface');
  }
  ...
  if (violations.length > 0) {
    console.error('❌ JAYT-242 QA GATE FAILED:');
    violations.forEach(v => console.error(`   - ${v}`));
    process.exit(1);
  }

  console.log('🟢 [JAYT-242-GATE-PASS] 100% P0 Pure Radar Remediation, Bundle Lineage & Truth Matrix v3.398.0 Verified!');
}
