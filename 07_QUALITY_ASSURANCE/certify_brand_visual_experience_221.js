...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_221_certification');
...
async function runCertifyBrandExperience221() {
  console.log('========================================================================');
  console.log('🌟 JAYT-221: 3-GATE PRODUCTION DEPLOYMENT & BRAND SYSTEM CERTIFICATION');
  ...
  for (let poll = 1; poll <= 5; poll++) {
    const ts = Date.now();
    const liveBrandText = await fetchText(`${liveUrl}jayt_brand_assets_221.js?v=3.361.0&nocache=${ts}`);
    liveBrandSha = sha256Buf(Buffer.from(liveBrandText, 'utf8'));

    const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.361.0&nocache=${ts}`);
  ...
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & BRAND SYSTEM ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3361 = bodyText.includes('OS 3.361') || bodyText.includes('Premium Brand & Discovery OS 3.361');
  ...
  console.log('   - OS 3.361 Rendered:                ' + (domReport.hasOs3361 ? '🟢 PASS' : '❌ FAIL'));
  ...
  const modalViewPath = path.join(artifactsDir, 'screenshot_221_modal_metiz_detail.png');
  ...
  const heroSpotlightPath = path.join(artifactsDir, 'screenshot_221_hero_spotlight_card.png');
  ...
  const desktopLightPath = path.join(artifactsDir, 'screenshot_221_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  ...
  const mobileLightPath = path.join(artifactsDir, 'screenshot_221_mobile_light.png');
  ...
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_221_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_221_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_221_hero_spotlight_card.png', path: heroSpotlightPath, sha256: sha256File(heroSpotlightPath), viewport: '1440x800' },
        { name: 'screenshot_221_modal_metiz_detail.png', path: modalViewPath, sha256: sha256File(modalViewPath), viewport: '1440x900' },
        { name: 'screenshot_221_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath), viewport: '1440x900' },
        { name: 'screenshot_221_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath), viewport: '390x844' },
        { name: 'screenshot_221_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath), viewport: '390x844' }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_221_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-221 PREMIUM BRAND & VISUAL DISCOVERY SYSTEM.');
  }
}

if (require.main === module) {
  runCertifyBrandExperience221().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyBrandExperience221 };
