/**
 * JAYT SUPPLY EXPANSION & LINEAGE FINALIZATION TEST SUITE (088C)
 * Directive: JAYT-088C-SUPPLY-EXPANSION-AND-LINEAGE-FINALIZATION
 *
 * Verifies:
 * 1. Strict DOM Lineage URL Parse & Resolution Verification:
 *    - Parent SHA-256, literal_href sliced at link_offset, and deterministic URL resolution matching requested_url.
 * 2. Physical Capture Parity: All 22 supply expansion target folders contain page.png, page.html, page.txt, receipt.
 * 3. 088B Candidates Preserved Pending CEO Review: Metiz U22 and CGV Payday maintained with verified physical locality.
 ...
 * 5. Zero Candidate Quota Forcing: Candidate count is derived dynamically from verified evidence (no >= N forcing).
...
const bundleManifestPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_088c', 'evidence_bundles_088c.json');
const reviewPackPath = path.join(repoRoot, '08_RELEASE_VAULT', 'JAYT_VERIFIED_SUPPLY_REVIEW_PACK_088C.md');
...
console.log('🧪 [JAYT-088C-TEST] Khởi chạy bộ kiểm thử Supply Expansion & Lineage Finalization 088C...\n');
...
// TEST 01: Strict DOM Lineage URL Parse & Resolution Verification
runTest('TEST_01_STRICT_DOM_LINEAGE_URL_PARSE_AND_RESOLVE_VERIFICATION', () => {
  assert.ok(fs.existsSync(manifest088cPath), 'Manifest 088C missing');
  const manifest = JSON.parse(fs.readFileSync(manifest088cPath, 'utf8'));
  assert.strictEqual(manifest.results.length, 22, 'Must have 22 supply targets');
  ...
  for (const item of manifest.results) {
    ...
    // Parse base domain from parent artifact receipt or URL
...
// TEST 02: All captured 088C artifacts exist with SHA-256
runTest('TEST_02_ALL_CAPTURED_088C_ARTIFACTS_EXIST_WITH_SHA256', () => {
  const manifest = JSON.parse(fs.readFileSync(manifest088cPath, 'utf8'));

  for (const item of manifest.results) {
    const itemDir = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'batch_capture_088c', 'captures_088c', item.target_id);
...
// TEST 03: 088B candidates preserved pending CEO review
runTest('TEST_03_TWO_EXISTING_088B_CANDIDATES_PRESERVED_PENDING_CEO_REVIEW', () => {
  const manifest = JSON.parse(fs.readFileSync(bundleManifestPath, 'utf8'));

  const metiz = manifest.bundles.find(b => b.bundle_id === 'BUNDLE_088C_METIZ_U22_RECURRING');
  const cgv = manifest.bundles.find(b => b.bundle_id === 'BUNDLE_088C_CGV_PAYDAY_30K_TIME_BOUNDED');
...
// TEST 05: No minimum candidate quota forcing - purely dynamic validation
runTest('TEST_05_NO_MINIMUM_CANDIDATE_QUOTA_FORCING', () => {
  const manifest = JSON.parse(fs.readFileSync(bundleManifestPath, 'utf8'));
  ...
  // Zero quota forcing: No assert.ok(verifiedCount >= N). Just confirm each verified candidate is valid.
  console.log(`     (Dynamic Evidence Audit: ${verifiedCount} deal candidate(s) grounded on physical disk)`);
});
...
runTest('TEST_06_LOYALTY_POLICIES_STRICTLY_ISOLATED_FROM_DEAL_CTAS', () => {
  ...
  assert.strictEqual(loyaltyBundles.length, 4, 'Must have exactly 4 loyalty references (Galaxy, CGV, Metiz, Gong Cha)');
...
if (passedTests === totalTests) {
  console.log(`🟢 [SUPPLY-LINEAGE-088C-SUMMARY] Kết quả kiểm thử: ${passedTests}/${totalTests} PASS!\n`);
} else {
  console.log(`❌ [SUPPLY-LINEAGE-088C-SUMMARY] Thất bại: ${passedTests}/${totalTests} PASS.\n`);
  process.exitCode = 1;
}
