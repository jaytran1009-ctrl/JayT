/**
 * SECTION EK — STATIC CONTRACT, REFERENTIAL INTEGRITY & STAGING AVAILABILITY VALIDATOR
 * Governing: JAYT-245 Section EK (Lines 3434-3461)
 */
const fs = require('fs');
...
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_ek.js');
...
const ledgerPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_PUBLIC_COUNT_LEDGER_EK.json');
const walletPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_WALLET_LEDGER_EK.json');
const voucherHubPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_voucher_hub_state.json');
const attestationPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_no_write_attestation.json');
const contractPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_unified_count_contract.json');
const snapshotPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_journey_count_snapshot.json');
const manifestEKPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_ek/EVIDENCE_MANIFEST_EK.json');
const affiliatePath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_AFFILIATE_RESEARCH_PORTFOLIO_OFFLINE.json');
const scorecardPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_supply_scorecard.json');
const scannerReportPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_content_scanner_report.json');
const rosterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_identity_roster.json');
const assetRegisterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_asset_register.json');
const quarantinePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_quarantine_manifest.json');
const runtimeTablePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_runtime_asset_table.json');
const ownershipRecordPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_asset_ownership_record.json');
const factLedgerPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_fact_ledger.json');
const expMatrixPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ek_experience_matrix.json');

console.log('========================================================================');
console.log('🛡️  SECTION EK — STATIC CONTRACT & REFERENTIAL INTEGRITY VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const ekModule = require(storefrontPath);
const items = ekModule.JAYT_DISCOVERY_ITEMS;
...
// Test 4: Fact Ledger EK Referential Integrity & Quarantined Facts (Mandate EK.1 & EK.2)
...
assertTest('Fact Ledger records strictly 0 verified field facts', factLedger.verified_field_facts && factLedger.verified_field_facts.length === 0);
assertTest('Fact Ledger logs 3 quarantined injected facts', factLedger.quarantined_injected_facts_log && factLedger.quarantined_injected_facts_log.length === 3);

// Test 5: Quarantine Manifest & Complete Isolation of 12 JPEGs (Mandate EK.1)
...
// Test 6: Complete Purge of False Provenance / Photographer CC Tokens & Injected Badges (Mandate EK.1)
...
// Test 8: Asset Ownership Record EK (Mandate EK.4)
...
// Test 9: Runtime Asset Audit Table EK (Mandate EK.4)
...
// Test 10: Vault EK 4 Lanes & Verbatim Timestamp Lineage (Mandate EK.3)
const manifestEK = JSON.parse(fs.readFileSync(manifestEKPath, 'utf8'));
assertTest('Evidence Manifest EK enforces 4 lanes summary and verbatim origin timestamps', manifestEK.four_lanes_summary && manifestEK.four_lanes_summary.LANE_A_PUBLIC_TRANSIT === 2);
assertTest('Backlog summary contains 8 exact-page candidates', manifestEK.field_facts_summary && manifestEK.field_facts_summary.backlog_exact_page_candidates === 8);
assertTest('Evidence Manifest EK confirms field_facts_verified_count equals 0', manifestEK.field_facts_summary.field_facts_verified_count === 0);
assertTest('Evidence Manifest EK records 3 quarantined injected facts', manifestEK.field_facts_summary.quarantined_injected_facts_count === 3);

const danabusEntry = manifestEK.artifacts.find(a => a.candidate_id === 'CAND_BUS_R16A');
...
manifestEK.artifacts.forEach(entry => {
...
assertTest('100% of raw artifact files exist on disk in EK vault with matching SHA-256 & byte size', allArtifactsFilesExist);

// Test 11: Pragmatic Experience Matrix (Mandate EK.4)
...
// Test 19: Scorecard EK vs Evidence Vault EK EXACT Match
const scorecardData = JSON.parse(fs.readFileSync(scorecardPath, 'utf8'));
assertTest('Evidence Vault EK contains exactly 8 CANONICAL_SOURCE_FOUND artifacts', manifestEK.canonical_sources_count === 8);
assertTest('Scorecard EK FIELD_VERIFIED_FACTS equals 0 (100% Reconciled)', scorecardData.scorecard.FIELD_VERIFIED_FACTS === 0);

// Test 20: HTML Content & Fingerprint Verifications
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_ek\.js"><\/script>/g) || [];
assertTest('Single script reference in index.html (no duplicates)', scriptMatches.length === 1);
assertTest('Version string is v3.465.0-staging.ek', htmlContent.includes('v3.465.0-staging.ek'));
assertTest('HTML body contains data-ledger-version="v3.465.0-staging.ek"', htmlContent.includes('data-ledger-version="v3.465.0-staging.ek"'));
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_ek_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION EK STATIC, REFERENTIAL INTEGRITY & AVAILABILITY PASS (66/66)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION EK STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
