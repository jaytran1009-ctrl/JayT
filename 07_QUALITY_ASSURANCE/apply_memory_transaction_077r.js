/**
 * JAYT PROJECT MEMORY TRANSACTION APPLICATION (077R)
 * Directive: JAYT-077R — COHORT TRUTH CORRECTION
...
| **Cohort Radar 077R** | \`COMMUNITY_SAVINGS_COHORT_REPORT_077R\` (32 probes) | 32 nguồn phân loại NETWORK_PROBE_METADATA_ONLY; tách biệt hoàn toàn 3 deal Staging đã có bằng chứng. |
...
const section5CriteriaText = `### 🎯 Mục tiêu: JAYT-077R — COHORT TRUTH CORRECTION

1. **Đính Chính Báo Cáo Cohort Radar 077R**: Ban hành [\`COMMUNITY_SAVINGS_COHORT_REPORT_077R.md\`](05_DEAL_AND_AFFILIATE/COMMUNITY_SAVINGS_COHORT_REPORT_077R.md) xác nhận 32 nguồn quét chỉ là \`NETWORK_PROBE_METADATA_ONLY\` (trạng thái truy cập mạng, không phải raw evidence bundle).
2. **Tách Rõ Bằng Chứng Staging & Probe Mạng**:
   - Tách biệt **3 deal Staging đã thẩm định** (Galaxy Happy Day + 2 deal Metiz có snapshot HTML/receipt độc lập trên đĩa) khỏi kết quả thăm dò mạng.
   - Xóa bỏ mọi logic gán nhãn hard-code theo source_id.
   - Giữ ranh giới nghiêm ngặt: Các tín hiệu \`PENDING_RECHECK\`, \`DISCOVERY_SIGNAL_ONLY\`, \`ACCOUNT_OR_CART_DEPENDENT\` chỉ thuộc về Radar, tuyệt đối không được đưa vào Lịch Tiết Kiệm Chính Thức.
3. **Bảo Tồn Bất Biến**: Giữ Production locked (\`deals_feed.json: []\`, \`is_approved: false\`), Staging giữ 3 deal sạch, Track 2 giữ PAUSED.`;

const section6LogEntry = `| \`2026-08-24T16:24:00+07:00\` | \`JAYT-077R\` | Đính chính toàn diện đợt quét Cohort 077 (ban hành COMMUNITY_SAVINGS_COHORT_REPORT_077R, phân loại 32 nguồn là NETWORK_PROBE_METADATA_ONLY, xóa hard-code source_id, tách biệt 3 deal Staging độc lập, rà soát Lịch 7 ngày sạch); cập nhật Project Memory v3.137.0. | [\`COMMUNITY_SAVINGS_COHORT_REPORT_077R.md\`](05_DEAL_AND_AFFILIATE/COMMUNITY_SAVINGS_COHORT_REPORT_077R.md)<br>[\`authorized_affiliate_accounts.json\`](05_DEAL_AND_AFFILIATE/feed_gateway/authorized_affiliate_accounts.json) | 100% PASS (6/6 Beta Readiness · 8/8 Staging Acceptance · 6/6 Cross-Layer Gate · 10/10 Memory Consistency · 8/8 Secret Hygiene · 5/5 Provenance Gate) | \`IMPLEMENTED_PENDING_CEO_AUDIT\` |`;
...
console.log('✅ [MEMORY-TRANSACTION-077R-SUCCESS]');
