...
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_196_containment');
...
async function certifyLiveState196() {
  console.log('========================================================================');
  console.log('🛡️ JAYT-196: EVIDENCE-BINDING CONTAINMENT & LIVE CERTIFICATION');
  ...
  try {
    const feedPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'generated_tiered_savings_feed_196.json');
  ...
  // --- GATE 2: LIVE DOM APPLICABILITY & CONTAINMENT ASSERTIONS ---
  console.log('\n--- GATE 2: LIVE DOM APPLICABILITY & CONTAINMENT ASSERTIONS ---');
  let browser = null;
  try {
    ...
    assert.strictEqual(confirmedCount, 1, 'local_confirmed_actionable_deals.length is not 1 (Contained state)');
    ...
    console.log('     Confirmed live feed: Exactly 1 Local Confirmed Deal (Mikazuki), 3 Scope Pending Deals, 10 Student Privileges.');

    // Check version badge 3.337
    assert(bodyText.includes('Daily Deal OS 3.337'), 'DOM missing version Daily Deal OS 3.337');
    console.log('     Confirmed Daily Deal OS 3.337 rendered.');
    ...
    // Check that batch 195 fake cards are NOT in the local confirmed list
    assert(!bodyText.includes('Metiz Cinema Đà Nẵng — Thứ Hai Siêu Hạng'), 'DOM improperly contains unverified Metiz Super Monday card');
    console.log('     Confirmed unverified batch 195 cards are completely removed from live site.');

    record('Live DOM Containment & Strict 1/30–50 KPI Binding', 'PASS', 0, 'OS 3.337, 1 Confirmed Deal, 3 Scope Pending Deals, 10 Student Privileges, Batch 195 Quarantined');
    ...
    const dPath = path.join(evidenceDir, 'screenshot_196_desktop_light.png');
    await page.screenshot({ path: dPath, fullPage: false });
    if (fs.existsSync(artifactsDir)) {
      fs.copyFileSync(dPath, path.join(artifactsDir, 'screenshot_196_desktop_light.png'));
    }
    screenshots.push({ name: 'screenshot_196_desktop_light.png', path: dPath, sha256: sha256File(dPath) });
    console.log('     📸 Saved: screenshot_196_desktop_light.png (' + screenshots[0].sha256.substring(0, 16) + '...)');

    // Mobile Light (iPhone 14)
    await page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
    await page.evaluate(() => document.body.setAttribute('data-theme', 'light'));
    await new Promise(r => setTimeout(r, 500));
    const mlPath = path.join(evidenceDir, 'screenshot_196_mobile_light.png');
    await page.screenshot({ path: mlPath, fullPage: false });
    if (fs.existsSync(artifactsDir)) {
      fs.copyFileSync(mlPath, path.join(artifactsDir, 'screenshot_196_mobile_light.png'));
    }
    screenshots.push({ name: 'screenshot_196_mobile_light.png', path: mlPath, sha256: sha256File(mlPath) });
    console.log('     📸 Saved: screenshot_196_mobile_light.png (' + screenshots[1].sha256.substring(0, 16) + '...)');

    // Mobile Dark (iPhone 14)
    await page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
    await page.evaluate(() => document.body.setAttribute('data-theme', 'dark'));
    await new Promise(r => setTimeout(r, 500));
    const mdPath = path.join(evidenceDir, 'screenshot_196_mobile_dark.png');
    await page.screenshot({ path: mdPath, fullPage: false });
    if (fs.existsSync(artifactsDir)) {
      fs.copyFileSync(mdPath, path.join(artifactsDir, 'screenshot_196_mobile_dark.png'));
    }
    screenshots.push({ name: 'screenshot_196_mobile_dark.png', path: mdPath, sha256: sha256File(mdPath) });
    console.log('     📸 Saved: screenshot_196_mobile_dark.png (' + screenshots[2].sha256.substring(0, 16) + '...)');
  ...
  // --- REPORT GENERATION ---
  const report = {
    certification_id: 'CERT_196_' + Date.now(),
  ...
  const reportPath = path.join(evidenceDir, 'CERTIFICATION_196_LIVE_REPORT.json');
  ...
  if (!report.all_passed) {
    ...
    console.log('\n🎉 ALL JAYT-196 LIVE CERTIFICATION GATES PASSED (SAFE CONTAINMENT & EVIDENCE BINDING)');
  }
}

if (require.main === module) {
  certifyLiveState196().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { certifyLiveState196 };
