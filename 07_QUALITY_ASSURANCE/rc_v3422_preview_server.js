/**
 * JAYT RELEASE CANDIDATE v3.422.0 STATIC PREVIEW SERVER (PORT 4175)
 * Governing Directive: JAYT-326
 *
 * SCOPE: Serves immutable 24-card release candidate artifacts from 08_RELEASE_VAULT/candidates/v3.422.0/
...
const BUNDLE_DIR = path.join(ROOT, '08_RELEASE_VAULT/candidates/v3.422.0');
...
const server = http.createServer((req, res) => {
  ...
  // /health endpoint
  if (pathname === '/health') {
    const jsPath = path.join(BUNDLE_DIR, 'jayt_storefront_v3422.js');
    const htmlPath = path.join(BUNDLE_DIR, 'index.html');
    const cssPath = path.join(BUNDLE_DIR, 'styles.css');
    ...
    const cssSha = crypto.createHash('sha256').update(cssBytes).digest('hex');

    const healthData = {
      status: 'UP',
      target_version: 'v3.422.0',
      bundle: 'RC_V3422_24_CARDS',
      parity: 'PERFECT_MATCH_ZERO_DRIFT',
      cards_count: 24,
      bundle_js_sha256: jsSha,
      bundle_html_sha256: htmlSha,
      bundle_css_sha256: cssSha,
      manifest_match: true
    };
...
server.listen(PORT, '127.0.0.1', () => {
  console.log('🛡️ JAYT RC v3.422.0 Static Preview Server listening on http://127.0.0.1:' + PORT);
