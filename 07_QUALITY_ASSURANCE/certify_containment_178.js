/**
 * JAYT-178: CONTAINMENT CERTIFICATION & SMOKE TEST
 ...
 * 2. Live DOM renders Daily Deal OS 3.319 and "0 Deal Đã Đối Soát".
...
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_178_containment');
...
async function run() {
  console.log('========================================================================');
  console.log('🛡️ JAYT-178: CONTAINMENT CERTIFICATION & SMOKE TEST');
  ...
  if (!puppeteer) {
    ...
    try {
      ...
      // Check version badge 3.319
      assert(bodyText.includes('Daily Deal OS 3.319') || bodyText.includes('3.319'), 'DOM missing version 3.319');
      console.log('     Confirmed Daily Deal OS 3.319 rendered.');

      // Check 0 Deal Đã Đối Soát
      assert(bodyText.includes('0 Deal Đã Đối Soát') || bodyText.includes('0 Deal'), 'DOM missing 0 Deal overview');
      assert(!bodyText.includes('7 Deal Đã Đối Soát'), 'DOM still contains "7 Deal Đã Đối Soát"');
      ...
      console.log('     Confirmed 0 Tier 1 deal cards in live DOM.');

      record('Live DOM containment verification', 'PASS', 0, 'Version 3.319, 0 deals overview, 0 Tier 1 cards');
      ...
      const dPath = path.join(evidenceDir, 'screenshot_178_desktop_light.png');
      await page.screenshot({ path: dPath, fullPage: false });
      screenshots.push({
        file: 'screenshot_178_desktop_light.png',
        viewport: '1440x900',
        theme: 'light',
      ...
      await page.evaluate(() => document.body.setAttribute('data-theme', 'light'));
      await new Promise(r => setTimeout(r, 500));
      const mLPath = path.join(evidenceDir, 'screenshot_178_mobile_light.png');
      await page.screenshot({ path: mLPath, fullPage: false });
      screenshots.push({
        file: 'screenshot_178_mobile_light.png',
        viewport: '390x844',
        theme: 'light',
      ...
      await new Promise(r => setTimeout(r, 500));
      const mDPath = path.join(evidenceDir, 'screenshot_178_mobile_dark.png');
      await page.screenshot({ path: mDPath, fullPage: false });
      screenshots.push({
        file: 'screenshot_178_mobile_dark.png',
        viewport: '390x844',
        theme: 'dark',
  ...
  const out = {
    ...
    verdict: failCount === 0 && inconclusiveCount === 0 ? 'LIVE_CONTAINMENT_178_VERIFIED' : 'LIVE_CONTAINMENT_FAILED'
  };

  const outPath = path.join(evidenceDir, 'CERTIFICATION_RESULT_178.json');
  fs.writeFileSync(outPath, JSON.stringify(out, null, 2), 'utf8');
  console.log(`📄 Result saved to: ${outPath}`);

  process.exit(out.verdict === 'LIVE_CONTAINMENT_178_VERIFIED' ? 0 : 1);
}
