/**
 * JAYT REAL BROWSER CDP E2E & MOBILE ACCESSIBILITY GATE (081T)
 * Launches real headless Chromium via Chrome DevTools Protocol (CDP).
 * Tests exact 390px mobile viewport, 4 immediate tabs fully contained in 390px,
 * keyboard Space/Enter step pill navigation, touch targets >= 44px,
...
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'e2e_081t');
...
console.log(`🌐 [REAL-BROWSER-081T] Tìm thấy trình duyệt: ${browserPath}`);
...
async function runRealBrowserSuite() {
  ...
  try {
    ...
    // --- 3. CHECK 4 IMMEDIATE MOBILE TABS FULLY CONTAINED IN 390PX ---
    ...
    assertCheck('CHECK_02_4_IMMEDIATE_MOBILE_TABS_CONTAINED',
      mTabs.allContained,
    ...
    // --- 4. ACCESSIBILITY: STEP PILLS ARE BUTTONS WITH ARIA & KEYBOARD SPACE/ENTER ---
    ...
    assertCheck('CHECK_03_STEP_PILLS_ACCESSIBILITY_AND_ARIA',
    ...
    assertCheck('CHECK_04_STEP_PILLS_KEYBOARD_SPACE_ENTER',
    ...
    // --- 5. GEOMETRY AUDIT: ALL VISIBLE TOUCH TARGETS >= 44PX ---
    ...
    assertCheck('CHECK_05_ALL_TOUCH_TARGETS_MIN_44PX',
    ...
    assertCheck('CHECK_06_USER_VOUCHER_SAVED_WITH_NEUTRAL_LABEL',
    ...
    // --- 7. CAPTURE MOBILE SCREENSHOT 390x844 ---
    ...
    // --- 8. SWITCH TO DESKTOP 1280x800 & CAPTURE DESKTOP SCREENSHOT ---
    await cdp.send('Emulation.setDeviceMetricsOverride', {
      width: 1280,
      height: 800,
      deviceScaleFactor: 1,
    ...
    assertCheck('CHECK_09_AUDIT_REPORT_SAVED',
      fs.existsSync(auditJsonPath),
      `Đã xuất bản báo cáo kiểm toán E2E hình học: ${path.relative(repoRoot, auditJsonPath)}`
    );

    console.log(`\n======================================================`);
    console.log(`🟢 [REAL-BROWSER-081T-SUMMARY] Kết quả kiểm thử trình duyệt thật: ${pass}/${pass + fail} PASS!\n`);
