...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_216_certification');
...
async function runCertifySyntheticVisualContainment216() {
  console.log('========================================================================');
  console.log('🌐 JAYT-216: 3-GATE PRODUCTION DEPLOYMENT & SYNTHETIC CONTAINMENT CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.356.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.356.0&t=${ts}`);
  ...
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & SYNTHETIC CONTAINMENT ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3356 = bodyText.includes('OS 3.356') || bodyText.includes('Live Visual OS 3.356');
    const hasVisualDashboard = bodyText.includes('ĐỐI SOÁT BẰNG CHỨNG HÌNH ẢNH') && bodyText.includes('0 overclaim · 100% Identity Canvas');
  ...
  console.log('   - OS 3.356 Rendered:                ' + (domReport.hasOs3356 ? '🟢 PASS' : '❌ FAIL'));
  ...
  const modalReport = await page.evaluate(() => {
    ...
    return {
      ...
      hasHonestProvenance: text.includes('chưa có ảnh ưu đãi/địa điểm xác minh') || text.includes('official_identity')
    };
  });

  console.log('   - Modal Opened:                     ' + (modalReport.modalOpened ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - Identity Hero Visual (Section 1): ' + (modalReport.hasSection1Hero ? '🟢 PASS' : '❌ FAIL'));
  ...
  console.log('   - Honest Provenance Label:          ' + (modalReport.hasHonestProvenance ? '🟢 PASS' : '❌ FAIL'));
  ...
  const modalViewPath = path.join(artifactsDir, 'screenshot_216_modal_identity_detail.png');
  ...
  // 2. First-View Card
  const firstViewCardPath = path.join(artifactsDir, 'screenshot_216_first_view_card.png');
  ...
  const desktopLightPath = path.join(artifactsDir, 'screenshot_216_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  ...
  const mobileLightPath = path.join(artifactsDir, 'screenshot_216_mobile_light.png');
  ...
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_216_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_216_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_216_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath), viewport: '1440x900' },
        { name: 'screenshot_216_first_view_card.png', path: firstViewCardPath, sha256: sha256File(firstViewCardPath), viewport: '1440x750' },
        { name: 'screenshot_216_modal_identity_detail.png', path: modalViewPath, sha256: sha256File(modalViewPath), viewport: '1440x900' },
        { name: 'screenshot_216_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath), viewport: '390x844' },
        { name: 'screenshot_216_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath), viewport: '390x844' }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_216_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-216 SYNTHETIC VISUAL CLAIM CONTAINMENT.');
  }
}

if (require.main === module) {
  runCertifySyntheticVisualContainment216().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifySyntheticVisualContainment216 };
