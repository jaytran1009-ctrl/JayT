...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_211_certification');
...
async function runCertifyRealVisuals211() {
  console.log('========================================================================');
  console.log('🌐 JAYT-211: 3-GATE PRODUCTION DEPLOYMENT & REAL VISUALS CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.351.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.351.0&t=${ts}`);
  ...
  // --- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & PREMIUM IDENTITY TILES RENDERING ---
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & PREMIUM IDENTITY TILES RENDERING ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3351 = bodyText.includes('OS 3.351') || bodyText.includes('Real Visuals OS 3.351');
  ...
  console.log('   - OS 3.351 Rendered:                ' + (domReport.hasOs3351 ? '🟢 PASS' : '❌ FAIL'));
  ...
  console.log('   - Monogram / Brand Tile:            ' + (modalReport.hasMonogramCrest ? '🟢 PASS' : '❌ FAIL'));
  ...
  const gate2Pass = (domReport.blueCount === 12 && domReport.purpleCount === 17 && domReport.totalDailyCards === 29 && domReport.hasHeadline && domReport.hasOs3351 && modalReport.modalOpened && modalReport.hasMonogramCrest);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Modal View with Premium Identity Tile
  const modalViewPath = path.join(artifactsDir, 'screenshot_211_modal_identity_tile.png');
  await page.screenshot({ path: modalViewPath, fullPage: false });
  console.log('   📸 Captured Modal Identity Tile View:  ' + modalViewPath);
  ...
  // 2. Desktop Light
  const desktopLightPath = path.join(artifactsDir, 'screenshot_211_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View:        ' + desktopLightPath);

  // 3. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_211_mobile_light.png');
  ...
  // 4. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_211_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_211_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_211_modal_identity_tile.png', path: modalViewPath, sha256: sha256File(modalViewPath) },
        { name: 'screenshot_211_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_211_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_211_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_211_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-211 REAL VISUALS & PREMIUM PRESENTATION.');
  }
}

if (require.main === module) {
  runCertifyRealVisuals211().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyRealVisuals211 };
