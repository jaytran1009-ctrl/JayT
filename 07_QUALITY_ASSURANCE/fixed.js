...
console.log('đŸ§ª [JAYT-VALIDATOR-TEST] Khá»Ÿi cháº¡y bá»™ kiá»ƒm thá»­ tĂ­nh toĂ n váº¹n vĂ  bĂ i test Ă¢m cho Candidate Validator...');

const repoRoot = path.resolve(__dirname, '..');
const tempTestDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jayt_val_test_'));
...
try {
  ...
  const validTextContent = 'Metiz Cinema: VĂ© 2D 55.000 VNÄ cho thĂ nh viĂªn U22 tá»« 01/01/2026 Ä‘áº¿n 31/12/2026.';
  fs.writeFileSync(validTextPath, validTextContent, 'utf8');
  ...
  fs.writeFileSync(validPngPath, validPngContent);
  ...
  const candidateValid = {
    ...
    merchant: "Metiz Cinema ÄĂ  Náºµng",
    title: "VĂ© 2D 55K U22",
    ...
    extracted_claims: {
      price_snippet: "55.000 VNÄ",
      conditions_snippet: "thĂ nh viĂªn U22",
      expiry_snippet: "31/12/2026"
    },
    observed_price_or_offer: "55.000 VNÄ",
    observed_conditions: "ThĂ nh viĂªn U22",
    expiry_basis: "Äáº¿n 31/12/2026",
    verification_readiness: "READY_FOR_CEO_REVIEW"
  };

  const res1 = validateCandidate(candidateValid, { domainCatalog: mockDomainCatalog });
  ...
  const emptyHomepageContent = 'ChĂ o má»«ng Ä‘áº¿n vá»›i Metiz Cinema. Trang chá»§ ráº¡p chiáº¿u phim.';
  fs.writeFileSync(emptyHomepageTextPath, emptyHomepageContent, 'utf8');
  ...
  const candidateFakeReview = {
    ...
    merchant: "Metiz Cinema ÄĂ  Náºµng",
    title: "VĂ© 2D 55K U22 (ChÆ°a CĂ³ Báº±ng Chá»©ng)",
    category: "local_entertainment",
    category_scope: "LOCAL_EXPERIENCE",
    affiliate_type: "DIRECT_DEAL",
    ...
    extracted_claims: {
      price_snippet: "55.000 VNÄ", // Not in homepage_only.txt
      conditions_snippet: "thĂ nh viĂªn U22", // Not in homepage_only.txt
  ...
  const res2 = validateCandidate(candidateFakeReview, { domainCatalog: mockDomainCatalog });
  ...
  const res3 = validateCandidate(candidateTamperedHash, { domainCatalog: mockDomainCatalog });
  ...
  const res4 = validateCandidate(candidateUnapprovedDomain, { domainCatalog: mockDomainCatalog });
  ...
  const res5 = validateCandidate(candidateMissingField, { domainCatalog: mockDomainCatalog });
  ...
  const candidateNeedsRecheck = {
    ...
    title: "Khuyáº¿n MĂ£i Highlands Coffee",
  ...
  const res6 = validateCandidate(candidateNeedsRecheck, { domainCatalog: mockDomainCatalog });
  ...
  const candidateMismatchSnippet = {
    ...
    extracted_claims: {
      price_snippet: "999.000 VNÄ", // Non-existent in validTextContent
  };
  const res7 = validateCandidate(candidateMismatchSnippet, { domainCatalog: mockDomainCatalog });
...
// --- Summary ---
console.log(\n\ [VALIDATOR-TEST-SUMMARY] Toàn bộ \ kiểm thử validator (bao gồm Negative Tests) đã \!);
if (!allPassed) {
  process.exit(1);
} else {
  process.exit(0);
