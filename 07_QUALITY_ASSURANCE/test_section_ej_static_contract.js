/**
 * SECTION EJ — STATIC CONTRACT, VERIFIED FIELD FACTS & STAGING AVAILABILITY VALIDATOR
 * Governing: JAYT-245 Section EJ (Lines 3401-3431)
 */
const fs = require('fs');
...
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_ej.js');
...
const ledgerPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_PUBLIC_COUNT_LEDGER_EJ.json');
const walletPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_WALLET_LEDGER_EJ.json');
const voucherHubPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_voucher_hub_state.json');
const attestationPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_no_write_attestation.json');
const contractPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_unified_count_contract.json');
const snapshotPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_journey_count_snapshot.json');
const manifestEJPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_ej/EVIDENCE_MANIFEST_EJ.json');
const affiliatePath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_AFFILIATE_RESEARCH_PORTFOLIO_OFFLINE.json');
const scorecardPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_supply_scorecard.json');
const scannerReportPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_content_scanner_report.json');
const rosterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_identity_roster.json');
const assetRegisterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_asset_register.json');
const quarantinePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_quarantine_manifest.json');
const runtimeTablePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_runtime_asset_table.json');
const ownershipRecordPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_asset_ownership_record.json');
const factLedgerPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_fact_ledger.json');
const expMatrixPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_ej_experience_matrix.json');

console.log('========================================================================');
console.log('🛡️  SECTION EJ — STATIC CONTRACT & VERIFIED FIELD FACTS VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const ejModule = require(storefrontPath);
const items = ejModule.JAYT_DISCOVERY_ITEMS;
...
// Test 4: Fact Ledger EJ Verified Field Facts (Mandate EJ.2)
const factLedger = JSON.parse(fs.readFileSync(factLedgerPath, 'utf8'));
assertTest('Fact Ledger records exactly 8 backlog candidate attempts', factLedger.attempts_and_artifacts && factLedger.attempts_and_artifacts.length === 8);
assertTest('Fact Ledger records exactly 3 verified field facts', factLedger.verified_field_facts && factLedger.verified_field_facts.length === 3);
assertTest('All 3 verified field facts have 10 mandatory fields complete', factLedger.verified_field_facts.every(f => (
  f.field_id && f.subject && f.fact_name && f.exact_value_or_quote && f.locator && f.canonical_url && f.geographic_scope && f.freshness_policy && f.original_timestamp && f.reviewer_decision === 'FIELD_VERIFIED'
)));

// Test 5: Quarantine Manifest & Complete Isolation of 12 JPEGs (Mandate EJ.1)
...
// Test 6: Complete Purge of False Provenance / Photographer CC Tokens (Mandate EJ.1 & EJ.3)
...
// Test 8: Asset Ownership Record EJ (Mandate EJ.5)
...
// Test 9: Runtime Asset Audit Table EJ (Mandate EJ.5)
...
// Test 10: Vault EJ 4 Lanes & Verbatim Timestamp Lineage (Mandate EJ.1)
const manifestEJ = JSON.parse(fs.readFileSync(manifestEJPath, 'utf8'));
assertTest('Evidence Manifest EJ enforces 4 lanes summary and verbatim origin timestamps', manifestEJ.four_lanes_summary && manifestEJ.four_lanes_summary.LANE_A_PUBLIC_TRANSIT === 2);
assertTest('Backlog summary contains 8 exact-page candidates', manifestEJ.field_facts_summary && manifestEJ.field_facts_summary.backlog_exact_page_candidates === 8);
assertTest('Evidence Manifest EJ confirms field_facts_verified_count equals 3', manifestEJ.field_facts_summary.field_facts_verified_count === 3);

const danabusEntry = manifestEJ.artifacts.find(a => a.candidate_id === 'CAND_BUS_R16A');
...
manifestEJ.artifacts.forEach(entry => {
...
assertTest('100% of raw artifact files exist on disk in EJ vault with matching SHA-256 & byte size', allArtifactsFilesExist);

// Test 11: Pragmatic Experience Matrix (Mandate EJ.4)
...
assertTest('Experience Matrix records 3 verified field facts', expMatrix.verified_field_facts_count === 3);
...
assertTest('Identity Roster verified field facts count equals 3', rosterData.roster_cardinality.verified_field_facts_count === 3);
...
assertTest('Snapshot JSON matches verified field facts count (3)', snapshot.data.verified_field_facts_count === 3);

// Test 19: Scorecard EJ vs Evidence Vault EJ EXACT Match
const scorecardData = JSON.parse(fs.readFileSync(scorecardPath, 'utf8'));
assertTest('Evidence Vault EJ contains exactly 8 CANONICAL_SOURCE_FOUND artifacts', manifestEJ.canonical_sources_count === 8);
assertTest('Scorecard EJ FIELD_VERIFIED_FACTS equals 3 (100% Reconciled)', scorecardData.scorecard.FIELD_VERIFIED_FACTS === 3);

// Test 20: HTML Content & Fingerprint Verifications
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_ej\.js"><\/script>/g) || [];
assertTest('Single script reference in index.html (no duplicates)', scriptMatches.length === 1);
assertTest('Version string is v3.464.0-staging.ej', htmlContent.includes('v3.464.0-staging.ej'));
assertTest('HTML body contains data-ledger-version="v3.464.0-staging.ej"', htmlContent.includes('data-ledger-version="v3.464.0-staging.ej"'));
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_ej_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION EJ STATIC, VERIFIED FIELD FACTS & AVAILABILITY PASS (64/64)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION EJ STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
