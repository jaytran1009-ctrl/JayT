/**
 * JAYT DEEP LINK INTEGRITY & QUALITY CONTROL TEST SUITE (064B)
 ...
 * 4. Rule 12: Strict Deep Promotion URL Filter (no fragments, no login, no nav pages, no duplicates).
 * 5. Hash Chain Verification across 19 historical receipts and 6 revisions.
 * 6. Honest naming: 'local_trust_anchor_reference' (UNANCHORED_LOCALLY_SECURED_PENDING_OFFSITE_BACKUP).
 */
...
const sandboxDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'private_sandbox', 'test_064b_sandbox');
...
// 5. Hash Chain Verification (19 receipts & 6 revisions)
function LL_05_APPEND_ONLY_IMMUTABILITY_HASH_CHAIN_AUDIT() {
  ...
  return ver.valid === true &&
         ver.verified_receipts_count === 19 &&
         chainLedger.revisions.length === 6;
}
...
// 12. Strict Deep Promotion URL Filter (Rule 12 / 064B)
function LL_12_DEEP_PROMOTION_URL_INTEGRITY_BINDING() {
  const mockLinks = [
    { href: 'https://jollibee.com.vn/khuyen-mai#contentarea', text: 'Skip to content' }, // Fragment -> MUST REJECT
    { href: 'https://jollibee.com.vn/khuyen-mai#social-login', text: 'Đăng nhập' },      // Fragment & Login -> MUST REJECT
    { href: 'https://shopee.vn/buyer/login?next=foo', text: 'Login' },                   // Auth -> MUST REJECT
    { href: 'https://otherdomain.com/khuyen-mai/deal-1', text: 'Deal' },                 // Wrong host -> MUST REJECT
    { href: 'https://dominos.vn/khuyen-mai/combo-mua-1-tang-1', text: 'Mua 1 tặng 1' },   // Valid Deep -> PASS
    { href: 'https://dominos.vn/khuyen-mai/combo-mua-1-tang-1', text: 'Mua 1 tặng 1' },   // Duplicate -> MUST DEDUPLICATE TO 1
    { href: 'https://be.com.vn/khach-hang-ca-nhan/tin-tuc-bao-chi/', text: 'Press' }     // Navigation -> MUST REJECT
  ];

  const filtered = filterStrictDeepPromotionUrls(mockLinks, 'dominos.vn');

  return filtered.length === 1 &&
         filtered[0].href === 'https://dominos.vn/khuyen-mai/combo-mua-1-tang-1' &&
         filtered[0].text === 'Mua 1 tặng 1';
}

// Master Test Runner
function run064bTests() {
  console.log('🧪 [JAYT-LESSONS-064B-TEST] Khởi chạy bộ kiểm thử Deep Link Integrity & Quality Control (064B)...\n');
  ...
  assertTest('LL_05_APPEND_ONLY_IMMUTABILITY_HASH_CHAIN_AUDIT', LL_05_APPEND_ONLY_IMMUTABILITY_HASH_CHAIN_AUDIT(), 'Chuỗi băm 19 receipt & 6 revision snapshot khớp 100%.');
  ...
  assertTest('LL_12_DEEP_PROMOTION_URL_INTEGRITY_BINDING', LL_12_DEEP_PROMOTION_URL_INTEGRITY_BINDING(), 'Rule 12: Bộ lọc Strict Deep Promotion URL loại bỏ 100% fragment, login, nav và trùng lặp.');

  console.log(`\n🟢 [LESSONS-064B-SUMMARY] TOÀN BỘ ${passedTests}/${totalTests} KIỂM THỬ ĐÃ ĐẠT [PASS]!\n`);
