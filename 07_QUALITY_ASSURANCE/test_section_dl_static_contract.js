/**
 * SECTION DL — STATIC CONTRACT & PROVENANCE VALIDATOR
 * Governing: JAYT-245 Section DL (Lines 2747-2764)
 */
const fs = require('fs');
const path = require('path');

const PROJECT_ROOT = 'd:/Công Việc MMO/OPC JayT/JayT-Dự Án Giá Trị Cộng Đồng';
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_dl.js');
const indexPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/index.html');

console.log('========================================================================');
console.log('🛡️  SECTION DL — STATIC CONTRACT & PROVENANCE VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const dlModule = require(storefrontPath);
const items = dlModule.JAYT_DISCOVERY_ITEMS;
const walletEntries = dlModule.JAYT_WALLET_ENTRIES;
...
// Test 15: HTML only references jayt_storefront_staging_dl.js once
const htmlContent = fs.readFileSync(indexPath, 'utf8');
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_dl\.js"><\/script>/g) || [];
...
assertTest('Version string is v3.440.0-staging.dl', htmlContent.includes('v3.440.0-staging.dl'));
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_dl_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION DL STATIC CONTRACT INTEGRITY PASS (18/18)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION DL STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
