/**
 * JAYT LIVE PUBLIC BETA UI DEPLOYMENT RUNNER (168)
 * Directive: JAYT-168: UI RELEASE, KHÔNG CÒN “INTERNAL-ONLY”
 * 
 * CORE RESPONSIBILITIES:
 * 1. Synchronizes 03_SOURCE_OF_TRUTH files to deploy/ and deploy/public/ with 100% SHA-256 parity.
 * 2. Deploys UI Public Beta to Vercel (https://deploy-ten-xi-48.vercel.app/).
 * 3. Audits the live site using Puppeteer.
 * 4. Captures 3 required live screenshots:
 *    - screenshot_168_live_desktop_light.png (1440x900)
 *    - screenshot_168_live_mobile_light.png (390x844)
 *    - screenshot_168_live_mobile_dark.png (390x844)
 * 5. Verifies 5 Category Hubs, 4 Card Tiers, "Hôm nay trên JayT" block on live production.
 */
...
console.log('======================================================');
console.log('🚀 JAYT-168: DEPLOYING LIVE PUBLIC BETA UI TO VERCEL');
console.log('======================================================\n');
...
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'screenshots_168');
...
// Also copy hybrid supply dashboard and contract
const extraFiles = [
...
(async () => {
  try {
    ...
    const desktopLightShot = path.join(evidenceDir, 'screenshot_168_live_desktop_light.png');
    ...
    const mobileLightShot = path.join(evidenceDir, 'screenshot_168_live_mobile_light.png');
    ...
    const mobileDarkShot = path.join(evidenceDir, 'screenshot_168_live_mobile_dark.png');
    ...
    const roadmapPresent = await page.$eval('#jayt-building-roadmap', el => Boolean(el));
    console.log(`  ✅ Live "JayT Đang Xây Gì" Roadmap Section: ${roadmapPresent ? 'PRESENT' : 'MISSING'}`);
    assert(roadmapPresent, 'Roadmap section must exist on live site');

    await browser.close();
    console.log('\n✨ LIVE PUBLIC BETA DEPLOYMENT AND VERIFICATION COMPLETED 100% CLEAN!');
    process.exit(0);
  } catch (err) {
