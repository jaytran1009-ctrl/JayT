...
console.log('========================================================================');
console.log('🛡️ JAYT-245 QA GATE: UPGRADE-ONLY VALIDATOR & FIELD-LEVEL RENDER GATE (BC)');
console.log('========================================================================\n');

let allPassed = true;

// 1. Audit START_HERE_AZ.md Entrypoint
console.log('🔍 [Audit 1] START_HERE_AZ.md Entrypoint & Mandatory Reading Order (BC)...');
...
if (fs.existsSync(startHerePath)) {
  ...
  if (content.includes('JAYT_CURRENT_STATE_BC.json') && content.includes('JAYT_WORKSPACE_RECONCILIATION_RECEIPT_BC.json')) {
    console.log('   ✅ START_HERE_AZ.md entrypoint verified with BC pointer & reconciliation receipt: PASS');
  } else {
    console.error('   ❌ START_HERE_AZ.md missing BC pointer references!');
    allPassed = false;
  }
} else {
...
// 2. Audit Current State Pointer BC & Active Epoch Integrity
console.log('\n🔍 [Audit 2] JAYT_CURRENT_STATE_BC.json Pointer & Active Epoch Integrity...');
const pointerPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_CURRENT_STATE_BC.json');
if (fs.existsSync(pointerPath)) {
  ...
  if (fs.existsSync(epochPath)) {
    ...
    if (calculatedHash === pointer.active_epoch_sha256) {
      console.log(`   ✅ Pointer BC matches Active Epoch SHA-256 (${calculatedHash.substring(0, 16)}...): PASS`);
  ...
  console.error('   ❌ JAYT_CURRENT_STATE_BC.json pointer does not exist!');
  allPassed = false;
}

// 3. Audit Full Workspace Reconciliation Receipt BC with Denominator & Exact Hashes
console.log('\n🔍 [Audit 3] JAYT_WORKSPACE_RECONCILIATION_RECEIPT_BC.json Denominator & Exact Hashes...');
const reconPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_WORKSPACE_RECONCILIATION_RECEIPT_BC.json');
if (fs.existsSync(reconPath)) {
  ...
  if (totalChecked === 15973 && trackedOnDisk === 15973 && recon.inventory_coverage_summary.missing_assets === 0) {
    ...
    console.log(`   ✅ Canonical Accounting: ${recon.canonical_public_accounting_bc.accounting_formula}`);
  } else {
    console.error(`   ❌ Workspace reconciliation incomplete or denominator missing: ${trackedOnDisk} / ${totalChecked}`);
    allPassed = false;
  }
} else {
  console.error('   ❌ JAYT_WORKSPACE_RECONCILIATION_RECEIPT_BC.json does not exist!');
  allPassed = false;
}

// 4. Field-Level Render Gate & DOM Verification on Storefront BC
console.log('\n🔍 [Audit 4] Strict Field-Level Render Gate Verification on Storefront BC...');
const storefrontBcPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_bc.js');
const storefrontBcCode = fs.readFileSync(storefrontBcPath, 'utf8');

// Check 1: Hero banner copy
if (storefrontBcCode.includes('Tất cả đều có điều kiện rõ ràng, đã đối soát độc lập')) {
  console.error('   ❌ Hero banner contains unproven broad claim!');
  allPassed = false;
} else if (storefrontBcCode.includes('Phân tầng minh bạch theo cấp độ xác minh')) {
...
const gatewaysSectionMatch = storefrontBcCode.match(/<section class="daily-gateways-section"[\s\S]*?<\/section>/);
if (gatewaysSectionMatch) {
  ...
  console.error('   ❌ Gateway section not found in storefront BC!');
  allPassed = false;
}
...
const { JAYT_DISCOVERY_ITEMS } = require(storefrontBcPath);
...
if (storefrontBcCode.includes('DEAL_120_CGV_ZALOPAY_12H')) {
...
if (storefrontBcCode.includes('accesstrade') || storefrontBcCode.includes('utm_source=jayt_affiliate')) {
...
console.log('\n🧪 [Phase 2] Running 9 Adversarial Mutation Tests on Real Fixture Copies (Section BC)...');
...
const isM2Rejected = !fakeCatalog.user_interfaces_and_design.some(u => u.id === "UI_STOREFRONT_BC");
...
const tamperedPointer = { active_epoch_file: "00_PROGRAM_BASELINE/JAYT_CURRENT_STATE_BC.json", active_epoch_sha256: "tampered_000000000000000000000000" };
...
const fakeStorefrontWithQuarantine = storefrontBcCode + '\n// DEAL_120_CGV_ZALOPAY_12H';
...
const fakeDeployTarget = { target: "production", version: "v3.422.2-staging.bc" };
...
const fakeStorefrontWithAffiliate = storefrontBcCode + '\nconst aff = "https://go.isclix.com/deep_link/123";';
...
console.log('\n------------------------------------------------------------------------');
if (!allPassed) {
  console.error('❌ [UPGRADE-ONLY-VALIDATOR-BC-FAIL] One or more validator audits failed!');
  process.exit(1);
}

console.log('🟢 [UPGRADE-ONLY-VALIDATOR-BC-PASS] 100% Field-Level Render Gate & Upgrade-Only Contract Validated (Section BC)!');
console.log('========================================================================');
