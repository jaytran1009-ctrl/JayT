...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_221R_certification');
...
async function runCertifyBrandExperience221R() {
  console.log('========================================================================');
  console.log('🌟 JAYT-221R: 3-GATE PRODUCTION DEPLOYMENT & IDENTITY FINISHING CERTIFICATION');
  ...
  for (let poll = 1; poll <= 5; poll++) {
    const ts = Date.now();
    const liveBrandText = await fetchText(`${liveUrl}jayt_brand_assets_221.js?v=3.361.1&nocache=${ts}`);
    liveBrandSha = sha256Buf(Buffer.from(liveBrandText, 'utf8'));

    const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.361.1&nocache=${ts}`);
    ...
    if (gate1BrandPass && gate1MainJsPass) break;
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3361R = bodyText.includes('OS 3.361R') || bodyText.includes('Premium Identity Finishing OS 3.361R');
  ...
  console.log('   - OS 3.361R Rendered:               ' + (domReport.hasOs3361R ? '🟢 PASS' : '❌ FAIL'));
  ...
  const modalViewPath = path.join(artifactsDir, 'screenshot_221R_modal_metiz_detail.png');
  ...
  const heroSpotlightPath = path.join(artifactsDir, 'screenshot_221R_hero_spotlight_card.png');
  ...
  const desktopLightPath = path.join(artifactsDir, 'screenshot_221R_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  ...
  const mobileLightPath = path.join(artifactsDir, 'screenshot_221R_mobile_light.png');
  ...
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_221R_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_221R_' + Date.now(),
    ...
    gate1_hash_parity: {
      pass: gate1BrandPass && gate1MainJsPass,
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_221R_hero_spotlight_card.png', path: heroSpotlightPath, sha256: sha256File(heroSpotlightPath), viewport: '1440x800' },
        { name: 'screenshot_221R_modal_metiz_detail.png', path: modalViewPath, sha256: sha256File(modalViewPath), viewport: '1440x900' },
        { name: 'screenshot_221R_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath), viewport: '1440x900' },
        { name: 'screenshot_221R_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath), viewport: '390x844' },
        { name: 'screenshot_221R_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath), viewport: '390x844' }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_221R_LIVE_REPORT.json');
  ...
  const allGatesPass = gate1BrandPass && gate1MainJsPass && gate2Pass;
  if (!allGatesPass) {
    console.error('❌ LIVE PRODUCTION CERTIFICATION FAILED!');
    process.exit(1);
  } else {
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-221R PREMIUM IDENTITY FINISHING PASS.');
  }
}

if (require.main === module) {
  runCertifyBrandExperience221R().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyBrandExperience221R };
