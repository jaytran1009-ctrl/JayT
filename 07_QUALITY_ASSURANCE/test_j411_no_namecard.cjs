...
(async () => {
  const base = process.env.JAYT_J411_TEST_URL || 'http://127.0.0.1:4181/';
  ...
  for (const viewport of [{ width:390, height:844 }, { width:1440, height:900 }]) {
    ...
    const m = await page.evaluate(() => {
      ...
      return {
        ...
        ,nameCardVisible:visible('.j406-pass-stage')
        ,radar:visible('.j411-radar')
      };
    });
    assert.strictEqual(m.version,'v3.462.0-j411');
    assert.strictEqual(m.marks,10);
    assert.strictEqual(m.oldCopy,false);
    assert(m.title.includes('Đà Nẵng'));
    assert(m.restored && m.radar && !m.nameCardVisible && !m.overflow && !m.affiliateLeak);
    ...
    console.log(`J411 ${viewport.width}px PASS | Name Card removed | city radar active | HUD ${hudMs.toFixed(2)}ms | 0 console errors`);
    await page.close();
  }
  await browser.close();
