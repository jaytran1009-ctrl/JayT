...
const registryJsSnippet = `  // =========================================================================
  // JAYT-218: CARD-LEVEL REAL VISUAL EVIDENCE REGISTRY (REAL CONTENT SUPPLY)
  // =========================================================================
  const CARD_VISUAL_EVIDENCE_REGISTRY_218 = ${JSON.stringify(registryObj, null, 2)};
  const CARD_VISUAL_EVIDENCE_REGISTRY_217 = CARD_VISUAL_EVIDENCE_REGISTRY_218;
  const CARD_VISUAL_EVIDENCE_REGISTRY_216 = CARD_VISUAL_EVIDENCE_REGISTRY_218;
  const CARD_VISUAL_EVIDENCE_REGISTRY_215 = CARD_VISUAL_EVIDENCE_REGISTRY_218;
  const CARD_VISUAL_EVIDENCE_REGISTRY_213 = CARD_VISUAL_EVIDENCE_REGISTRY_218;

  function findCardVisualEvidence(dealId, brandName) {
    if (dealId) {
      const found = CARD_VISUAL_EVIDENCE_REGISTRY_218.cards.find(c => c.deal_id === dealId || c.card_id === dealId);
      if (found) return found;
    }
    if (brandName) {
      const norm = brandName.toLowerCase();
      const found = CARD_VISUAL_EVIDENCE_REGISTRY_218.cards.find(c => c.brand && c.brand.toLowerCase().includes(norm));
      if (found) return found;
    }
  ...
  // JAYT-218: VISUAL TRUTH CARD HEADER RENDERER (REAL CONTENT SUPPLY)
  function renderCardVisualHeaderHTML(visualEv, brandName, categoryName, tierBadgeHTML) {
    ...
    if (isVerifiedExact) {
      return \`
        <!-- Verified Exact Real Asset Header (JAYT-218) -->
        <div class="jayt-card-visual-canvas" style="min-height:156px; height:165px; position:relative; overflow:hidden; border-bottom:1px solid rgba(0,0,0,0.15); display:flex; flex-direction:column; justify-content:space-between; padding:16px; background:\${visualEv.theme_gradient || 'linear-gradient(135deg, #0F172A, #1E293B)'};">
          <div style="position:absolute; top:0; left:0; width:100%; height:100%; display:flex; align-items:center; justify-content:center; padding:16px; background:rgba(0,0,0,0.25); z-index:1;">
            <img src="\${esc(visualEv.asset_file_or_embed_url)}" alt="\${esc(brandName)}" style="max-width:85%; max-height:85px; object-fit:contain; filter:drop-shadow(0 4px 10px rgba(0,0,0,0.35));" />
          </div>
          <div style="position:absolute; top:0; left:0; width:100%; height:100%; background:linear-gradient(180deg, rgba(15,23,42,0.65) 0%, rgba(15,23,42,0.05) 50%, rgba(15,23,42,0.8) 100%); z-index:1; pointer-events:none;"></div>
    ...
    return \`
      <!-- Large Visual Canvas Header (JayT Identity Visual - JAYT-218) -->
...
const oldRegRegex = /\/\/\s*==+\s*\n\s*\/\/\s*JAYT-217: CARD-LEVEL REAL VISUAL EVIDENCE REGISTRY[\s\S]*?const findBrandIdentityTile = findBrandVisualProfile;/;
...
const modalSection1New = `<!-- SECTION 1: HERO VISUAL LỚN (JAYT-218 REAL CONTENT SUPPLY) -->
...
code = code.replace(/Visual Truth OS 3\.357/g, 'Real Content Supply OS 3.358');
code = code.replace(/Chuẩn JAYT-217/g, 'Chuẩn JAYT-218');

// Update visual evidence dashboard snippet to the exact 3 KPIs with 7 verified assets
const oldDashboardRegex = /<!-- KHỐI ĐỐI SOÁT HÌNH ẢNH CẤP CARD[\s\S]*?<!-- KHỐI TỔNG QUAN 5 TẦNG NGUỒN CUNG/;
const newDashboardSnippet = `<!-- KHỐI ĐỐI SOÁT HÌNH ẢNH CẤP CARD (REAL CONTENT SUPPLY - JAYT-218) -->
        <section class="jayt-visual-evidence-dashboard-card" style="background:var(--bg-surface); border:1.5px solid #3B82F6; border-radius:var(--radius-lg); padding:12px 14px; margin-top:8px; margin-bottom:10px; box-shadow:var(--shadow-sm);">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; flex-wrap:wrap; gap:6px;">
            <div style="font-size:11.5px; font-weight:800; color:#1D4ED8; text-transform:uppercase; letter-spacing:0.5px;">
              📊 NGUỒN CUNG HÌNH ẢNH THỰC TẾ (REAL CONTENT SUPPLY)
            </div>
            <span class="apex-badge" style="background:#EFF6FF; color:#1E40AF; border:1px solid #BFDBFE; font-weight:800; font-size:10.5px; padding:2px 8px;">
              🎯 Đã kiểm định 7 asset thật (4 Ưu Đãi + 3 Địa Điểm)
            </span>
          </div>
          ...
          <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:8px;">
            <div style="padding:8px 10px; background:var(--bg-surface-subtle); border-radius:var(--radius-sm); border-left:3px solid #059669;">
              ...
              <div style="font-size:14px; font-weight:800; color:#059669; margin-top:2px;">7/29 (24.1%)</div>
              <div style="font-size:10px; color:var(--text-muted);">Spotify, MSFT, Figma, AWS, Metiz, CGV, DVC</div>
            </div>
            <div style="padding:8px 10px; background:var(--bg-surface-subtle); border-radius:var(--radius-sm); border-left:3px solid #3B82F6;">
              ...
              <div style="font-size:14px; font-weight:800; color:#1D4ED8; margin-top:2px;">22/29 (75.9%)</div>
...
console.log('✅ Successfully updated jayt_apex_interface.js for JAYT-218 Real Content Supply!');
