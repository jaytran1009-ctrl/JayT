...
code = code.replace(/OS 3\.361/g, 'OS 3.361R');
code = code.replace(/Premium Brand & Discovery OS 3\.361/g, 'Premium Identity Finishing OS 3.361R');
code = code.replace(/v=3\.361\.0/g, 'v=3.361.1');

// 2. Add Clean CSS for Brand System & 24px SVG icons
const style221R = `
  /* JAYT-221R: PREMIUM IDENTITY FINISHING SYSTEM */
  .jayt-cat-svg {
    width: 24px;
    height: 24px;
  ...
  .jayt-brand-wordmark-title {
    font-size: 13.5px;
    font-weight: 800;
    color: #FFFFFF;
    letter-spacing: -0.2px;
  }
  .jayt-brand-logo-thumb {
    max-height: 20px;
    max-width: 40px;
    object-fit: contain;
...
if (!code.includes('JAYT-221R: PREMIUM IDENTITY FINISHING SYSTEM')) {
  code = code.replace('function injectStyles() {', 'function injectStyles() {\n    const css221R = `' + style221R + '`;\n    const st221R = document.createElement("style"); st221R.innerHTML = css221R; document.head.appendChild(st221R);\n');
}

// 3. Modernize Card Visual Header Renderer (No Acronyms, Clean Wordmark or Official Logo)
const newVisualHeaderFunc = `
  // JAYT-221R: PREMIUM VISUAL CARD HEADER RENDERER (OFFICIAL LOGO ASSET VS JAYT TEXT IDENTIFIER)
  function renderCardVisualHeaderHTML(visualEv, brandName, categoryName, tierBadgeHTML) {
    const isDark = state.theme === 'dark';
    const brandInfo = (typeof window !== 'undefined' && window.JayTBrandAssets) ? window.JayTBrandAssets.getBrandIdentifier(brandName) : null;
    const isOfficialLogo = (brandInfo && brandInfo.kind === 'OFFICIAL_LOGO_ASSET' && brandInfo.logoUrl) || (visualEv && visualEv.visual_kind === 'OFFICIAL_IDENTITY_ASSET' && visualEv.asset_file_or_embed_url);
    const logoUrl = (brandInfo && brandInfo.logoUrl) ? brandInfo.logoUrl : (visualEv ? visualEv.asset_file_or_embed_url : null);
    const resolvedBrand = (brandInfo && brandInfo.brand) || brandName || 'JayT Partner';
    const identifierLabel = isOfficialLogo ? 'Official logo asset' : 'JayT text identifier';
    ...
    if (visualEv && visualEv.visual_kind === 'EXACT_PROMOTION' && visualEv.asset_file_or_embed_url) {
      return \`
        <!-- Exact Promotion Poster Canvas (16:9, JAYT-221R) -->
        <div class="jayt-card-visual-canvas" style="aspect-ratio:16/9; width:100%; position:relative; overflow:hidden; border-top-left-radius:20px; border-top-right-radius:20px; background:#0B0F19; display:flex; flex-direction:column; justify-content:space-between; padding:14px;">
          <img src="\${esc(visualEv.asset_file_or_embed_url)}" alt="\${esc(resolvedBrand)}" style="position:absolute; top:0; left:0; width:100%; height:100%; object-fit:cover; object-position:center top; z-index:1;" class="jayt-poster-img" />
          ...
          <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px; position:relative; z-index:2;">
            <!-- Brand Wordmark / Official Logo Anchor -->
            <div class="jayt-brand-wordmark-pill">
              \${(isOfficialLogo && logoUrl) ? \`<img src="\${esc(logoUrl)}" alt="\${esc(resolvedBrand)}" class="jayt-brand-logo-thumb" />\` : ''}
              <span class="jayt-brand-wordmark-title">\${esc(resolvedBrand)}</span>
            </div>
            \${tierBadgeHTML}
          </div>
          <div style="font-size:10.5px; color:#F8FAFC; background:rgba(11,15,25,0.82); backdrop-filter:blur(8px); padding:4px 10px; border-radius:6px; align-self:flex-start; border:1px solid rgba(255,255,255,0.25); position:relative; z-index:2; font-weight:700; display:flex; align-items:center; gap:6px;">
            <span style="width:6px; height:6px; border-radius:50%; background:#10B981; display:inline-block;"></span>
            \${esc(identifierLabel)} · \${esc(visualEv.render_label || 'Poster gốc đối soát')}
          </div>
        </div>
      \`;
    }
    ...
    if (visualEv && visualEv.visual_kind === 'OFFICIAL_IDENTITY_ASSET' && visualEv.asset_file_or_embed_url) {
      const grad = (brandInfo && brandInfo.bgGradient) || visualEv.theme_gradient || 'linear-gradient(135deg, #0F172A, #1E293B)';
      return \`
        <!-- Official Brand Logo Canvas (16:9, JAYT-221R) -->
        <div class="jayt-card-visual-canvas" style="aspect-ratio:16/9; width:100%; position:relative; overflow:hidden; border-top-left-radius:20px; border-top-right-radius:20px; background:\${grad}; display:flex; flex-direction:column; justify-content:space-between; padding:14px;">
          <div style="position:absolute; top:0; left:0; width:100%; height:100%; display:flex; align-items:center; justify-content:center; padding:16px; background:radial-gradient(circle, rgba(255,255,255,0.06) 0%, rgba(0,0,0,0.35) 100%); z-index:1;">
            <img src="\${esc(visualEv.asset_file_or_embed_url)}" alt="\${esc(resolvedBrand)}" style="max-width:65%; max-height:65px; object-fit:contain; filter:drop-shadow(0 6px 14px rgba(0,0,0,0.45));" />
          </div>
          ...
          <div style="font-size:10.5px; color:#F1F5F9; background:rgba(11,15,25,0.75); backdrop-filter:blur(6px); padding:3.5px 9px; border-radius:6px; align-self:flex-start; border:1px solid rgba(255,255,255,0.2); position:relative; z-index:2; font-weight:600;">
            🏛️ Official logo asset · \${esc(visualEv.render_label)}
          </div>
        </div>
      \`;
    }

    // 3. JAYT TEXT IDENTIFIER / BRAND CANVAS (Clean Editorial Typography Wordmark)
    const gradient = (brandInfo && brandInfo.bgGradient) || (visualEv && (visualEv.theme_gradient || visualEv.gradient)) || 'linear-gradient(135deg, #1E293B, #334155)';
    const accent = (brandInfo && brandInfo.accentColor) || (visualEv && visualEv.accent_color) || '#93C5FD';

    return \`
      <!-- Editorial Brand Wordmark Canvas (16:9, JAYT-221R) -->
      <div class="jayt-card-visual-canvas" style="aspect-ratio:16/9; width:100%; background:\${gradient}; padding:16px; border-top-left-radius:20px; border-top-right-radius:20px; display:flex; flex-direction:column; justify-content:space-between; position:relative; overflow:hidden;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px; position:relative; z-index:2;">
          <div class="jayt-brand-wordmark-pill">
            <span class="jayt-brand-wordmark-title">\${esc(resolvedBrand)}</span>
          </div>
        ...
        <div>
          <div style="font-size:11.5px; color:\${accent}; font-weight:700; margin-bottom:4px;">
            \${esc(categoryName || 'Nguồn chính thức đã ghi nhận')}
          </div>
          <div style="font-size:10.5px; color:#E2E8F0; background:rgba(11,15,25,0.65); backdrop-filter:blur(6px); padding:3.5px 8px; border-radius:6px; display:inline-block; border:1px solid rgba(255,255,255,0.18); font-weight:600;">
            🏷️ JayT text identifier — chưa có ảnh ưu đãi/địa điểm xác minh
          </div>
        </div>
      </div>
    \`;
...
if (headerRegex.test(code)) {
  ...
  console.log('✅ Updated renderCardVisualHeaderHTML for JAYT-221R');
}

// 4. Update Category Hubs Renderer to strictly use 24px SVG Icons (Zero Emojis)
const newCategoryHubsFunc = `
  function renderCategoryHubsCenter163() {
    ...
    // Group cards into Curated Rails
    ...
    return \`
      <div class="jayt-hubs-experience-root" style="max-width:1280px; margin:0 auto; padding:16px 20px 48px;">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
            ...
            <div style="display:flex; align-items:center; gap:8px;">
              <span class="apex-badge" style="background:\${isDark ? '#064E3B' : '#ECFDF5'}; color:\${isDark ? '#6EE7B7' : '#065F46'}; border:1px solid #10B981; font-weight:800; font-size:11.5px; padding:6px 12px; border-radius:10px;">
                Premium Identity Finishing OS 3.361R
              </span>
            </div>
          </div>
        </section>
        ...
        <!-- 3. CURATED DISCOVERY RAILS (5 NGỮ CẢNH TRỌNG TÂM - 24PX SVG ICONS, ZERO EMOJIS) -->
        ...
        <!-- RAIL 3: CÀ PHÊ & KHÔNG GIAN HỌC BÀI -->
        <section class="jayt-rail-section" style="margin-bottom:36px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px;">
            <div style="display:flex; align-items:center; gap:12px;">
        ...
        <section id="transparency-governance-drawer" style="margin-top:40px; padding:20px; background:\${isDark ? '#111827' : '#F8FAFC'}; border-radius:20px; border:1px solid \${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)'};">
          <details style="cursor:pointer;">
            <summary style="font-size:13.5px; font-weight:800; color:\${isDark ? '#94A3B8' : '#64748B'}; outline:none;">
              Bảng Đối Soát Bằng Chứng & Kỷ Luật Sự Thật (Transparency & Quality Governance)
            </summary>
...
if (hubsRegex.test(code)) {
  ...
  console.log('✅ Updated renderCategoryHubsCenter163 for JAYT-221R');
}

fs.writeFileSync(interfacePath, code, 'utf8');
console.log('✅ Applied JAYT-221R finishing pass to jayt_apex_interface.js!');
