...
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_185_containment');
...
async function certifyLiveState185() {
  console.log('========================================================================');
  console.log('🛡️ JAYT-185: SUPPLY TRUTH & LIVE STATE CERTIFICATION');
  ...
  try {
    const feedPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'generated_verified_deals_185.json');
    ...
    // Fetch Live Module
    const liveModuleRes = await fetchUrl('https://deploy-ten-xi-48.vercel.app/jayt_verified_deals_module.js?nocache=' + Date.now());
    ...
    // Fetch Live Main JS
    const liveJsRes = await fetchUrl('https://deploy-ten-xi-48.vercel.app/jayt_apex_interface.js?nocache=' + Date.now());
  ...
  try {
    ...
    // Check version badge 3.326
    assert(bodyText.includes('Daily Deal OS 3.326') || bodyText.includes('3.326'), 'DOM missing version 3.326');
    console.log('     Confirmed Daily Deal OS 3.326 rendered.');
    ...
    record('Live DOM Runtime Binding & Verification', 'PASS', 0, 'Version 3.326, exactly 3 deals live (Spotify, JetBrains, YouTube)');
    ...
    const dPath = path.join(evidenceDir, 'screenshot_185_desktop_light.png');
    await page.screenshot({ path: dPath, fullPage: false });
    screenshots.push({
      file: 'screenshot_185_desktop_light.png',
      viewport: '1440x900',
      theme: 'light',
    ...
    await page.evaluate(() => document.body.setAttribute('data-theme', 'light'));
    await new Promise(r => setTimeout(r, 500));
    const mLPath = path.join(evidenceDir, 'screenshot_185_mobile_light.png');
    await page.screenshot({ path: mLPath, fullPage: false });
    screenshots.push({
      file: 'screenshot_185_mobile_light.png',
      viewport: '390x844',
      theme: 'light',
    ...
    await new Promise(r => setTimeout(r, 500));
    const mDPath = path.join(evidenceDir, 'screenshot_185_mobile_dark.png');
    await page.screenshot({ path: mDPath, fullPage: false });
    screenshots.push({
      file: 'screenshot_185_mobile_dark.png',
      viewport: '390x844',
      theme: 'dark',
  ...
  const finalOutput = {
    test_suite: 'JAYT-185-LIVE-CERTIFICATION',
  ...
  const resPath = path.join(evidenceDir, 'CERTIFICATION_RESULT_185.json');
