...
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_eq.js');
...
const ledgerPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_PUBLIC_COUNT_LEDGER_EQ.json');
const walletPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_WALLET_LEDGER_EQ.json');
const goLiveBoardPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/GO_LIVE_EVIDENCE_BOARD_EQ.json');
const voucherHubPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_voucher_hub_state.json');
const attestationPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_no_write_attestation.json');
const contractPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_unified_count_contract.json');
const manifestEQPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_eq/EVIDENCE_MANIFEST_EQ.json');
const githubArtifactV2Path = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_eq/artifact_cand_github_education_docs_v2.md');
const githubReceiptPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_eq/artifact_cand_github_education_docs_v2_receipt.json');
const proposedFactsPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_desk_eq/PROPOSED_FACTS_REGISTRY_EQ.json');
const discoveryPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_desk_eq/DISCOVERY_REGISTRY_EQ.json');
const rosterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_identity_roster.json');
const assetRegisterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_asset_register.json');
const quarantinePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_quarantine_manifest.json');
const runtimeTablePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_runtime_asset_table.json');
const ownershipRecordPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_asset_ownership_record.json');
const factLedgerPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eq_fact_ledger.json');

console.log('========================================================================');
console.log('🛡️  SECTION EQ — STATIC CONTRACT & GO-LIVE GAP AUDIT VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const eqModule = require(storefrontPath);
const items = eqModule.JAYT_DISCOVERY_ITEMS;
...
// Test 4: Go-Live Evidence Board EQ (Mandate EQ.2)
assertTest('GO_LIVE_EVIDENCE_BOARD_EQ.json exists on disk', fs.existsSync(goLiveBoardPath));
const goLiveBoardData = JSON.parse(fs.readFileSync(goLiveBoardPath, 'utf8'));
assertTest('Go-Live Evidence Board records overall_go_live_status = NOT_GO_LIVE', goLiveBoardData.overall_go_live_status === 'NOT_GO_LIVE');
assertTest('Go-Live Evidence Board tracks exactly 10 items (P0, P1, P2)', goLiveBoardData.items.length === 10);
assertTest('P0 Production Exposure Containment item GL-P0-01 exists and mapped', goLiveBoardData.items.some(i => i.item_id === 'GL-P0-01' && i.state === 'CONTAINMENT_MAPPED_ON_STAGING'));

// Test 5: GitHub Docs Artifact v2 and Capture Receipt (Mandate EP/EQ)
...
// Test 6: Proposed Facts 12-Point Schema Verification
...
// Test 7: Cleaned Discovery Registry EQ
...
// Test 8: Evidence Manifest EQ
const manifestEQ = JSON.parse(fs.readFileSync(manifestEQPath, 'utf8'));
assertTest('Evidence Manifest EQ records total_discovery_dossiers_evaluated = 6', manifestEQ.summary.total_discovery_dossiers_evaluated === 6);
assertTest('Evidence Manifest EQ records captured_artifacts_with_receipt_count = 1', manifestEQ.summary.captured_artifacts_with_receipt_count === 1);
assertTest('Evidence Manifest EQ records field_facts_verified_in_runtime_storefront = 0', manifestEQ.summary.field_facts_verified_in_runtime_storefront === 0);

// Test 9: Fact Ledger EQ
const factLedger = JSON.parse(fs.readFileSync(factLedgerPath, 'utf8'));
assertTest('Fact Ledger EQ records evaluated_citation_facts_count = 2', factLedger.fact_ledger_policy.evaluated_citation_facts_count === 2);
assertTest('Fact Ledger EQ records strictly 0 verified field facts on runtime storefront', factLedger.verified_field_facts.length === 0);

// Test 10: Quarantine Manifest & Complete Isolation of 12 JPEGs
...
// Test 13: Asset Ownership Record EQ
...
// Test 14: Runtime Asset Audit Table EQ
...
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_eq\.js"><\/script>/g) || [];
assertTest('Single script reference in index.html (no duplicates)', scriptMatches.length === 1);
assertTest('Version string is v3.471.0-staging.eq', htmlContent.includes('v3.471.0-staging.eq'));
assertTest('HTML body contains data-ledger-version="v3.471.0-staging.eq"', htmlContent.includes('data-ledger-version="v3.471.0-staging.eq"'));
...
if (passedTests === totalTests) {
  console.log('🎉 SECTION EQ STATIC, GO-LIVE GAP AUDIT & CITATION PACK PASS (100%)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION EQ STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
