...
(async () => {
  const base = process.env.JAYT_J410_TEST_URL || 'http://127.0.0.1:4191/';
  ...
  for (const viewport of [{ width:390, height:844 }, { width:1440, height:900 }]) {
    ...
    assert.strictEqual(m.version,'v3.461.0-j410');
    assert.strictEqual(m.marks,10);
    assert.strictEqual(m.oldCopy,false);
    assert.strictEqual(m.title,'Đà Nẵng, vừa đúng nhịp.');
    ...
    console.log(`J410 ${viewport.width}px PASS | 10 local brand badges | HUD ${hudMs.toFixed(2)}ms | 0 console errors`);
    await page.close();
  }
  await browser.close();
