...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_218S_certification');
...
async function runCertifyPosterBinding218S() {
  console.log('========================================================================');
  console.log('🌐 JAYT-218S: 3-GATE PRODUCTION DEPLOYMENT & POSTER BINDING CERTIFICATION');
  ...
  for (let poll = 1; poll <= 5; poll++) {
    const ts = Date.now();
    const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.360.0&nocache=${ts}`);
    liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

    const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.360.0&nocache=${ts}`);
  ...
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & POSTER BINDING ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3360 = bodyText.includes('OS 3.360') || bodyText.includes('Exact Poster Binding OS 3.360');
    const hasExactPromoMedia = bodyText.includes('Exact Promotion Media') && bodyText.includes('3/29 (10.3%)');
    const hasExact4Layer = bodyText.includes('Exact 4-Layer Binding') && bodyText.includes('3/29 (10.3%)');
  ...
  console.log('   - OS 3.360 Rendered:                ' + (domReport.hasOs3360 ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - Exact Promotion Media (3/29):     ' + (domReport.hasExactPromoMedia ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - Exact 4-Layer Binding (3/29):     ' + (domReport.hasExact4Layer ? '🟢 PASS' : '❌ FAIL'));
  ...
  // Test Modal Interaction on Metiz Card 1 (55K T3-T5)
  console.log('\n   🖱️ Testing Live Modal Interaction on Exact Poster Card (Metiz 55K T3-T5)...');
  ...
  const gate2Pass = (
    ...
    domReport.hasOs3360 &&
    domReport.hasExactPromoMedia &&
    domReport.hasExact4Layer &&
  ...
  const modalViewPath = path.join(artifactsDir, 'screenshot_218S_modal_metiz_55k_t3_t5_detail.png');
  ...
  // 2. Verified Metiz 55K T3-T5 Card
  const verifiedMetizPath = path.join(artifactsDir, 'screenshot_218S_metiz_55k_t3_t5_card.png');
  await page.screenshot({ path: verifiedMetizPath, clip: { x: 0, y: 0, width: 1440, height: 750 } });
  console.log('   📸 2/5 Captured Metiz 55K Promo Card:   ' + verifiedMetizPath);

  // 3. Desktop Light View (1440x900)
  const desktopLightPath = path.join(artifactsDir, 'screenshot_218S_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  ...
  const mobileLightPath = path.join(artifactsDir, 'screenshot_218S_mobile_light.png');
  ...
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_218S_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_218S_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_218S_metiz_55k_t3_t5_card.png', path: verifiedMetizPath, sha256: sha256File(verifiedMetizPath), viewport: '1440x750' },
        { name: 'screenshot_218S_modal_metiz_55k_t3_t5_detail.png', path: modalViewPath, sha256: sha256File(modalViewPath), viewport: '1440x900' },
        { name: 'screenshot_218S_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath), viewport: '1440x900' },
        { name: 'screenshot_218S_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath), viewport: '390x844' },
        { name: 'screenshot_218S_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath), viewport: '390x844' }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_218S_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-218S EXACT POSTER BINDING.');
  }
}

if (require.main === module) {
  runCertifyPosterBinding218S().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyPosterBinding218S };
