/**
 * QA TEST SUITE: JAYT-117 TRUSTED DAILY HABIT
 * 
 * Verifies:
 * 1. Strict 3-Tier Classification & 100% Claim-Level Fidelity on Disk.
 * 2. Local-Only Wording Enforcement (no fake community sharing claims for local storage).
 * 3. Tier 3 Clean Pricing (no fake strikethrough original prices).
 * 4. Supply Gap Board 5x5 Matrix (25 cells audited).
 * 5. Official Asset Pipeline & Community Backend specs present.
 * 6. Usability Test Report 8 real Da Nang participants present with SUS > 80.
 * 7. Freshness Gate & System Standardized Timestamps.
 * 8. Max 3 Action Cards constraint in Hero Decision Hub.
 * 9. Progressive Disclosure (collapsed initial screen, expand on demand).
 * 10. Compact Venue Cards without repetitive disclaimers.
 * 11. Puppeteer Mobile 390px & Desktop 1440px (Theme toggle, 0 targets < 44px, 8 persona tests <30s).
 */
...
const screenshotsDir = path.join(repoRoot, '08_RELEASE_VAULT', 'screenshots_117');
...
async function runTestSuite117() {
  console.log('\n================================================================');
  console.log('      STARTING JAYT-117 TRUSTED DAILY HABIT QA SUITE            ');
  console.log('================================================================\n');

  // --- TEST 1: SSOT 3-TIER DATASET & 100% CLAIM-LEVEL FIDELITY ---
  console.log('--- TEST 1: SSOT 3-Tier Dataset & Claim-Level Fidelity ---');
  const feedPath = path.join(sotDir, 'daily_supply_feed_117.json');
  assert(fs.existsSync(feedPath), 'daily_supply_feed_117.json exists in SOT');
  ...
  assert(Array.isArray(feedData.public_menu_combos) && feedData.public_menu_combos.length === 5, 'Tier 3 public_menu_combos contains strictly 5 combos');
  ...
  // Check Tier 3 Public Menu Combos (No fake strikethrough)
  for (const menu of feedData.public_menu_combos) {
    ...
    assert(!menu.original_listed_price_vnd, `Menu Combo ${menu.id} contains NO fake strikethrough price`);
  ...
  // --- TEST 2: SUPPLY GAP BOARD 5x5 MATRIX AUDIT ---
  console.log('\n--- TEST 2: Supply Gap Board 5x5 Matrix Audit ---');
  const gapBoardPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'supply_gap_board_117.json');
  assert(fs.existsSync(gapBoardPath), 'supply_gap_board_117.json exists');
  ...
  assert(gapData.summary_metrics.gap_high_priority_cells > 0, 'Gap board identifies high priority supply gaps');

  // --- TEST 3: ASSET PIPELINE & BACKEND ARCHITECTURE SPECS ---
  console.log('\n--- TEST 3: Asset Pipeline & Backend Architecture Specs ---');
  const assetSpecPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'official_asset_pipeline_spec_117.md');
  assert(fs.existsSync(assetSpecPath), 'official_asset_pipeline_spec_117.md exists');
  const assetSpecText = fs.readFileSync(assetSpecPath, 'utf8');
  assert(assetSpecText.includes('Cấm tuyệt đối ảnh AI giả mạo cửa hàng'), 'Asset spec strictly bans AI fake store images');

  const backendSpecPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'community_backend_architecture_spec_117.md');
  assert(fs.existsSync(backendSpecPath), 'community_backend_architecture_spec_117.md exists');
  const backendSpecText = fs.readFileSync(backendSpecPath, 'utf8');
  assert(backendSpecText.includes('Zero-PII'), 'Backend spec enforces Zero-PII sanitization');
  assert(backendSpecText.includes('Abuse Reporting'), 'Backend spec enforces abuse reporting & auto-hide');
  ...
  assert(utReportText.includes('Nguyễn Văn An') && utReportText.includes('Hoàng Minh Đức'), 'Usability test report contains real Da Nang participants');
  assert(utReportText.includes('SUS'), 'Usability test report contains SUS quantitative scores');

  // --- TEST 4: LOCAL-ONLY WORDING & FRESHNESS GATE IN JS ---
  console.log('\n--- TEST 4: Local-Only Wording & Freshness Gate in JS ---');
  const interfaceJs = fs.readFileSync(path.join(sotDir, 'jayt_apex_interface.js'), 'utf8');
  assert(interfaceJs.includes('daily_supply_feed_117.json'), 'Interface loads daily_supply_feed_117.json');
  ...
  assert(!interfaceJs.includes('Tín Hiệu Ưu Đãi Do Cộng Đồng Gửi Về'), 'Interface has removed misleading community sharing heading');
  assert(interfaceJs.includes('btn-toggle-full-catalog'), 'Interface defines progressive disclosure button');

  // --- TEST 5: 26 LOCATIONS & COMMERCIAL LOCK INVARIANT ---
  console.log('\n--- TEST 5: 26 Watchlist Locations & Commercial Lock Invariant ---');
  ...
  // --- TEST 6: HEADLESS BROWSER & 8 PERSONA SIMULATION ---
  console.log('\n--- TEST 6: Headless Browser & 8 Persona Simulation ---');
  ...
  for (const vp of viewports) {
    ...
    assert(menuCards === 5, `Viewport ${vp.name} renders strictly 5 Public Menu cards in Tier 3 (found: ${menuCards})`);
    ...
    // 6. Verify Local-Only Language on Layer 3
    const layer3Title = await page.$eval('#community-radar-section .apex-section-title', el => el.innerText);
    assert(layer3Title.includes('Ghi Chú & Tín Hiệu Trên Thiết Bị Này'), `Viewport ${vp.name} uses truthful Local-Only heading in Layer 3`);

    const submitBtnLabel = await page.$eval('#btn-submit-community-signal', el => el.innerText);
    assert(submitBtnLabel.includes('Lưu ghi chú'), `Viewport ${vp.name} uses truthful Local-Only button label`);
  ...
  console.log('\n================================================================');
  console.log(`  ALL ${passedAssertions}/${totalAssertions} ASSERTIONS PASSED FOR JAYT-117!`);
  console.log('================================================================\n');
}

runTestSuite117().catch(err => {
  console.error('\n❌ Test Suite 117 Failed:', err);
  process.exit(1);
});
