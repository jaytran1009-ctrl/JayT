/**
 * SECTION DJ — STATIC CONTRACT & PROVENANCE VALIDATOR
 * Governing: JAYT-245 Section DJ (Lines 2695-2722)
 */
const fs = require('fs');
const path = require('path');

const PROJECT_ROOT = 'd:/Công Việc MMO/OPC JayT/JayT-Dự Án Giá Trị Cộng Đồng';
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_dj.js');
const indexPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/index.html');

console.log('========================================================================');
console.log('🛡️  SECTION DJ — STATIC CONTRACT & PROVENANCE VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const djModule = require(storefrontPath);
const items = djModule.JAYT_DISCOVERY_ITEMS;
const walletEntries = djModule.JAYT_WALLET_ENTRIES;
...
// Test 11: Zero unverified claims in Wallet copy
...
// Test 12: Asset Quarantine - Bảo tàng Đà Nẵng must NOT use Cham Museum photo
...
// Test 13: Asset Matching - Bảo tàng Chăm has Cham Museum photo
...
// Test 14: HTML only references jayt_storefront_staging_dj.js once
const htmlContent = fs.readFileSync(indexPath, 'utf8');
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_dj\.js"><\/script>/g) || [];
assertTest('Single script reference in index.html (no duplicates)', scriptMatches.length === 1);

// Test 15: Hero CTA Hierarchy in HTML
...
// Test 16: Version string in HTML
assertTest('Version string is v3.438.0-staging.dj', htmlContent.includes('v3.438.0-staging.dj'));

// Test 17: Drawer element exists in HTML
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_dj_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION DJ STATIC CONTRACT INTEGRITY PASS (17/17)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION DJ STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
