...
console.log('========================================================================');
console.log('🔍 JAYT-141S: SEMANTIC DELTA COMPILER & RED-TEAM AUDIT');
console.log('========================================================================\n');

const repoRoot = path.resolve(__dirname, '..');
const compilerPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'generic_compiler_141s.js');
...
test('Sum of 4-tier semantic delta categories equals exact total sources (15 == 15)', () => {
  const sd = manifest.semantic_delta_summary;
  const sum = (
    sd.unchanged_identical_count +
    sd.unchanged_render_variation_count +
    sd.semantic_changed_review_count +
    sd.offer_relevant_delta_count +
    sd.http_error_backoff_count
  );
  console.log(`     Total Sources:                    ${sd.total_sources_in_registry}`);
  console.log(`     UNCHANGED_IDENTICAL:              ${sd.unchanged_identical_count}`);
  console.log(`     UNCHANGED_RENDER_VARIATION:       ${sd.unchanged_render_variation_count}`);
  console.log(`     SEMANTIC_CHANGED_REVIEW_REQUIRED: ${sd.semantic_changed_review_count}`);
  console.log(`     OFFER_RELEVANT_DELTA:             ${sd.offer_relevant_delta_count}`);
  console.log(`     HTTP_ERROR_BACKOFF (Metiz 404):   ${sd.http_error_backoff_count}`);
  console.log(`     Conservation Sum:                 ${sum}`);

  assert.strictEqual(sd.total_sources_in_registry, 15, 'Expected 15 total sources');
  assert.strictEqual(sum, 15, 'Metric conservation broken');
  assert.strictEqual(sd.metric_conservation_check, 15, 'Manifest internal check failed');
});

console.log('\n--- GATE 2: 2-LAYER HASHING LINEAGE (RAW VS SEMANTIC) ---');
test('100% of monitored active sources have raw_html_sha256 and semantic_content_sha256', () => {
  for (const src of registry.sources) {
    if (src.state !== 'HTTP_ERROR_BACKOFF') {
      assert(src.hashes.raw_html_baseline_sha256, `Missing raw baseline hash for ${src.source_id}`);
      assert(src.hashes.raw_html_current_sha256, `Missing raw current hash for ${src.source_id}`);
      assert(src.hashes.semantic_content_baseline_sha256, `Missing semantic baseline hash for ${src.source_id}`);
      assert(src.hashes.semantic_content_current_sha256, `Missing semantic current hash for ${src.source_id}`);
    }
  }
  console.log('     Verified 2-layer hashing across all active sources.');
});

console.log('\n--- GATE 3: ACCURATE DEMOTION OF RAW RENDER VARIANCES (0 FALSE-POSITIVE DEALS) ---');
test('Exact counts: 1 Identical, 10 Render Variations, 3 Semantic Changes, 0 Offer-Relevant Deltas', () => {
  const sd = manifest.semantic_delta_summary;
  assert.strictEqual(sd.unchanged_identical_count, 1, 'Expected 1 identical (Starlight)');
  assert.strictEqual(sd.unchanged_render_variation_count, 10, 'Expected 10 render variations');
  assert.strictEqual(sd.semantic_changed_review_count, 3, 'Expected 3 general copy changes');
  assert.strictEqual(sd.offer_relevant_delta_count, 0, 'Expected 0 offer-relevant deltas (0 fake deals)');
  assert.strictEqual(sd.http_error_backoff_count, 1, 'Expected 1 Metiz 404 error');
});
...
test('Manifest 141S contains ZERO "VERIFIED" statuses or CTA buy/book buttons', () => {
...
test('generic_compiler_141s.js contains ZERO hardcoded brands, venues, malls, landmarks, or voucher codes', () => {
...
console.log('\n--- GATE 8: PRODUCTION LOCK & ZERO LIVE DEPLOYMENT IN 141S ---');
...
if (failCount > 0) {
  ...
  console.log('✨ ALL 8 JAYT-141S SEMANTIC FRESHNESS GATES PASSED 100% CLEAN!');
  process.exit(0);
}
