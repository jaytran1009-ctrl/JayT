...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_206_certification');
...
async function runCertifySourceFirst206() {
  console.log('========================================================================');
  console.log('🌐 JAYT-206: 3-GATE PRODUCTION DEPLOYMENT & SOURCE-FIRST CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.346.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.346.0&t=${ts}`);
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasHeadline = bodyText.includes('Hôm nay: 0 🟢 đã xác nhận · 10 🔵 ưu đãi chính thức · 15 🟣 điểm hẹn đã xác minh');
    const hasOs3346 = bodyText.includes('OS 3.346') || bodyText.includes('Source-First OS 3.346');
  ...
  console.log('   - 🔵 Ưu đãi chính thức:            ' + domReport.blueCount + ' (Expected: 10)');
  console.log('   - 🟣 Điểm hẹn giá tốt:             ' + domReport.purpleCount + ' (Expected: 15)');
  console.log('   - 🎯 TỔNG SỐ CARD TRONG NGÀY:      ' + domReport.totalDailyCards + ' (Expected: 25)');
  ...
  console.log('   - OS 3.346 Rendered:                ' + (domReport.hasOs3346 ? '🟢 PASS' : '❌ FAIL'));

  const gate2Pass = (domReport.blueCount === 10 && domReport.purpleCount === 15 && domReport.totalDailyCards === 25 && domReport.hasHeadline && domReport.hasOs3346);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Desktop Light
  const desktopLightPath = path.join(artifactsDir, 'screenshot_206_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View: ' + desktopLightPath);

  // 2. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_206_mobile_light.png');
  ...
  // 3. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_206_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_206_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_206_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_206_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_206_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_206_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-206 SOURCE-FIRST ENGINE.');
  }
}

if (require.main === module) {
  runCertifySourceFirst206().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifySourceFirst206 };
