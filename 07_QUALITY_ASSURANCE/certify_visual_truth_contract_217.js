...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_217_certification');
...
async function runCertifyVisualTruthContract217() {
  console.log('========================================================================');
  console.log('🌐 JAYT-217: 3-GATE PRODUCTION DEPLOYMENT & VISUAL TRUTH CERTIFICATION');
  ...
  for (let poll = 1; poll <= 5; poll++) {
    const ts = Date.now();
    const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.357.0&nocache=${ts}`);
    liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

    const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.357.0&nocache=${ts}`);
  ...
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & VISUAL TRUTH GATE ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3357 = bodyText.includes('OS 3.357') || bodyText.includes('Visual Truth OS 3.357');
    const hasVisualDashboard = bodyText.includes('HỢP ĐỒNG SỰ THẬT THỊ GIÁC') && bodyText.includes('Stop-Ship cho đến khi có chứng cứ gốc');

    // Forbidden synthetic claims check
    const hasForbiddenClaim1 = bodyText.includes('OFFICIAL PROVENANCE ASSET');
    const hasForbiddenClaim2 = bodyText.includes('Địa điểm thực địa đối soát') || bodyText.includes('ĐỊA ĐIỂM THỰC ĐỊA ĐỐI SOÁT');
  ...
  console.log('   - OS 3.357 Rendered:                ' + (domReport.hasOs3357 ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - Visual Truth Dashboard DOM:       ' + (domReport.hasVisualDashboard ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - Forbidden Synthetic Claims Zero:  ' + (!domReport.hasForbiddenClaim1 && !domReport.hasForbiddenClaim2 ? '🟢 PASS' : '❌ FAIL'));
  ...
  const modalReport = await page.evaluate(() => {
    ...
    return {
      ...
      hasHonestProvenance: text.includes('nhận diện thương hiệu jayt')
    };
  });

  console.log('   - Modal Opened:                     ' + (modalReport.modalOpened ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - JayT Identity Hero (Section 1):   ' + (modalReport.hasSection1Hero ? '🟢 PASS' : '❌ FAIL'));
  ...
  console.log('   - Honest JayT Identity Label:       ' + (modalReport.hasHonestProvenance ? '🟢 PASS' : '❌ FAIL'));
  ...
  const gate2Pass = (
    ...
    !domReport.hasForbiddenClaim1 &&
    !domReport.hasForbiddenClaim2 &&
    ...
    modalReport.hasHonestProvenance
  );
  ...
  const modalViewPath = path.join(artifactsDir, 'screenshot_217_modal_identity_detail.png');
  await page.screenshot({ path: modalViewPath, fullPage: false });
  console.log('   📸 1/5 Captured Modal Identity Detail:   ' + modalViewPath);
  ...
  // 2. First-View Card
  const firstViewCardPath = path.join(artifactsDir, 'screenshot_217_first_view_card.png');
  await page.screenshot({ path: firstViewCardPath, clip: { x: 0, y: 0, width: 1440, height: 750 } });
  console.log('   📸 2/5 Captured First Viewport Card:     ' + firstViewCardPath);

  // 3. Desktop Light View (1440x900)
  const desktopLightPath = path.join(artifactsDir, 'screenshot_217_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  ...
  const mobileLightPath = path.join(artifactsDir, 'screenshot_217_mobile_light.png');
  ...
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_217_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_217_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_217_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath), viewport: '1440x900' },
        { name: 'screenshot_217_first_view_card.png', path: firstViewCardPath, sha256: sha256File(firstViewCardPath), viewport: '1440x750' },
        { name: 'screenshot_217_modal_identity_detail.png', path: modalViewPath, sha256: sha256File(modalViewPath), viewport: '1440x900' },
        { name: 'screenshot_217_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath), viewport: '390x844' },
        { name: 'screenshot_217_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath), viewport: '390x844' }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_217_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-217 VISUAL TRUTH CONTRACT.');
  }
}

if (require.main === module) {
  runCertifyVisualTruthContract217().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyVisualTruthContract217 };
