...
async function main() {
  console.log('🧪 [JAYT-097-TEST] Khởi chạy bộ kiểm thử Field Validation & Supply Cohort 097 (REAL PUPPETEER BROWSER + HTTP STAGING)...\n');
  ...
  // TEST 02: Correction Receipt 097 Exists With Accurate Gaps
  runSyncTest('TEST_02_CORRECTION_RECEIPT_097_EXISTS_WITH_ACCURATE_GAPS', () => {
    const receiptPath = path.join(repoRoot, '08_RELEASE_VAULT', 'CORRECTION_RECEIPT_097_TAXONOMY_AND_BROWSER_E2E_RIGOR.json');
    assert.ok(fs.existsSync(receiptPath), 'CORRECTION_RECEIPT_097_TAXONOMY_AND_BROWSER_E2E_RIGOR.json missing');
    const receipt = JSON.parse(fs.readFileSync(receiptPath, 'utf8'));

    assert.strictEqual(receipt.receipt_id, 'CORRECTION_RECEIPT_097_TAXONOMY_AND_BROWSER_E2E_RIGOR');
    assert.strictEqual(receipt.work_order, 'JAYT-097-FIELD-VALIDATION-AND-SUPPLY-COHORT');
    assert.ok(Array.isArray(receipt.root_causes) && receipt.root_causes.length === 2);
    assert.strictEqual(receipt.governance_invariants.release_candidate_emitted, false);
    assert.strictEqual(receipt.governance_invariants.production_lock_enforced, true);
  });

  // TEST 03: Negative Taxonomy Enforcement for Rejected & Pending WOs (094, 094A, 094B, 095, 096, 097)
  runSyncTest('TEST_03_NEGATIVE_TAXONOMY_ENFORCEMENT_FOR_REJECTED_AND_PENDING_WOS', () => {
    const { validateGlobalStatusTaxonomy067 } = require('./memory_transaction_manager_057.js');
    const nonApprovedWos = ['094', '094A', '094B', '095', '096', '097'];
  ...
  // Start HTTP Staging Server for Real Browser Testing
  ...
  // TEST 04: Puppeteer Real Browser Time Dock 5 Slots & Live DOM Mutations
  await runAsyncTest('TEST_04_PUPPETEER_REAL_BROWSER_TIME_DOCK_5_SLOTS_AND_DOM_MUTATIONS', async () => {
  ...
  await runAsyncTest('TEST_05_PUPPETEER_REAL_BROWSER_SPLIT_BILL_INTERACTIVE_AND_ESCAPE', async () => {
    ...
    // Assert live calculated per-person amount: (300,000 - 60,000) / 4 = 60,000 VND
    const perPersonText = await page.$eval('#sheet-val-per-person', el => el.textContent);
    assert.ok(perPersonText.includes('60.000') || perPersonText.includes('60,000'), `Calculated value mismatch: ${perPersonText}`);
  ...
  await runAsyncTest('TEST_06_PUPPETEER_REAL_BROWSER_PII_SUBMISSION_AND_LOCALSTORAGE', async () => {
    ...
    const piiRawText = 'Quán A giảm 20% liên hệ số 0905123456 hoặc email deal.hot@gmail.com, CCCD 048192001234';
    ...
    assert.ok(!firstSig.includes('0905123456'), 'Raw phone number MUST NOT be in localStorage');
    assert.ok(!firstSig.includes('deal.hot@gmail.com'), 'Raw email MUST NOT be in localStorage');
    assert.ok(!firstSig.includes('048192001234'), 'Raw CCCD MUST NOT be in localStorage');
  ...
  // TEST 08: Three-Tier Fixture Provenance (four_layer_dataset.json)
  runSyncTest('TEST_08_THREE_TIER_FIXTURE_PROVENANCE', () => {
    const dsPath = path.join(sotDir, 'four_layer_dataset.json');
    assert.ok(fs.existsSync(dsPath), 'four_layer_dataset.json missing');
    const ds = JSON.parse(fs.readFileSync(dsPath, 'utf8'));

    assert.strictEqual(ds.dataset_id, 'JAYT_FOUR_LAYER_DATASET_097');
    assert.strictEqual(ds.version, '2.0.0');

    // Emerald candidates
    assert.ok(Array.isArray(ds.layer_1_pending_candidates) && ds.layer_1_pending_candidates.length >= 2);
    ...
    assert.ok(Array.isArray(ds.layer_2_watchlist.verified_locations) && ds.layer_2_watchlist.verified_locations.length >= 9);
  ...
  // TEST 09: Strict Language Invariants (No Prohibited Words in Blue/Amber Tiers)
  runSyncTest('TEST_09_STRICT_LANGUAGE_INVARIANTS_NO_PROHIBITED_WORDS', () => {
  ...
  // TEST 10: Strict Touch Targets >= 44px
  runSyncTest('TEST_10_STRICT_TOUCH_TARGETS_GE_44PX', () => {
  ...
  // TEST 11: Pre-generated HTTP Staging Evidence Metadata Parity
  runSyncTest('TEST_11_PRE_GENERATED_HTTP_STAGING_EVIDENCE_METADATA_PARITY', () => {
    const metaPath = path.join(__dirname, 'runtime_evidence', 'screenshots_097', 'evidence_metadata_097.json');
    assert.ok(fs.existsSync(metaPath), 'evidence_metadata_097.json missing');
    const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));

    assert.strictEqual(meta.work_order, 'JAYT-097-FIELD-VALIDATION-AND-SUPPLY-COHORT');
    assert.strictEqual(meta.release_candidate_emitted, false);

    const expectedScreens = [
      'desktop_1440px_field_validation_097.png',
      'tablet_768px_field_validation_097.png',
      'mobile_390px_field_validation_097.png'
    ];

    for (const scr of expectedScreens) {
      assert.ok(meta.viewports[scr], `Metadata missing entry for ${scr}`);
      const scrPath = path.join(__dirname, 'runtime_evidence', 'screenshots_097', scr);
  ...
  // TEST 12: Three-Layer Byte Parity (03_SOURCE_OF_TRUTH === deploy/public === staging_instance)
  runSyncTest('TEST_12_THREE_LAYER_BYTE_PARITY', () => {
  ...
  // TEST 13: Release Candidate Freeze Protocol
  runSyncTest('TEST_13_RELEASE_CANDIDATE_FREEZE_PROTOCOL_ENFORCED', () => {
    const rc097Path = path.join(repoRoot, '08_RELEASE_VAULT', 'RELEASE_CANDIDATE_097.json');
    assert.strictEqual(fs.existsSync(rc097Path), false, 'RELEASE_CANDIDATE_097.json must NOT exist in 097');
  });

  // TEST 14: Production Lock Invariants
  runSyncTest('TEST_14_PRODUCTION_LOCK_INVARIANTS', () => {
  ...
  if (passCount === totalCount) {
    console.log(`🟢 [FIELD-VALIDATION-097-SUMMARY] Toàn bộ ${passCount}/${totalCount} KIỂM THỬ ĐÃ ĐẠT [PASS]!\n`);
  } else {
    console.error(`❌ [FIELD-VALIDATION-097-SUMMARY] Thất bại: ${passCount}/${totalCount} PASS.\n`);
    process.exit(1);
  }
}
