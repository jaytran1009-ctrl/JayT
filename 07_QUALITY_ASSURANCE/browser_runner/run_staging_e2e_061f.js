/**
 * JAYT STAGING REAL BROWSER E2E SMOKE RUNNER (061F)
 * Directive: JAYT-STAGING-LINEAGE-REPAIR-061F
 */
...
const stagingEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'staging_061f');
const receiptPath = path.join(stagingEvidenceDir, 'STAGING_E2E_RECEIPT_061F.json');
...
async function runStagingBrowserE2E061F(options = {}) {
  const startedAt = new Date().toISOString();
  console.log('\n=============================================================');
  console.log('🚀 [STAGING-E2E-061F] KHỞI CHẠY STAGING BROWSER E2E SMOKE (061F)');
  console.log('   Directive:   JAYT-STAGING-LINEAGE-REPAIR-061F');
  ...
  console.log('   Watermark:   STAGING · Nguồn chính thức đã capture · Chờ kiểm toán độc lập CEO (061F)');
  console.log('   Governance:  Lineage Repair · Fail-Closed Drift Quarantine · 7-Day TTL');
  console.log('=============================================================\n');

  fs.mkdirSync(stagingEvidenceDir, { recursive: true });

  const port = 8110 + Math.floor(Math.random() * 50);
  ...
  const cdpPort = 9340 + Math.floor(Math.random() * 100);
  const userDataDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', `.chrome_profile_staging_061f_${Date.now()}`);
  ...
  try {
    ...
    console.log(`   - Watermark: ${galaxyDeal.watermark_badge}`);
    ...
    // Verify Truthful Watermark
    if (!galaxyDeal.watermark_badge.includes('Chờ kiểm toán độc lập CEO (061F)')) {
      throw new Error(`STAGING_E2E_FAILED: Watermark badge must state pending CEO audit: '${galaxyDeal.watermark_badge}'`);
    }
    ...
    const evalRes = await sendSession('Runtime.evaluate', {
      expression: `JSON.stringify({
        ...
        watermark: document.querySelector('.watermark-badge') ? document.querySelector('.watermark-badge').innerText : null,
    ...
    console.log(`   - Rendered Watermark:  ${pageData.watermark}`);
    ...
    const hasWatermark061F = pageData.watermark && pageData.watermark.includes('Chờ kiểm toán độc lập CEO (061F)');

    if (!has50kText || !has70kText || !hasTuesday || !hasAllCust || !hasDanang || !hasAeon || !hasWatermark061F) {
      throw new Error('STAGING_E2E_FAILED: Real browser DOM did not render all required 061F Galaxy facts.');
    }
    ...
    const screenshotRel = '07_QUALITY_ASSURANCE/runtime_evidence/staging_061f/galaxy_staging_smoke_061f.png';
    ...
    const e2eReceipt = {
      ...
      run_id: 'run_staging_e2e_061f',
      executed_at: completedAt,
      status: 'STAGING_E2E_VERIFIED_SUCCESS_PENDING_CEO_AUDIT',
      server_config: {
        host: '127.0.0.1',
        port: port,
        base_url: baseUrl,
      ...
      verified_staging_deal: {
        ...
        watermark_badge: galaxyDeal.watermark_badge,
      ...
      e2e_verifications: {
        ...
        truthful_watermark_verified: true,
      ...
      artifacts: {
        ...
        incident_receipt_path: '07_QUALITY_ASSURANCE/runtime_evidence/INCIDENT_MUTATION_DISCLOSURE_RECEIPT_061F.json',
        staging_manifest_path: '07_QUALITY_ASSURANCE/runtime_evidence/STAGING_ACCEPTANCE_MANIFEST_061F.json',
        correction_receipt_path: '07_QUALITY_ASSURANCE/runtime_evidence/correction_receipt_061f_galaxy_happy_day.json'
      }
    };
...
module.exports = {
  runStagingBrowserE2E061F
};

if (require.main === module) {
  runStagingBrowserE2E061F()
    .then(() => {
      console.log('🟢 STAGING E2E 061F COMPLETED SUCCESSFULLY');
      process.exit(0);
    })
    .catch(err => {
      console.error('❌ STAGING E2E 061F FAILED:', err && err.stack ? err.stack : err);
      process.exit(1);
    });
}
