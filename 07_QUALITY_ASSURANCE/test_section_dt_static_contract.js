/**
 * SECTION DT — STATIC CONTRACT, CHRONOLOGY & FIELD-LEVEL EVIDENCE VALIDATOR
 * Governing: JAYT-245 Section DT (Lines 2935-2962)
 */
const fs = require('fs');
...
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_dt.js');
const indexPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/index.html');
const snapshotPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_dt_journey_count_snapshot.json');
const matrixPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_dt_field_evidence_matrix.json');
const manifestPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_dt/EVIDENCE_MANIFEST_DT.json');
const candidatePath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_SUPPLY_CANDIDATE_COHORT_DT.json');
const scorecardPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_dt_supply_scorecard.json');

console.log('========================================================================');
console.log('🛡️  SECTION DT — STATIC CONTRACT, CHRONOLOGY & FIELD EVIDENCE VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const dtModule = require(storefrontPath);
const items = dtModule.JAYT_DISCOVERY_ITEMS;
const walletEntries = dtModule.JAYT_WALLET_ENTRIES;
...
// Test 11: Field-Level Evidence Matrix Existence and Structure
const matrixData = JSON.parse(fs.readFileSync(matrixPath, 'utf8'));
assertTest('Field Evidence Matrix DT exists with 5 candidate entities', matrixData.matrix && matrixData.matrix.length === 5);

// Test 12: Field-Level Validation Rules
let fieldValidationPass = true;
matrixData.matrix.forEach(entity => {
  entity.fields.forEach(f => {
    if (f.status === 'VERIFIED_FROM_SOURCE') {
      if (!f.locator || !f.captured_at_utc) fieldValidationPass = false;
    } else if (f.status === 'UNKNOWN_AWAITING_CAPTURE') {
      if (f.locator !== null) fieldValidationPass = false;
    }
  });
});
assertTest('Field-Level Evidence Matrix adheres to verification rules (no fake locators for unknown fields)', fieldValidationPass);

// Test 13: Evidence Vault Manifest DT Verification
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
assertTest('Evidence Manifest DT exists with 5 genuine artifact records', manifest.artifacts && manifest.artifacts.length === 5);
...
assertTest('100% of raw artifact files exist on disk in DT vault with matching SHA-256 & byte size', allFilesMatch);
...
manifest.artifacts.forEach(a => {
  const fetchTime = Date.parse(a.captured_at_utc);
  if (isNaN(fetchTime) || fetchTime > genTime) chronoPass = false;
});
assertTest('Chronology Contract holds for all evidence artifacts (fetched_at <= generated_at <= signed_at)', chronoPass);

// Test 16: Scorecard DT Stage Breakdown
...
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_dt\.js"><\/script>/g) || [];
...
assertTest('Version string is v3.448.0-staging.dt', htmlContent.includes('v3.448.0-staging.dt'));
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_dt_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION DT STATIC & FIELD-LEVEL EVIDENCE INTEGRITY PASS (33/33)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION DT STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
