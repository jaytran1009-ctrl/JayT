/**
 * SECTION EG — STATIC CONTRACT, QUARANTINE MANIFEST & ZERO FALSE PROVENANCE VALIDATOR
 * Governing: JAYT-245 Section EG (Lines 3312-3339)
 */
const fs = require('fs');
...
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_eg.js');
...
const ledgerPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_PUBLIC_COUNT_LEDGER_EG.json');
const walletPath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_WALLET_LEDGER_EG.json');
const voucherHubPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_voucher_hub_state.json');
const attestationPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_no_write_attestation.json');
const contractPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_unified_count_contract.json');
const snapshotPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_journey_count_snapshot.json');
const manifestEGPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/evidence_vault_eg/EVIDENCE_MANIFEST_EG.json');
const affiliatePath = path.join(PROJECT_ROOT, '00_PROGRAM_BASELINE/JAYT_AFFILIATE_RESEARCH_PORTFOLIO_OFFLINE.json');
const scorecardPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_supply_scorecard.json');
const scannerReportPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_content_scanner_report.json');
const rosterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_identity_roster.json');
const assetRegisterPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_asset_register.json');
const quarantinePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_quarantine_manifest.json');
const runtimeTablePath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_runtime_asset_table.json');
const expMatrixPath = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/staging_eg_experience_matrix.json');

console.log('========================================================================');
console.log('🛡️  SECTION EG — STATIC CONTRACT, QUARANTINE & ZERO FALSE PROVENANCE VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const egModule = require(storefrontPath);
const items = egModule.JAYT_DISCOVERY_ITEMS;
...
// Test 4: Quarantine Manifest & Complete Isolation of 12 JPEGs (Mandate EG.1)
...
// Test 5: Complete Purge of False Provenance / Photographer CC Tokens (Mandate EG.1 & EG.3)
...
// Test 6: Single Instance of Voucher (0) in Navigation
...
// Test 7: Truth-Bound Asset Register EG (Mandate EG.2)
...
// Test 8: Runtime Asset Audit Table EG (Mandate EG.4)
...
assertTest('All runtime assets have permission_status = VERIFIED_INTERNAL_ORIGINAL_VECTOR', runtimeTable.runtime_elements.every(e => e.permission_status === 'VERIFIED_INTERNAL_ORIGINAL_VECTOR'));
assertTest('All runtime assets have truthful alt and credit strings', runtimeTable.runtime_elements.every(e => e.effective_alt.includes('Đồ họa minh họa') && e.effective_credit.includes('Đồ họa nguyên bản JayT')));

// Test 9: Vault EG 4 Cohorts & Verbatim Timestamp Lineage
const manifestEG = JSON.parse(fs.readFileSync(manifestEGPath, 'utf8'));
assertTest('Evidence Manifest EG enforces cohort summary and verbatim origin timestamps', manifestEG.provenance_rules && manifestEG.provenance_rules.cohort_based_read_only_acquisition === true);
assertTest('Cohort summary contains 4 distinct cohorts totaling 12 processed candidates', manifestEG.cohort_summary && manifestEG.cohort_summary.TOTAL_PROCESSED_CANDIDATES === 12);

const danabusEntry = manifestEG.artifacts.find(a => a.candidate_id === 'CAND_BUS_R16A');
...
manifestEG.artifacts.forEach(entry => {
...
assertTest('100% of raw artifact files exist on disk in EG vault with matching SHA-256 & byte size', allArtifactsFilesExist);

// Test 10: Pragmatic Experience Matrix (Mandate EG.3)
...
// Test 11: 50-Item Identity Roster Verification
...
// Test 12: Honest 2-Tier Content Scanner Report
...
// Test 13: Voucher Hub State Verification
...
// Test 15: Offline Affiliate Portfolio AccessTrade Status
...
// Test 17: Snapshot JSON matches runtime computation
...
// Test 18: Scorecard EG vs Evidence Vault EG EXACT Match (8 Canonical Sources)
const scorecardData = JSON.parse(fs.readFileSync(scorecardPath, 'utf8'));
const canonicalInVault = manifestEG.artifacts.filter(a => a.pipeline_stage === 'CANONICAL_SOURCE_FOUND').length;
assertTest('Evidence Vault EG contains exactly 8 CANONICAL_SOURCE_FOUND artifacts', canonicalInVault === 8);
assertTest('Scorecard EG CANONICAL_SOURCE_FOUND equals 8 (100% Reconciled)', scorecardData.scorecard.CANONICAL_SOURCE_FOUND === 8);

// Test 19: HTML Content & Fingerprint Verifications
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_eg\.js"><\/script>/g) || [];
assertTest('Single script reference in index.html (no duplicates)', scriptMatches.length === 1);
assertTest('Version string is v3.461.0-staging.eg', htmlContent.includes('v3.461.0-staging.eg'));
assertTest('HTML body contains data-ledger-version="v3.461.0-staging.eg"', htmlContent.includes('data-ledger-version="v3.461.0-staging.eg"'));
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_eg_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION EG STATIC, QUARANTINE & TRUTH-BOUND PROVENANCE INTEGRITY PASS (58/58)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION EG STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
