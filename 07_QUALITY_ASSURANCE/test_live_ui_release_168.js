/**
 * JAYT LIVE UI RELEASE RED-TEAM TEST SUITE (168)
...
console.log('========================================================================');
console.log('🧪 JAYT-168: LIVE UI RELEASE & DEPLOYMENT AUDIT (https://deploy-ten-xi-48.vercel.app)');
console.log('========================================================================\n');
...
const screenshotsDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'screenshots_168');
const liveDesktopLightPath = path.join(screenshotsDir, 'screenshot_168_live_desktop_light.png');
const liveMobileLightPath = path.join(screenshotsDir, 'screenshot_168_live_mobile_light.png');
const liveMobileDarkPath = path.join(screenshotsDir, 'screenshot_168_live_mobile_dark.png');
...
async function runLiveUiRelease168Audit() {
  ...
  test('Active pipeline contains zero static dictionaries, unproven cinema prices, or synthetic brand maps', () => {
    ...
    for (const dir of targetDirs) {
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.js') && !f.includes('quarantine') && !f.includes('test_live_ui_release_168.js'));
  ...
  test('https://deploy-ten-xi-48.vercel.app/ is live with 168 UI update', async () => {
  ...
  console.log('\n--- GATE 4: 3 LIVE SCREENSHOTS INTEGRITY AUDIT ---');
  ...
  console.log('\n--- GATE 5: WORKSTREAM C 3 DISPLAY TIERS & ZERO FILLER CARDS ---');
  test('Live interface correctly supports 4 tiers: 🟢 Verified Deal, 🔵 Verified Venue, 🟣 Tracked Source, ⚪ Empty State', () => {
    ...
    assert(jsContent.includes('jayt-today-overview'));
    console.log('     Verified 4 Display Tiers and "Hôm Nay Trên JayT" in web interface.');
  });
  ...
  try {
    ...
    assert.strictEqual(clusterChipsCount, 6, 'Must render exactly 6 community cluster chips on live site');

    await browser.close();
    browserTestPassed = true;
    console.log(`     Live Puppeteer audit: 5 Hubs and 6 Clusters verified.`);
  } catch (err) {
  ...
  if (failCount > 0) {
    ...
    console.log('✨ ALL 10 JAYT-168 LIVE UI RELEASE TESTS PASSED 100% CLEAN!');
    process.exit(0);
  }
}

if (require.main === module) {
  runLiveUiRelease168Audit();
}
