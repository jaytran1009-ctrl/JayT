/**
 * SECTION DK — STATIC CONTRACT & PROVENANCE VALIDATOR
 * Governing: JAYT-245 Section DK (Lines 2725-2744)
 */
const fs = require('fs');
const path = require('path');

const PROJECT_ROOT = 'd:/Công Việc MMO/OPC JayT/JayT-Dự Án Giá Trị Cộng Đồng';
const storefrontPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/jayt_storefront_staging_dk.js');
const indexPath = path.join(PROJECT_ROOT, '03_SOURCE_OF_TRUTH/index.html');

console.log('========================================================================');
console.log('🛡️  SECTION DK — STATIC CONTRACT & PROVENANCE VALIDATOR');
console.log('========================================================================\n');
...
// 1. Load JS Module
const dkModule = require(storefrontPath);
const items = dkModule.JAYT_DISCOVERY_ITEMS;
const walletEntries = dkModule.JAYT_WALLET_ENTRIES;
...
// Test 15: HTML only references jayt_storefront_staging_dk.js once
const htmlContent = fs.readFileSync(indexPath, 'utf8');
const scriptMatches = htmlContent.match(/<script src="jayt_storefront_staging_dk\.js"><\/script>/g) || [];
...
assertTest('Version string is v3.439.0-staging.dk', htmlContent.includes('v3.439.0-staging.dk'));
...
const qaDest = path.join(PROJECT_ROOT, '07_QUALITY_ASSURANCE/test_section_dk_static_contract.js');
fs.copyFileSync(__filename, qaDest);
console.log(`✅ Saved QA test script -> ${qaDest}`);

if (passedTests === totalTests) {
  console.log('🎉 SECTION DK STATIC CONTRACT INTEGRITY PASS (18/18)\n');
  process.exit(0);
} else {
  console.error('💥 SECTION DK STATIC CONTRACT FAILED!\n');
  process.exit(1);
}
