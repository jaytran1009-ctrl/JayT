...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_207_certification');
...
async function runCertifyClaimSafe207() {
  console.log('========================================================================');
  console.log('🌐 JAYT-207: 3-GATE PRODUCTION DEPLOYMENT & CLAIM-SAFE CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.347.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.347.0&t=${ts}`);
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasHeadline = bodyText.includes('Hôm nay: 0 🟢 · 3 🔵 ưu đãi chính thức · 22 🟣 nguồn chính thức đã ghi nhận');
    const hasOs3347 = bodyText.includes('OS 3.347') || bodyText.includes('Claim-Safe OS 3.347');
  ...
  console.log('   - 🔵 Ưu đãi chính thức:            ' + domReport.blueCount + ' (Expected: 3)');
  console.log('   - 🟣 Nguồn chính thức đã ghi nhận: ' + domReport.purpleCount + ' (Expected: 22)');
  ...
  console.log('   - OS 3.347 Rendered:                ' + (domReport.hasOs3347 ? '🟢 PASS' : '❌ FAIL'));

  const gate2Pass = (domReport.blueCount === 3 && domReport.purpleCount === 22 && domReport.totalDailyCards === 25 && domReport.hasHeadline && domReport.hasOs3347);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Desktop Light
  const desktopLightPath = path.join(artifactsDir, 'screenshot_207_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View: ' + desktopLightPath);

  // 2. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_207_mobile_light.png');
  ...
  // 3. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_207_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_207_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_207_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_207_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_207_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_207_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-207 CLAIM-SAFE DIRECTORY.');
  }
}

if (require.main === module) {
  runCertifyClaimSafe207().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyClaimSafe207 };
