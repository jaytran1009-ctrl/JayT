/**
 * LIVE VERCEL PRODUCTION DEPLOYMENT & PARITY AUDIT (114A)
...
const receiptPath = path.join(repoRoot, '08_RELEASE_VAULT', 'DEPLOYMENT_RECEIPT_114A.json');
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'live_beta_114a');
...
async function deployAndAudit() {
  console.log('🚀 [DEPLOY-114A] Bắt đầu quy trình Deploy Live Vercel Beta & Audit 114A...\n');
  ...
  const viewports = [
    { name: 'live_114a_mobile_390px', width: 390, height: 844 },
    { name: 'live_114a_desktop_1440px', width: 1440, height: 900 }
  ];
  ...
  for (const vp of viewports) {
    ...
    // Assert Today Board
    const todayBoardExists = await page.$('.apex-today-board');
    console.log(`   [${vp.name}] Today Board rendered: ${todayBoardExists !== null}`);
  ...
  const receipt = {
    release_id: 'JAYT_114A_TRUTHFUL_DAILY_UTILITY_RESET',
    version: 'v3.230.0',
    directive: 'JAYT-114A-TRUTHFUL-DAILY-UTILITY-RESET',
  ...
  console.log(`\n✅ [DEPLOY-114A] Đã phát hành DEPLOYMENT_RECEIPT_114A.json tại:\n   ${receiptPath}\n`);
}
