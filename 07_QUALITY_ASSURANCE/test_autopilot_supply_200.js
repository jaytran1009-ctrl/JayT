...
function runAutopilotTests200() {
  console.log('========================================================================');
  console.log('🧪 JAYT-200: AUTOPILOT SUPPLY & 5-PART GATE TEST SUITE');
  ...
  const feed200Path = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'generated_tiered_savings_feed_200.json');
  assert(fs.existsSync(feed200Path), 'Feed 200 not found');
  const feed = JSON.parse(fs.readFileSync(feed200Path, 'utf8'));
  ...
  // --- TEST 2: ZERO UNVERIFIED GREEN DEALS ---
  try {
    ...
    record('TEST_02_ZERO_UNVERIFIED_GREEN_DEALS', true, 'Zero unverified green deals. 100% fail-closed on 5-part verbatim rule.');
  } catch (err) {
    record('TEST_02_ZERO_UNVERIFIED_GREEN_DEALS', false, err.message);
  }

  // --- TEST 3: 12 SCOPE PENDING DEALS BOUND WITH WHAT TO CHECK ---
  try {
    ...
    scopeDeals.forEach(d => {
      ...
      assert(d.branch_address && d.branch_address.trim().length > 0, `Deal ${d.deal_id} missing branch_address`);
      assert(d.what_to_check && d.what_to_check.trim().length > 0, `Deal ${d.deal_id} missing what_to_check guidance`);
    });
    record('TEST_03_12_SCOPE_PENDING_BOUND_WITH_WHAT_TO_CHECK', true, '12 scope pending deals bound with explicit offer quotes, branch addresses, and what_to_check instructions.');
  } catch (err) {
    record('TEST_03_12_SCOPE_PENDING_BOUND_WITH_WHAT_TO_CHECK', false, err.message);
  }
  ...
  if (!allPassed) {
    console.error('❌ JAYT-200 TEST SUITE FAILED!');
    process.exit(1);
  } else {
    console.log(`🎉 ALL ${tests.length}/${tests.length} JAYT-200 TESTS PASSED [100% EXCELLENCE]`);
  }
}

if (require.main === module) {
  runAutopilotTests200();
}

module.exports = { runAutopilotTests200 };
