/**
 * JAYT COMPLETE THREE-COHORT REAL-SUPPLY ENGINE RED-TEAM TEST SUITE (151)
...
const { readMemoryStrict, isRejectedByNegativeFilter } = require('../05_DEAL_AND_AFFILIATE/jayt_autonomous_worker_151');

console.log('========================================================================');
console.log('🧪 JAYT-151: THREE-COHORT REAL-SUPPLY RECOVERY RED-TEAM AUDIT');
console.log('========================================================================\n');
...
async function runSupplyEngine151Audit() {
  ...
  test('Active pipeline contains zero static dictionary or string fallback patterns', () => {
    ...
    for (const dir of targetDirs) {
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.js') && !f.includes('quarantine') && !f.includes('test_supply_engine_151.js'));
  ...
  console.log('\n--- GATE 3: THREE-COHORT COVERAGE (COHORT A, B, C PRESENT) ---');
  test('Latest run processed targets across Cohort A, Cohort B, and Cohort C', () => {
    const runDirs = fs.readdirSync(runsBaseDir).filter(d => d.startsWith('RUN_'));
    const latestDirName = runDirs[runDirs.length - 1];
    const manifest = JSON.parse(fs.readFileSync(path.join(runsBaseDir, latestDirName, 'RUN_MANIFEST.json'), 'utf8'));

    const cs = manifest.cohort_summary;
    assert(cs.COHORT_A_CINEMA_ENTERTAINMENT > 0, 'Cohort A must have processed items');
    assert(cs.COHORT_B_FNB_COFFEE > 0, 'Cohort B must have processed items');
    assert(cs.COHORT_C_TRANSIT_STUDENT > 0, 'Cohort C must have processed items');
    assert.strictEqual(manifest.summary.items_processed, 21);
    console.log(`     Verified three-cohort coverage: Cohort A (${cs.COHORT_A_CINEMA_ENTERTAINMENT}), Cohort B (${cs.COHORT_B_FNB_COFFEE}), Cohort C (${cs.COHORT_C_TRANSIT_STUDENT}).`);
  });

  console.log('\n--- GATE 4: STRICT RECEIPT & TAXONOMY PARITY (MANIFEST AS SSOT) ---');
  test('Network failures, receipt errors, and 404s are strictly ERROR_OR_BLOCKED_SOURCE', () => {
    const runDirs = fs.readdirSync(runsBaseDir).filter(d => d.startsWith('RUN_'));
    const latestDirName = runDirs[runDirs.length - 1];
    const manifest = JSON.parse(fs.readFileSync(path.join(runsBaseDir, latestDirName, 'RUN_MANIFEST.json'), 'utf8'));

    for (const c of manifest.captures) {
      if (!c.is_trusted || c.http_status >= 400) {
        assert.strictEqual(c.classification, 'ERROR_OR_BLOCKED_SOURCE', `Item ${c.item_id} with untrusted receipt or HTTP error must be ERROR_OR_BLOCKED_SOURCE!`);
      }
    }
    console.log('     Verified 100% receipt & taxonomy parity with zero misclassified items.');
  });

  console.log('\n--- GATE 5: 6-STEP TAXONOMY & SOURCE REPAIR ERROR CLASSIFICATION ---');
  test('Classification sum equals total items and stale sources are isolated', () => {
    const runDirs = fs.readdirSync(runsBaseDir).filter(d => d.startsWith('RUN_'));
    const latestDirName = runDirs[runDirs.length - 1];
    const manifest = JSON.parse(fs.readFileSync(path.join(runsBaseDir, latestDirName, 'RUN_MANIFEST.json'), 'utf8'));
    ...
    assert.strictEqual(b.INCOMPLETE_OFFER_EVIDENCE, 3, 'Expected 3 incomplete offer pages in this batch');
  ...
  if (failCount > 0) {
    ...
    console.log('✨ ALL 9 JAYT-151 THREE-COHORT REAL-SUPPLY TESTS PASSED 100% CLEAN!');
    process.exit(0);
  }
}

if (require.main === module) {
  runSupplyEngine151Audit();
}
