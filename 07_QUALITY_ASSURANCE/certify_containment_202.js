...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_202_containment');
...
async function runCertifyContainment202() {
  console.log('========================================================================');
  console.log('🌐 JAYT-202: 3-GATE PRODUCTION CONTAINMENT DEPLOYMENT & LIVE CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.343.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.343.0&t=${ts}`);
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasHeadline = bodyText.includes('Hôm nay: 0 đã xác nhận · 0 ưu đãi chính thức cần kiểm tra phạm vi');
    const hasOs3343 = bodyText.includes('OS 3.343') || bodyText.includes('Daily Deal OS 3.343');
  ...
  console.log('   - Actionable Official Offers (🔵):  ' + domReport.scopeCount + ' (Expected: 0)');
  console.log('   - Strategic Savings KPI (🟢 + 🔵):  ' + domReport.totalDealsKpi + ' (Expected: 0)');
  ...
  console.log('   - Rendered Deal Cards in DOM:       ' + domReport.renderedCardsCount + ' (Expected: 0)');
  console.log('   - Headline Match:                   ' + (domReport.hasHeadline ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - OS 3.343 Rendered:                ' + (domReport.hasOs3343 ? '🟢 PASS' : '❌ FAIL'));

  const gate2Pass = (domReport.readyCount === 0 && domReport.scopeCount === 0 && domReport.communityCount === 0 && domReport.totalDealsKpi === 0 && domReport.renderedCardsCount === 0 && domReport.hasHeadline && domReport.hasOs3343);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Desktop Light
  const desktopLightPath = path.join(artifactsDir, 'screenshot_202_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View: ' + desktopLightPath);

  // 2. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_202_mobile_light.png');
  ...
  // 3. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_202_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_202_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_202_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_202_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_202_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_202_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    console.error('❌ LIVE PRODUCTION CONTAINMENT CERTIFICATION FAILED!');
    process.exit(1);
  } else {
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONTAINED & CERTIFIED.');
  }
}

if (require.main === module) {
  runCertifyContainment202().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyContainment202 };
