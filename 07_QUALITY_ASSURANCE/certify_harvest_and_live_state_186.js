...
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_186_containment');
...
async function certifyLiveState186() {
  console.log('========================================================================');
  console.log('🛡️ JAYT-186: EVIDENCE CUSTODY & LIVE STATE CERTIFICATION');
  ...
  try {
    const feedPath = path.join(repoRoot, '05_DEAL_AND_AFFILIATE', 'generated_verified_deals_186.json');
  ...
  try {
    ...
    // Check version badge 3.327
    assert(bodyText.includes('Daily Deal OS 3.327') || bodyText.includes('3.327'), 'DOM missing version 3.327');
    console.log('     Confirmed Daily Deal OS 3.327 rendered.');
    ...
    record('Live DOM Runtime Binding & Verification', 'PASS', 0, 'Version 3.327, exactly 3 deals live (Spotify, JetBrains, YouTube)');
    ...
    const dPath = path.join(evidenceDir, 'screenshot_186_desktop_light.png');
    await page.screenshot({ path: dPath, fullPage: false });
    screenshots.push({
      file: 'screenshot_186_desktop_light.png',
      viewport: '1440x900',
      theme: 'light',
    ...
    await page.evaluate(() => document.body.setAttribute('data-theme', 'light'));
    await new Promise(r => setTimeout(r, 500));
    const mLPath = path.join(evidenceDir, 'screenshot_186_mobile_light.png');
    await page.screenshot({ path: mLPath, fullPage: false });
    screenshots.push({
      file: 'screenshot_186_mobile_light.png',
      viewport: '390x844',
      theme: 'light',
    ...
    await new Promise(r => setTimeout(r, 500));
    const mDPath = path.join(evidenceDir, 'screenshot_186_mobile_dark.png');
    await page.screenshot({ path: mDPath, fullPage: false });
    screenshots.push({
      file: 'screenshot_186_mobile_dark.png',
      viewport: '390x844',
      theme: 'dark',
  ...
  const finalOutput = {
    test_suite: 'JAYT-186-LIVE-CERTIFICATION',
  ...
  const resPath = path.join(evidenceDir, 'CERTIFICATION_RESULT_186.json');
