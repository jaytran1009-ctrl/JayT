...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_213_certification');
...
async function runCertifyCardLevelVisualEvidence213() {
  console.log('========================================================================');
  console.log('🌐 JAYT-213: 3-GATE PRODUCTION DEPLOYMENT & CARD-LEVEL EVIDENCE CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.353.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.353.0&t=${ts}`);
  ...
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & HONEST DASHBOARD BREAKDOWN ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3353 = bodyText.includes('OS 3.353') || bodyText.includes('Real Visual Evidence OS 3.353');
  ...
  console.log('   - OS 3.353 Rendered:                ' + (domReport.hasOs3353 ? '🟢 PASS' : '❌ FAIL'));
  ...
  const gate2Pass = (domReport.blueCount === 12 && domReport.purpleCount === 17 && domReport.totalDailyCards === 29 && domReport.hasHeadline && domReport.hasOs3353 && domReport.hasVisualDashboard && modalReport.modalOpened && modalReport.hasSection1Hero && modalReport.hasSection2Gallery && modalReport.hasSection3Quote && modalReport.hasSection4Guide && modalReport.hasSection6Cta && modalReport.hasSection7Provenance && modalReport.hasSection8Honesty);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Modal View with Visual Hero, Gallery & Evidence Breakdown
  const modalViewPath = path.join(artifactsDir, 'screenshot_213_modal_evidence_breakdown.png');
  await page.screenshot({ path: modalViewPath, fullPage: false });
  console.log('   📸 Captured Modal Evidence View:        ' + modalViewPath);

  // Close modal for fullpage screenshots
  await page.click('#btn-close-deal-detail');
  await new Promise(r => setTimeout(r, 400));

  // 2. Desktop Light (including Visual Evidence Dashboard)
  const desktopLightPath = path.join(artifactsDir, 'screenshot_213_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View:          ' + desktopLightPath);

  // 3. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_213_mobile_light.png');
  ...
  // 4. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_213_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_213_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_213_modal_evidence_breakdown.png', path: modalViewPath, sha256: sha256File(modalViewPath) },
        { name: 'screenshot_213_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_213_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_213_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_213_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-213 CARD-LEVEL REAL VISUAL EVIDENCE GATE.');
  }
}

if (require.main === module) {
  runCertifyCardLevelVisualEvidence213().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyCardLevelVisualEvidence213 };
