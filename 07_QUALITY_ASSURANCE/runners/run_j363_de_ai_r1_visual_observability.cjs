...
const RECEIPT_PATH = path.join(WS1, '07_QUALITY_ASSURANCE/runtime_evidence/J363_DE_AI_R1_VISUAL_REMEDIATION_RECEIPT.json');

const STAGING_LOCAL_URL = 'http://127.0.0.1:4176';
const VERCEL_PREVIEW_URL = 'https://deploy-o641pdb45-kuntran777-6857s-projects.vercel.app';
const VERCEL_DEPLOYMENT_ID = 'dpl_D3cJjwW24BrUWc3wjC4HR4PXSZqd';
...
async function runR1VisualObservability() {
  console.log('================================================================');
  console.log('JAYT-363 DE-AI R1: VISUAL & USABILITY REMEDIATION OBSERVABILITY');
  console.log('Mandate: CHAIRMAN-DIRECTIVE-2026-0909-DE-AI-OVERHAUL');
  console.log('Work Order: WORK_ORDER_J363_DE_AI_R1_VISUAL_REMEDIATION.json');
  console.log('Governance Gate: JAYT_363_CEO_DE_AI_R1_VISUAL_ACCEPTANCE_GATE');
  ...
  const receipt = {
    receipt_id: 'REC_J363_DE_AI_R1_VISUAL_' + Date.now(),
    work_order_id: 'J363_DE_AI_R1_VISUAL_REMEDIATION',
    mandate: 'CHAIRMAN-DIRECTIVE-2026-0909-DE-AI-OVERHAUL',
    governance_gate: 'JAYT_363_CEO_DE_AI_R1_VISUAL_ACCEPTANCE_GATE',
    ...
    visual_layout_remediation: {
      header_height_contract: {
        desktop: '64px',
        tablet: '60px',
        mobile: '56px'
      },
      collision_resolved: false,
      empty_controls_removed: false,
      consumer_language_verified: false,
      catalog_card_evidence_bound: false
    }
  };
  ...
  // -----------------------------------------------------------------------
  // GATE 4: UX, ACCESSIBILITY, VISUAL REMEDIATION AUDIT ACROSS 3 VIEWPORTS
  // -----------------------------------------------------------------------
  console.log('\n[4/8] Running Deep Puppeteer UX, Visual Remediation & Accessibility Audit across 3 Viewports...');
  ...
  for (const vp of viewports) {
    ...
    const headerCheck = await page.evaluate(() => {
      ...
      return {
        ...
        mainTop: mainRect ? Math.round(mainRect.top) : 0,
    ...
    console.log(`  ✓ Header Height: ${headerCheck.headerHeight}px, Vertical Gap to Content: ${headerCheck.verticalGap}px, Collision: ${headerCheck.hasCollision ? 'FAIL (COLLISION)' : 'PASS (CLEAR)'}`);

    // Capture screenshot 1: Viewport at scrollY=0
    const ssScrollY0Name = `j363_r1_${vp.name}_scrollY0.png`;
    ...
    const ssFullPageName = `j363_r1_${vp.name}_fullpage.png`;
    ...
    if (vp.name === 'desktop_1440') {
      ...
      const ssScrolledName = 'j363_r1_desktop_header_scrolled.png';
      ...
      console.log(`  ✓ Screenshot (Fixed header after scroll) saved: ${ssScrolledName} (scrollY=${scrolledHeaderCheck.scrollY}px, headerTop=${scrolledHeaderCheck.headerTop}px)`);

      await page.evaluate(() => window.scrollTo(0, 0));
    }

    // B. Corrected Touch Target Scanner (Strictly interactive controls only)
    ...
    // C. Blank Visual Controls & Icon Verification
    ...
    console.log(`  ✓ Consumer Language Pass: ${languageScan.language_pass ? 'PASS (0 internal labels found)' : 'FAIL: ' + languageScan.forbidden_internal_labels_found.join(', ')}`);

    // E. Catalog Card Evidence Binding (Query article.rail-card-cn and .rail-deal-card)
    ...
    // F. WCAG AA Contrast Evaluation (Consistent with original runner text.length >= 2)
    ...
    // G. Modal State Geometry & Keyboard Navigation
    await page.evaluate(() => {
  ...
  // H. Browser Zoom 200% Check on Desktop Viewport
  ...
  receipt.visual_layout_remediation.collision_resolved = allCollisionsResolved;
  receipt.visual_layout_remediation.empty_controls_removed = allBlankControlsClean;
  receipt.visual_layout_remediation.consumer_language_verified = allLanguagePassed;
  receipt.visual_layout_remediation.catalog_card_evidence_bound = allCatalogCardsBound;
  receipt.visual_layout_remediation.browser_zoom_200_verified = zoom200Check;
  receipt.visual_layout_remediation.screenshots = screenshotsCaptured;
  ...
  // -----------------------------------------------------------------------
  // GATE 8: DUAL VERDICT SEPARATION (STAGING REMEDIATION VS FACTUAL HOLD)
  // -----------------------------------------------------------------------
  console.log('\n[8/8] Computing Dual Gate Verdict (Visual Remediation & Provenance Hold)...');
  ...
  const gate4_visual_and_a11y = allCollisionsResolved && allBlankControlsClean && allLanguagePassed && allTouchPassed && allContrastPassed && zeroErrors && zeroOverflow;
  ...
  receipt.gate_checks = {
    ...
    gate_4_visual_layout_remediation_and_accessibility: {
      passed: gate4_visual_and_a11y,
      evaluation: 'Zero header-content collision at scrollY=0 and zoom 200%, blank controls eliminated with SVGs and visible text, DEAL XAC MINH cleaned, touch scanner 100% compliant on real controls, WCAG AA contrast 100%.'
    },
  ...
  const stagingVisualPassed = (
    gate1_previewBinding &&
    gate2_parity &&
    gate3_provenance &&
    gate4_visual_and_a11y &&
    gate5_perf &&
    gate6_modules &&
  ...
  receipt.dual_verdict = {
    technical_staging_and_visual_remediation: stagingVisualPassed ? 'PASSED_FOR_STAGING_PREVIEW' : 'FAILED_STAGING_GATES',
    ...
    overall_verdict: stagingVisualPassed
      ? 'VISUAL_REMEDIATION_ACCEPTED_FOR_STAGING__FACTUAL_CONTENT_HELD__RELEASE_NOT_AUTHORIZED'
      : 'REMEDIATION_FAILED'
  };

  receipt.verdict = receipt.dual_verdict.overall_verdict;

  const runnerFile = path.join(WS1, '07_QUALITY_ASSURANCE/runners/run_j363_de_ai_r1_visual_observability.cjs');
  ...
  // Sync receipt to WS2
  const ws2Receipt = path.join(WS2, '07_QUALITY_ASSURANCE/runtime_evidence/J363_DE_AI_R1_VISUAL_REMEDIATION_RECEIPT.json');
  fs.writeFileSync(ws2Receipt, receiptStr, 'utf8');
  fs.writeFileSync(ws2Receipt + '.sha256', sha256(Buffer.from(receiptStr, 'utf8')), 'utf8');

  console.log('\n================================================================');
  console.log('REMEDIATION OBSERVABILITY COMPLETED');
  ...
  console.log('Visual Layout & A11y:       ', gate4_visual_and_a11y ? 'PASS' : 'FAIL');
...
runR1VisualObservability().catch(err => {
