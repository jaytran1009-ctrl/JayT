/**
 * LIVE VERCEL PRODUCTION DEPLOYMENT & PARITY AUDIT (122)
...
const receiptPath = path.join(repoRoot, '08_RELEASE_VAULT', 'DEPLOYMENT_RECEIPT_122.json');
const evidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'live_beta_122');
...
async function deployAndAudit122() {
  console.log('🚀 [DEPLOY-122] Bắt đầu quy trình Deploy Live Vercel Beta & Audit 122...\n');
  ...
  const viewports = [
    { name: 'live_122_mobile_390px', width: 390, height: 844 },
    { name: 'live_122_tablet_768px', width: 768, height: 1024 },
    { name: 'live_122_desktop_1440px', width: 1440, height: 900 }
  ];
  ...
  for (const vp of viewports) {
    ...
    // Assert Multi-Intent cards in hero
  ...
  const receipt = {
    release_id: 'JAYT_122_INTENT_DIVERSITY_AND_PREMIUM_MOMENT',
    version: 'v3.239.0',
    directive: 'JAYT-122-INTENT-DIVERSITY-AND-PREMIUM-MOMENT',
    ...
    multi_intent_hero_status: 'ACTIVE_3_DISTINCT_INTENTS_PER_SLOT',
    decision_questions_status: 'ACTIVE_5_SLOT_QUESTIONS',
    deal_expired_recovery_status: 'ACTIVE_RECOVERY_MODAL',
    audit_files: auditReport,
    viewport_evidence: screenshots
  };

  fs.writeFileSync(receiptPath, JSON.stringify(receipt, null, 2), 'utf8');
  console.log(`\n🧾 [RECEIPT] Đã xuất Deployment Receipt 122: ${receiptPath}`);
  console.log('🎉 [DEPLOY-122] TOÀN BỘ QUY TRÌNH DEPLOY & AUDIT 122 ĐÃ HOÀN TẤT THÀNH CÔNG RỰC RỠ!\n');
}

deployAndAudit122().catch(err => {
  console.error('❌ [DEPLOY-122 ERROR]', err);
  process.exit(1);
});
