...
const certEvidenceDir = path.join(repoRoot, '07_QUALITY_ASSURANCE', 'runtime_evidence', 'evidence_210_certification');
...
async function runCertifyVisualAssets210() {
  console.log('========================================================================');
  console.log('🌐 JAYT-210: 3-GATE PRODUCTION DEPLOYMENT & VISUAL ASSET CERTIFICATION');
  ...
  const ts = Date.now();
  const liveModuleText = await fetchText(`${liveUrl}jayt_verified_deals_module.js?v=3.350.0&t=${ts}`);
  const liveModuleSha = sha256Buf(Buffer.from(liveModuleText, 'utf8'));

  const liveMainJsText = await fetchText(`${liveUrl}jayt_apex_interface.js?v=3.350.0&t=${ts}`);
  ...
  // --- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & VISUAL ASSET RENDERING ---
  console.log('\n--- GATE 2: LIVE DOM PUPPETEER ASSERTIONS & VISUAL ASSET RENDERING ---');
  ...
  const domReport = await page.evaluate(() => {
    ...
    const hasOs3350 = bodyText.includes('OS 3.350') || bodyText.includes('Visual Asset OS 3.350');

    const verifiedVisualImages = Array.from(document.querySelectorAll('.jayt-card-verified-deal img'));
    ...
    return {
      ...
      verifiedVisualImagesCount: verifiedVisualImages.length,
      hasHeadline,
      hasOs3350
    };
  });
  ...
  console.log('   - Verified Visual Images on Cards:  ' + domReport.verifiedVisualImagesCount);
  console.log('   - Headline Breakdown Match:         ' + (domReport.hasHeadline ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - OS 3.350 Rendered:                ' + (domReport.hasOs3350 ? '🟢 PASS' : '❌ FAIL'));

  // Test Modal Interaction on Verified Visual Deal (Metiz)
  console.log('\n   🖱️ Testing Live Modal Interaction on Verified Deal (Metiz)...');
  ...
  const modalReport = await page.evaluate(() => {
    ...
    return {
      ...
      hasImageBanner: !!img && img.src.includes('cinema-context-v1.png'),
      hasAttribution: text.includes('ảnh: không gian rạp chiếu phim đà nẵng'),
      hasRightsBasisBadge: text.includes('owner_uploaded_original') || text.includes('official_press_kit_license'),
  ...
  console.log('   - Verified Image Banner Rendered:   ' + (modalReport.hasImageBanner ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - Attribution & Rights Basis Badge: ' + (modalReport.hasAttribution && modalReport.hasRightsBasisBadge ? '🟢 PASS' : '❌ FAIL'));
  console.log('   - 5 Sections Rendered:              ' + (modalReport.hasTitle && modalReport.hasVerbatimSection && modalReport.hasGuideSection && modalReport.hasAnalysisSection && modalReport.hasChecklist ? '🟢 PASS' : '❌ FAIL'));

  const gate2Pass = (domReport.blueCount === 12 && domReport.purpleCount === 17 && domReport.totalDailyCards === 29 && domReport.hasHeadline && domReport.hasOs3350 && modalReport.modalOpened && modalReport.hasImageBanner);
  console.log(`   Gate 2 Live DOM Assertions: ${gate2Pass ? '🟢 PASS' : '❌ FAIL'}`);

  // --- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---
  console.log('\n--- GATE 3: VISUAL CAPTURE & SCREENSHOT ARTIFACTS ---');
  // 1. Modal View with Verified Image Banner
  const modalViewPath = path.join(artifactsDir, 'screenshot_210_modal_verified_asset.png');
  await page.screenshot({ path: modalViewPath, fullPage: false });
  console.log('   📸 Captured Modal Verified Asset View: ' + modalViewPath);
  ...
  // 2. Desktop Light
  const desktopLightPath = path.join(artifactsDir, 'screenshot_210_desktop_light.png');
  await page.setViewport({ width: 1440, height: 900 });
  await page.screenshot({ path: desktopLightPath, fullPage: false });
  console.log('   📸 Captured Desktop Light View:        ' + desktopLightPath);

  // 3. Mobile Light
  const mobileLightPath = path.join(artifactsDir, 'screenshot_210_mobile_light.png');
  ...
  // 4. Mobile Dark
  const mobileDarkPath = path.join(artifactsDir, 'screenshot_210_mobile_dark.png');
  await page.evaluate(() => {
  ...
  // Save report
  const certReport = {
    certification_id: 'CERT_210_' + Date.now(),
    ...
    gate3_visual_capture: {
      pass: true,
      screenshots: [
        { name: 'screenshot_210_modal_verified_asset.png', path: modalViewPath, sha256: sha256File(modalViewPath) },
        { name: 'screenshot_210_desktop_light.png', path: desktopLightPath, sha256: sha256File(desktopLightPath) },
        { name: 'screenshot_210_mobile_light.png', path: mobileLightPath, sha256: sha256File(mobileLightPath) },
        { name: 'screenshot_210_mobile_dark.png', path: mobileDarkPath, sha256: sha256File(mobileDarkPath) }
      ]
    }
  };

  const reportFilePath = path.join(certEvidenceDir, 'CERTIFICATION_210_LIVE_REPORT.json');
  ...
  if (!allGatesPass) {
    ...
    console.log('\n🎉 ALL 3 GATES PASSED! LIVE VERCEL PRODUCTION 100% CONFORMANT TO JAYT-210 OFFICIAL VISUAL ASSET PROGRAM.');
  }
}

if (require.main === module) {
  runCertifyVisualAssets210().catch(err => {
    console.error('Fatal Certification Error:', err);
    process.exit(1);
  });
}

module.exports = { runCertifyVisualAssets210 };
