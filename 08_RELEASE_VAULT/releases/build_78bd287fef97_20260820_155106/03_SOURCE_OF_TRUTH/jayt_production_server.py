...
def verify_build_manifest_integrity():
    ...
    # Kiểm tra Runtime Server Binding: server runtime phải chạy đúng tại canonical build root
    if canon_vault_root != canon_ptr_path:
        dev_manifest = os.path.join(canon_vault_root, "BUILD_MANIFEST.json")
        if not (os.path.exists(dev_manifest) and "08_RELEASE_VAULT" in canon_vault_root):
    ...
    if not manifest_digest:
        return False, build_id, manifest_hash, ["ERR_BUILD_MANIFEST_MISSING_CONTENT_DIGEST"]
    ...
    try:
        sorted_files = sorted(files_list, key=lambda x: x.get("relative_path", ""))
    except Exception as e:
    ...
    for file_entry in sorted_files:
        ...
        fname = file_entry.get("file_name") or os.path.basename(rel_path.replace("\\", "/"))
        ...
        combined_builder += f"{fname}:{actual_hash.upper()};"

    # Tính toán Merkle Tree Digest thực tế từ toàn bộ cây tệp
    recomputed_tree_digest = hashlib.sha256(combined_builder.encode("utf-8")).hexdigest().lower()

    if recomputed_tree_digest != manifest_digest.lower():
    ...
    if pointer_digest and recomputed_tree_digest != pointer_digest.lower():
