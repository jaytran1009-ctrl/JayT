// =============================================================================
// JAYT — TRỢ LÝ TIẾT KIỆM HẰNG NGÀY ĐÀ NẴNG 43 (APEX INTERFACE v8.0)
// Chỉ thị: JAYT-RETENTION-VALUE-ENGINE-001 (3 Intent Flows • Right-Now Engine • Need Collections)
// =============================================================================
(function () {
  'use strict';
  ...
  function verifyLiveAvailability(deal, evidenceStore, daNangCtx, options = {}) {
    ...
    // Gate: Anti-Future Timestamp Protection (Skew tolerance: 60s)
  ...
  function css() {
    ...
    /* CONTEXTUAL REASON (WHY THIS FITS YOU) */
    ...
    .apex-save-pill { font-size: 10.5px; font-weight: 900; color: #10b981; background: rgba(16,185,129,.14); padding: 3px 7px; border-radius: 6px; border: 1px solid rgba(16,185,129,.3); }
    ...
    <article class="apex-deal" data-deal-id="${dealDomId}" data-taxonomy="${d.taxonomy}">
      ...
      <div class="apex-deal-body">
        ...
        <div class="apex-contextual-reason">
          💡 <b>Vì sao hợp lúc này:</b> ${esc(d.contextual_reason)}
        </div>` : ''}
        ...
        <div class="apex-deal-actions">
          <button class="hunt" data-apex="hunt" data-id="${dealDomId}">🎯 Xem Điều Kiện & Săn</button>
  ...
  function renderRecommendationSection() {
    ...
    <section class="apex-rec-box" id="apex-rec-box">
      <div class="apex-rec-top">
        <div class="apex-rec-title">🎯 3 KÈO HỢP VỚI BẠN HÔM NAY</div>
  ...
  function render() {
    ...
    if (!isHttpRuntime()) {
      ...
      <div class="apex-safe-card" style="border-color:rgba(255,255,255,.1);background:#0d2820;">
        <h3 style="color:#fff;">🔍 CHƯA TÌM THẤY KÈO PHÙ HỢP</h3>
        <p style="color:var(--apex-muted);">Không có ưu đãi nào khớp với bộ lọc hoặc từ khóa tìm kiếm hiện tại.</p>
          ...
          <section class="apex-section">
            <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:14px;">
              <h3 style="margin:0;font-size:17px;color:var(--apex-gold);font-weight:900;">
                ${state.viewMode==='saved' ? '⭐ KÈO BẠN ĐÃ LƯU' : state.viewMode==='plan' ? '📋 KẾ HOẠCH SĂN KÈO HÔM NAY' : '🛍️ DANH MỤC ƯU ĐÃI ĐÃ KIỂM CHỨNG'}
              </h3>
              <span style="font-size:12px;color:var(--apex-muted);">Hiển thị ${filteredDeals.length} ưu đãi</span>
            </div>
            ${dealsSectionContent}
          </section>
        </div>
  ...
  function renderActiveModal() {
    ...
    <div class="apex-modal" id="apex-active-modal">
      <div class="apex-modal-card">
        <span class="apex-persona-tag">🔍 BẢO VỆ NGƯỜI DÙNG</span>
        <h2>${esc(d.merchant)}</h2>
        <p><b>Ưu đãi:</b> ${esc(d.title || d.merchant)}</p>
        ${d.contextual_reason ? `<p style="color:#f1df9a;">💡 <b>Gợi ý:</b> ${esc(d.contextual_reason)}</p>` : ''}
        <p><b>Giá áp dụng:</b> <span style="color:#fff;font-weight:900;font-size:16px;">${money(d.price_num)}</span> ${d.original_price ? `<del style="color:#788a82;font-size:12px;">${money(d.original_price)}</del>` : ''}</p>
        
        <div class="apex-feedback-local-notice">
          🛡️ <b>Kiểm tra điều kiện:</b> ${esc(d.disclosure || 'Chương trình khuyến mãi công khai. Vui lòng xuất trình thông tin hợp lệ tại điểm áp dụng.')}
        </div>

        <div class="actions">
          <button class="no" data-apex="close">Đóng / Hủy</button>
  ...
  function renderOnboardingModal() {
    ...
    <div class="apex-modal" id="apex-onboard-modal">
      <div class="apex-onboard-card">
        ...
        <h2 style="color:var(--apex-gold);margin:0 0 6px;">Chào bạn đến với Bộ Nhớ Khuyến Mãi Đà Nẵng 43</h2>
        <p style="color:var(--apex-muted);font-size:13px;margin:0 0 16px;">Chọn đối tượng để JayT gợi ý chính xác những ưu đãi gần bạn nhất:</p>
        ...
        <button class="apex-onboard-skip" data-apex="onboard-skip">Bỏ qua, xem tất cả ưu đãi</button>
      </div>
    </div>`;
  }
  ...
  function renderFeedbackModal() {
    ...
    <div class="apex-modal" id="apex-feedback-modal">
      <div class="apex-modal-card">
        <h2>🚩 Báo Cáo Thông Tin Khuyến Mãi</h2>
        <p style="margin:4px 0 12px;color:var(--apex-gold);font-weight:700;">${esc(state.feedbackDealTitle || 'Ưu đãi')}</p>
        <div class="apex-feedback-local-notice">
  ...
  async function handleEvent(e) {
    ...
    if (action === 'quick-intent') {
      ...
      if (idx > -1) {
        ...
        state.saved.push(id);
        showToast('⭐ Đã lưu ưu đãi vào danh sách cá nhân!');
      }
      ...
      if (idx > -1) {
        ...
        if (state.todayPlan.length >= 3) {
          showToast('⚠️ Kế hoạch hôm nay tối đa 3 ưu đãi!');
          return;
        }
