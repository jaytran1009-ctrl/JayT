# -*- coding: utf-8 -*-
"""
JAYT APEX PRODUCTION SERVER — STRICT TLS-SNI IP PINNING & 2xx HOP RESOLVER
=============================================================================
...
4. /api/token/issue: Exact Deal ID matching & Secret Governed Token Issuer.
5. /out: TRUE TLS-SNI IP-Pinned, 2xx Terminal, Hop-by-Hop Redirect Gateway.
=============================================================================
"""
import os
import sys
...
def get_domain_catalog():
    """Nạp danh mục domain với đầy đủ chính sách kiểm toán (Fail-Closed)."""
    vault_root = get_vault_root()
...
def validate_catalog_policy_for_domain(hostname, catalog):
    """
    KIỂM TOÁN CHÍNH SÁCH CATALOG TOÀN DIỆN:
    1. Domain tồn tại trong Catalog
    2. status == ACTIVE
    3. emergency_kill_switch_active == False
    4. is_enabled != False
    5. protocol strictly HTTPS
    6. Hạn dùng expiry UTC còn hiệu lực
    """
    ...
    allowed_protocols = matched.get("allowed_protocols", ["HTTPS"])
    if "HTTPS" not in [p.upper() for p in allowed_protocols]:
        return False, None, f"ERR_PROTOCOL_NOT_PERMITTED: {hostname_clean}"
    ...
    if expiry_str:
        try:
            ...
            return False, None, f"ERR_INVALID_EXPIRY_TIMESTAMP: {str(e)}"

    return True, matched, None

def make_true_tls_ip_pinned_request(url, timeout=4):
    """
    TRUE TLS-SNI IP PINNING:
    Kết nối socket trực tiếp tới Pinned IP, thiết lập Host header và TLS SNI server_hostname.
    Loại bỏ hoàn toàn cửa sổ DNS Rebinding!
    """
    parsed = urlparse(url)
    ...
    # Resolve all IPv4 & IPv6 records and validate anti-SSRF
    ...
    conn = http.client.HTTPSConnection(pinned_ip, port=port, timeout=timeout, context=ctx)
    ...
    conn.close()
    return status, headers, pinned_ip

def execute_strict_ip_pinned_hop_resolution(initial_url, catalog, max_hops=5, timeout=4):
    """
    REAL HOP-BY-HOP IP-PINNED RESOLUTION (STRICT 2xx TERMINAL):
    1. Kiểm tra chính sách Catalog & Scheme trên từng hop
    2. Thực thi request ghim IP (TLS-SNI Pinned)
    3. Theo vết Location header
    4. Chỉ chấp nhận đích cuối là HTTP 2xx (200..299)
    """
    ...
    for hop in range(max_hops):
        ...
        # 1. Thẩm định đầy đủ chính sách Catalog
        ...
        # 2. Gửi request TLS-SNI IP Pinned
        try:
            ...
            # ĐÍCH CUỐI BẮT BUỘC LÀ 2xx (200 <= status <= 299)
            elif 200 <= status_code <= 299:
                return True, current_url, None
            else:
                # 4xx / 5xx terminal -> FAIL-CLOSED
...
def verify_and_consume_token_atomic(token):
    """
    QUY TRÌNH KIỂM TOÁN NGUYÊN TỬ VÀ TIÊU THỤ HẬU XÁC MINH:
    1. Kiểm tra cấu trúc & chữ ký token
    2. Tìm chính xác deal_id (KHÔNG substring)
    3. Thực thi IP-Pinned Hop-by-Hop Resolver & kiểm tra 2xx terminal
    4. CHỈ TIÊU THỤ NGUYÊN TỬ KHI TOÀN BỘ CHUỖI ĐÃ PASS 100%!
    """
    if not TOKEN_SIGNING_CONFIGURED:
    ...
    # BẮT BUỘC KHỚP TUYỆT ĐỐI 1-1 (ZERO SUBSTRING MATCHING)
    ...
    # IP-PINNED HOP RESOLUTION (2xx TERMINAL REQUIRED)
    ok, final_url, hop_err = execute_strict_ip_pinned_hop_resolution(canonical_url, catalog)
    if not ok:
        # FAIL-CLOSED: Giữ nguyên token cho người dùng nếu destination tạm thời lỗi
    ...
    def do_GET(self):
        ...
        # 2. HEALTH & RUNTIME METADATA ENDPOINT
            ...
            health_data = {
                ...
                "resolver_engine": "TRUE_TLS_SNI_IP_PINNED_v3"
            }
        ...
        # 5. TOKEN ISSUANCE ENDPOINT (/api/token/issue?deal_id=...)
            ...
            deals = get_deals_feed()
            # BẮT BUỘC KHỚP TUYỆT ĐỐI 1-1 (ZERO SUBSTRING)
            deal = deals.get(deal_id)
            if not deal:
