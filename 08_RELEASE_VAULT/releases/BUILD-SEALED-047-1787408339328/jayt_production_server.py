...
# 2. ATOMIC POST-VALIDATION TOKEN STORE (TIMESTAMP-TRACKED WITH EXPIRY CLEANUP)
CONSUMED_TOKENS = {}  # token -> consumed_at_timestamp (float)
TOKEN_LOCK = threading.Lock()
TOKEN_EXPIRY_SECONDS = 15
REPLAY_PROTECTION_CLEANUP_WINDOW_SECONDS = 60

# 2b. IMMUTABLE RUNTIME REQUEST AUDIT LEDGER (CAPPED AT 1000 ENTRIES UNDER LOCK)
SERVER_REQUEST_AUDIT_LOG = []
AUDIT_LOCK = threading.Lock()
MAX_AUDIT_RECORDS = 1000

def record_server_audit(method, path, query, status_code, extra=None):
    with AUDIT_LOCK:
        SERVER_REQUEST_AUDIT_LOG.append({
        ...
        if len(SERVER_REQUEST_AUDIT_LOG) > MAX_AUDIT_RECORDS:
            del SERVER_REQUEST_AUDIT_LOG[:-MAX_AUDIT_RECORDS]
...
def get_active_build_id_safe():
    try:
        bid, _, _ = get_active_build_metadata()
        if bid:
            return bid
    except Exception:
        pass
...
def get_evidence_store():
    vault_root = get_vault_root()
    ev_file = os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "evidence_store.json")
    if not os.path.exists(ev_file):
        ev_file = os.path.join(os.path.dirname(vault_root), "05_DEAL_AND_AFFILIATE", "evidence_store.json")
    if not os.path.exists(ev_file):
        return {}
    try:
        with open(ev_file, "r", encoding="utf-8-sig") as f:
            data = json.load(f)
...
VALID_VERIFIABLE_CAPTURE_METHODS = {'BROWSER_FULLPAGE_SCREENSHOT', 'RAW_HTTPS_RESPONSE_PAYLOAD', 'PDF_DOCUMENT_SNAPSHOT', 'ARCHIVE_SNAPSHOT', 'AUTOMATED_PUBLIC_BROWSER_CAPTURE'}

def is_deal_render_eligible(deal, evidence_data, vault_root):
    """
    SERVER-ENFORCED VISIBILITY GATE (JAYT-CATALOG-VISIBILITY-GATE-002)
    Fail-closed calculation: All criteria must be strictly satisfied.
    """
    ...
    # 1. Lifecycle status must NOT be NEEDS_RECHECK or ARCHIVED
    lifecycle = deal.get("lifecycle_status", "PROBING")
    if lifecycle in ("NEEDS_RECHECK", "ARCHIVED"):
        return False
    
    # 2. Taxonomy must be PROBING (VERIFIED is strictly rejected without cryptographic authorization)
    taxonomy = deal.get("taxonomy")
    if taxonomy not in ("PROBING", "UNVERIFIED"):
        return False

    # 3. Evidence reference check
    ev_ref = deal.get("evidence_ref")
    if not ev_ref or not isinstance(ev_ref, str) or ev_ref not in evidence_data:
        return False

    ev = evidence_data[ev_ref]
    if not isinstance(ev, dict) or ev.get("deal_id") != did:
        return False

    # 4. Raw capture artifact requirements (Capture Authenticity Gate)
    cap_file = ev.get("capture_file") or ev.get("artifact_screenshot")
    if not cap_file or not isinstance(cap_file, str) or not cap_file.strip():
        return False
    if cap_file.endswith(".txt") or cap_file.endswith(".md"):
        return False

    content_hash = ev.get("evidence_content_hash")
    if not content_hash or not isinstance(content_hash, str) or len(content_hash.strip()) != 64:
        return False

    cap_method = ev.get("capture_method")
    if not cap_method or cap_method not in VALID_VERIFIABLE_CAPTURE_METHODS:
        return False

    mime_type = ev.get("artifact_mime_type")
    if not mime_type or mime_type not in AUTHENTIC_VERIFIABLE_MIMES:
        return False

    # 5. URL match check
    source_url = deal.get("source_url") or deal.get("url")
    artifact_url = ev.get("artifact_source_url") or ev.get("source_url")
    if artifact_url and source_url:
        if artifact_url.rstrip("/") != source_url.rstrip("/"):
            return False

    # 6. Specificity & observed fields
    if not ev.get("observed_price_or_offer") or not ev.get("observed_conditions") or not ev.get("expiry_basis"):
        return False

    # 7. Physical artifact verification on disk inside vault
    candidate_paths = [
        os.path.join(vault_root, cap_file),
        os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", cap_file),
        os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "evidence_artifacts", os.path.basename(cap_file)),
        os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "candidates", "evidence_artifacts", os.path.basename(cap_file)),
        os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "candidates", "evidence_snapshots", os.path.basename(cap_file)),
        os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "candidates", "pending_review", "artifacts", os.path.basename(cap_file)),
        os.path.join(vault_root, "..", "05_DEAL_AND_AFFILIATE", "candidates", "evidence_snapshots", os.path.basename(cap_file)),
        os.path.join(vault_root, "..", "05_DEAL_AND_AFFILIATE", "candidates", "pending_review", "artifacts", os.path.basename(cap_file)),
        os.path.join(vault_root, "..", "..", "05_DEAL_AND_AFFILIATE", "candidates", "evidence_snapshots", os.path.basename(cap_file)),
        os.path.join(vault_root, "..", "..", "05_DEAL_AND_AFFILIATE", "candidates", "pending_review", "artifacts", os.path.basename(cap_file)),
        os.path.join(vault_root, "07_QUALITY_ASSURANCE", "fixtures", "evidence_artifacts", os.path.basename(cap_file))
    ]
        ...
        if os.path.exists(p) and os.path.isfile(p):
            ...
            try:
                with open(p, "rb") as af:
                    actual_sha = hashlib.sha256(af.read()).hexdigest().lower()
                if actual_sha != content_hash.strip().lower():
                    return False # Hash mismatch -> Fail closed
            except Exception:
                return False
            break
...
def generate_ephemeral_token(deal_id, target_url, correlation_id):
    if not TOKEN_SIGNING_CONFIGURED:
        return None, "ERR_TOKEN_SECRET_NOT_CONFIGURED"
    if not correlation_id or not isinstance(correlation_id, str):
        return None, "ERR_MISSING_CORRELATION_ID"
    now = int(time.time())
    payload = f"{deal_id}|{correlation_id}|{target_url}|{now}"
    signature = hmac.new(HMAC_SECRET, payload.encode("utf-8"), hashlib.sha256).hexdigest()
    token = f"{deal_id}.{correlation_id}.{now}.{signature}"
    return token, None
...
def verify_and_consume_token_atomic(token, correlation_id):
    if not TOKEN_SIGNING_CONFIGURED:
        return False, None, "ERR_TOKEN_SECRET_NOT_CONFIGURED"
    if not correlation_id or not isinstance(correlation_id, str):
        return False, None, "ERR_MISSING_CORRELATION_ID"
        
    now_ts = time.time()
    with TOKEN_LOCK:
        # 1. Periodic cleanup of tokens outside the replay protection window
        expired_cutoff = now_ts - REPLAY_PROTECTION_CLEANUP_WINDOW_SECONDS
        stale_tokens = [t for t, consumed_at in CONSUMED_TOKENS.items() if consumed_at < expired_cutoff]
        for t in stale_tokens:
            del CONSUMED_TOKENS[t]
    ...
    deal_id, token_cid, ts_str, sig = parts
    
    # 1. Cryptographic Correlation ID Binding Match Check
    if not hmac.compare_digest(token_cid, correlation_id):
        return False, None, "ERR_CORRELATION_ID_MISMATCH"
    ...
    expected_payload = f"{deal_id}|{token_cid}|{canonical_url}|{issued_at}"
    ...
    with TOKEN_LOCK:
        ...
        CONSUMED_TOKENS[token] = time.time()
...
class JayTComprehensiveHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def send_response(self, code, message=None):
        ...
        self.send_header('X-JayT-Active-Build', get_active_build_id_safe())
    ...
    def do_GET(self):
        ...
        # 6b. DEALS FEED, EVIDENCE & ZONE DATA ADAPTER ENDPOINT (/api/deals) — SERVER-ENFORCED VISIBILITY GATE
        elif parsed.path == "/api/deals":
            errors = []
            vault_root = get_vault_root()
            feed_file = os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "deals_feed.json")
            ev_file = os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "evidence_store.json")
            zone_file = os.path.join(vault_root, "05_DEAL_AND_AFFILIATE", "zone_catalog.json")
            ...
            if not os.path.exists(zone_file):
                errors.append("ERR_ZONE_CATALOG_FILE_NOT_FOUND")
            ...
            if not errors:
                try:
                    with open(feed_file, "r", encoding="utf-8-sig") as ff:
                        ...
                        if not isinstance(raw_deals, list):
                            errors.append("ERR_DEALS_FEED_NOT_LIST")
                except Exception as e:
                ...
                try:
                    with open(zone_file, "r", encoding="utf-8-sig") as zf:
                        raw_zones = json.load(zf)
                        if not isinstance(raw_zones, list):
                            errors.append("ERR_ZONE_CATALOG_NOT_LIST")
                        else:
                            for z in raw_zones:
                                if isinstance(z, dict) and z.get("zone_id"):
                                    valid_zone_ids.add(z.get("zone_id"))
                except Exception as e:
                    errors.append(f"ERR_ZONE_CATALOG_MALFORMED_JSON: {str(e)}")
            ...
            # SERVER-SIDE RENDER ELIGIBILITY EVALUATION (JAYT-CATALOG-VISIBILITY-GATE-002)
            render_eligible_deals = []
            for d in raw_deals:
                if is_deal_render_eligible(d, evidence_data, vault_root):
                    deal_copy = dict(d)
            ...
            if len(render_eligible_deals) == 0:
                resp_data = {
                    "status": "OK",
                    "catalog_state": "NO_RENDER_ELIGIBLE_DEALS",
                    "deals": [],
                    "evidence": {},
                    "zones": raw_zones,
                    "total_count": 0,
                    "total_stored_deals": len(raw_deals),
                    "note": "Toàn bộ danh mục ưu đãi đang trong giai đoạn thẩm định nguồn (PROBING) theo cổng Capture Authenticity Gate."
                }
            else:
                eligible_ev_refs = {d["evidence_ref"] for d in render_eligible_deals if d.get("evidence_ref")}
                resp_data = {
                    "status": "OK",
                    "catalog_state": "ACTIVE_RENDER_ELIGIBLE_DEALS",
                    "deals": render_eligible_deals,
                    "evidence": {ref: evidence_data[ref] for ref in eligible_ev_refs if ref in evidence_data},
                    "zones": raw_zones,
        ...
        # 7. TOKEN ISSUANCE ENDPOINT — STRICT METHOD REJECTION (GET is FORBIDDEN -> 405)
            ...
            self.wfile.write(json.dumps({
                ...
                "message": "Token issuance requires HTTP POST with Content-Type: application/json"
            }).encode("utf-8"))
            return

                # 8b. REQUEST AUDIT LEDGER ENDPOINT
        elif parsed.path == "/api/request-audit":
            with AUDIT_LOCK:
                log_copy = list(SERVER_REQUEST_AUDIT_LOG)
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self.send_header("Cache-Control", "no-store")
            self.end_headers()
        ...
        # 8. TOKENIZED FAIL-CLOSED REDIRECT RESOLVER (/out?token=...&cid=...)
        elif parsed.path == "/out":
            qs = parse_qs(parsed.query)
            token = qs.get("token", [""])[0]
            correlation_id = qs.get("cid", [""])[0]
            
            if not token:
                record_server_audit("GET", "/out", qs, 403, {"error": "ERR_MISSING_EPHEMERAL_TOKEN", "correlation_id": correlation_id})
            ...
            if not correlation_id:
                record_server_audit("GET", "/out", qs, 403, {"error": "ERR_MISSING_CORRELATION_ID", "token": token})
                self.send_response(403)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "FORBIDDEN", "reason": "ERR_MISSING_CORRELATION_ID"}).encode("utf-8"))
                return
                
            valid_token, target_url, token_err = verify_and_consume_token_atomic(token, correlation_id)
            if not valid_token:
                record_server_audit("GET", "/out", qs, 403, {"error": token_err, "correlation_id": correlation_id})
                self.send_response(403)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                ...
                record_server_audit("GET", "/out", qs, 302, {
    ...
    def handle_token_issue(self, deal_id):
        if not TOKEN_SIGNING_CONFIGURED:
        ...
        if not deal and deal_id:
            target_lower = deal_id.strip().lower()
            for did, d in deals.items():
                if did.strip().lower() == target_lower:
                    deal = d
                    deal_id = did
                    break
        if not deal:
        ...
        # STRICT SERVER VISIBILITY GATE: Deal must be render_eligible to issue token!
        evidence_data = get_evidence_store()
        vault_root = get_vault_root()
        if not is_deal_render_eligible(deal, evidence_data, vault_root):
            self.send_response(403)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self.send_header("Cache-Control", "no-store")
            self.end_headers()
        ...
        correlation_id = f"cid_{os.urandom(8).hex()}"
        canonical_url = deal.get("source_url") or deal.get("url")
        token, err = generate_ephemeral_token(deal_id, canonical_url, correlation_id)
        if err:
            self.send_response(500)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self.end_headers()
        ...
        token_resp = {
            ...
            "outbound_endpoint": f"/out?token={token}&cid={correlation_id}"
        }
        record_server_audit("POST", "/api/token/issue", "", 200, {
            "deal_id": deal_id,
            "token": token,
            "issued_token": token,
            "correlation_id": correlation_id
    ...
    def do_POST(self):
        parsed = urlparse(self.path)
        if parsed.path == "/api/token/issue":
            ...
            if content_length <= 0:
                ...
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "EMPTY_REQUEST_BODY"}).encode("utf-8"))
                return
            ...
            try:
                ...
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "INVALID_JSON_BODY", "detail": str(e)}).encode("utf-8"))
                return
            ...
            if not isinstance(data, dict):
                ...
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "JSON_BODY_MUST_BE_OBJECT"}).encode("utf-8"))
                return
            ...
            if not deal_id or not isinstance(deal_id, str) or not deal_id.strip():
                ...
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"error": "MISSING_OR_INVALID_DEAL_ID"}).encode("utf-8"))
                return

            return self.handle_token_issue(deal_id.strip())
        else:
            self.send_error(404, "Endpoint Not Found")
