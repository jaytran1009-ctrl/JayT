/* =============================================================================
   ...
   Chỉ thị: JAYT-COMMUNITY-RETENTION-002 (Evidence-Backed Labeling • Asia/Ho_Chi_Minh Timezone)
   ============================================================================= */
(function () {
  'use strict';
  ...
  function getDaNangDiurnalContext(mockMinutes = null) {
    let nowMinutes = 0;
    let timeStr = '12:00';
    let dayOfWeek = 'Mon';
  ...
  function evaluateRecommendations() {
    ...
    if (inTimingDeals.length > 0) {
      return inTimingDeals.map(d => {
        // Evidence-backed claim discipline: PROBING deals get "Phù hợp khung giờ này"
        let statusLabel = '🕒 Phù hợp khung giờ này';
        if (d.taxonomy === 'VERIFIED' && d.availability_schedule) {
          statusLabel = '🟢 ĐANG ÁP DỤNG NGAY';
        }
        return {
          deal: d,
