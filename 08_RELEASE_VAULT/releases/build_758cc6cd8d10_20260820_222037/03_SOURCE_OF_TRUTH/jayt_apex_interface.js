// =============================================================================
// JAYT — CÔNG CỤ QUYẾT ĐỊNH CHI TIÊU HẰNG NGÀY ĐÀ NẴNG 43 (APEX INTERFACE v11.0)
// Chỉ thị: JAYT-FUNCTION-FIRST-EXPERIENCE-001
// =============================================================================
(function () {
  'use strict';
  ...
  const state = {
    activeProductTab: 'local', // 'local' (Địa Phương Đà Nẵng) | 'online' (Sàn Online & Săn Mã)
    activeScenario: null,      // null | 'student' | 'office' | 'group'
    scenarioStep: 1,           // 1 -> 2 -> 3 -> 4 (Result Decision)
    ...
    rightNowMode: false,       // boolean: when true, MUST strictly pass all 6 conditions (including current live time)
    viewMode: 'all',           // 'all' | 'saved' | 'plan' | 'ledger' | 'intent_student' | 'intent_office' | 'intent_group'
    query: '',
    saved: [],                 // array of deal_id
    todayPlan: [],             // array of deal_id
    planDetails: {},           // { [deal_id]: { slot: 'lunch', status: 'need_check'|'opened'|'claimed'|'not_fit', note: '' } }
    savingsLedger: {
      totalSavedVnd: 0,
      claimedDeals: [],        // array of { deal_id, merchant, savedVnd, claimedAt }
      openedHistory: [],       // array of { deal_id, merchant, url, openedAt }
      topNeedsCount: {}        // { [needKey]: count }
    },
    activeModal: null,
  ...
  function loadLocalStorageData() {
    try {
      ...
      const rawLedger = localStorage.getItem('jayt_savings_ledger');
      if (rawLedger) {
        const parsed = JSON.parse(rawLedger);
        if (parsed && typeof parsed === 'object') {
          state.savingsLedger = {
            totalSavedVnd: Number(parsed.totalSavedVnd) || 0,
            claimedDeals: Array.isArray(parsed.claimedDeals) ? parsed.claimedDeals : [],
            openedHistory: Array.isArray(parsed.openedHistory) ? parsed.openedHistory : [],
            topNeedsCount: parsed.topNeedsCount && typeof parsed.topNeedsCount === 'object' ? parsed.topNeedsCount : {}
          };
        }
      }
    } catch (e) {
      console.warn('[JAYT-STATE] Lỗi đọc localStorage:', e);
    }
  }

  function saveSavingsLedgerToStorage() {
    try {
      localStorage.setItem('jayt_savings_ledger', JSON.stringify(state.savingsLedger));
    } catch (e) {
      console.warn('[JAYT-STATE] Lỗi lưu savings ledger:', e);
    }
  }
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.date_key === currentKey && Array.isArray(parsed.deal_ids)) {
          ...
          state.planDetails = parsed.plan_details || {};
          return;
        }
      }
    } catch (e) {
  ...
  function saveTodayPlanToStorage() {
    ...
    const payload = {
      ...
      plan_details: state.planDetails,
      plan_status: Object.keys(state.planDetails).reduce((acc, k) => {
        acc[k] = state.planDetails[k]?.status || 'need_check';
        return acc;
      }, {}),
  ...
  function getDaNangDiurnalContext() {
    ...
    if (typeof window !== 'undefined' && window.__mockDayOfWeekNum !== undefined) {
      dayOfWeekNum = Number(window.__mockDayOfWeekNum);
      const revMap = { 1: 'Mon', 2: 'Tue', 3: 'Wed', 4: 'Thu', 5: 'Fri', 6: 'Sat', 7: 'Sun' };
      dayOfWeek = revMap[dayOfWeekNum] || 'Thu';
    }
    ...
    try {
      if (isHttpRuntime()) {
        ...
        dataStore.deals = validDeals.map(d => {
          ...
          const origPrice = Number(d.original_price) || Number(rawPrice);
          const dealPrice = Number(rawPrice) || 0;
          return {
            ...d,
            price_num: dealPrice,
            saving_vnd: Math.max(0, origPrice - dealPrice),
            is_local: ['local_food', 'local_beverage', 'local_entertainment'].includes(d.category),
            is_online: ['ecommerce_shopee', 'ecommerce_lazada', 'ecommerce_tiktok'].includes(d.category),
  ...
  function matchProductTab(deal, tab) {
    if (tab === 'local') return deal.is_local;
    if (tab === 'online') return deal.is_online;
    return true;
  }
  ...
  function evaluateFilteredDeals() {
    ...
    return dataStore.deals.filter(d => {
      // Isolate Product Tab: Local vs Online
      if (!matchProductTab(d, state.activeProductTab)) return false;
      ...
      // When in rightNowMode, MUST pass current minute and day of week
      if (state.rightNowMode) {
        if (!isCurrentlyWithinTiming(d, ctx.nowMinutes, ctx.dayOfWeekNum)) return false;
      } else {
  ...
  function evaluateDecisionMatrix() {
    if (!dataStore.isLoaded || !dataStore.deals) return null;
    const ctx = getDaNangDiurnalContext();

    // 6-condition filter on candidate pool
    const candidates = dataStore.deals.filter(d => {
      if (!matchPersona(d, state.persona)) return false;
      if (!matchZone(d, state.selectedZone)) return false;
      if (!matchBudget(d, state.budgetTier)) return false;
      if (!matchDuration(d, state.durationMins)) return false;
    ...
    // Rank: 1 Top Pick + up to 2 Alternatives
    candidates.sort((a, b) => {
      const aScore = (state.todayPlan.includes(a.deal_id) ? 3 : 0) + (a.discount_pct || 0);
      const bScore = (state.todayPlan.includes(b.deal_id) ? 3 : 0) + (b.discount_pct || 0);
    ...
    return {
      ...
      rankingReason: `Tối ưu tốt nhất cho mức giá ${money(topPick.price_num)}, thời lượng ~${topPick.duration_mins || 30}p và đang trong khung giờ mở cửa tại Đà Nẵng lúc ${ctx.timeStr}.`,
      unverifiedConditions: topPick.disclosure || 'Chương trình tham khảo theo trang chủ. Cần hỏi lại điều kiện áp dụng tại quầy/app trước khi gọi món.'
    };
  }
  ...
  function evaluateRecommendations() {
    ...
    const candidateDeals = dataStore.deals.filter(d => {
      if (!matchPersona(d, state.persona)) return false;
      if (!matchZone(d, state.selectedZone)) return false;
      if (!matchBudget(d, state.budgetTier)) return false;
      if (!matchDuration(d, state.durationMins)) return false;
  ...
  function css() {
    ...
    /* TWO PRODUCT TABS (ĐỊA PHƯƠNG vs ONLINE) */
    .apex-product-tabs {
      display: flex; gap: 12px; border-bottom: 2px solid var(--apex-line); padding-bottom: 12px;
    }
    .apex-prod-tab-btn {
      min-height: 48px; padding: 0 20px; border-radius: 12px; border: 1px solid var(--apex-line);
      background: rgba(255,255,255,.04); color: #c6d0ca; font-weight: 800; font-size: 14px;
      cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: all .2s;
    }
    .apex-prod-tab-btn:hover, .apex-prod-tab-btn.active {
    ...
    /* INTERACTIVE DECISION FLOW (QUICK DECISION STEPPER) */
    .apex-decision-box {
      background: linear-gradient(145deg, #0d2820, #061a14); border: 1px solid var(--apex-gold);
      border-radius: 20px; padding: 22px; display: grid; gap: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.5);
    }
    .apex-stepper-head { display: flex; justify-content: space-between; align-items: baseline; flex-wrap: wrap; gap: 8px; }
    .apex-stepper-title { font-weight: 900; font-size: 17px; color: var(--apex-gold); display: flex; align-items: center; gap: 8px; }
    .apex-stepper-scenarios { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 12px; }
    ...
    /* BEST RESULT FOR YOU (TOP DECISION MATRIX) */
    .apex-best-result-card {
      background: linear-gradient(145deg, #10382c, #061a14); border: 2px solid var(--apex-gold);
      border-radius: 18px; padding: 22px; display: grid; gap: 14px; margin-top: 10px;
    }
    .apex-top-pick-badge {
      display: inline-flex; align-items: center; gap: 6px; background: var(--apex-gold); color: #061a14;
      padding: 4px 12px; border-radius: 999px; font-weight: 900; font-size: 12px; letter-spacing: .04em;
    }
    .apex-decision-reason-box {
      background: rgba(0,0,0,.4); border-left: 4px solid var(--apex-gold); border-radius: 8px;
      padding: 10px 14px; font-size: 12.5px; color: #f6f4ed; line-height: 1.45;
    }
    .apex-unverified-box {
      background: rgba(248,113,113,.1); border: 1px solid rgba(248,113,113,.3); border-radius: 8px;
      padding: 10px 14px; font-size: 12px; color: #fca5a5; line-height: 1.4;
    }
    .apex-alt-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 12px; margin-top: 8px; }
    .apex-alt-card {
      background: #061a14; border: 1px solid var(--apex-line); border-radius: 12px; padding: 12px;
    ...
    .apex-planner-status-select {
      min-height: 36px; width: 100%; background: #09211a; border: 1px solid var(--apex-line); color: #fff;
      padding: 4px 8px; border-radius: 8px; font-size: 11px; font-weight: 700; outline: 0; cursor: pointer;
    }
    .apex-planner-note-input {
      width: 100%; background: #061a14; border: 1px solid rgba(255,255,255,.1); color: #fff;
      padding: 6px 10px; border-radius: 8px; font-size: 11px; outline: 0;
    }

    /* SAVINGS LEDGER (SỔ TIẾT KIỆM CÁ NHÂN) */
    .apex-ledger-box {
      background: linear-gradient(145deg, #103328, #061a14); border: 1px solid var(--apex-gold);
      border-radius: 20px; padding: 22px; display: grid; gap: 14px;
    }
    .apex-ledger-head { display: flex; justify-content: space-between; align-items: baseline; flex-wrap: wrap; gap: 8px; }
    .apex-ledger-title { font-weight: 900; font-size: 17px; color: var(--apex-gold); }
    .apex-ledger-metrics { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px; }
    .apex-ledger-card {
      background: rgba(0,0,0,.4); border: 1px solid var(--apex-line); border-radius: 14px; padding: 14px;
  ...
  function renderProductTabs() {
    ...
    <div class="apex-product-tabs">
      <button class="apex-prod-tab-btn ${state.activeProductTab==='local'?'active':''}" data-apex="switch-product-tab" data-tab="local">
        <span>📍</span> Địa Phương Đà Nẵng 43 (Ăn Uống, Cafe, Phim)
      </button>
      <button class="apex-prod-tab-btn ${state.activeProductTab==='online'?'active':''}" data-apex="switch-product-tab" data-tab="online">
        <span>📦</span> Sàn Online & Săn Mã (Shopee, Lazada, TikTok)
      </button>
    </div>`;
  }
  ...
  function renderPersonalBrief() {
    ...
    const totalSaved = money(state.savingsLedger.totalSavedVnd);
    ...
    <section class="apex-daily-brief" id="apex-personal-brief">
      ...
      <div class="apex-brief-grid">
        ...
        <div class="apex-brief-pill">
          <span class="apex-brief-label">Kế Hoạch Hôm Nay</span>
          <span class="apex-brief-val" style="color:var(--apex-gold);">${planCount}/3 Mục Đã Lập (Reset 00:00)</span>
        </div>
        <div class="apex-brief-pill">
          <span class="apex-brief-label">Sổ Tiết Kiệm Đã Dùng</span>
          <span class="apex-brief-val" style="color:#10b981;">💰 ${totalSaved}</span>
        </div>
      </div>
    </section>`;
  }

  function renderDecisionFlowSection() {
    const matrix = evaluateDecisionMatrix();
    const ctx = getDaNangDiurnalContext();

    return `
    <section class="apex-decision-box" id="apex-decision-flow">
      <div class="apex-stepper-head">
        <div class="apex-stepper-title">
          <span>🎯 BỘ CÔNG CỤ RA QUYẾT ĐỊNH CHI TIÊU NHANH (<= 3 THAO TÁC)</span>
        </div>
        <span style="font-size:12px;color:var(--apex-muted);">Chọn tình huống để JayT tính toán phương án tối ưu:</span>
      </div>

      <div class="apex-stepper-scenarios">
        <button class="apex-intent-btn ${state.viewMode==='intent_student'?'active':''}" data-apex="quick-intent" data-intent="student">
          <div class="apex-intent-title">🎓 1. Sinh Viên Giữa 2 Tiết</div>
          <div class="apex-intent-desc">Còn dưới 50K, cần phương án ăn/uống 30p gần campus Bách Khoa / Sư Phạm.</div>
        </button>
        <button class="apex-intent-btn ${state.viewMode==='intent_office'?'active':''}" data-apex="quick-intent" data-intent="office">
          <div class="apex-intent-title">💼 2. Nghỉ Trưa Văn Phòng 45p</div>
          <div class="apex-intent-desc">45 phút nghỉ trưa, cơm văn phòng Hải Châu tiết kiệm dưới 50K.</div>
        </button>
        <button class="apex-intent-btn ${state.viewMode==='intent_group'?'active':''}" data-apex="quick-intent" data-intent="group">
          <div class="apex-intent-title">👥 3. Đi Chơi Nhóm Cuối Tuần</div>
      ...
      ${matrix ? `
      <div class="apex-best-result-card" id="apex-best-decision-result">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
          <span class="apex-top-pick-badge">🏆 LỰA CHỌN ƯU TIÊN SỐ 1 CHO BẠN</span>
          <span style="font-size:11.5px;color:#10b981;font-weight:800;">⏰ Khớp giờ Đà Nẵng: ${esc(ctx.timeStr)}</span>
        </div>

        <div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:8px;">
          <div>
            <h3 style="margin:0 0 4px;font-size:18px;color:#fff;">${esc(matrix.topPick.merchant)}</h3>
            <div style="font-size:12px;color:var(--apex-gold);font-weight:700;">${esc(matrix.topPick.title)}</div>
          </div>
          <div style="text-align:right;">
            <div style="font-size:22px;font-weight:900;color:#fff;">${money(matrix.topPick.price_num)} <small style="font-size:11px;color:var(--apex-gold);">(Tham khảo)</small></div>
            ${matrix.topPick.saving ? `<span class="apex-save-pill">${esc(matrix.topPick.saving)}</span>` : ''}
          </div>
        </div>

        <div class="apex-decision-reason-box">
          💡 <b>Lý do xếp hạng:</b> ${esc(matrix.rankingReason)}
        </div>

        <div class="apex-unverified-box">
          ⚠️ <b>Điều kiện chưa xác minh độc lập:</b> ${esc(matrix.unverifiedConditions)}
        </div>

        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          <button class="hunt" style="flex:2;min-height:44px;background:linear-gradient(135deg,#10b981,#059669);color:#fff;border:0;border-radius:10px;font-weight:900;cursor:pointer;" data-apex="hunt" data-id="${matrix.topPick.deal_id.toLowerCase().includes('metiz')?'metiz':matrix.topPick.deal_id}">
            🎯 Xem Chi Tiết & Mở Nguồn
          </button>
          <button class="plan-btn" style="flex:1.2;min-height:44px;background:rgba(212,175,55,.18);color:var(--apex-gold);border:1px solid var(--apex-gold);border-radius:10px;font-weight:900;cursor:pointer;" data-apex="toggle-plan" data-id="${matrix.topPick.deal_id}">
            ➕ Đưa Vào Kế Hoạch Hôm Nay
          </button>
        </div>

        ${matrix.alternatives.length > 0 ? `
        <div style="margin-top:10px;border-top:1px dashed rgba(255,255,255,.1);padding-top:10px;">
          <div style="font-size:12px;font-weight:800;color:var(--apex-gold-light);margin-bottom:6px;">🥈 2 LỰA CHỌN THAY THẾ KHẢ DỤNG KHÁC:</div>
          <div class="apex-alt-grid">
            ${matrix.alternatives.map(alt => `
            <div class="apex-alt-card">
              <div style="font-size:11px;font-weight:900;color:var(--apex-gold);">${esc(alt.brand || alt.merchant)}</div>
              <div style="font-size:13px;font-weight:800;color:#fff;">${esc(alt.merchant)}</div>
              <div style="font-size:15px;font-weight:900;color:#fff;">${money(alt.price_num)}</div>
              <button style="min-height:36px;background:rgba(255,255,255,.08);color:#fff;border:0;border-radius:8px;font-size:11px;font-weight:800;cursor:pointer;" data-apex="hunt" data-id="${alt.deal_id.toLowerCase().includes('metiz')?'metiz':alt.deal_id}">
                Xem Phương Án Này →
              </button>
            </div>`).join('')}
          </div>
        </div>` : ''}
      </div>` : (state.rightNowMode ? `
      <div class="apex-unverified-box" style="margin-top:10px;background:#0d2820;border-color:rgba(255,255,255,.15);color:var(--apex-muted);">
        🕒 <b>Chưa có phương án khớp 100% lúc ${esc(ctx.timeStr)}:</b> Không có nguồn ưu đãi nào đang trong khung giờ mở thỏa mãn đồng thời cả 6 điều kiện bạn chọn. Hãy thử lại vào khung giờ tiếp theo hoặc đặt lại bộ lọc.
      </div>` : '')}
    </section>`;
  }

  function renderSavingsLedgerSection() {
    const ledger = state.savingsLedger;
    return `
    <section class="apex-ledger-box" id="apex-savings-ledger">
      <div class="apex-ledger-head">
        <div class="apex-ledger-title">
          <span>📒 SỔ TIẾT KIỆM CÁ NHÂN (LƯU CỤC BỘ TRÊN THIẾT BỊ)</span>
        </div>
        <span style="font-size:12px;color:var(--apex-muted);">Chỉ ghi nhận khi bạn chủ động bấm "Đã dùng"</span>
      </div>

      <div class="apex-ledger-metrics">
        <div class="apex-ledger-card">
          <span class="apex-brief-label">Tổng Tiền Tiết Kiệm Tham Khảo</span>
          <span style="font-size:22px;font-weight:900;color:#10b981;">💰 ${money(ledger.totalSavedVnd)}</span>
        </div>
        <div class="apex-ledger-card">
          <span class="apex-brief-label">Số Kèo Đã Dùng Thành Công</span>
          <span style="font-size:22px;font-weight:900;color:var(--apex-gold);">${ledger.claimedDeals.length} Lần</span>
        </div>
        <div class="apex-ledger-card">
          <span class="apex-brief-label">Số Nguồn Đã Mở Tham Khảo</span>
          <span style="font-size:22px;font-weight:900;color:#fff;">${ledger.openedHistory.length} Nguồn</span>
        </div>
      </div>

      ${ledger.claimedDeals.length > 0 ? `
      <div style="font-size:12px;color:var(--apex-muted);margin-top:8px;">
        <b>Nhật ký gần đây:</b> ${ledger.claimedDeals.slice(-3).reverse().map(c => `${esc(c.merchant)} (+${money(c.savedVnd)})`).join(' • ')}
      </div>` : '<div style="font-size:12px;color:var(--apex-muted);">Chưa có ghi nhận nào. Khi bạn dùng một ưu đãi và bấm "Đã Dùng", số tiền tham khảo sẽ tự động tích lũy tại đây.</div>'}
    </section>`;
  }
  ...
  function renderRecommendationSection() {
    ...
    if (recItems.length === 0) {
      ...
      <section class="apex-rec-box" id="apex-rec-box">
        ...
        <p style="font-size:12.5px;color:var(--apex-muted);margin:0;">
          🕒 Chưa có nguồn ưu đãi nào đang trong khung giờ vàng lúc <b>${esc(ctx.timeStr)}</b> khớp với toàn bộ điều kiện lọc bạn đã chọn. Vui lòng thử lại vào khung giờ tiếp theo hoặc đặt lại bộ lọc.
        </p>
      </section>`;
    }
    ...
    <section class="apex-schedule-box" id="apex-today-schedule">
      <div class="apex-schedule-head">
        <div class="apex-schedule-title">
          <span>📋 CÔNG CỤ LẬP KẾ HOẠCH HÔM NAY (${esc(ctx.dateStr)})</span>
        </div>
        <div class="apex-schedule-progress">
          Tiến độ: <b>${selectedCount}/3</b> thời điểm đã lập
        </div>
      </div>
        ...
        <div class="apex-slot-card ${ctx.slotKey === 'morning' ? 'current-active' : ''}" data-slot="morning">
          ...
          <div class="apex-slot-deals">
            ${planSlots.morning.length > 0 ? planSlots.morning.map(d => {
              ...
              const detail = state.planDetails[d.deal_id] || { status: 'need_check', note: '' };
              return `
              <div class="apex-mini-deal">
                ...
                ${inPlan ? `
                <select class="apex-planner-status-select" data-apex="change-plan-status" data-id="${d.deal_id}">
                  <option value="need_check" ${detail.status==='need_check'?'selected':''}>🔍 Trạng thái: Cần kiểm tra</option>
                  <option value="opened" ${detail.status==='opened'?'selected':''}>🌐 Trạng thái: Đã mở nguồn</option>
                  <option value="claimed" ${detail.status==='claimed'?'selected':''}>💰 Trạng thái: Đã dùng thành công</option>
                  <option value="not_fit" ${detail.status==='not_fit'?'selected':''}>⏭️ Trạng thái: Không phù hợp</option>
                </select>
                <input class="apex-planner-note-input" type="text" placeholder="Ghi chú local (VD: Hẹn 8h tại Bạch Đằng)..." value="${esc(detail.note || '')}" data-apex="change-plan-note" data-id="${d.deal_id}" />
                ` : ''}
                ...
                <div class="apex-mini-deal-actions">
                  ...
                  <button style="flex:1;background:${inPlan?'#10b981':'rgba(255,255,255,.1)'};color:#fff;" data-apex="toggle-plan" data-id="${d.deal_id}">
                    ${inPlan ? '✔️ Đã Trong Lịch' : '➕ Thêm Vào Lịch'}
                  </button>
                </div>
              </div>`;
          ...
          <div class="apex-slot-deals">
            ${planSlots.lunch.length > 0 ? planSlots.lunch.map(d => {
              ...
              <div class="apex-mini-deal">
                ...
                <input class="apex-planner-note-input" type="text" placeholder="Ghi chú local (VD: 45p ăn trưa nhóm 3 người)..." value="${esc(detail.note || '')}" data-apex="change-plan-note" data-id="${d.deal_id}" />
                ` : ''}
          ...
          <div class="apex-slot-deals">
            ${planSlots.evening.length > 0 ? planSlots.evening.map(d => {
              ...
              <div class="apex-mini-deal">
                ...
                <input class="apex-planner-note-input" type="text" placeholder="Ghi chú local (VD: Xem phim suất 19h45)..." value="${esc(detail.note || '')}" data-apex="change-plan-note" data-id="${d.deal_id}" />
                ` : ''}
  ...
  function render() {
    ...
    if (!isHttpRuntime()) {
      ...
      <div class="apex-safe-card" style="border-color:rgba(255,255,255,.1);background:#0d2820;">
        ...
        <p style="color:var(--apex-muted);">
          ${state.rightNowMode
            ? `Hiện không có nguồn ưu đãi nào đang trong khung giờ mở lúc <b>${esc(ctx.timeStr)}</b> khớp với toàn bộ điều kiện lọc bạn đã chọn.`
            : 'Không có nguồn ưu đãi nào khớp với bộ lọc hoặc từ khóa tìm kiếm hiện tại.'}
        </p>
    ...
    <div class="apex-layout">
      ...
      <aside class="apex-side">
        <div class="apex-brand">
          ...
          <div>
            <b>JAYT ĐÀ NẴNG</b>
            <small>CÔNG CỤ QUYẾT ĐỊNH 43</small>
          </div>
        </div>

        <nav class="apex-nav">
          <button class="${state.viewMode==='all' && !state.rightNowMode?'active':''}" data-apex="nav" data-view="all" data-persona="all">
          ...
          <button class="${state.viewMode==='ledger'?'active':''}" data-apex="nav" data-view="ledger" data-persona="all">
            <span>📒 Sổ Tiết Kiệm</span>
            <small style="color:var(--apex-gold-light);font-weight:900;">${money(state.savingsLedger.totalSavedVnd)}</small>
          </button>
        </nav>
          ...
          <section class="apex-section">
            <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:14px;">
              <h3 style="margin:0;font-size:17px;color:var(--apex-gold);font-weight:900;">
                ${state.viewMode==='saved' ? '⭐ KÈO BẠN ĐÃ LƯU' : state.viewMode==='plan' ? '📋 KẾ HOẠCH SĂN KÈO HÔM NAY' : state.activeProductTab==='online' ? '📦 SÀN ONLINE & SĂN MÃ — LIÊN KẾT TIẾP THỊ MINH BẠCH' : '🛍️ DANH MỤC ĐANG KHẢO SÁT — KIỂM TRA ĐIỀU KIỆN TẠI NGUỒN TRƯỚC KHI DÙNG'}
              </h3>
    ...
    <div class="apex-mobile-nav-bar">
      <button class="apex-mobile-nav-item ${state.viewMode==='all' && !state.rightNowMode?'active':''}" data-apex="nav" data-view="all" data-persona="all">
        <span>🌟</span>
        <span>Khám Phá</span>
      </button>
    ...
    <div class="apex-modal" id="apex-active-modal">
      <div class="apex-modal-card">
        ...
        <div class="apex-feedback-local-notice">
          🛡️ <b>Kiểm tra điều kiện tại nguồn:</b> ${esc(d.disclosure || 'Chương trình tham khảo theo thông tin trang chủ. Vui lòng kiểm tra điều kiện áp dụng tại điểm bán / ứng dụng đối tác.')}
        </div>
  ...
  async function handleEvent(e) {
    ...
    if (action === 'switch-product-tab') {
      state.activeProductTab = tab || 'local';
      state.persona = 'all';
      state.timeSlot = 'all';
      ...
      state.rightNowMode = false;
      state.viewMode = 'all';
      state.query = '';
      render();
    } else if (action === 'quick-intent') {
      state.rightNowMode = true;
      if (intent === 'student') {
      ...
      // Record to savingsLedger topNeedsCount
      state.savingsLedger.topNeedsCount[intent] = (state.savingsLedger.topNeedsCount[intent] || 0) + 1;
      saveSavingsLedgerToStorage();
      render();
    } else if (action === 'toggle-right-now') {
      state.rightNowMode = !state.rightNowMode;
      render();
      ...
      if (idx > -1) {
        ...
        state.planDetails[id] = { slot: 'lunch', status: 'need_check', note: '' };
        showToast('📋 Đã thêm vào kế hoạch hôm nay!');
      }
      saveTodayPlanToStorage();
      render();
    } else if (action === 'change-plan-status') {
      const newStatus = target.value;
      if (state.planDetails[id]) {
        const oldStatus = state.planDetails[id].status;
        state.planDetails[id].status = newStatus;
        
        // P005: When user actively marks 'claimed', compute savings into ledger
        if (newStatus === 'claimed' && oldStatus !== 'claimed') {
          const dealObj = dataStore.deals.find(d => d.deal_id === id);
          if (dealObj) {
            const savedVnd = dealObj.saving_vnd || 0;
            state.savingsLedger.totalSavedVnd += savedVnd;
            state.savingsLedger.claimedDeals.push({
              deal_id: id,
              merchant: dealObj.merchant,
              savedVnd: savedVnd,
              claimedAt: new Date().toISOString()
            });
            saveSavingsLedgerToStorage();
            showToast(`🎉 Đã ghi nhận tiết kiệm tham khảo +${money(savedVnd)} vào Sổ Cá Nhân!`);
          }
        }
        saveTodayPlanToStorage();
        render();
      }
    } else if (action === 'change-plan-note') {
      const newNote = target.value;
      if (state.planDetails[id]) {
        state.planDetails[id].note = newNote;
        saveTodayPlanToStorage();
      }
    } else if (action === 'hunt') {
      ...
      // Record outbound open into savings ledger history
      if (targetDeal) {
        state.savingsLedger.openedHistory.push({
          deal_id: targetDeal.deal_id,
          merchant: targetDeal.merchant,
          url: targetDeal.source_url,
