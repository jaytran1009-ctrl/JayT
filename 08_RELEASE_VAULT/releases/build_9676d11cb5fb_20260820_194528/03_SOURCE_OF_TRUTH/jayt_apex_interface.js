/* =============================================================================
   ...
   Chỉ thị: JAYT-COMMUNITY-RETENTION-001 (Real-Time Diurnal Recommendation • Zero-PII)
   ============================================================================= */
(function () {
  'use strict';
  ...
  function getClientDiurnalContext(mockMinutes = null) {
    let nowMinutes = 0;
    let timeStr = '12:00';
    ...
    let slotName = 'Tối';
    let slotIcon = '🎬';
    let vibeText = 'Khung giờ vàng giải trí, xem phim & tụ tập bạn bè tại Helio / Vincom Sơn Trà.';
    ...
    if (nowMinutes >= 420 && nowMinutes < 660) {
      ...
      slotName = 'Trưa';
      slotIcon = '☀️';
      vibeText = 'Khung giờ nạp năng lượng & nghỉ trưa công sở / campus sinh viên.';
    } else if (nowMinutes >= 840 && nowMinutes < 1050) {
      slotKey = 'afternoon';
      slotName = 'Chiều';
      slotIcon = '🧋';
      vibeText = 'Khung giờ trà sữa, tán gẫu & giờ tan trường / tan sở Điện Biên Phủ.';
    } else {
      slotKey = 'evening';
      slotName = 'Tối';
      slotIcon = '🎬';
      vibeText = 'Khung giờ vàng giải trí, xem phim & tụ tập bạn bè tại Helio / Vincom Sơn Trà.';
    }

    return { nowMinutes, timeStr, slotKey, slotName, slotIcon, vibeText };
  }
  ...
  function evaluateRecommendations() {
    ...
    const ctx = getClientDiurnalContext();
    ...
    // 1. First priority: deals active RIGHT NOW (start_minutes <= now < end_minutes)
    const activeNowDeals = candidateDeals.filter(d => {
    ...
    if (activeNowDeals.length > 0) {
      return activeNowDeals.map(d => ({ deal: d, isActiveNow: true, isPinned: state.saved.includes(d.deal_id) }));
    }
    ...
    return inSlotDeals.map(d => ({ deal: d, isActiveNow: false, isPinned: state.saved.includes(d.deal_id) }));
  }
  ...
  function css() {
    ...
    .apex-live-pill {
      font-size: 10.5px; font-weight: 900; background: rgba(16,185,129,.2); color: #10b981;
      padding: 3px 8px; border-radius: 6px; border: 1px solid rgba(16,185,129,.4); display: flex; align-items: center; gap: 4px;
    }
    .apex-pinned-pill {
      font-size: 10.5px; font-weight: 900; background: rgba(212,175,55,.2); color: var(--apex-gold);
      padding: 3px 8px; border-radius: 6px; border: 1px solid rgba(212,175,55,.4);
    }
  ...
  function renderRecommendationSection() {
    ...
    <section class="apex-rec-box" id="apex-rec-box">
      <div class="apex-rec-top">
        ...
        <div class="apex-rec-clock">📍 Đà Nẵng 43 • <b>${esc(ctx.timeStr)}</b> (${esc(ctx.slotIcon)} Buổi ${esc(ctx.slotName)})</div>
      </div>
        ...
        ${recItems.map(item => {
          ...
          <div class="apex-rec-card" data-rec-deal-id="${dealDomId}">
            <div class="apex-rec-card-badge-row">
              ${item.isActiveNow ? `<span class="apex-live-pill">🟢 ĐANG ÁP DỤNG NGAY</span>` : `<span class="apex-live-pill" style="color:#d4af37;background:rgba(212,175,55,.15);border-color:rgba(212,175,55,.3);">⏳ SẮP MỞ BUỔI ${esc(ctx.slotName).toUpperCase()}</span>`}
