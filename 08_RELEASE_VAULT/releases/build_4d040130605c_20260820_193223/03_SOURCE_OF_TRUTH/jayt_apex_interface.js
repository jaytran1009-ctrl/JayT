/* =============================================================================
   ...
   Chỉ thị: JAYT-COMMUNITY-VALUE-002 (Numeric Time Boundaries • Pure HTTP API Isolation)
   ============================================================================= */
(function () {
  'use strict';
  ...
  async function handleConfirm(dealId) {
    ...
    console.log('[JAYT-DEBUG] handleConfirm called with dealId:', dealId, 'found deal:', deal?.deal_id, 'isHttp:', isHttpRuntime());
    if (!deal) {
      console.warn('[JAYT-DEBUG] deal not found, aborting handleConfirm');
      return;
    }
    ...
    try {
      console.log('[JAYT-DEBUG] Calling POST /api/token/issue for deal:', deal.deal_id);
      ...
      console.log('[JAYT-DEBUG] Token issue response:', data);
      ...
      if (popup) {
        ...
        console.log('[JAYT-DEBUG] Navigating popup to:', fullOutUrl);
        popup.location.href = fullOutUrl;
      }
    } catch (err) {
      console.error('[JAYT-DEBUG] Token Gateway Error:', err);
