// =============================================================================
// JAYT — TRỢ LÝ TIẾT KIỆM HẰNG NGÀY ĐÀ NẴNG 43 (APEX INTERFACE v10.0)
// Chỉ thị: JAYT-TRUST-FIRST-RETENTION-003 (Strict 6-Condition RightNowMode • Honest Semantics)
// =============================================================================
(function () {
  'use strict';
  ...
  function evaluateRecommendations() {
    ...
    // Apply FULL filter state to candidate deals
    ...
    // 1. Strict Timing: Deals currently active right now (all 6 conditions pass)
    ...
    // P003 Mandate: Zero Fallback to out-of-hours deals in recommendations
    return [];
  }
  ...
  function renderRightNowModule() {
    ...
    <section class="apex-right-now-box" id="apex-right-now-module">
      <div class="apex-right-now-title">
        <span>⚡ NGAY LÚC NÀY TẠI ĐÀ NẴNG ${state.rightNowMode ? '• <small style="color:var(--apex-gold);font-weight:900;">(ĐANG BẬT LỌC ĐỒNG THỜI 6 ĐIỀU KIỆN)</small>' : ''}</span>
      ...
      <div class="apex-right-now-filters">
        <button class="apex-select-pill" style="background:${state.rightNowMode?'var(--apex-gold)':'#061a14'};color:${state.rightNowMode?'#061a14':'#fff'};font-weight:900;" data-apex="toggle-right-now">
          ${state.rightNowMode ? '🟢 Lọc Đang Diễn Ra (Bật)' : '⚡ Xem Kèo Ngay Lúc Này'}
        </button>
    ...
    if (recItems.length === 0) {
      ...
      <section class="apex-rec-box" id="apex-rec-box">
        ...
        <p style="font-size:12.5px;color:var(--apex-muted);margin:0;">
          🕒 Chưa có nguồn ưu đãi nào đang trong khung giờ vàng lúc <b>${esc(ctx.timeStr)}</b> khớp với toàn bộ điều kiện lọc bạn đã chọn (đối tượng, khu vực, ngân sách, thời lượng, nhu cầu). Vui lòng thử lại vào khung giờ tiếp theo hoặc đặt lại bộ lọc.
        </p>
      </section>`;
    }
