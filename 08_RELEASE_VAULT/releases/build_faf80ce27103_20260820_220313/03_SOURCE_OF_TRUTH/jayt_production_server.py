...
# 2b. IMMUTABLE RUNTIME REQUEST AUDIT LEDGER
...
class JayTComprehensiveHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 6b. DEALS FEED, EVIDENCE & ZONE DATA ADAPTER ENDPOINT (/api/deals) — FAIL-CLOSED
            ...
            zone_file = os.path.join(get_vault_root(), "05_DEAL_AND_AFFILIATE", "zone_catalog.json")
                ...
                try:
                    with open(zone_file, "r", encoding="utf-8-sig") as zf:
                        ...
                        if not isinstance(raw_zones, list) or len(raw_zones) == 0:
                            errors.append("ERR_ZONE_CATALOG_EMPTY_OR_NOT_LIST")
                        else:
                            for z in raw_zones:
            ...
            if not errors:
                # Validate each deal and cross-reference evidence & zones
                for d in raw_deals:
                    ...
                    # Zone referential integrity check
                    d_zone = d.get("zone")
                    if not d_zone or not isinstance(d_zone, str):
                        errors.append(f"ERR_DEAL_MISSING_ZONE: {did}")
                    elif d_zone not in valid_zone_ids:
                        errors.append(f"ERR_DEAL_ZONE_NOT_IN_CATALOG: {did} -> {d_zone}")

                    # Numeric time slot minutes validation
                    sm = d.get("start_minutes")
                    em = d.get("end_minutes")
                    if not isinstance(sm, int) or not isinstance(em, int) or sm < 0 or em > 1440 or sm >= em:
                        errors.append(f"ERR_DEAL_INVALID_TIME_MINUTES: {did} -> start={sm}, end={em}")
                    
                    # Check end-of-day boundary precision for deals stating 23:59
                    bt = str(d.get("best_time", ""))
                    if "23:59" in bt and em != 1440:
                        errors.append(f"ERR_DEAL_END_OF_DAY_BOUNDARY_MISMATCH: {did} -> best_time={bt}, end_minutes={em} (Must be 1440 for half-open [start, 1440))")
