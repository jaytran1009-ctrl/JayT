...
def make_true_tls_ip_pinned_request(url, timeout=5):
    """
    TRUE TLS-SNI IP PINNING VIA RAW SOCKET + SSL CONTEXT WRAP:
    ...
    # 1. Connect raw socket to pinned IP
    raw_sock = socket.create_connection((pinned_ip, port), timeout=timeout)
    
    # 2. TLS wrap with server_hostname = hostname (SNI & Cert verified against hostname)
