# -*- coding: utf-8 -*-
"""
JAYT APEX PRODUCTION SERVER — STRICT TLS-SNI IP PINNING & CATALOG POLICY
=============================================================================
...
6. /__test/slow: Internal test endpoint for true in-flight graceful drain testing.
=============================================================================
"""
import os
import sys
...
class JayTComprehensiveHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 3. INTERNAL TEST SLOW ENDPOINT (STRICTLY TEST-GATED)
        elif parsed.path == "/__test/slow":
            if os.environ.get("JAYT_ENABLE_TEST_ENDPOINTS") != "true":
                self.send_error(404, "Test Endpoints Disabled")
                return
            qs = parse_qs(parsed.query)
            delay_ms = int(qs.get("ms", ["600"])[0])
            delay_sec = max(0.01, min(delay_ms / 1000.0, 5.0))
