...
2. /readyz: Strict Traffic Readiness probe:
   ...
   - Build Manifest & Physical File Hash Integrity Verification.
...
def verify_build_manifest_integrity():
    """KIỂM TOÁN TÍNH TOÀN VẸN CỦA BUILD MANIFEST & FILE DIGESTS"""
    vault_root = get_vault_root()
    manifest_file = os.path.join(vault_root, "BUILD_MANIFEST.json")
    if not os.path.exists(manifest_file):
        manifest_file = os.path.join(vault_root, "08_RELEASE_VAULT", "RELEASE_MANIFEST.json")
    ...
    if isinstance(files_list, list):
        for file_entry in files_list:
            rel_path = file_entry.get("relative_path", "").lstrip("\\/")
            expected_hash = file_entry.get("sha256", "").lower()
            target_path = os.path.join(vault_root, rel_path)
            if os.path.exists(target_path) and expected_hash:
                try:
                    with open(target_path, "rb") as pf:
                        actual_hash = hashlib.sha256(pf.read()).hexdigest().lower()
                    if actual_hash != expected_hash:
                        return False, build_id, manifest_hash, [f"ERR_FILE_DIGEST_MISMATCH: {rel_path}"]
                except Exception as e:
                    return False, build_id, manifest_hash, [f"ERR_READING_FILE: {rel_path} ({str(e)})"]
...
class JayTComprehensiveHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 3. READINESS PROBE (/readyz) — STRICT FAIL-CLOSED (Secret + Full Catalog + Manifest Integrity)
            ...
            # Check 3: Build manifest & file integrity validation
            ...
            if not_ready_reasons:
                ...
                ready_data = {
                    ...
                    "resolver_engine": "TRUE_TLS_SNI_IP_PINNED_v6_FULL_CATALOG_MANIFEST_GATED"
                }
                self.wfile.write(json.dumps(ready_data, ensure_ascii=False, indent=2).encode("utf-8"))
                return
