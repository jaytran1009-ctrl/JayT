...
(function () {
  ...
  const STATIC_BASELINE_EVIDENCE = {
    "EVID_METIZ_20260820_WEB_AUDIT": {
      "deal_id": "DNG-METIZ-45K",
      "source_url": "https://metiz.vn/",
      "evidence_type": "OFFICIAL_PARTNER_POLICY",
      "checked_at": "2026-08-20T10:00:00+07:00",
      "sha256_hash": "2f42be9ec56f3f01c801e0a29486c8f9db1dd8256a4697920ab0b08055627263"
    },
    "EVID_CGV_20260820_WEB_AUDIT": {
      "deal_id": "DNG-CGV-55K",
      "source_url": "https://www.cgv.vn/",
      "evidence_type": "OFFICIAL_PROMOTION_PORTAL",
      "checked_at": "2026-08-20T10:00:00+07:00",
      "sha256_hash": "060d4b9b940984ee4fa41893c52e850b535ff4591a27e8d64190c74f51e06915"
    },
    "EVID_HIGHLANDS_20260820_WEB_AUDIT": {
      "deal_id": "DNG-HL-BK-19K",
      "source_url": "https://www.highlandscoffee.com.vn/",
      "evidence_type": "OFFICIAL_PROMOTION_PORTAL",
      "checked_at": "2026-08-20T10:00:00+07:00",
      "sha256_hash": "a4d9c7cb32103f6f96be4b8417c88b22a6136d8194ad699e1ee10df6ae578508"
    },
    "EVID_KATINAT_20260820_WEB_AUDIT": {
      "deal_id": "DNG-KAT-BK-PROBING",
      "source_url": "https://katinat.vn/",
      "evidence_type": "OFFICIAL_PROMOTION_PORTAL",
      "checked_at": "2026-08-20T10:00:00+07:00",
      "sha256_hash": "ff2262d1a3c61ae7d06e2c340d8aaefba4dbbb91a6299b806d6a5c2d33aa3824"
    }
  };

  const STATIC_BASELINE_DEALS = [
    {
      "deal_id": "DNG-METIZ-45K",
      ...
      "saving_num": 35000,
      "best_time": "Thứ 4 Vui Vẻ",
      "taxonomy": "VERIFIED",
      "source_type": "AFFILIATE_REVENUE_SHARE",
      "affiliate_program_id": "METIZ_AFFILIATE_DIRECT_2026",
      "source_url": "https://metiz.vn/",
      "evidence_ref": "EVID_METIZ_20260820_WEB_AUDIT",
      "disclosure": "Giá vé đồng giá 45.000đ vào ngày Thứ 4 hàng tuần cho thành viên Metiz.",
      "code": "METIZWED45K"
    },
    {
      "deal_id": "DNG-CGV-55K",
      ...
      "saving_num": 40000,
      "best_time": "Thứ 2 Cuối Tháng",
      "taxonomy": "VERIFIED",
      "source_type": "AFFILIATE_REVENUE_SHARE",
      "affiliate_program_id": "CGV_AFFILIATE_DIRECT_2026",
      "source_url": "https://www.cgv.vn/",
      "evidence_ref": "EVID_CGV_20260820_WEB_AUDIT",
      "disclosure": "Giá vé 55.000đ dành cho thành viên U22 và Culture Day.",
      "code": "CGVCULTURE55"
    },
    {
      "deal_id": "DNG-HL-BK-19K",
      "merchant": "Highlands Coffee Bách Khoa",
      ...
      "taxonomy": "VERIFIED",
      "source_type": "AFFILIATE_REVENUE_SHARE",
      "affiliate_program_id": "HL_AFFILIATE_DIRECT_2026",
      "source_url": "https://www.highlandscoffee.com.vn/",
      "evidence_ref": "EVID_HIGHLANDS_20260820_WEB_AUDIT",
      "disclosure": "Combo Phin Sữa Tươi Cà Phê sáng sớm giá 19.000đ khi đặt trước.",
      "code": "HLBK19K"
    },
    {
      "deal_id": "DNG-KAT-BK-PROBING",
      "merchant": "Katinat Nguyễn Văn Linh",
      ...
      "taxonomy": "PROBING",
      "source_type": "DIRECT_COMMUNITY_AUDIT",
      "source_url": "https://katinat.vn/",
      "evidence_ref": "EVID_KATINAT_20260820_WEB_AUDIT",
      "disclosure": "Ưu đãi khảo sát giảm 35% cho sinh viên mang thẻ.",
      "code": "KATNVL39K"
    }
  ];
  ...
  async function loadSourceOfTruthData() {
    try {
      if (isHttp) {
        ...
        // file:// Protocol: Dùng static baseline đã kiểm chứng, nghiêm cấm điều hướng outbound
        const validDeals = STATIC_BASELINE_DEALS.filter(d => validateDealItem(d, STATIC_BASELINE_EVIDENCE));
        dataStore.deals = validDeals;
        dataStore.evidence = STATIC_BASELINE_EVIDENCE;
        dataStore.isLoaded = true;
        dataStore.loadError = null;
      }
    } catch (err) {
  ...
  function evaluateLiveDeals(profile) {
    ...
    if (window.JayTEligibilityEngine && typeof window.JayTEligibilityEngine.executeEligibilityGate === 'function') {
      try {
        const res = window.JayTEligibilityEngine.executeEligibilityGate(dataStore.deals, profile, dataStore.evidence, null);
        if (res && ((res.verified_hero_deals && res.verified_hero_deals.length > 0) || (res.probing_deals && res.probing_deals.length > 0))) {
          return res;
        }
      } catch (e) {
        console.warn('[JAYT-ADAPTER] Engine error:', e);
      }
    }

    return {
      verified_hero_deals: dataStore.deals.filter(d => d.taxonomy === 'VERIFIED'),
      probing_deals: dataStore.deals.filter(d => d.taxonomy === 'PROBING'),
      unverified_deals: dataStore.deals.filter(d => d.taxonomy === 'UNVERIFIED'),
  ...
  async function handleOutboundRedirect(d, targetTab) {
    if (!isHttp) {
    ...
    try {
      const issueResp = await fetch('/api/token/issue', {
        method: 'POST',
        headers: {
      ...
      if (!issueResp.ok) {
        throw new Error('ERR_TOKEN_ISSUE_HTTP_' + issueResp.status);
      }
      ...
      toast('✓ Đã cấp token an toàn (TTL 15s) — Đang chuyển hướng qua cổng bảo mật JayT');
      if (targetTab && !targetTab.closed) {
        targetTab.location.href = tokenData.outbound_endpoint;
      } else {
        window.location.href = tokenData.outbound_endpoint;
      }
    } catch (e) {
      console.error('[JAYT-GATEWAY] Lỗi cấp token an toàn:', e);
      if (targetTab && !targetTab.closed) {
        try { targetTab.close(); } catch (err) {}
      }
  ...
  document.addEventListener('click', e => {
    ...
    if (a === 'confirm' && d) {
      // Synchronously open blank window during user gesture click event
      let targetTab = null;
      if (isHttp) {
        try {
          targetTab = window.open('about:blank', '_blank');
        } catch (err) {
          console.warn('[JAYT-GATEWAY] window.open failed:', err);
        }
      }
      ...
      const m = document.getElementById('apex-active-modal') || b.closest('.apex-modal');
      if (m) m.remove();
      handleOutboundRedirect(d, targetTab);
      render();
    }
    if (a === 'share') {
