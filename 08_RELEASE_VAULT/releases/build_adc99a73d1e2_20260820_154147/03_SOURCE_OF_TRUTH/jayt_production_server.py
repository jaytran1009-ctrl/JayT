...
RAW_SECRET = os.environ.get("JAYT_TOKEN_SECRET", "").strip()
...
def verify_build_manifest_integrity():
    """KIỂM TOÁN TÍNH TOÀN VẸN CỦA BUILD MANIFEST, PHYSICAL FILE DIGESTS & POINTER BINDING"""
    vault_root = os.path.abspath(get_vault_root())
    
    # 1. Pointer Binding & Vault Containment Check
    # Tìm kiếm pointer file ở vault releases hoặc parent releases
    pointer_candidates = [
        os.path.join(vault_root, "08_RELEASE_VAULT", "releases", "active_release_pointer.json"),
        os.path.join(os.path.dirname(vault_root), "active_release_pointer.json"),
        os.path.join(os.path.dirname(vault_root), "08_RELEASE_VAULT", "releases", "active_release_pointer.json"),
        os.path.join(vault_root, "active_release_pointer.json")
    ]
    ...
    pointer_build_id = None
    pointer_build_path = None

    if pointer_file and os.path.exists(pointer_file):
        try:
            ...
            return False, None, None, [f"ERR_CORRUPTED_POINTER_FILE: {str(e)}"]

        if not pointer_build_id or not pointer_build_path:
            return False, None, None, ["ERR_POINTER_MISSING_ACTIVE_BUILD"]

        # Kiểm tra chống Path Traversal và giam hãm an toàn trong thư mục Release Vault
        norm_ptr_path = os.path.abspath(os.path.normpath(pointer_build_path))
        if ".." in pointer_build_path or not ("08_RELEASE_VAULT" in norm_ptr_path and "releases" in norm_ptr_path):
            return False, None, None, [f"ERR_POINTER_PATH_OUTSIDE_VAULT: {pointer_build_path}"]
    ...
    # Đối chiếu Pointer Build ID và Manifest Build ID
    if pointer_build_id and pointer_build_id != build_id:
        return False, build_id, manifest_hash, [f"ERR_POINTER_BUILD_ID_MISMATCH: pointer={pointer_build_id}, manifest={build_id}"]

    # 3. Files List Validation (Fail-Closed 100%)
    ...
    for file_entry in files_list:
        ...
        target_path = os.path.join(vault_root, clean_rel)

        # FAIL CLOSED: File phải tồn tại trên đĩa, KHÔNG ĐƯỢC bỏ qua
