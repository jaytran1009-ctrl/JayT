/* =============================================================================
   ...
   Chỉ thị: JAYT-COMMUNITY-VALUE-004 (Kế Hoạch Hôm Nay • Ngày Mai Có Gì • Báo Deal Cộng Đồng)
   ============================================================================= */
(function () {
  'use strict';
  ...
  const state = {
    ...
    todayPlan: JSON.parse(localStorage.getItem('jayt_today_plan') || '[]'),
  ...
  function css() {
    ...
    .apex-nav button {
      ...
      border-radius: 10px; padding: 11px 13px; text-align: left; cursor: pointer; font-weight: 700; font-size: 13px;
    ...
    .apex-content { max-width: 1400px; margin: auto; padding: 24px clamp(16px, 3vw, 40px) 60px; }
    ...
    .apex-hero {
      ...
      border: 1px solid var(--apex-line); border-radius: 24px; padding: clamp(20px, 3.5vw, 36px);
    ...
    .apex-hero h1 {
      font: 700 clamp(24px, 3.5vw, 44px)/1.15 'Newsreader', Georgia, serif;
      letter-spacing: -.03em; max-width: 760px; margin: 10px 0 8px; color: #fff8e8;
    }
    .apex-hero p { max-width: 660px; color: #c1cec5; font-size: 14px; line-height: 1.55; margin: 0; }
    ...
    .apex-schedule-box {
      margin-top: 24px; background: linear-gradient(145deg, #0e3025, #082019);
      border: 1px solid var(--apex-gold); border-radius: 20px; padding: 22px;
      box-shadow: 0 12px 35px rgba(0,0,0,.35);
    }
    .apex-schedule-head { display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 10px; margin-bottom: 14px; }
    .apex-schedule-title { font-size: 17px; font-weight: 900; color: var(--apex-gold); display: flex; align-items: center; gap: 8px; }
    ...
    .apex-slots-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px; }
    ...
    .apex-slot-name { font-size: 14.5px; font-weight: 800; color: #fff; display: flex; align-items: center; gap: 6px; }
    ...
    .apex-mini-deal-actions button {
      padding: 6px 10px; font-size: 11px; font-weight: 800; border-radius: 6px; border: 0; cursor: pointer;
    }

    /* SECTION 2: TOMORROW'S PREVIEW */
    .apex-tomorrow-box {
      margin-top: 24px; background: linear-gradient(145deg, #09241b, #051813);
      border: 1px solid rgba(212,175,55,.3); border-radius: 20px; padding: 20px 22px;
    }
    ...
    .apex-filter-btn {
      ...
      color: #c6d0ca; border-radius: 999px; padding: 6px 13px; font-size: 12px; font-weight: 700;
    ...
    .apex-head h2 { font-size: 20px; margin: 0; font-weight: 800; color: #fff8e8; }
    .apex-head p { font-size: 12.5px; color: var(--apex-muted); margin: 4px 0 0; }
    
    .apex-deals { display: grid; grid-template-columns: repeat(auto-fill, minmax(290px, 1fr)); gap: 18px; }
    .apex-deal {
    ...
    .apex-deal-actions button {
      border: 0; border-radius: 8px; padding: 9px 10px; font-size: 12px; font-weight: 800;
    ...
    .apex-report-link { font-size: 10.5px; color: #7f9087; text-align: right; background: none; border: 0; cursor: pointer; padding: 2px 0; margin-top: 2px; }
    ...
    .apex-modal-card button {
      flex: 1; padding: 11px; border-radius: 9px; border: 1px solid var(--apex-line);
  ...
  function renderFeedbackModal(deal) {
    ...
    <div class="apex-modal-card">
      ...
      <textarea id="apex-feedback-notes" class="apex-feedback-textarea" placeholder="Nhập thêm ghi chú để đội kiểm định kiểm tra thực địa..."></textarea>

      <div class="actions">
        <button data-apex="close-feedback">Đóng</button>
        <button class="yes" style="background:#f87171;color:#fff;" data-apex="submit-feedback" data-id="${esc(deal.deal_id)}">Gửi Phản Hồi</button>
      </div>
    </div>`;
  }
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (act === 'submit-feedback') {
      ...
      closeFeedbackModal();
      toast('🚩 Cảm ơn bạn! Phản hồi đã được ghi nhận để kiểm định.');
    }
    ...
    if (act === 'toggle-plan') {
      ...
      try { localStorage.setItem('jayt_today_plan', JSON.stringify(state.todayPlan)); } catch {}
      render();
    }
