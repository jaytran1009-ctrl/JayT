# -*- coding: utf-8 -*-
"""
JAYT APEX PRODUCTION SERVER — STRICT TLS-SNI IP PINNING & FAIL-CLOSED SECRET
=============================================================================
1. /healthz & /health: Dynamic active_build_id & build_manifest_hash & secret_configured.
...
6. /__test/*: Internal test endpoint (HARD-BLOCKED in Production unconditionally).
=============================================================================
"""
import os
import sys
...
# 1. STRICT SECRET GOVERNANCE (FAIL-CLOSED: NO DEFAULT/FALLBACK STRINGS IN CODE)
    ...
    def do_GET(self):
        ...
        # 2. HEALTH & RUNTIME METADATA ENDPOINT
            ...
            health_data = {
                ...
                "resolver_engine": "TRUE_TLS_SNI_IP_PINNED_v4_STRICT"
            }
            self.wfile.write(json.dumps(health_data, ensure_ascii=False, indent=2).encode("utf-8"))
            return

        # 3. INTERNAL TEST SLOW ENDPOINT (HARD-BLOCKED IN PRODUCTION)
        elif parsed.path.startswith("/__test/"):
            # Hard-block in production environment unconditionally
        ...
        # 4. TRUSTED SERVER TIME BASELINE ENDPOINT (RFC-3339 Asia/Ho_Chi_Minh)
        ...
        # 5. SINGLE SOURCE OF TRUTH STATUS ENDPOINT
        ...
        # 6. TOKEN ISSUANCE ENDPOINT (/api/token/issue?deal_id=...)
        ...
        # 7. TOKENIZED FAIL-CLOSED REDIRECT RESOLVER (/out?token=...)
