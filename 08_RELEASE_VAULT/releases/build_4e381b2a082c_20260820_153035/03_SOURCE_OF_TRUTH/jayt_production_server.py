# -*- coding: utf-8 -*-
"""
JAYT APEX PRODUCTION SERVER — SEPARATE LIVENESS (/healthz) & READINESS (/readyz)
=============================================================================
1. /healthz & /livez: Process liveness probe (UP).
2. /readyz: Strict Traffic Readiness probe (Secret >= 32 chars, Catalog valid, Manifest valid).
...
class JayTComprehensiveHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 2. LIVENESS PROBE (/healthz, /livez, /health) -> Báo tiến trình còn sống
        ...
        # 3. READINESS PROBE (/readyz) -> Kiểm tra sẵn sàng phục vụ traffic (Secret, Catalog, Manifest)
            ...
            if not catalog:
                not_ready_reasons.append("ERR_DOMAIN_CATALOG_UNAVAILABLE_OR_EMPTY")
            if not manifest_hash or manifest_hash == "UNTRACKED_DEV_HASH":
                not_ready_reasons.append("ERR_BUILD_MANIFEST_NOT_TRACKED")
            ...
            if not_ready_reasons:
                ...
                ready_data = {
                    ...
                    "resolver_engine": "TRUE_TLS_SNI_IP_PINNED_v5_READINESS_GATED"
                }
                self.wfile.write(json.dumps(ready_data, ensure_ascii=False, indent=2).encode("utf-8"))
                return
        ...
        # 4. INTERNAL TEST SLOW ENDPOINT (HARD-BLOCKED IN PRODUCTION)
            ...
            # HARD BLOCK: Nếu là Production hoặc không bật test -> 404
