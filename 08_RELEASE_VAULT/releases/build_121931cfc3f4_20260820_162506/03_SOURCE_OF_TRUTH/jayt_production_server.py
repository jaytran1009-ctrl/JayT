...
class JayTComprehensiveHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 8. TOKENIZED FAIL-CLOSED REDIRECT RESOLVER (/out?token=...)
            ...
            if not valid_token:
                record_server_audit("GET", "/out", qs, 403, {"error": token_err})
                self.send_response(403)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                ...
                record_server_audit("GET", "/out", qs, 302, {"target_url": target_url, "token": token})
    ...
    def handle_token_issue(self, deal_id):
        ...
        record_server_audit("POST", "/api/token/issue", "", 200, {"deal_id": deal_id, "token": token})
