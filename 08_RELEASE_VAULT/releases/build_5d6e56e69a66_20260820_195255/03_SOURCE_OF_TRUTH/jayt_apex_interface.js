/* =============================================================================
   ...
   Chỉ thị: JAYT-COMMUNITY-RETENTION-003 (Verified Availability Gate • Fail-Closed Claim Matrix)
   ============================================================================= */
(function () {
  'use strict';
  ...
  function verifyLiveAvailability(deal, evidenceStore, daNangCtx) {
    ...
    const MAX_AGE_MS = 30 * 24 * 3600 * 1000; // 30 days maximum freshness window
    if (Date.now() - evTime > MAX_AGE_MS) return false;
  ...
  function evaluateRecommendations() {
    ...
    if (inTimingDeals.length > 0) {
      return inTimingDeals.map(d => {
        // Strict Fail-Closed Availability Gate (JAYT-COMMUNITY-RETENTION-003)
