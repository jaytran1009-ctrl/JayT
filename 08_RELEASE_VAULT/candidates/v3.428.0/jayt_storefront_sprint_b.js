 ...
 * Version: v3.428.0 (Production Release — JAYT-355)
...
(function(root) {
  ...
  // =========================================================================
  // JAYT-353-R3 BATCH 17: SMART VALUE RADAR (15 APPROVED PROVENANCE ITEMS)
  // =========================================================================
  ...
  "monday": {
    ...
    "has_deals": false,
    "items": [],
    "empty_state_note": "Thứ Hai đầu tuần: Chưa có chương trình ưu đãi định kỳ hàng tuần được xác minh bằng chứng trong Trust Vault. (Lưu ý: CGV Culture Day chỉ diễn ra vào Thứ Hai CUỐI CÙNG của mỗi tháng; Metiz Super Monday hiện chưa có bằng chứng nguồn gốc hợp lệ)."
  },
  "tuesday": {
    "name_vi": "Thứ Ba",
    "has_deals": true,
    "items": [
      {
        ...
        "price_rule": "Đồng giá vé 2D chỉ 50.000đ tại Galaxy Coopmart Đà Nẵng",
        "frequency": "Thứ Ba hàng tuần",
        "exceptions": "Không áp dụng ngày Lễ/Tết, suất chiếu đặc biệt, định dạng IMAX Laser; phụ thu 3D",
        "source_url": "https://www.galaxycine.vn/khuyen-mai/happy-day---ve-chi-tu-45k/",
        "raw_file": "05_DEAL_AND_AFFILIATE/runs/RUN_20260827_115134_fb687e/LEAF_GALAXY_CINEMA_dd674d28088a/page.html",
  ...
  "wednesday": {
    ...
    "has_deals": false,
    "items": [],
    "empty_state_note": "Thứ Tư hiện chưa có chương trình ưu đãi định kỳ hàng tuần được xác minh. (Metiz U22 tạm giữ do cần bổ sung xác minh địa điểm cụ thể theo lệnh J352-R1; CGV Culture Day chỉ áp dụng vào Thứ Tư cuối cùng của tháng)."
  },
  "thursday": {
    "name_vi": "Thứ Năm",
    "has_deals": false,
    "items": [],
    "empty_state_note": "Thứ Năm hiện chưa có chương trình ưu đãi định kỳ hàng tuần được xác minh. (Metiz U22 tạm giữ theo lệnh J352-R1). Tiện ích công ích và thư viện phục vụ bình thường theo lịch niêm yết."
  },
  "friday": {
    "name_vi": "Thứ Sáu",
    "has_deals": false,
    "items": [],
    "empty_state_note": "Thứ Sáu áp dụng biểu giá ngày thường tiêu chuẩn. (Chính sách Galaxy U22 là biểu giá đối tượng thành viên, không phải ưu đãi định kỳ riêng ngày Thứ Sáu; tạm giữ theo lệnh J352-R1)."
  },
  "saturday": {
    "name_vi": "Thứ Bảy",
    "has_deals": false,
  ...
  function renderVoucherVaultView() {
    ...
    const actionableItems = VOUCHER_VAULT_ITEMS.filter(i => !i.held);
    const heldItems = VOUCHER_VAULT_ITEMS.filter(i => i.held);
    ...
    const countHeld = heldItems.length;
    ...
    return `
      <div class="sprint-b-view-container">
        ...
        <!-- 5-TIER FILTER BAR WITH HELD TAB -->
  ...
  function renderVoucherCard(item) {
    ...
    return `
      <article class="vault-card" data-card-id="${item.id}" data-card-tier="${cardTierAttr}" data-actionable="${isActionable}" style="${isHeld ? 'display: none; border-color: #fca5a5; background: #fff5f5;' : ''}" aria-label="${item.title}">
        <div class="vault-card-header">
          ${tierBadge}
        ...
        <div class="vault-card-actions" style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 14px;">
          ...
          ${(item.price_vnd && !isHeld) ? `
            <button type="button" class="btn-split-shortcut" data-split-amount="${item.price_vnd}" data-split-desc="${item.title}" aria-label="Chia bill nhóm cho ${item.title}">
              🧮 Chia bill nhóm
            </button>
          ` : ''}
        </div>
    ...
    buttons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('#voucher-vault-grid .vault-card').forEach(card => {
          ...
          let show = false;
          if (filter === 'ALL') {
            show = isAct; // Exclude held records from actionable ALL view
          } else if (filter === 'HELD') {
            show = false;
          } else {
        ...
        if (statusEl) {
          if (filter === 'ALL') {
            ...
            statusEl.textContent = `Đang hiển thị ${visible} ưu đãi bị tạm giữ đối soát theo lệnh J354-R1 (0 nút sao chép).`;
          } else {
  ...
  function attachSplitBillProEvents() {
    ...
    function update() {
      ...
      const zaloMessage = [
        ...
        payer ? `💳 Thông tin chuyển khoản: ${payer}` : '',
        `----------------------------------`,
      ...
      if (resContainer) {
        ...
        copyBtn?.addEventListener('click', async () => {
          ...
          if (success) {
            try { navigator.vibrate?.(50); } catch(e) {}
            copyBtn.textContent = '✓ Đã sao chép lời rủ!';
          ...
          ${dayTabs.map(tab => {
            ...
            return `
              <button class="calendar-day-tab ${isToday ? 'active today-marker' : ''}" data-calendar-day="${tab.key}" role="tab" aria-selected="${isToday ? 'true' : 'false'}">
                ...
                <small style="font-size: 0.75rem; opacity: 0.85;">${tab.key === 'tuesday' ? 'Có ưu đãi' : 'Biểu giá chuẩn'}</small>
              </button>
            `;
          }).join('')}
        </div>
