...
(function(root) {
  ...
  function renderVoucherCard(item) {
    ...
    if (item.tier === 'PRICE_OBSERVATION') {
      tierBadge = '<span class="vault-tier-badge badge-price-observation">📋 Giá quan sát</span>';
        ...
        ${item.price_vnd ? `
          <div class="vault-card-price">
            ...
            <small style="font-size: 0.78rem; font-weight: 500; color: var(--text-muted);">
              ${item.tier === 'PRICE_OBSERVATION' ? '(Giá quan sát niêm yết)' : '(Ưu đãi tại quầy)'}
            </small>
          </div>
        ` : ''}
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (fallbackBox) {
            ...
            alert('Không thể tự động sao chép mã. Mã của bạn là: ' + code);
          }
          setTimeout(() => {
