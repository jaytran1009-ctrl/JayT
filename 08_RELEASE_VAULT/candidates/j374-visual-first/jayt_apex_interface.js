...
function getOfferVisualAsset(offer) {
  ...
  if (brand.includes('popeyes')) {
    if (id.includes('89k')) return '/assets/images/brands/popeyes_core_89k.leaf.png';
    if (id.includes('145k') || title.includes('145')) return '/assets/images/brands/popeyes_bo_doi_145k.leaf.png';
    if (id.includes('99k') || title.includes('99')) return '/assets/images/brands/popeyes_bogo_deli_99k.leaf.png';
    return '/assets/images/brands/popeyes_promotions_page.leaf.png';
  }
  if (brand.includes('pizza company') || brand.includes('tpc')) {
    if (id.includes('mam_keo') || id.includes('479k')) return '/assets/images/brands/tpc_combo_cot_mam_keo.leaf.png';
    if (id.includes('mai_man') || id.includes('599k')) return '/assets/images/brands/tpc_combo_cot_mai_man.leaf.png';
  ...
  if (brand.includes('katinat')) {
    if (title.includes('app') || id.includes('loyalty')) return '/assets/images/brands/katinat_app_loyalty.leaf.png';
  ...
  if (brand.includes('highlands')) return '/assets/images/brands/highlands_coffee_banner.svg';
  ...
  // 3. Authentic Local Da Nang Food Photo
    ...
    <div class="schedule-rhythm-section" aria-label="Gợi ý ưu đãi theo nhịp thời gian Đà Nẵng">
      <div class="schedule-rhythm-header">
        <div class="schedule-active-indicator">
          ...
          ${state.scheduleManualOverride ? '<span class="schedule-override-tag">Đang chọn thủ công</span>' : ''}
        </div>
        ${state.scheduleManualOverride ? `
          <button type="button" id="btn-reset-schedule-time" class="btn-reset-schedule" title="Khôi phục giờ thực tế Đà Nẵng">
            ↺ Giờ thực tế (${currentRealSlot.name})
          </button>
        ` : ''}
      </div>

      <div class="schedule-chips-container" role="group" aria-label="Chọn khung giờ xem ưu đãi">
        ${J374_SCHEDULE_SLOTS.map(slot => `
          <button 
            type="button" 
            class="schedule-slot-btn ${slot.id === activeSlotId ? 'is-active' : ''}" 
...
function renderSingleVisualCard(offer) {
  ...
  let savingBadgeHtml = '';
  if (offer.saving_amount && offer.saving_amount > 0) {
  ...
  el.innerHTML = `
    <div class="visual-card-media-wrap">
      <img class="visual-card-img" 
           ...
           width="640" 
           height="400" 
           onerror="this.onerror=null; this.src='/assets/images/danang_real_photo_mi_quang.jpg';" />
      ${savingBadgeHtml}
    ...
    <div class="visual-card-body">
      ...
      <div class="visual-card-actions">
        <button type="button" class="btn-card-detail" data-action="open-drawer" data-offer-id="${offer.offer_id}" aria-label="Xem chi tiết ${escapeHtml(offer.title)}">
          Chi tiết ↗
        </button>
        <button type="button" class="btn-card-invite" data-action="open-invite" data-offer-id="${offer.offer_id}" aria-label="Rủ bạn kèo này cho ${escapeHtml(offer.title)}">
          👥 Rủ bạn kèo này
        </button>
      </div>
    </div>
