...
(function(root) {
  ...
  const VOUCHER_VAULT_ITEMS = [
  {
    ...
    "app_url": null,
    "locality_basis": "Starlight Đà Nẵng: Tầng 4, Tòa nhà Nguyễn Kim, 46 Điện Biên Phủ, Đà Nẵng",
  ...
  },
  {
    ...
    "app_url": null,
    "locality_basis": "The Pizza Company Lotte Mart Đà Nẵng; The Pizza Company 173 Nguyễn Văn Thoại, Q. Sơn Trà; The Pizza Company Co.opmart 478 Điện Biên Phủ, Q. Thanh Khê; The Pizza Company Nguyễn Văn Linh, Q. Hải Châu",
  ...
  },
  {
    ...
    "app_url": null,
    "locality_basis": "Gong Cha Đà Nẵng: 01 Nguyễn Văn Linh, P. Bình Hiên, Q. Hải Châu, Đà Nẵng",
  ...
  },
  {
    ...
    "locality_basis": "Katinat Bạch Đằng: 9 Bạch Đằng, P. Thạch Thang, Q. Hải Châu, Đà Nẵng",
  ...
  },
  {
    ...
    "app_url": null,
    "locality_basis": "Popeyes Núi Thành: 179 Núi Thành, Phường Hòa Cường Bắc, Quận Hải Châu, Đà Nẵng",
    "validity": "Áp dụng thường nhật 2026",
    "conditions": "Combo 4 Miếng Gà Giòn + 2 Nước ngọt (giảm 47%, giá gốc 186.000đ). Áp dụng đặt giao hàng qua Hotline 1900 6008 hoặc Website/App Popeyes.",
    "budget_warning": "Kiểm tra hóa đơn và điều kiện trước khi thanh toán.",
    "has_code": false,
    "code": null,
    "action_type": "COUNTER_MEMBER",
    ...
    "member_badge": "Ưu đãi tại quầy / Đặt món",
    "is_discovery": false,
    "claim_instruction": "Combo 4 Miếng Gà Giòn + 2 Nước ngọt giá 99.000đ đặt qua hotline 1900 6008 hoặc website Popeyes giao từ 179 Núi Thành, Đà Nẵng.",
  ...
  },
  {
    ...
    "locality_basis": "Popeyes Núi Thành: 179 Núi Thành, Phường Hòa Cường Bắc, Quận Hải Châu, Đà Nẵng",
    "validity": "Áp dụng thường nhật 2026",
    "conditions": "Combo 7 món: 2 Mì Ý Phô Mai + 2 Gà Giòn + 1 Khoai tây chiên (S) + 2 Nước (giá gốc 225.000đ). Áp dụng khi đặt qua Website/Ứng dụng Popeyes – Hotline 1900 6008.",
    "budget_warning": "Kiểm tra hóa đơn và điều kiện trước khi thanh toán.",
    "has_code": false,
    "code": null,
    "action_type": "COUNTER_MEMBER",
...
,
  ...
  },
  {
    ...
    "batch18_id": "P2O_GALAXY_SHOPEEPAY_SEP_2026",
  ...
  function renderDailyGuideHome() {
    ...
    return `
      <div class="cr-experience-container">
        ...
        <!-- HOME SECTION: SMART VALUE RADAR 15 CARDS (JAYT-361 MANDATE) -->
        <section class="home-radar-section" aria-label="Smart Value Radar — 15 Mục Quan Sát Trên Trang Chủ" style="margin-top: 32px; margin-bottom: 24px;">
          <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
            <div>
              <h2 style="font-size: 1.35rem; font-weight: 800; color: var(--text-primary); margin: 0 0 4px 0;">
                📡 Smart Value Radar — Thiết Bị Học Tập &amp; KTX Đối Soát
              </h2>
              <p style="font-size: 0.88rem; color: var(--text-secondary); margin: 0;">
                15 sản phẩm công nghệ quan sát thực tế tại đối tác Đà Nẵng (Phi Long Technology). Giá quan sát đối soát &mdash; Bấm để kiểm tra tại website chính hãng.
              </p>
            </div>
            <button class="btn-rail-action" data-nav="RADAR" style="background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; font-weight: 700; padding: 8px 16px; border-radius: 6px; cursor: pointer;" aria-label="Mở toàn bộ màn hình Smart Value Radar">
              Xem chi tiết Radar &rarr;
            </button>
          </div>

          <div class="radar-grid" id="home-radar-grid" role="feed" aria-label="Danh sách thiết bị quan sát trên trang chủ">
            ${VALUE_RADAR_ITEMS.map(item => `
              <article class="radar-card" data-category="${item.category}" aria-label="${item.name}">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; gap: 6px;">
                  <span style="font-size: 0.75rem; font-weight: 700; color: #0369a1; background: #e0f2fe; padding: 3px 8px; border-radius: 4px;">
                    ${item.category === 'USB_STORAGE' ? 'Lưu trữ USB 3.2' : item.category === 'SSD_STORAGE' ? 'Ổ cứng SSD' : item.category === 'RAM_MEMORY' ? 'RAM Laptop DDR5' : 'Chương trình HSSV'}
                  </span>
                  <span style="font-size: 0.75rem; color: var(--text-muted); font-family: monospace;">Model: ${item.model}</span>
                </div>
                ...
                `}
                <div class="radar-stock-badge">${item.stock_status}</div>
                <div style="font-size: 0.78rem; color: var(--text-muted); margin-bottom: 14px;">
                  🕒 Ghi nhận theo HTTP Date: ${item.captured_at}
                </div>
                <div style="margin-top: auto; display: flex; flex-direction: column; gap: 8px;">
                  <a href="${item.source_url}" target="_blank" rel="noopener noreferrer nofollow" class="btn-rail-action" style="background: var(--bg-card-subtle); color: var(--text-primary); border: 1px solid var(--border-subtle); padding: 8px 14px; border-radius: 6px; text-decoration: none; font-size: 0.85rem; font-weight: 600; display: inline-flex; align-items: center; gap: 4px; min-height: 44px; width: 100%; box-sizing: border-box; justify-content: center;" aria-label="Kiểm tra ${item.name} tại website chính hãng Phi Long">
