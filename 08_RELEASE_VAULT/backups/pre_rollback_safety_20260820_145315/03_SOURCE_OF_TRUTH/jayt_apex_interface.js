...
(function () {
  ...
  function evaluateLiveDeals(profile) {
    ...
    // Fallback nếu engine chưa nạp
    return {
      verified_hero_deals: candidateDeals.filter(d => d.taxonomy === 'VERIFIED'),
      probing_deals: candidateDeals.filter(d => d.taxonomy === 'PROBING'),
      unverified_deals: candidateDeals.filter(d => d.taxonomy === 'UNVERIFIED'),
